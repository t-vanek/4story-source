...
/*void CTClientGame::SendCS_HACKINFO_REQ(BYTE bDisguise)
{
	CPacket packet;

	packet.SetID(CS_HACKINFO_REQ)
		<< bDisguise;
...
void CTClientGame::SendCS_PETRIDING_REQ(DWORD dwMonID, BOOL bRide)
{
	BYTE bAction = bRide? PETACTION_RIDING: PETACTION_DISMOUNT;

	CPacket packet;

	packet.SetID(CS_PETRIDING_REQ)
		<< dwMonID
		<< bAction;
...
void CTClientGame::SendCS_DELRECALLMON_REQ( DWORD dwMonID, BYTE bMonType)
{
	CPacket packet;

	packet.SetID(CS_DELRECALLMON_REQ)
		<< dwMonID
		<< bMonType;
...
void CTClientGame::SendCS_ACTITEMSUSE_REQ(CTClientObjBase *pATTACK,
										  CTClientObjBase *pDEFEND,
										  LPTSKILL pTSKILL)
{
	if (!pATTACK || !pDEFEND || !pTSKILL)
		return;
	...
	packet.SetID(CS_ACTITEMSUSE_REQ)
		<< pATTACK->m_dwID
		<< pATTACK->m_bType
		<< m_pMainWnd->m_bChannel
		<< m_vMAP.m_wMapID
