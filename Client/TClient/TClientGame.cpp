...
DWORD CTClientGame::m_vTICONNA[TSKILLSLOT_COUNT] = {
	ID_CTRLINST_ICON_NA_1,
	ID_CTRLINST_ICON_NA_2,
	ID_CTRLINST_ICON_NA_3,
	ID_CTRLINST_ICON_NA_4,
	ID_CTRLINST_ICON_NA_5,
	ID_CTRLINST_ICON_NA_6,
	ID_CTRLINST_ICON_NA_7,
	ID_CTRLINST_ICON_NA_8,
	ID_CTRLINST_ICON_NA_9,
	ID_CTRLINST_ICON_NA_10,
	ID_CTRLINST_ICON_NA_11,
	ID_CTRLINST_ICON_NA_12,
	ID_CTRLINST_ICON_NA_13,
	ID_CTRLINST_ICON_NA_14};
...
WORD CTClientGame::m_vBattleSlotID[BATTLEINVEN_SLOT_COUNT] = {
	27301,
	27302,
	27303,
	27304,
...
WORD CTClientGame::m_vBattleGauge[BATTLEINVEN_SLOT_COUNT] = {
	27318,
	27319,
	27320,
	27321,
...
DWORD CTClientGame::m_dwHideMaintain[2] =
{
	SDT_STATUS_HIDE,
	SDT_STATUS_STANDHIDE
};

D3DXVECTOR3 CTClientGame::m_vBOWPos[TCONTRY_B] = 
{
	D3DXVECTOR3(277.30f, 422.0f, 510.61f),
	D3DXVECTOR3(743.67f, 423.0f, 512.95f)
};

WORD CTClientGame::m_wBOWSkills[PREMIUM_SKILLS] = {
	1501,
	1502,
	1503,
	1504,
...
WORD CTClientGame::m_wPremiumSkills[PREMIUM_SKILLS] = {
	1552,
	1553,
	1554,
	1555,
...
WORD CTClientGame::m_wSkillSystemExceptions[SKILL_EXCEPTIONS_COUNT] = {
	235,
	437,
	637,
	638,
...
CString CTClientGame::m_strBRCommands[13] = {
	_T("Yes!"),
	_T("No"),
	_T("Help"),
	_T("Buff"),
...
CString CTClientGame::GetSHA1String(const CString& strSTRING)
{
	SHA1 hasher;

	string string((LPCTSTR)strSTRING);
...
CTClientGame::CTClientGame()
{
	...
	m_bRSCS = FALSE;
	m_bStartGame = FALSE;
	m_bIsCinematic = FALSE;
	m_bCanRender = TRUE;
	m_bCharLoaded = FALSE;
	m_bAutoHelp = FALSE;
	m_bSkillUseLocked = FALSE;
	...
	ZeroMemory(m_pCustomImages, sizeof(CD3DImage*) * TCUSTOMTEX_COUNT);
	...
	m_strMACAddr = GetMACAddress();
	m_bLastSVR = SVR_NONE;
	InitPetSizes();
	m_dwLastQueueTick = GetTickCount();

	for (BYTE i = 0; i < PING_MEASUREMENT_COUNT; ++i)
		m_dwPing[i] = 0;

	m_dwEstPing = 35;
	m_strBRInviter.Empty();
	m_vBRTEAM.clear();
}
...
void CTClientGame::CalcSCENE( DWORD dwTick)
{
	...
	else
	{
		...
		if( m_pCtrlHost->m_bType == OT_COMPANION )
			CalcSPOLECNIK( (CTClientSpolecnik*) m_pCtrlHost, dwTick);
	...
	if (GetTickCount() - m_dwMountTick > 500 && (m_wPetID || (m_nNextPetID && !static_cast<CTPetManageDlg*>(m_vTFRAME[TFRAME_PET_MANAGE])->IsRecalling())))
	{
		if (m_nNextPetID && !m_wPetID)
		{
			m_pSESSION->SendCS_PETRECALL_REQ((WORD) m_nNextPetID);
			static_cast<CTPetManageDlg*>(m_vTFRAME[TFRAME_PET_MANAGE])->SetRecalling(TRUE);
			m_nNextPetID = 0;
		}
		else
		{
	...
	try
	{
	if (!m_pMainChar->m_bSubActionLock && !m_qSkillQueue.empty() && !m_bCHARGE)
	{
		SKILLUSE SkillUse = *m_qSkillQueue.front();	
		delete m_qSkillQueue.front();
		m_qSkillQueue.pop();
		UseTSkill(SkillUse.m_wID, SkillUse.m_bActivate, SkillUse.m_bCheckGround);
	}
	}
	catch(...)
	{
		m_pMainWnd->MessageBoxOK(
			"Error occured in the new skill system. Code : 0x0A, Please report this to the 4Story Team.",
			TSTR_CANCEL,
			GM_CLOSE_MSGBOX,
			0 );
	}
	...
	if (GetTickCount() - dwLastUpdate > 500)
	{
		CTClientNpc* pSelectedNPC = (CTClientNpc*) m_vMAP.FindTNPC(m_dwSelectedNPC);
		FLOAT fLowestDiff = 5.0f;
		DWORD dwLowestNPC = 0;
	
		if (pSelectedNPC)
		{
			if (GetDifferencial(m_pMainChar->GetPositionMat(), pSelectedNPC->m_vWorld) > TARGETUI_DIST)
			{
				pSelectedNPC->ClearTActionSFX(&pSelectedNPC->m_vTACTIONSFX);
				m_dwSelectedNPC = 0;
			}
		}

		for (MAPOBJECT::iterator itNpc = m_vMAP.m_mapTNpc.begin(); itNpc != m_vMAP.m_mapTNpc.end(); ++itNpc)
		{
			FLOAT fDiff = GetDifferencial(m_pMainChar->GetPositionMat(), (*itNpc).second->m_vWorld);
			CTClientNpc* pNPC = (CTClientNpc*) (*itNpc).second;
			if (!pNPC || !pNPC->m_bCanSelected || pNPC->m_wItemID)
				continue;

			if (fDiff > TARGETUI_DIST)
			{
				static_cast<CTClientNpc*>((*itNpc).second)->ClearTActionSFX(
					&static_cast<CTClientNpc*>((*itNpc).second)->m_vTACTIONSFX);
			}

			if (fDiff < fLowestDiff)
			{
				fLowestDiff = fDiff;
				dwLowestNPC = pNPC->m_dwID;
			}
		}

		if (dwLowestNPC != m_dwSelectedNPC)
		{
			pSelectedNPC = (CTClientNpc*) m_vMAP.FindTNPC(m_dwSelectedNPC);
			if (pSelectedNPC)
				pSelectedNPC->ClearTActionSFX(&pSelectedNPC->m_vTACTIONSFX);
		}

		pSelectedNPC = (CTClientNpc*) m_vMAP.FindTNPC(dwLowestNPC);

		if (fLowestDiff < TARGETUI_DIST && pSelectedNPC && m_dwSelectedNPC != pSelectedNPC->m_dwID)
		{
			m_dwSelectedNPC = dwLowestNPC;
			if (!m_vTOPTION.m_bDisableAutoSelectSFX)
				pSelectedNPC->PlayRawSFX(&pSelectedNPC->m_vTACTIONSFX, 287879825, TRUE);
		}
	...
	CalcMONSTER(dwTick);
	CalcRECALL(dwTick);
	CalcSPOLECNIK(dwTick);
	CalcPLAYER(dwTick);

	CalcSLAVERECALL(dwTick);
	CalcSLAVESPOLECNIK(dwTick);
	CalcSLAVEMON(dwTick);
	CalcARROW(dwTick);
...
void CTClientGame::LoadCustomTextures(DWORD dwID, const CString strPath)
{
	...
	m_pCustomImages[nIndex] = new CD3DImage();
	...
	if (vFILE.Open(strPath, CFile::modeRead | CFile::typeBinary))
	{
		...
		if (dwSIZE)
		{
			...
			//vFILE.Seek(1, CFile::begin);
			vFILE.Read(pDATA, dwSIZE);

			m_pCustomImages[nIndex]->LoadImageFile(pDevice, pDATA, dwSIZE);

			delete[] pDATA;
		}
	}

	ImagesToTextures(dwID, FALSE);
}

LPTEXTURESET CTClientGame::ImagesToTextures(DWORD dwID, BOOL bResetDevice, CD3DDevice* pDevve)
{
	if (!m_pCustomImages[dwID - 1])
		return nullptr;
	...
	hResult = D3DXCreateTexture(
		pDevice,
		(UINT)m_pCustomImages[nIdx]->GetWidth(),
		(UINT)m_pCustomImages[nIdx]->GetHeight(), 1,
	...
	rc.left = 0;
	rc.top = 0;
	rc.right = (LONG)m_pCustomImages[nIdx]->GetWidth(); //long? Is it converting to my D?
	rc.bottom = (LONG)m_pCustomImages[nIdx]->GetHeight();

	m_pCustomImages[nIdx]->Render(pDevice, D3DCOLOR_ARGB(255,255,255,255), 0,0, &rc);
	...
	pT3DTEX->m_pTDATA = (LPBYTE) pRNDTEX;
	pT3DTEX->m_bEnabled = TRUE;
	pTextureSet->PushTexturePtr(pT3DTEX);

	if( !bResetDevice )
		m_mapCUSTOMTEX.insert( MAPCUSTOMTEX::value_type( dwID, pTextureSet ) );
...
HRESULT CTClientGame::Render( DWORD dwTickCount)
{
	...
	if (GetTickCount() - dwPrevTick >= 1000)
	{
	//	if (m_pSESSION)
	//		m_pSESSION->SendCS_PINGMEASUREMENT_REQ();
	...
	if( m_pMainChar->IsFlashed() || m_dwFLASHTick )
		RenderFLASH();

	//if(m_pMainChar->m_bFlash)
	//	m_pChatFrame->m_vListArray[0]->ShowComponent(FALSE);
...
void CTClientGame::ResetVisibleOBJ( DWORD dwTick)
{
	...
	MAPSPOLECNIK::iterator itSPOLECNIK;
	...
	{
		CTClientChar* pHOST = FindPC((*itRECALL).second->GetHostID());	
		if (pHOST)
		{
			if (dynamic_cast<CTClientFake*> ((*itRECALL).second) == NULL)
			{
				(*itRECALL).second->m_bDetect = CanCharacterSee( m_pMainChar, pHOST );
				(*itRECALL).second->m_bHide = pHOST->m_bHide;
			} //test
		}
	...
	{
		CTClientChar* pHOST = FindPC( (*itSPOLECNIK).second->GetHostID() );	
		ResetDetect((*itSPOLECNIK).second);

		if( pHOST )
		{
/*			if( CanCharacterSee( m_pMainChar, pHOST ) )
			{
				(*itSPOLECNIK).second->m_bDetect = TRUE;
				(*itSPOLECNIK).second->m_bHide = FALSE;
			}
			else
			{
				(*itSPOLECNIK).second->m_bDetect = FALSE;
				(*itSPOLECNIK).second->m_bHide = TRUE;
			}
*/

			(*itSPOLECNIK).second->m_bDetect = CanCharacterSee( m_pMainChar, pHOST );
			(*itSPOLECNIK).second->m_bHide = pHOST->m_bHide;
			if (IsInBRMap())
				(*itSPOLECNIK).second->m_bDetect = IsInMyBRTeam(pHOST->m_dwID);
		}

		(*itSPOLECNIK).second->ResetVisible(
			m_pCamera,
			m_vMAP.m_dwHouseID,
			m_vMAP.m_bHouseMesh,
			dwTick);

		if( (*itSPOLECNIK).second->m_bVisible && !m_vTSCENE_PLAYER.IsRunning() )
			PushDrawOBJ((*itSPOLECNIK).second);
		else
			(*itSPOLECNIK).second->CalcRender(m_pDevice);
	}
...
void CTClientGame::UpdateBoxGauge(DWORD dwTick)
{
	CTBoxOpenDlg* pDLG = static_cast<CTBoxOpenDlg*>( m_vTFRAME[TFRAME_OPENBOX] );
	if( !pDLG->IsVisible() )
		return;
		
	BYTE bInvenID = (BYTE) LOWORD(pDLG->GetParam());
	BYTE bItemID = (BYTE) HIWORD(pDLG->GetParam());
	...
	if( pDLG->Update(dwTick))
	{
		pDLG->m_pOpen->EnableComponent(TRUE);
		pDLG->Release();
		...
		CTClientItem* pItem = pTINVEN->FindTItem(bItemID);
		if (!pItem || !pItem->GetTITEM())
			return;
		...
		{
			m_pSESSION->SendCS_ITEMCHANGE_REQ(bInvenID, bItemID);
		}
	}
}
...
void CTClientGame::PushDrawOBJ( CTClientObjBase *pTOBJ)
{
	...
	case OT_PC		:
		{
			if ((pTOBJ->m_bType == OT_COMPANION && static_cast<CTClientSpolecnik*>(pTOBJ)->GetHostID() != m_pMainChar->m_dwID) || pTOBJ->m_bType != OT_RECALL && pTOBJ->m_bType != OT_COMPANION || (pTOBJ->m_bType == OT_RECALL && !static_cast<CTClientPet*>(pTOBJ)))
			{
				if (m_vTOPTION.m_bHideTournament)
				{
					if (IsTournamentMap() && m_pMainChar->m_bTournamentTeam != TTOURNAMENT_TEAM_NONE && pTOBJ->m_bType != OT_NPC &&
						static_cast<CTClientChar*>(pTOBJ)->m_bTournamentTeam == TTOURNAMENT_TEAM_NONE)
					{
						bDraw = FALSE;
					}
				}

				if (m_vTOPTION.m_bHideDuel)
				{
					if (m_bDuel && GetDuelTarget() && static_cast<CTClientChar*>(pTOBJ)->m_dwID != GetDuelTarget()->m_dwID)
						bDraw = FALSE;
				}

				if (m_vTOPTION.m_bHideArena)
				{
					if (m_pMainChar->m_bChallengeTeam != TTOURNAMENT_TEAM_NONE && m_pMainChar->m_wChallengeArenaID != static_cast<CTClientChar*>(pTOBJ)->m_wChallengeArenaID)
					{
						bDraw = FALSE;
					}
				}
...
D3DXVECTOR3 CTClientGame::GetBaseNamePosition(CTClientObjBase *pTOBJ)
{
 ...
 if( pTOBJ->m_bType == OT_RECALL && ((CTClientRecall *) pTOBJ)->m_bRecallType == TRECALLTYPE_PET && !((CTClientRecall *) pTOBJ)->m_wCompanionID )
 {
	 CTClientChar *pTCHAR = ((CTClientPet* ) pTOBJ)->GetTakeUpChar();

	 if(pTCHAR)
	 {
		 if(((CTClientPet*)pTOBJ)->GetPetID() == 31 || ((CTClientPet*)pTOBJ)->GetPetID() == 32  )
			 vTPOS.y += 1.70f + pTCHAR->m_fSizeY * TMOUNTTEXT_SCALE;
		 else if(((CTClientPet*)pTOBJ)->GetPetID() == 35 || ((CTClientPet*)pTOBJ)->GetPetID() == 36 || ((CTClientPet*)pTOBJ)->GetPetID() == 37)
			 vTPOS.y += 1.30f + pTCHAR->m_fSizeY * TMOUNTTEXT_SCALE;
		 else if(((CTClientPet*)pTOBJ)->GetPetID() == 38)
			 vTPOS.y += 2.20f + pTCHAR->m_fSizeY * TMOUNTTEXT_SCALE;
		 else if (((CTClientPet*)pTOBJ)->GetPetID() == 39)
			 vTPOS.y += 2.6f + pTCHAR->m_fSizeY * TMOUNTTEXT_SCALE;
	...
	if( pTOBJ->GetDrawName() || (pTOBJ == m_pTARGET && pTOBJ->m_bType != OT_PC) )
	{
		...
		if(!pTHOST->IsDead())
		{
			if( pTHOST->m_bType == OT_PC )
			{
				...
				else if(!pTCHAR->m_strGUILD.IsEmpty() || IsInBOWMap() || IsInBRMap())
				{



					if(IsInBOWMap() || IsInBRMap())
						strTITLE = "4Story";
					else
		...
		//if(RenderTRANK( pTHOST, vTPOS, 32.0F, -fRIGHT))
		//	vTPOS.y += 32;
		...
		if(RenderTRANK( pTHOST, vTPOS, 32.0F, -8.0f))
			vTPOS.y += 36;
	}
...
BYTE CTClientGame::RenderTRANK( CTClientObjBase *pTOBJ, const D3DXVECTOR3 &vPROJ, FLOAT fBaseHeight, FLOAT fRight)
{
	LPDIRECT3DTEXTURE9 pTEX = NULL;
	if( pTOBJ->m_bType != OT_PC )
		return FALSE;

	D3DXVECTOR3 vBackProj = vPROJ;
	if (abs(vPROJ.x - vBackProj.x) > 0.075f ||
		abs(vPROJ.y - vBackProj.y) > 0.075f ||
		abs(vPROJ.z - vBackProj.z) > 0.075f)
	{
		vBackProj = vPROJ;
	}

	CTClientChar* pCHAR = dynamic_cast<CTClientChar*>(pTOBJ);
	if (!pCHAR)
		return FALSE;

	if ((IsInBRMap() && !IsInMyBRTeam(pCHAR->m_dwID)) || IsInBOWMap())
		return FALSE;

	BYTE bRank = CTChart::FindRank(pCHAR->m_dwRankPoint); bRank -= 1;
	pTEX = m_pRankTexture->GetTexture(bRank);
	if(!pTEX)
		return FALSE;
	...
	D3DXMatrixRotationX(
		...
		-D3DX_PI / 2.0f);
	vTICON *= vTROT;
	vTICON._42 = vBackProj.y + fBaseHeight;

	if(bRank < 35)
		vTICON._41 = vBackProj.x + fRight - 8.0f;
	else
	{
		vTICON._41 = vBackProj.x + fRight - 16.0f;
		vTICON._42 += 5.0f;
	}

	vTICON._43 = vBackProj.z;
	...
	case OT_PC	:
		{
			...
			else if(pTCHAR->m_pGuildMark && !IsInBOWMap() && !IsInBRMap())
...
void CTClientGame::RenderFLASH()
{
	...
	DWORD dwFlashColor = (m_pMainChar->GetFlashColor() | (DWORD(m_dwFLASHTick * 0xFF / TFLASH_TICK) << 24));
...
void CTClientGame::RenderTCMDDIR()
{
	...
	((CTCharNewDlg *) m_vTFRAME[TFRAME_CHARINFO_NEW])->EnableTLIGHT( &vCAM, TRUE);
	CTachyonMesh::BeginGlobalDraw(m_pDevice->m_pDevice);

	CTCustomCloakDlg* pDlgCustom = static_cast<CTCustomCloakDlg*>(m_vTFRAME[TFRAME_CUSTOMCLOAK]);
	pDlgCustom->EnableTLIGHT(&vCAM, TRUE);
	...
	((CTCharNewDlg *) m_vTFRAME[TFRAME_CHARINFO_NEW])->EnableTLIGHT( &vCAM, FALSE);
	CTachyonMesh::EndGlobalDraw(m_pDevice->m_pDevice);
...
void CTClientGame::CalcBREATH( DWORD dwTick)
{
	...
	{
		if(pTOBJ->m_bSwim != pTOBJ->m_bSwimBefore)
		{
			pTOBJ->m_bSwimBefore = pTOBJ->m_bSwim;
			if(m_pSESSION)
				m_pSESSION->SendCS_HIDECOMPANION_REQ(pTOBJ->m_bSwim);
		}
	}*/
}

void CTClientGame::CalcLAVA( DWORD dwTick)
{
	static DWORD dwDamageTick = 0;
	...
	if(pTOBJ->m_bSwimLava || pTOBJ->m_bInLava)
	{
		dwDamageTick += dwTick;
...
void CTClientGame::CalcCHARGE( DWORD dwTick)
{
	if (m_wActSkillID)
	{
		LPTSKILL pSKILL = CTChart::FindTSKILLTEMP(m_wActSkillID);
		if (pSKILL && pSKILL->m_bKind == 21)
		{
			if(m_bCHARGE)
			{
				...
				{
					if(pSKILL && (pSKILL->m_dwSpellTick ||
						!CTChart::GetTACTION(pSKILL->m_bActionID[TSKILLACTION_MOVEACTIVATE] )->m_bSubAction) && (
					...
					m_vTGAUGE[TGAUGE_CHARGE]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
						!m_dwMaxChargeTick ? 0 : m_dwChargeTick * 100 / m_dwMaxChargeTick);
				...
				else if(m_dwChargeTick >= m_dwMaxChargeTick)
				{
					m_vTGAUGE[TGAUGE_CHARGE]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
						100);
	...
	{
		if( pTSKILL && (pTSKILL->m_pTSKILL->m_dwSpellTick ||
...
void CTClientGame::CalcReuseTick( DWORD dwTick)
{
	...
	MAPSPOLECNIK::iterator itTSPOLECNIK;
	...
	for( itTSPOLECNIK = m_mapSLAVESPOLECNIK.begin(); itTSPOLECNIK != m_mapSLAVESPOLECNIK.end(); itTSPOLECNIK++)
		for( itTSKILL = (*itTSPOLECNIK).second->m_mapTSKILL.begin(); itTSKILL != (*itTSPOLECNIK).second->m_mapTSKILL.end(); itTSKILL++)
		{
			if( (*itTSKILL).second->m_dwTick && (*itTSKILL).second->m_bTimerON )
			{
				(*itTSKILL).second->m_dwTick = (*itTSKILL).second->m_dwTick > dwTick ? (*itTSKILL).second->m_dwTick - dwTick : 0;
...
void CTClientGame::CalcMainChar( DWORD dwTick)
{
	...
	if (bSIDE)
	{
		...
		if ((m_pMainChar->m_dwActionID == 26846 && m_pMainChar->m_dwAniID == 26848) || (m_pMainChar->m_dwActionID == 25118 && m_pMainChar->m_dwAniID == 25166) ||
			(m_pMainChar->m_dwActionID == 28310 && m_pMainChar->m_dwAniID == 27562))
			return;
	}
	...
	{
		LPTQUEST pTMISSION = nullptr;

		
		for (const auto& QClass : m_pMainChar->m_mapTQCLASS)
			for (const auto& Quest : QClass.second->m_mapTQUEST)
			{
				if (!Quest.second->m_pTQUEST)
					continue;

				pTMISSION = Quest.second->m_pTQUEST;
				...
				for (const auto& Term : pTMISSION->m_vTTERMPOS)
				{
					
					if ((Term->m_dwID == 666 || Term->m_dwID == 667) && Term->m_bType == QTT_HUNT)
					{

					
						BOOL Continue = FALSE;
						
						for (auto QTerm : Quest.second->m_vTTERM)
						{
							if (QTerm->m_pTTERM && QTerm->m_pTTERM->m_dwID == Term->m_dwID)
								if (QTerm->m_bCount)
								{
									Continue = TRUE;
									break;
						...
						D3DXMATRIX mat;
						mat._41 = Term->m_fPosX;
						mat._42 = Term->m_fPosY;
						mat._43 = Term->m_fPosZ;

						if (GetDifferencial(m_pMainChar->m_pRidingPet ? m_pMainChar->m_pRidingPet->GetPositionMat() : m_pMainChar->GetPositionMat(), mat) < 8.0f)
						{

							if (m_pSESSION)
								m_pSESSION->SendCS_QUESTPOSEXEC_REQ(pTMISSION->m_dwID, Term->m_dwID);
						}
					}
				}
			}
...
FLOAT CTClientGame::CheckCamPos(DWORD dwTick)
{
	...
	FLOAT fLavaHeight = FLOAT(INT(100.0f * m_vMAP.GetLavaHeight(
		vPOS.x,
		vPOS.z))) / 100.0f;

	if (fLavaHeight > fWaterHeight)
		fWaterHeight = fLavaHeight;
		...
		default					:
			{
				...
				if(dwCOLOR)
				{
					...
					pTSFX->m_fSCALE = bCritical ? 1.5f : 1.0f;
	...
	if( m_pTAUTOPATH && CanLookAt() && m_pMainChar->CanAutoPath() )
	{
		...
		if( bUPDATE && !m_bTAUTOPAUSE )
		{
			...
			else
			{
				...
				if(pTARGET->m_bNPCType == TNPC_AUCTION)
	...
	while(itTARROW != m_vTARROW.end())
	{
		...
		if(pTARGET)
		{
			...
			else
			{
				...
				{
					BOOL bFound = FALSE;
					for (BYTE i = 0; i < SKILL_EXCEPTIONS_COUNT; ++i)
						if (m_wSkillSystemExceptions[i] == pTSKILL->m_wSkillID)
						{
							bFound = TRUE;
							break;
						}
				...
				//	if ((pTHOST->m_bType == OT_PC || pTHOST->m_bType == OT_SELF) && !bFound)
										if ((pTHOST->m_bType == OT_PC || pTHOST->m_bType == OT_RECALL || pTHOST->m_bType == OT_SELF) && !bFound)
					{
						VTSKILLTARGET vSKILLTARGET; vSKILLTARGET.clear();
						TSKILLTARGET SKILLTARGET;

						SKILLTARGET.m_dwID = pTARGET->m_dwID;
						SKILLTARGET.m_bType = pTARGET->m_bType;
						vSKILLTARGET.push_back(&SKILLTARGET);

						FinishSkill(pTHOST, &pTARROW->m_vTSKILLDATA.m_vTGROUND, pTSKILL, vSKILLTARGET);
					}
					else
...
CTClientObjBase *CTClientGame::SeekSpolecnikTarget( CTClientSpolecnik *pSPOLECNIK)
{
	CTClientObjBase *pTARGET = NULL;
	FLOAT fDIST = 0.0f;
		...
		for(; itPATHMON != end ; ++itPATHMON )
		{
				...
				fDist - fAB > 0.125f )
			{
				...
				if( index >= 0 && index < ADJACENCY_COUNT )
				{
					...
					else if( vCROSS.y < 0.0f ) //���� ���Ϳ��� ���ʿ� ������ ũ�ν� ��������� Y���� ������.
									...
									DWORD dwTick)
{
	...
	//		pTRECALL->m_pTCURSKILL = pTRECALL->GetBestTSKILL(pTARGET);
		...
		if (!pTRECALL->m_pTCURSKILL || !pTRECALL->m_pTCURSKILL->m_pTSKILL)
			return;
...
void CTClientGame::CalcSLAVESPOLECNIK( DWORD dwTick)
{
	static DWORD dwPATH = 0;
	static DWORD dwSYNC = 0;
	static DWORD dwUPDATE = GetTickCount();

	MAPSPOLECNIK::iterator itTSPOLECNIK;

	D3DXMATRIX vTROT;
	VTMONMOVE vTMONMOVE;
	VTSPOLECNIK vTATTACK;
	...
	for( itTSPOLECNIK = m_mapSPOLECNIK.begin(); itTSPOLECNIK != m_mapSPOLECNIK.end(); itTSPOLECNIK++)


		if( !(*itTSPOLECNIK).second->IsDead() &&
			!(*itTSPOLECNIK).second->IsBlock() &&
			!(*itTSPOLECNIK).second->IsPush() )
		{
			CTClientSpolecnik *pTRECALL = (*itTSPOLECNIK).second;
			//--------------------------------------------

			CTClientObjBase *pTARGET = FindPC((*itTSPOLECNIK).second->m_dwHostID);
			if (!pTARGET) continue;


			D3DXMatrixRotationY(
				&vTROT,
				FLOAT(pTARGET->m_wDIR) * D3DX_PI / 900.0f);

			D3DXVECTOR3 vTPOS = pTRECALL->GetPosition();
			BYTE bROAM = TRUE;

			if( !pTARGET && pTRECALL->m_bAI == TRECALLAI_ACTIVE )
				pTARGET = SeekSpolecnikTarget(pTRECALL);

			if( pTARGET && (pTARGET->IsDead() || (pTARGET->m_bHide && !pTARGET->m_bDetect)) )
				pTARGET = NULL;
			...
			if(bROAM && pTARGET)
			{
				...
				FLOAT fStandBound = 2.0f;//1.55f;

				if( pTRECALL->m_bTrans )
					fStandBound = 0.8f;

				if( bActionID != TA_STAND || fDIST > fStandBound * fStandBound || fDIST < fRadius * fRadius )
				...
				if(bCORRECT || bPATH )
					...
					else
					{
						...
						if( fabs(fHEIGHT - vTARGET.y) > pTARGET->m_fSizeY )
						{
						...
						if(pTARGET->IsFlyOBJ() && !pTRECALL->m_bTrans)
							vTARGET.y = max( vTARGET.y, pTARGET->GetPositionY() + TPATH_HEIGHT) + pTRECALL->m_fBreathHeight;
					...
					if (pTARGET == m_pMainChar && GetTickCount() - dwUPDATE > 1000)
					{
						dwUPDATE = GetTickCount();
						SendCS_MONMOVE_REQ(
							pTRECALL,
							m_pMainWnd->m_bChannel, 
							m_vMAP.m_wMapID, 
...
void CTClientGame::CalcSPOLECNIK( DWORD dwTick)
{
	BYTE bRESET = FALSE;	

	MAPSPOLECNIK::iterator itSPOLECNIK = m_mapSPOLECNIK.begin();
	while( itSPOLECNIK != m_mapSPOLECNIK.end() )
	{
		CTClientSpolecnik* pSpolecnik = (*itSPOLECNIK).second;

		if( !pSpolecnik->m_bDEAD )
		{
			if( m_pCtrlHost != pSpolecnik )
				CalcSPOLECNIK( pSpolecnik, dwTick);				

			++itSPOLECNIK;
		}
		else
		{
			bRESET = DeleteSpolecnik(pSpolecnik->m_dwID, false);
			itSPOLECNIK = m_mapSPOLECNIK.erase(itSPOLECNIK);
		}
	}

}

void CTClientGame::CalcSPOLECNIK( CTClientSpolecnik *pTSPOLECNIK,
							   DWORD dwTick)
{
	BYTE bHandling = IsHostingOBJ( pTSPOLECNIK->m_dwID, pTSPOLECNIK->m_bType);
	LPTREGIONINFO pTREGION = pTSPOLECNIK->m_pTREGION;
	D3DXVECTOR3 vPOS = pTSPOLECNIK->GetPosition();

	pTSPOLECNIK->CalcDIR(
		dwTick,
		pTSPOLECNIK->m_bMouseDIR,
		pTSPOLECNIK->m_bKeyDIR);

	pTSPOLECNIK->CalcMaintain(
		dwTick,
		bHandling,
		this);
	...
	if( IsCtrlSPOLECNIK(pTSPOLECNIK) && (bHandling || pTSPOLECNIK->m_bBlock) )
	{
		D3DXVECTOR3 vTNEXT = pTSPOLECNIK->GetPosition();
		FLOAT fNextH = vPOS.y;

		pTSPOLECNIK->ForecastHeight(
			&m_vMAP,
			&fNextH,
			NULL,
			NULL,
		...
		if( pTSPOLECNIK == m_pCtrlHost )
			CheckTPORTAL( pTSPOLECNIK, &vPOS, &vTNEXT, fNextH);
		...
		if(bBLOCK)
		{
			pTSPOLECNIK->m_vWorld._41 += vTNEXT.x - pTSPOLECNIK->GetPositionX();
			pTSPOLECNIK->m_vWorld._43 += vTNEXT.z - pTSPOLECNIK->GetPositionZ();
			...
			if( bHandling && !m_pMainChar->m_bGhost && !pTSPOLECNIK->m_bBlock )
			{
				pTSPOLECNIK->m_bBlock = TRUE;
				...
				SendCS_BLOCK_REQ(
					...
					pTSPOLECNIK->m_wPITCH,
					pTSPOLECNIK->m_wDIR,
					pTSPOLECNIK->m_bAction,
					pTSPOLECNIK->m_bBlock);

				if(GetPartyID(m_pMainChar))
					m_pMainChar->m_bTCORPSMOVE = TCORPSMOVE_UNIT;
			}
		}
		else if( bHandling && !m_pMainChar->m_bGhost && pTSPOLECNIK->m_bBlock )
		{
			pTSPOLECNIK->m_bBlock = FALSE;
	...
	else if( pTSPOLECNIK->m_bSlide || pTSPOLECNIK->IsPush() )
	{
		FLOAT fNextH = vPOS.y;
		D3DXVECTOR3 vTNEXT;
		...
		if(m_vMAP.CheckCollision( pTSPOLECNIK, &vTNEXT, &vPOS, fNextH))
		{
			pTSPOLECNIK->m_vWorld._41 += vTNEXT.x - pTSPOLECNIK->GetPositionX();
			pTSPOLECNIK->m_vWorld._43 += vTNEXT.z - pTSPOLECNIK->GetPositionZ();
	...
	if(bHandling)
	{
		if( pTSPOLECNIK->m_pTREGION != pTREGION )
		{
			SendCS_REGION_REQ(
				pTSPOLECNIK->m_dwID,
				pTSPOLECNIK->m_bType,
				m_pMainWnd->m_bChannel,
				m_vMAP.m_wMapID,
				pTSPOLECNIK->m_pTREGION->m_dwID,
				pTSPOLECNIK->m_pTREGION->m_wLocalID);
		}

		

		if(pTSPOLECNIK->m_bDIE)
			m_pSESSION->SendCS_DELSPOLECNIKMON_REQ(pTSPOLECNIK->m_dwID, pTSPOLECNIK->m_bType);
	}

	if(pTSPOLECNIK->m_bDIE)
		pTSPOLECNIK->OnDIE( m_pDevice, m_pRES);

	pTSPOLECNIK->m_dwDropDamage = 0;
	pTSPOLECNIK->m_bDIE = FALSE;
...
void CTClientGame::CalcSelectCharacter(DWORD dwTick)
{
	...
	else
	{
		...
		if( m_dwLeftTickCHAR/TNOTIFY_PERIOD != dwLEFT/TNOTIFY_PERIOD )
			...
			m_pOutro->UpdateTick(m_dwLeftTickCHAR);
...
void CTClientGame::CalcExitGame(DWORD dwTick)
{
	...
	else
	{
		...
		if( m_dwLeftTickEXIT/TNOTIFY_PERIOD != dwLEFT/TNOTIFY_PERIOD )
			...
			m_pOutro->UpdateTick(m_dwLeftTickEXIT);
...
int CTClientGame::OnCommand( TCOMMAND Command)
{
	...
	ON_GM_COMMAND(GM_USEITEM_MOVETOCHAR_FROMTARGET)
	...
	ON_GM_COMMAND(GM_CLOSE_BOX)
	ON_GM_COMMAND(GM_TOGGLE_LIST)
	...
	ON_GM_COMMAND(GM_GMP_VOLUNTEER_WOODLAND)
	...
	ON_GM_COMMAND(GM_PET_EFFECTCHANGE)
	ON_GM_COMMAND(GM_PET_CHANGEEFFECT)
	ON_GM_COMMAND(GM_PET_EFFECTDELETE)
	ON_GM_COMMAND(GM_PET_DELETEEFFECT)
	...
	ON_GM_COMMAND(GM_GVFPM_MAIL2)
	ON_GM_COMMAND(GM_GMP_WHISPER2)
	...
	ON_GM_COMMAND(GM_COMPANION_DELETEREQ)
	ON_GM_COMMAND(GM_COMPANION_DELETE)
	ON_GM_COMMAND(GM_GSKILL_RENEW_YES)
...
//	ON_GM_COMMAND( GM_TEXTOOL_COMPO )
	...
	ON_GM_COMMAND(GM_USE_SADDLE)
	ON_GM_COMMAND(GM_UB_SHOP_1)
	ON_GM_COMMAND(GM_UB_SHOP_2)
	ON_GM_COMMAND(GM_UB_SHOP_3)
	ON_GM_COMMAND(GM_UB_SHOP_4)
	ON_GM_COMMAND(GM_UB_SHOP_5)
	ON_GM_COMMAND(GM_UB_SHOP_6)
	ON_GM_COMMAND(GM_UB_SHOP_7)
	ON_GM_COMMAND(GM_UB_SHOP_8)
	ON_GM_COMMAND(GM_UB_SHOP_9)
	ON_GM_COMMAND(GM_UB_SHOP_10)
	ON_GM_COMMAND(GM_UB_SHOP_CANCEL)
	ON_GM_COMMAND(GM_OPEN_BOX)
#ifdef ADD_SKYGARDEN
	...
	ON_GM_COMMAND(GM_LEVEL1)
	ON_GM_COMMAND(GM_LEVEL2)
	ON_GM_COMMAND(GM_LEVEL3)
	ON_GM_COMMAND(GM_CANCEL_VETERAN)
	ON_GM_COMMAND(GM_OPEN_MENU_COMMUNITY)
	ON_GM_COMMAND(GM_OPEN_MENU_WAR)
	ON_GM_COMMAND(GM_OPEN_MENU_MESSENGER)
	ON_GM_COMMAND(GM_OPEN_MENU_CHAR)
	ON_GM_COMMAND(GM_OPEN_MENU_INVEN)
	ON_GM_COMMAND(GM_OPEN_MENU_SYSTEM)
	ON_GM_COMMAND(GM_OPEN_SOUL_LOTTERY)
	ON_GM_COMMAND(GM_SOUL_LOTTERY_TRIGGER)
	ON_GM_COMMAND(GM_SC_LEFTBTN)
	ON_GM_COMMAND(GM_SC_MIDDLEBTN)
	ON_GM_COMMAND(GM_SC_RIGHTBTN)
	ON_GM_COMMAND(GM_TOGGLE_TWITTER)
	ON_GM_COMMAND(GM_TOGGLE_FACEBOOK)
	ON_GM_COMMAND(GM_CREATE_COMPANION)
	ON_GM_COMMAND(GM_COMPANION_EFFECT)
	ON_GM_COMMAND(GM_COMPANION_EFFECTREQ)
	ON_GM_COMMAND(GM_WARN_COMPITEM)
	ON_GM_COMMAND(GM_TOGGLE_COMPANION)
	ON_GM_COMMAND(GM_USE_POWDER)
	ON_GM_COMMAND(GM_TOGGLE_CLOAKHIDE)
	ON_GM_COMMAND(GM_TOGGLE_TITLE)
	ON_GM_COMMAND(GM_CANCEL_POST)
	ON_GM_COMMAND(GM_CANCEL_POST_CONFIRM)
	ON_GM_COMMAND(GM_USE_MEDALS)

	ON_GM_COMMAND(GM_OPEN_REGISTER_UI)
	ON_GM_COMMAND(GM_BOW_REGISTER)

	ON_GM_COMMAND(GM_CANCEL_BOW_QUEUE)
	ON_GM_COMMAND(GM_CANCEL_BOW_QUEUE_CONFIRM)
	ON_GM_COMMAND(GM_BOW_CASH_RESPAWN)
	ON_GM_COMMAND(GM_BP_EXCHANGE)
	ON_GM_COMMAND(GM_BP_EXCHANGE_CONFIRM)

	ON_GM_COMMAND(GM_BRTEAMMATEADD_REQ)
	ON_GM_COMMAND(GM_BRTEAMMATEADD_ANSWER)

	ON_GM_COMMAND(GM_BATTLERANK_TOGGLE)

	ON_GM_COMMAND(GM_DONATE_LIFE)
	ON_GM_COMMAND(GM_USE_COMPRESET)
	ON_GM_COMMAND(GM_TOGGLE_GUILD_BASE_NEW)
	ON_GM_COMMAND(GM_TOGGLE_CUSTOMCLOAK)
	ON_GM_COMMAND(GM_CUSTOMCLOAK_WARN)
	ON_GM_COMMAND(GM_CUSTOMCLOAK_PREVIEW)
	ON_GM_COMMAND(GM_CUSTOMCLOAK_APPLY)
	ON_GM_COMMAND(GM_ARENA_TOP)
...
int CTClientGame::OnSessionMsg( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	ON_TRECEIVE(CS_SOULUP_ACK)
	ON_TRECEIVE(CS_SOULLOTTERY_ACK)
	ON_TRECEIVE(CS_SOULLOTTERY_TRIGGER_ACK)
	ON_TRECEIVE(CS_WHIP_ACK)
	...
	ON_TRECEIVE(CS_ADDSPOLECNIKMON_ACK)
	ON_TRECEIVE(CS_DELSPOLECNIKMON_ACK)	
	...
	ON_TRECEIVE(CS_PETEFFECTCHANGE_ACK)
	...
	// Title Message
	ON_TRECEIVE(CS_TITLELIST_ACK)
	ON_TRECEIVE(CS_TITLEGAIN_ACK)
	ON_TRECEIVE(CS_TITLERESET_ACK)
	...
	ON_TRECEIVE(CS_LEAVESKYGARDEN_ACK)
	ON_TRECEIVE(CS_SKYGARDEN_OCCUPY_LEFT_ACK)
	ON_TRECEIVE(CS_SKYGARDEN_OCCUPY_CENTER_ACK)
	ON_TRECEIVE(CS_SKYGARDEN_OCCUPY_RIGHT_ACK)
	...
	ON_TRECEIVE( CS_EXITTOURNAMETMAINTAIN_ACK )
	ON_TRECEIVE( CS_EXITTOURNAMETHOTKEY_ACK )
...
#endif
	ON_TRECEIVE(CS_GETREWARDS_ACK)

	 //TSecuritysystem
	 ON_TRECEIVE( CS_SC_CREATECODE_ACK )
	 ON_TRECEIVE( CS_SC_CHANGECODE_ACK )
	 ON_TRECEIVE( CS_SC_DISABLECODE_ACK )
	 ON_TRECEIVE( CS_SC_ENABLECODE_ACK )
	 ON_TRECEIVE( CS_SC_UNLOCK_ACK )
	 ON_TRECEIVE( CS_SC_CHECKCODE_ACK )
	 ON_TRECEIVE( CS_SC_GETLOCKTIME_ACK )

	 ON_TRECEIVE( CS_COMPANIONLIST_ACK )
	 ON_TRECEIVE( CS_COMPANIONUPDATE_REQ )
     ON_TRECEIVE( CS_COMPANIONLUPDATE_REQ )
	 ON_TRECEIVE( CS_UPDATECOMPANIONBYITEM_REQ )
	 ON_TRECEIVE( CS_UPDATECOMPANIONBYSYSTEM_REQ )
	 ON_TRECEIVE( CS_UPDATESPAWNEDCOMPANION_REQ )
	 ON_TRECEIVE( CS_CREATECOMPANION_ACK )
	 ON_TRECEIVE( CS_UPDATECOMPANIONITEMS_REQ )
	 ON_TRECEIVE( CS_UPDATECOMPANIONBONUS_REQ )
	 ON_TRECEIVE( CS_UPDATEMEDALS_REQ )

	 ON_TRECEIVE(CS_BOWTIMEUPDATE_ACK)
	 ON_TRECEIVE(CS_REGISTERBOW_ACK)
	 ON_TRECEIVE(CS_RELEASEFORBOW_REQ)
	 ON_TRECEIVE(CS_CANCELBOWQUEUE_ACK)
	 ON_TRECEIVE(CS_FORWARDBOWCHAT_ACK)
	 ON_TRECEIVE(CS_BOWNOTIFY_ACK)
	 ON_TRECEIVE(CS_PINGMEASUREMENT_ACK)
	 ON_TRECEIVE(CS_NOTIFYNONQUEUEDPLAYER_ACK)
	 ON_TRECEIVE(CS_UPDATEBONUSPOINT_ACK)
	 ON_TRECEIVE(CS_BOWKILLNOTIFY_ACK)
	 ON_TRECEIVE(CS_BOWRANKING_ACK)
	 ON_TRECEIVE(CS_LEAVEBATTLEFIELD_ACK)

	 ON_TRECEIVE(CS_BRTIMEUPDATE_ACK)
	 ON_TRECEIVE(CS_BRTEAMMATEADD_ACK)
	 ON_TRECEIVE(CS_UPDATEBRTEAM_ACK)
	 ON_TRECEIVE(CS_UPDATEBRMATE_ACK)
	 ON_TRECEIVE(CS_INITBRTRANKING_ACK)
	 ON_TRECEIVE(CS_UPDATEBRTPLAYER_ACK)
	 ON_TRECEIVE(CS_ADDTOBRQUEUE_ACK)
	 ON_TRECEIVE(CS_UPDATEREMAININGPLAYER_ACK)
     ON_TRECEIVE(CS_CUSTOMCLOAK_LIST_ACK)
	 ON_TRECEIVE(CS_CUSTOMCLOAK_CHANGE_ACK)

	 ON_TRECEIVE(CS_GUILDSKILLACTION_ACK)
	 ON_TRECEIVE(CS_GUILDSKILLUPDATE_ACK)
	 ON_TRECEIVE(CS_ARENAINFO_ACK)
	 ON_TRECEIVE(CS_ARENARANKING_ACK)
	default	: break;
	}
...
void CTClientGame::ReleaseGame()
{
	...
	static_cast<CBRTRanking*>(m_vTFRAME[TFRAME_BRTRANKING])->ReleaseTeams();
	static_cast<CBRTeamDlg*>(m_vTFRAME[TFRAME_BRTEAMS])->LeaveTeam();
	SiegeMgr->Release();
	...
	for( itSPOLECNIK = m_mapSPOLECNIK.begin(); itSPOLECNIK != m_mapSPOLECNIK.end(); itSPOLECNIK++)
		delete (*itSPOLECNIK).second;
	m_mapSLAVESPOLECNIK.clear();
	m_mapSPOLECNIK.clear();
	...
	while (!m_vBRTEAM.empty())
	{
		delete m_vBRTEAM.back();
		m_vBRTEAM.pop_back();
	}
	...
	m_bBRStatus = BS_NORMAL;

	ResetDoEXIT();
	ClearTASSITANT();
	/*m_bChannelID = 1;
	GetMainWnd()->m_bChannel = 1;*/
}
...
BOOL CTClientGame::IsItemUI(CTClientUIBase* pUI)
{
	if( m_vTFRAME[TFRAME_TRADE] == pUI ||
		...
		m_vTFRAME[TFRAME_CHARINFO_NEW] == pUI ||
...
void CTClientGame::DoHotKey(THOTKEY_BASE eTYPE, BYTE bHotkeyID)
{
	...
	if( m_bRPSGaming && m_bRPSKeyEnable )
	{
		switch(bHotkeyID)
		{
		case 0: GetSession()->SendCS_RPSGAME_REQ( RPSCARD_SCISSORS ); break; //1Ű
		case 1: GetSession()->SendCS_RPSGAME_REQ( RPSCARD_ROCK ); break; //2Ű
		case 2: GetSession()->SendCS_RPSGAME_REQ( RPSCARD_PAPER ); break; //3Ű
		}
		return ;
	}
#endif
...
#ifdef NEW_IF
	else if(bFrameID == TFRAME_POTIONS) //VALLAH
		return(BYTE)T_INVALID;
#endif
	else if(bFrameID == TFRAME_SOULLOTTERY)
	
		return (BYTE)T_INVALID;
	
	else if(bFrameID == TFRAME_BOW_WAITING)
		return (BYTE) T_INVALID;
	else if(bFrameID == TFRAME_BOW_RESPAWN)
		return (BYTE) T_INVALID;
	else if(bFrameID == TFRAME_PREMIUMSKILLS)
		return (BYTE) T_INVALID;
	else if(bFrameID == TFRAME_BOWSKILLS)
		return (BYTE) T_INVALID;
	else if(bFrameID == TFRAME_BRTRANKING)
		return (BYTE) T_INVALID;
	else if(bFrameID == TFRAME_BRPANNELS)
		return (BYTE) T_INVALID;
	else if(bFrameID == TFRAME_BRTEAMS)
		return (BYTE) T_INVALID;
...
BYTE CTClientGame::GetTopFrame()
{
	...
	for(; itr!=m_kids.rend(); ++itr)
	{
		...
		if( pKid->IsVisible() &&
			...
			pKid != m_vTFRAME[TFRAME_EQUIP_DETAIL_INFO] &&
			pKid != m_vTFRAME[TFRAME_EQUIP_DETAIL_INFO_2] &&
...
void CTClientGame::OnActivateKEY( WORD wSCAN)
{
	...
	case TKEY_HOTKEY_ACTION		: DoDEFACTION(TRUE); break;
	case TKEY_TRSCS_ME			:
	case TKEY_TRSCS_F1			:
	case TKEY_TRSCS_F2			:
	case TKEY_TRSCS_F3			:
	...
	case TKEY_HOTKEY_F1			: 
		{
			SelectHotKey(THOTKEYBASE_DEF, 0);
			SelectHotKey(THOTKEYBASE_SUB1, 1);
		}
		break;
	case TKEY_HOTKEY_F2			: 
		{
			SelectHotKey(THOTKEYBASE_DEF, 2); 
			SelectHotKey(THOTKEYBASE_SUB1, 3);
		}
		break;
	case TKEY_HOTKEY_F3			: 
		{
			SelectHotKey(THOTKEYBASE_DEF, 4); 
			SelectHotKey(THOTKEYBASE_SUB1, 5);
		}
		break;
	case TKEY_HOTKEY_F4			: 
		{
			SelectHotKey(THOTKEYBASE_DEF, 6);
			SelectHotKey(THOTKEYBASE_SUB1, 7);
		}
		break;
	case TKEY_HOTKEY_F5			: 
		{
			SelectHotKey(THOTKEYBASE_DEF, 8); 
			SelectHotKey(THOTKEYBASE_SUB1, 9);
		}
		break;
	...
	case TKEY_PSKILL1           : UsePremiumSkill(0); break;
	case TKEY_PSKILL2           : UsePremiumSkill(1); break;
	case TKEY_PSKILL3           : UsePremiumSkill(2); break;
	case TKEY_PSKILL4           : UsePremiumSkill(3); break;
	case TKEY_PSKILL5           : UsePremiumSkill(4); break;
	case TKEY_PSKILL6           : UsePremiumSkill(5); break;
	case TKEY_PSKILL7           : UsePremiumSkill(6); break;
    case TKEY_PSKILL8           : UsePremiumSkill(7); break;
	case TKEY_PSKILL9           : UsePremiumSkill(8); break;
	case TKEY_PSKILL10          : UsePremiumSkill(9); break;
	case TKEY_PSKILL11          : UsePremiumSkill(10); break;
	case TKEY_PSKILL12          : UsePremiumSkill(11); break;
	case TKEY_PICK_NEXT			: PickNext(TPICK_ALIVE_ALL); break;
	case TKEY_PICK_PLAYER       : PickNext(TPICK_ALIVE_PLAYER); break;
	...
	case TKEY_MODE				:
		...
		{
			if( GetTickCount() - m_pMainChar->m_dwReskillTick < 230 || m_bCHARGE)
				return;
...
case TKEY_COMPANION:
		{
			OnGM_TOGGLE_COMPANION();
		}
		break;

	case TKEY_TOGGLE_GUILD_UI	:
		{
			OnGM_TOGGLE_GUILD_BASE_NEW();
		}
		break;
	...
	case TKEY_RUN:
		{
			if( GetTickCount() - m_pMainChar->m_dwWalkTick < 250 ||
				GetTickCount() - m_pMainChar->m_dwReskillTick < 500 )
				return;

			m_pMainChar->m_bRun = !m_pMainChar->m_bRun;
			m_pMainChar->m_dwWalkTick = GetTickCount();
		}
		break;
...
#ifdef NEW_IF
		{
			CTRankingNewMain *pDlgMain = static_cast<CTRankingNewMain*>( m_vTFRAME[TFRAME_RANKING_NEW] );
...
void CTClientGame::OnRegionChanged()
{
	...
	if(m_pMainChar->m_pTREGION)
	{
		...
		if(IsInBRMap())
			strMSG.Replace("(Mission)", "");

		CString strReplacedMsg = strMSG;
...
#ifdef NEW_IF
		CTGaugePannel* pTGaugePannel = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE]);
		pTGaugePannel->m_pRegionNew->m_strText = strReplacedMsg;
#else
	...
	if (m_vMAP.m_wMapID != SKYGARDEN_MAPID)
	{
		CTSkygardenMap* pSkygardenMapUI = static_cast<CTSkygardenMap*>(m_vTFRAME[TFRAME_SKYGARDEN_MAP]);
		pSkygardenMapUI->ShowComponent(false);
		pSkygardenMapUI->EnableComponent(false);
	}
}
										   ...
										   BYTE bType)
{
	switch(bType)
	{
	case OT_COMPANION: return FindSpolecnik(dwID);
	...
	//case OT_PET		: return static_cast<CTClientPet*>( FindRecall(dwID) );
	}
...
CTClientRecall *CTClientGame::FindSlaveCompanion( DWORD dwID)
{
	MAPRECALL::iterator finder = m_mapSLAVECOMPANION.find(dwID);

	if( finder != m_mapSLAVECOMPANION.end() )
		return (*finder).second;

	return NULL;
}
CTClientSpolecnik *CTClientGame::FindSlaveSpolecnik( DWORD dwID)
{
	MAPSPOLECNIK::iterator finder = m_mapSLAVESPOLECNIK.find(dwID);

	if( finder != m_mapSLAVESPOLECNIK.end() )
		return (*finder).second;
...
void CTClientGame::FindSlaveSpolecnikByHost( DWORD dwHostID, LPVTSPOLECNIK vTSPOLECNIK )
{
	MAPSPOLECNIK::iterator it, end;
	it = m_mapSLAVESPOLECNIK.begin();
	end = m_mapSLAVESPOLECNIK.end();
	for(; it != end ; ++it )
	{
		CTClientSpolecnik* pTRECALL = it->second;
		if( pTRECALL )
		{
			if( pTRECALL->m_dwHostID == dwHostID )
				vTSPOLECNIK->push_back( pTRECALL );
		}
	}
}
...
CTClientSpolecnik *CTClientGame::FindSpolecnik( DWORD dwID)
{
	MAPSPOLECNIK::iterator finder = m_mapSPOLECNIK.find(dwID);

	if( finder != m_mapSPOLECNIK.end() )
		return (*finder).second;

	return NULL;
}

CTClientSpolecnik *CTClientGame::FindSpolecnikByMon( WORD wID)
{
	MAPSPOLECNIK::iterator it, end;
	it = m_mapSPOLECNIK.begin();
	end = m_mapSPOLECNIK.end();
	for(; it != end ; ++it )
	{
		CTClientSpolecnik* pSpolecnik = it->second;
		if(pSpolecnik)
			if(pSpolecnik->m_wMonID ==wID)
				return it->second;
...
void CTClientGame::FindSpolecnikByHost( DWORD dwHostID, LPVTSPOLECNIK vTRECALL )
{
	MAPSPOLECNIK::iterator it, end;
	it = m_mapSPOLECNIK.begin();
	end = m_mapSPOLECNIK.end();
...
CTClientSpolecnik* CTClientGame::FindSpolecnikByMonAndHost( DWORD dwHostID, WORD wMonID )
{
	MAPSPOLECNIK::iterator it, end;
	it = m_mapSPOLECNIK.begin();
	end = m_mapSPOLECNIK.end();
	for(; it != end ; ++it )
	{
		CTClientSpolecnik* pTRECALL = it->second;
		if( pTRECALL )
			if( pTRECALL->m_dwHostID == dwHostID && pTRECALL->m_wMonID == wMonID )
				return pTRECALL;
	}
...
CTClientSpolecnik *CTClientGame::FindSlaveFixSpolecnik( DWORD dwID)
{
	
	return NULL;
}
CTClientSpolecnik *CTClientGame::FindFixSpolecnik( DWORD dwID)
{	
	return NULL;
}
...
TEdit *CTClientGame::GetCurEdit()
{
	...
	pEdit = static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS])->GetTextBoxContent();
	if( pEdit )
		return pEdit;
	...
	pEdit = static_cast< CTGuildMainNew* >( m_vTFRAME[ TFRAME_GUILD_BASE_NEW ] )->GetCurEdit();
	if( pEdit )
		return pEdit;

#ifdef NEW_IF
	pEdit = m_pChatFrame->GetChatEditMenu();
	if( pEdit )
		return pEdit;
#endif
...
BYTE CTClientGame::RotateCamera( CPoint &point, CPoint &pt, BYTE bMOVE )
{
	...
	if(m_pMainChar->CountMaintainFunc(0, SDT_RUNAWAY, 1))
		bMOVE = FALSE;
	else
		bMOVE = TRUE;
	return bMOVE;
...
void CTClientGame::OnChar( UINT nChar, int nRepCnt, UINT nFlags)
{
	...
	else if( wKEY == TKEY_CHAT )
	{
		...
		if (CTMessengerChat::GetCurMsgChatEdit())
			bChat = FALSE;
#ifdef NEW_IF
		else if (m_pChatFrame->GetChatEditMenu())
		{
			m_pChatFrame->GetChatMenu()->SetTitle();
			bChat = FALSE;
		}
#endif
			...
			bChat = FALSE;
		else if (static_cast<CTGuildMainNew*>(m_vTFRAME[TFRAME_GUILD_BASE_NEW])->GetCurEdit())
			bChat = FALSE;
		else if (static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS])->m_pMiddleBtn->IsVisible() && 
			static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS])->IsVisible())
			bChat = FALSE;
#ifdef ADD_TEXTOOL
...
#ifdef NEW_IF
	else if( pTEDIT && pTEDIT == m_pChatFrame->GetChatEditMenu() )
	{
		if(pTEDIT->IsVisible())
			pTEDIT->OnChar( nChar, nRepCnt, nFlags);
	}
#endif
...
void CTClientGame::OnKeyDown( UINT nChar, int nRepCnt, UINT nFlags)
{
	...
	{
		if(pTEDIT->IsVisible())
			pTEDIT->OnKeyDown(nChar,nRepCnt,nFlags);
	}
#endif
	if(!CanProcess())
		return;
...
void CTClientGame::OnLButtonDown( UINT nFlags, CPoint pt)
{
	...
	{
		if(m_pTRSCS->m_pRaidNew->HitTest(pt))
		{
			m_pTRSCS->OnLButtonDown(nFlags, pt);
		}
	}
#endif
...
#ifdef NEW_IF
	if( !pChatEdit )
		pChatEdit = m_pChatFrame->GetChatEditMenu();
#endif
	...
	if( !pChatEdit )
		pChatEdit = static_cast< CTGuildMainNew* >( m_vTFRAME[TFRAME_GUILD_BASE_NEW] )->GetCurEdit();
	...
	else
	{
		...
		if( m_pTRSCS->IsVisible() && m_pTRSCS->OnBeginDrag(NULL, pt) )
			m_bNeedDrag = TRUE;
		m_ptCurLBDown = pt;
	}
}
...
void CTClientGame::OnLButtonDblClk( UINT nFlags, CPoint pt)
{
	...
	if( bTFRAME < TFRAME_COUNT )
	{
		...
		case TFRAME_INVEN_5		:
			{
				...
				if(pTINVEN)
				{
					...
					BYTE j = 0;
					if(bTFRAME == TFRAME_BATTLEINVEN)
					{
						for(BYTE l = 0; l < BATTLEINVEN_SLOT_COUNT * 2; ++l)
						{
							TImageList* pTSLOT = nullptr;
							if(l < BATTLEINVEN_SLOT_COUNT)
								pTSLOT = dynamic_cast<TImageList*>(m_vTFRAME[bTFRAME]->FindKid(m_vBattleSlotID[l]));
							else
								pTSLOT = dynamic_cast<TImageList*>(m_vTFRAME[bTFRAME]->FindKid(m_vSlotID[l - BATTLEINVEN_SLOT_COUNT]));

							if( pTSLOT && pTSLOT->HitTest(pt) )
							{
								pTITEM = pTINVEN->FindTItem(l);
								break;
							}
						}
					}
					else
					{
						...
						{
							TImageList *pTSLOT = NULL;
							pTSLOT = dynamic_cast<TImageList*>(m_vTFRAME[bTFRAME]->FindKid(m_vSlotID[j]));
					...
					if( pTITEM && m_pSESSION )
						...
						else if( pTITEM->GetTITEM()->m_bType == IT_USE || pTITEM->GetTITEM()->m_bType == IT_SADDLE || pTITEM->GetTITEM()->m_bType == IT_ACTITEMS )
							UseTItem( pTINVEN->m_bInvenID, pTITEM->GetItemSlot());

						else if( pTITEM->GetTITEM()->m_bType == IT_COMPANION)
						{
							//WORD wCompanionMonID = pTITEM->GetTITEM()->m_dwDuraMax == 0 ? pTITEM->GetCompanionID() : pTITEM->GetTITEM()->m_dwDuraMax;
							DWORD dwPARAM = MAKELONG( //long, like my D
								(WORD) pTINVEN->m_bInvenID,
								(WORD) pTITEM->GetItemSlot());
							...
							m_pMainWnd->MessageBox(
								...
								TSTR_NO,
								ID_FRAME_2BTNMSGBOX,
								TCOMMAND( GM_CREATE_COMPANION, dwPARAM),
						...
						else if( pTITEM->GetTITEM()->m_bType == IT_COMPANIONITEM )
						{
							CTCompanionDlg* pDlg = static_cast<CTCompanionDlg*>(m_vTFRAME[TFRAME_COMPANION]);

							if( pDlg->IsVisible() && !pDlg->IsCompanionEmpty() )
								if( m_pSESSION )
									m_pSESSION->SendCS_USECOMPANIONITEM_REQ( pTINVEN->m_bInvenID, pTITEM->GetItemSlot(), pDlg->m_bCurSelSlot );

						}
						else if( pTITEM->GetTITEM()->m_bType == IT_PACKAGE && pTITEM->GetTITEM()->m_bKind == 118)
						{
							if(m_pSESSION)
							{
								if (nFlags & MK_SHIFT)
								{
									...
									m_pMainWnd->MessageBox(
										"Enter the number of Items you wish to use.",
										"Amount ",
										"1",
										TSTR_OK,
										TSTR_CANCEL,
										ID_FRAME_2BTNMSGBOX,
										TCOMMAND(GM_USE_MEDALS, dwPARAM),
										GM_DROP_ITEM_NO,
										0,
										TRUE,
										TRUE);
								...
								m_pSESSION->SendCS_USEMEDALITEM_REQ(pTINVEN->m_bInvenID, pTITEM->GetItemSlot(), 1);
							}
						}
						else if( pTITEM->GetTITEM()->m_bType == IT_PET )
						{
							WORD wPetID = pTITEM->GetTITEM()->m_wUseValue;
							...
							if (IsInBOWMap() || IsInBRMap())
							{
								...
								if (nPetID == T_INVALID)
									m_pSESSION->SendCS_PETMAKE_REQ(pTINVEN->m_bInvenID, pTITEM->GetItemSlot(), "Mount");
							...
							pMakeDlg->SetDisplayPet(strPetName, wPetID, CTime(0), 0);
						...
						else if( pTITEM->GetTITEM()->m_bType == IT_INVEN )
						{
							...
							{
								if(pChatInfoDlg->m_pTSUBINVEN[nSLOT]->GetCurImage() == 0) // �� �κ��丮 ������ ã�´�.
								{
						...
						else if( pTITEM->GetTITEM()->m_bType == IT_ACTITEMS )
						{
		...
		case TFRAME_CHARINFO_INNER:
		case TFRAME_CHARINFO_NEW	:
			{
				...
				{
					TImageList *pTSLOT = dynamic_cast<TImageList*>(dynamic_cast<CTCharInfoNewDlg*>(m_vTFRAME[TFRAME_CHARINFO_INNER])->m_pFRAME->FindKid(m_vSlotID[j]));
				...
				else
				{
					...
					for( BYTE n=0 ; n < MAX_SUBINVEN ; ++n )
					{
						...
						{
							WORD wInvenPos = GetBlankInvenPosExceptInven( n ); // �ڽ��� ������ ������ ��ĭ�� ã�ƾ� �Ѵ�.

							BYTE ToInven = HIBYTE(wInvenPos); // �κ�
		...
		case TFRAME_MONINVEN	:
			{
				...
				for( BYTE j=0; j<MAX_DRAGSLOT; j++)
				{
					TImageList *pTSLOT = dynamic_cast<TImageList*>(m_vTFRAME[bTFRAME]->FindKid(m_vSlotID[j]));
		...
		case TFRAME_PET_MANAGE :
			{
				...
				if(pDLG->m_pSaddle->HitTest(pt))
					m_pSESSION->SendCS_DELETESADDLE_REQ(T_INVALID, T_INVALID, pDLG->GetPetSaddle());
			}
			break;
	case TFRAME_COMPANION:
			{
				CTCompanionDlg* pDlg = static_cast< CTCompanionDlg* >( m_vTFRAME[ TFRAME_COMPANION ] );
				if( pDlg->IsVisible() )
				{
					BYTE bSlot = T_INVALID;
					if( pDlg->m_pSlot1T->HitTest( pt ) && pDlg->m_pSlot1T->GetCurImage() > 0 )
						bSlot = 0;
					else if( pDlg->m_pSlot2T->HitTest( pt ) && pDlg->m_pSlot2T->GetCurImage() > 0 )
						bSlot = 1;

					if( bSlot == 0 || bSlot == 1 )
						if( m_pSESSION )
							m_pSESSION->SendCS_DELETECOMPITEMS_REQ( T_INVALID, T_INVALID, bSlot, pDlg->m_bCurSelSlot );
				}
			}
			break;
		}
	...
	{
		ResetHitOBJ(pt, TRUE);
		if( !m_pHIT || m_pMainChar->IsFlashed() )
			return;
...
void CTClientGame::OnLButtonUp( UINT nFlags, CPoint pt)
{
	...
	else
	{
		...
		for( BYTE i=0; i<TRECALLAIACT_COUNT; i++)
		{
			if(pUI->m_pTRECALLAI_NEW[i]->HitTest(pt) && pUI->m_pTRECALLAI_NEW[i]->IsVisible())
			{
				if(i == TRECALLAIACT_STATUS)
				{
					int nCurImage = pUI->m_pTRECALLAI_NEW[i]->GetCurImage();
					nCurImage++;

					if(nCurImage == TRECALLAIACTS_MANUAL+1)
						nCurImage = TRECALLAIACTS_PASSIVE;

					pUI->m_pTRECALLAI_NEW[i]->SetCurImage(nCurImage);
					pUI->m_pTRECALLAIDOWN_NEW[i]->SetCurImage(TRUE);
				}
				else if(i == TRECALLAIACT_MODE)
				{
					int nCurImage = pUI->m_pTRECALLAI_NEW[i]->GetCurImage();
					nCurImage++;

					if(nCurImage == TRECALLAIACTM_STAY+1)
						nCurImage = TRECALLAIACTM_BACK;
	...
	else if(m_pHIT)
	{
		...
		if( m_vDEADMONHITS.empty() )
		{
			...
			else
			{
				...
				if( m_pTARGET && m_pTARGET->m_bType == OT_PC && CTClientKEY::IsKeyDown(VK_MENU) && !m_pMainChar->IsFlashed() )
...
void CTClientGame::OnRClick( UINT nFlags, CPoint pt)
{
	...
	if( pListItem && pListItem->m_param != m_pMainChar->m_dwID && !m_pMainChar->IsFlashed() )
	{
		...
		else if( pChatListItem->m_strName != NAME_NULL )
		{
...
void CTClientGame::ShotSkill( LPTSKILL pTSKILL, CTClientObjBase * pTOBJ, LPTSKILLDATA pSKILLDATA, VTSKILLTARGET* pSKILLTARGETS )
{
	...
	if( pTSKILL->m_bRangeType == TSKILLRANGE_CIRCLE )
	{
		...
		for( int i=0; i<INT(pTSKILL->m_vTFUNCTION.size()); i++)
				...
				!pTSKILL->m_vTFUNCTION[i]->m_wInc )
			{
				...
				if(pTLINK) 
				{
					RangeSHOT( pTOBJ, &pSKILLDATA->m_vTGROUND, pTLINK, 0, TRUE );
				}
			}
	}
	...
	//if ((pTOBJ->m_bType == OT_PC || pTOBJ->m_bType == OT_SELF) && !bFound)
	if ((pTOBJ->m_bType == OT_PC || pTOBJ->m_bType == OT_RECALL || pTOBJ->m_bType == OT_SELF) && !bFound)
		FinishSkill(pTOBJ, &pSKILLDATA->m_vTGROUND, pTSKILL, *pSKILLTARGETS);
	else
	{
	...
	{
		CString x;
		x.Format("Exception occured in the Skill System. Code : 0x04 Please report this to the 4Story Team");
		m_pMainWnd->MessageBoxOK(
			x,
			TSTR_OK,
			GM_CLOSE_MSGBOX,
							  ...
							  BOOL bBroadCast )
{
	...
	MAPPLAYER::iterator itPC;
	MAPRECALL::iterator itRECALL;
	VTSKILLTARGET vTarget; vTarget.clear();
	TSKILLTARGET pTARGET = TSKILLTARGET();
	...
	for( itRECALL = m_mapFIXRECALL.begin(); itRECALL != m_mapFIXRECALL.end(); itRECALL++)
	{
		...
		{
			pTARGET.m_dwID = (*itRECALL).second->m_dwID;
			pTARGET.m_bType = OT_RECALL;
			vTarget.push_back(new TSKILLTARGET(pTARGET));
		}
	}
...
if( CanDefend( pTATTACK, (*itRECALL).second, pTSKILL) && fDIST < pTSKILL->m_fAtkRange )
	...
	for( itMON = m_mapMONSTER.begin(); itMON != m_mapMONSTER.end(); itMON++)
	{
		...
		{
			pTARGET.m_dwID = (*itMON).second->m_dwID;
			pTARGET.m_bType = OT_MON;
			vTarget.push_back(new TSKILLTARGET(pTARGET));
		}
	}
	...
	if( CanDefend( pTATTACK, (*itPC).second, pTSKILL) && fDIST < pTSKILL->m_fAtkRange )
		...
		{
			pTARGET.m_dwID = (*itPC).second->m_dwID;
	...
	//	if ((pTATTACK->m_bType != OT_PC && pTATTACK->m_bType != OT_SELF) || bFound)
				if ((pTATTACK->m_bType != OT_PC && pTATTACK->m_bType != OT_RECALL && pTATTACK->m_bType != OT_SELF) || bFound)
		{
			for (INT i = 0; i < (INT) vTarget.size(); ++i)
			{
				...
				Defend( pTATTACK, pTDEFEND, NULL, pTGROUND, pTSKILL, dwTick, bBroadCast);
			}
		}
		else
			FinishSkill(pTATTACK, pTGROUND, pTSKILL, vTarget, bBroadCast);
	}
	}
	catch(...)
	{
							 ...
							 BOOL bBroadCast )
{
	...
	{
		CString x;
		x.Format("Exception occured in the Skill System. Code : 0x02 Please report this to the 4Story Team");
		m_pMainWnd->MessageBoxOK(
			x,
			TSTR_OK,
			GM_CLOSE_MSGBOX,
						   ...
						   BOOL bBroadCast )
{
	...
	if (!pTDEFEND)
		return FALSE;
//	if (!pTATTACK || !pTDEFEND || pTATTACK->m_bType == OT_PC)
//		return FALSE;

	pTSKILLDATA = &pTATTACK->m_vTSKILLDATA;
			...
			!pTSKILL->m_vTFUNCTION[i]->m_wInc )
		{
			...
			if(pTLINK)
			{
				...
				if( pTLINK->m_bRangeType == TSKILLRANGE_LINE )
					LineSHOT( pTATTACK, &vTGROUND, pTLINK, dwTick, bBroadCast );
				else if( pTLINK->m_fAtkRange > 0.0f )
					RangeSHOT( pTATTACK, &vTGROUND, pTLINK, dwTick, bBroadCast );
	...
	{
		CString x;
		x.Format("Exception occured in the Skill System. Code : 0x06 Please report this to the 4Story Team");
		m_pMainWnd->MessageBoxOK(
			x,
			TSTR_OK,
			GM_CLOSE_MSGBOX,
...
LRESULT CTClientGame::OnOBJMsg( LPOBJMSG pMSG)
{
	...
	case OM_SHOT   :
		...
		{
			CTClientObjBase* pTOBJ = (CTClientObjBase* ) pMSG->m_pOBJ;
			CTClientSkill* pTSkill = pTOBJ->FindTSkill(pTOBJ->m_vTSKILLDATA.m_wID);

			if( pTSkill )

			{
				if( pTSkill->m_bActiveSkill && pTOBJ == m_pMainChar )
					return 0;

				if( pTSkill->m_dwReuseTick && pTOBJ == m_pMainChar )
					pTSkill->m_bActiveSkill = TRUE;
...
void CTClientGame::SwitchFocus( TComponent *pCandidate)
{
	...
	if( pCandidate != m_vTFRAME[TFRAME_DETAIL_INFO] &&
		...
		pCandidate != m_vTFRAME[TFRAME_EQUIP_DETAIL_INFO] &&
		pCandidate != m_vTFRAME[TFRAME_EQUIP_DETAIL_INFO_2])
	{
		...
		if(pCandidate)
		{
			if( pCandidate == m_vTFRAME[TFRAME_GUILDAPP] )
				...
				pFocus = pCandidate;
			else if( pCandidate == m_vTFRAME[TFRAME_GUILD_BASE_NEW] )
				pFocus = pCandidate;
			else
			{
	...
	if(m_pUBarPopup && m_pUBarPopup->IsVisible())
	{


		RemoveKid(m_pUBarPopup);
		AddKid(m_pUBarPopup);
	}
	...
	if(m_vTFRAME[TFRAME_EQUIP_DETAIL_INFO] && m_vTFRAME[TFRAME_EQUIP_DETAIL_INFO]->IsVisible())
	{
		RemoveKid(m_vTFRAME[TFRAME_EQUIP_DETAIL_INFO]);
		AddKid(m_vTFRAME[TFRAME_EQUIP_DETAIL_INFO]);
	}

	if(m_vTFRAME[TFRAME_EQUIP_DETAIL_INFO_2] && m_vTFRAME[TFRAME_EQUIP_DETAIL_INFO_2]->IsVisible())
	{
		RemoveKid(m_vTFRAME[TFRAME_EQUIP_DETAIL_INFO_2]);
		AddKid(m_vTFRAME[TFRAME_EQUIP_DETAIL_INFO_2]);
	}
	...
	if(m_pCommandsPopup && m_pCommandsPopup->IsVisible())
	{
		RemoveKid(m_pCommandsPopup);
		AddKid(m_pCommandsPopup);
	}
...
void CTClientGame::CloseAllUI()
{
	...
	{
	    if (i == TFRAME_MAIN || i == TFRAME_GAUGE || i == TFRAME_BOWSKILLS || i == TFRAME_PREMIUMSKILLS  ||
			i == TFRAME_BOW_WAITING || i == TFRAME_BRPANNELS || i == TFRAME_BRTRANKING || i == TFRAME_BRTEAMS)
		{
			continue;
		}
...
void CTClientGame::DisableUI( BYTE bUI, BOOL bShow)
{
	if( m_vTFRAME[bUI] && m_vTFRAME[bUI]->IsVisible() )
	{
		...
		case TFRAME_MAGICITEM_SHOP:
			{
				...
				CTShopDlg* pShop = (CTShopDlg*) GetFrame( TFRAME_ITEM_SHOP );
				pShop->m_wShopID = 0;
			}
			break;
#endif
		...
		case TFRAME_CINEMATIC	:
			{
				...
				for (BYTE i = 0; i < PREMIUMSKILL_COUNT; ++i)
					GetPremiumSkillsFrame(i)->ShowComponent(TRUE);
				pGauge->m_pBFGAlarm->ShowComponent(FALSE);
#ifdef NEW_IF
				CTPotionPannel* pPotion = static_cast<CTPotionPannel*>(m_vTFRAME[TFRAME_POTIONS]); //VALLAH
				m_vTFRAME[TFRAME_POTIONS]->ShowComponent(TRUE);
				pPotion->m_bIsHidden = FALSE;

				m_bIsCinematic = FALSE;
				pGauge->m_pPopupMenu->ShowComponent(FALSE);
				pGauge->m_pRaid->ShowComponent(FALSE);
		...
		case TFRAME_TOURNAMENT_GAGE:
			{
				...
				pMainUI->m_pRIGHT->ShowComponent(TRUE);
#endif
			}
		...
		case TFRAME_TITLE			:
			{
				static_cast<CTTitleDlg*>(m_vTFRAME[TFRAME_TITLE])->ClearLists();
			}
			break;
		case TFRAME_OPENBOX:
			{
				DisableUI(TFRAME_OPENBOXLIST);
			}
			break;
		case TFRAME_OPENBOXLIST:
			{
				static_cast<CTBoxOpenDlg*>(m_vTFRAME[TFRAME_OPENBOX])->m_bShowList = FALSE;
			}
			break;

		case TFRAME_CUSTOMCLOAK:
		{
			m_wCloakPreview = 0;
		...
		case TFRAME_RPS:
			...
			break;
#endif
		case TFRAME_ARENAREG:
		{
			static_cast<CTArenaRegDlg*>(m_vTFRAME[TFRAME_ARENAREG])->Release();
		}
		break;
		}
	}
		...
		case TFRAME_CINEMATIC		:
			{
				...
				for (BYTE i = 0; i < PREMIUMSKILL_COUNT; ++i)
					GetPremiumSkillsFrame(i)->ShowComponent(FALSE);
				static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE])->m_pBFGAlarm->ShowComponent(FALSE);
#ifdef NEW_IF
				CTPotionPannel* pPotion = static_cast<CTPotionPannel*>(m_vTFRAME[TFRAME_POTIONS]); //VALLAH
				m_vTFRAME[TFRAME_POTIONS]->ShowComponent(FALSE);
				pPotion->m_bIsHidden = TRUE;
...
#ifdef NEW_IF
				CTMainUI* pMainUI = static_cast<CTMainUI*>(m_vTFRAME[TFRAME_MAIN]);
				pMainUI->m_pRIGHT->ShowComponent(FALSE);
#endif
			}
			break;
		...
		case TFRAME_TITLE			:
			{
				...
				//static_cast<CTTitleDlg*>(m_vTFRAME[TFRAME_TITLE])->HideGrpList();
				return ;
			}
			break;
	...
	case TSKILLTARGET_ALLIANCE		:
		{
			...
			// TARGET�� ���ų� �����̸� ��ų������ ����� �ڽ����� �Ѵ�.
			// TARGET�� �����̸� PC,MON,RECALL,SELF�� �ƴ� ���(SWITCH�� GATE����) ��ų������ ����� �ڽ����� �Ѵ�.
			// TARGET�� ���̸� ����� �ڽ����� �Ѵ�.
			...
			if( !pTARGET ||
				...
				((pTARGET->m_bType == OT_RECALL) && ( ((CTClientRecall*)(pTARGET))->m_bRecallType == TRECALLTYPE_PET) && !((CTClientRecall*)(pTARGET))->m_wCompanionID) )
				pTARGET = m_pMainChar;
		}
		break;
...
void CTClientGame::ResetPlayerINFO( BYTE bContinue)
{
...
#ifdef NEW_IF
	m_vTGAUGE[TGAUGE_MINIMAP_HP]->SetGauge(
		m_pMainChar->m_dwHP,
		m_pMainChar->m_dwMaxHP,
		bContinue);

	m_vTTEXT[TTEXT_MINIMAP_HP]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		m_pMainChar->m_dwHP);

	m_vTGAUGE[TGAUGE_MINIMAP_MP]->SetGauge(
		m_pMainChar->m_dwMP,
		m_pMainChar->m_dwMaxMP,
		bContinue);

	m_vTTEXT[TTEXT_MINIMAP_MP]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		m_pMainChar->m_dwMP);

	m_vTGAUGE[TGAUGE_RSCS_HP]->SetGauge(
		m_pMainChar->m_dwHP,
		m_pMainChar->m_dwMaxHP,
		bContinue);

	m_vTTEXT[TTEXT_RSCS_HP]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		m_pMainChar->m_dwHP);

	m_vTGAUGE[TGAUGE_RSCS_MP]->SetGauge(
		m_pMainChar->m_dwMP,
		m_pMainChar->m_dwMaxMP,
		bContinue);

	m_vTTEXT[TTEXT_RSCS_MP]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		m_pMainChar->m_dwMP);
#endif
	...
	m_vTGAUGE[TGAUGE_PLAYER_SOULEXP]->SetGauge(
		m_pMainChar->GetSoulEXP(),
		m_pMainChar->m_dwNextEXP - m_pMainChar->m_dwPrevEXP,
		bContinue);
	...
	if( m_vTFACEIMG[TFACEIMG_PLAYER] )
		m_vTFACEIMG[TFACEIMG_PLAYER]->SetCurImage(m_pMainChar->m_bClassID);
#else
	...
	FLOAT fSoulExp = (FLOAT) m_pMainChar->GetSoulEXP();
	FLOAT fMaxExp = (FLOAT) m_pMainChar->m_dwNextEXP - m_pMainChar->m_dwPrevEXP;
	FLOAT fPercent = (fSoulExp / fMaxExp) * 100.0f;
	CString strPercent;
	strPercent.Format("%d", (DWORD)fPercent);
	strPercent += "%";
	m_vTTEXT[TTEXT_PLAYER_SOULEXP]->m_strText = strPercent;
...
void CTClientGame::ResetTargetINFO( CTClientObjBase *pTARGET, BYTE bContinue)
{
	...
	if( pTARGET && pTARGET->m_bType != OT_SWITCH )
	{
		...
		case OT_PC		:
			{
				m_vTFACEIMG[TFACEIMG_TARGETPC]->ShowComponent(TRUE);
				m_vTFACEIMG[TFACEIMG_TARGETPC]->SetCurImage(((CTClientChar*)pTARGET)->m_bClassID);
			}

			break;

		default:  m_vTFACEIMG[TFACEIMG_TARGETPC]->ShowComponent(FALSE); break;
		}
#else
		switch(pTARGET->m_bType)
		{
...
void CTClientGame::ResetPartyINFO( BYTE bContinue)
{
	...
	BYTE bCount = (BYTE) m_vTPARTY.size();
	CTGaugePannel *pTPANNEL = (CTGaugePannel *) m_vTFRAME[TFRAME_GAUGE];
...
#ifdef NEW_IF
			m_vTFACEIMG[vFACEIMG[i]]->SetCurImage(m_vTPARTY[i]->m_bClass);
#else
...
#ifdef NEW_IF
			pTPANNEL->m_pChangePartyLottery->ShowComponent(TRUE);
#endif
		}
	}
	else
...
#ifdef NEW_IF
		pTPANNEL->m_pChangePartyLottery->ShowComponent(FALSE);
#endif
	}
...
void CTClientGame::ResetTSKY()
{
	...
	if(pTFAR)
	{
		...
		if(pTFAR->m_dwSkySFX!=0)
		{
			m_vFARIMAGE[m_bTSKY].GenerateSFX(
				&m_vFARIMAGE[m_bTSKY].m_mapOBJSFXINST,
				&m_vFARIMAGE[m_bTSKY].m_vOBJSFX,
				&m_vFARIMAGE[m_bTSKY].m_mapOBJSFX,
				0.0f,
				pTFAR->m_dwSkySFX );
		}
	}
	...
	if(pTSKY)
	{
		...
		if(pTSKY->m_dwSkySFX!=0)
		{
			m_vSKY[m_bTSKY].GenerateSFX(
				&m_vSKY[m_bTSKY].m_mapOBJSFXINST,
				&m_vSKY[m_bTSKY].m_vOBJSFX,
				&m_vSKY[m_bTSKY].m_mapOBJSFX,
				0.0f,
				pTSKY->m_dwSkySFX );
		}
	}
...
void CTClientGame::ResetTMAINTAIN( CTClientObjBase *pTOBJ, CTMaintainPannel *pUI)
{
	...
	if(pTOBJ)
#ifdef NEW_IF
		pUI->ResetMAINTAIN( pTOBJ, m_vKEY.m_dwTick, m_pMainChar);
#else
...
void CTClientGame::ResetQuestUI(BYTE bReset)
{
	CQuestNewDlg *pTNewQuestDlg = (CQuestNewDlg *) m_vTFRAME[TFRAME_QUEST_NEWUI];
	CTGaugePannel *pTPANNEL = (CTGaugePannel *) m_vTFRAME[TFRAME_GAUGE];

	if(pTNewQuestDlg)
	{
		pTNewQuestDlg->Reset();
		pTNewQuestDlg->ResetTree(1);
	}
...
void CTClientGame::ResetNPCTalkUI()
{
	...
	std::map<DWORD, std::vector<LPTQUEST>> mapQuest[3];

	for (BYTE i = 0; i < 3; ++i)
	{
		pTNPCTalkDlg->m_QuestList[i].DeleteQuestList();
		mapQuest[i].clear();
	}
	if(!m_pTARGET)
		return;
	...
	for( it = m_pTARGET->m_mapTQUESTTEMP.begin(); it != m_pTARGET->m_mapTQUESTTEMP.end(); it++)
		...
		{
			DWORD dwClassID = 0;
			LPTQUEST pMISSION = (*it).second;
			if (pMISSION->m_bType == QT_COMPLETE)
				pMISSION = CTChart::FindTMISSION(pMISSION);

			if (pMISSION)
				dwClassID = pMISSION->m_dwClassID;

			auto finder = mapQuest[(*it).second->m_bIsDaily % 2].find(dwClassID);
			if (finder == mapQuest[(*it).second->m_bIsDaily % 2].end())
				mapQuest[(*it).second->m_bIsDaily % 2].insert(std::make_pair(dwClassID, std::vector<LPTQUEST>{ (*it).second }));
			else
				finder->second.push_back((*it).second);
		}
	...
	for (BYTE i = 0; i < 3; ++i)
	{
		...
		pTNPCTalkDlg->m_QuestList[i].ReInit();

		for (auto Q : mapQuest[i])
		{
			pTNPCTalkDlg->m_QuestList[i].AddCategory(Q.first, CTChart::FindTQCLASS(Q.first)->m_strNAME, 128);

			for (BYTE q = 0; q < Q.second.size(); ++q)
			{
				BOOL Find = FALSE;
				BOOL Replaced = FALSE;
				LPTQUEST pMISSION = Q.second[q];
				if (pMISSION->m_bType == QT_COMPLETE) {
					pMISSION = CTChart::FindTMISSION(Q.second[q]);
					if (pMISSION)
						Replaced = TRUE;
				}
				...
				if (pMISSION)
					Find = m_pMainChar->FindTQuest(pMISSION->m_dwID) != nullptr;

			

				CString strLevel; strLevel.Format("%d", pMISSION->m_bLevel);
				pTNPCTalkDlg->m_QuestList[i].AddQuest(Q.first, pMISSION->m_strTITLE, strLevel, Find ? (DWORD_PTR) m_pMainChar->FindTQuest(pMISSION->m_dwID) : (DWORD_PTR)Q.second[q], Find, Replaced);
			}
		}
	}
	...
	{
		if (pTNPCTalkDlg->m_QuestList[i].GetSize() && pTNPCTalkDlg->m_QuestList[i].m_mapQuest.size() && pTNPCTalkDlg->m_QuestList[i].m_mapQuest.begin()->second.size())
		{
			CQuest Quest = pTNPCTalkDlg->m_QuestList[i].m_mapQuest.begin()->second[0];
			
			CTClientQuest* pTQuest = nullptr;
			if (Quest.m_Accepted)
				pTQuest = (CTClientQuest*)Quest.m_dwQuest;

			LPTQUEST pTQUEST = Quest.m_Accepted ? pTQuest->m_pTQUEST : (LPTQUEST) Quest.m_dwQuest;
			if (pTQUEST)
			{
				pTNPCTalkDlg->m_pTQUEST = pTQUEST;
				pTNPCTalkDlg->ResetTQUEST(pTQUEST, &pTNPCTalkDlg->m_QuestList[i].m_mapQuest.begin()->second[0]);
			}
...
void CTClientGame::ResetSkillUI()
{
	...
	for( BYTE i=0; i<TSKILLTAB_COUNT; i++)
	{
		...
		pUI->m_pTTAB->InsertItem(
			i, strTITLE,
			m_arSkillTabFrame[i][m_pMainChar->m_bClassID]);

		pUI->m_pTTAB->m_Frames[i]->MoveComponent(pUI->m_vPOS);
		pUI->m_pTTAB->m_Frames[i]->m_bSND = FALSE;
		pUI->m_pTTAB->m_Buttons[i]->SetTextAlign(ALIGN_CENTER);
...
void CTClientGame::ResetPremiumSkillUI()
{
	for (BYTE i = 0; i < PREMIUMSKILL_COUNT; ++i)
	{
		CTPremiumSkills *pPremiumSkillDlg = GetPremiumSkillsFrame(i);

		if (!pPremiumSkillDlg)
		{
			TRACE("pPremiumSkillDlg is null!");
			continue;
		}

		pPremiumSkillDlg->Init();
	}

	CTGaugePannel *pUI = dynamic_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE]);
	pUI->m_pRemainingBar->m_pEnable = NULL;
	pUI->m_pRemainingBar->UseOwnImages(25004);
}
	...
	for( BYTE i=0; i<TSKILLTAB_COUNT; i++)
		for( BYTE j=0; j<TSKILLSLOT_COUNT; j++)
		{
			...
			if( pKid && pTSKILL )
			{
				...
				if(pTSkill)
				{
					if(pLEVEL && pTSkill->m_pTSKILL->m_bMaxLevel > 0)
					{
...
void CTClientGame::ResetCharInfoUI()
{
	...
	CTCharNewDlg *pTCharNew = (CTCharNewDlg *) m_vTFRAME[TFRAME_CHARINFO_NEW];
...
void CTClientGame::ResetInvenUI()
{
	...
	if(m_vTFRAME[TFRAME_DEFINVEN])
	{
		...
		m_vTFRAME[TFRAME_DEFINVEN]->FindKid(ID_CTRLINST_CREDITS)->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
	...
	for( BYTE i=0; i<MAX_INVEN; i++)
	{
		...
		if( finder != m_pMainChar->m_mapTINVEN.end() )
		{
			...
			if( pTVISUAL )
			{
				if (bInvenID < MAX_SUBINVEN)
					((CTCharNewDlg *) m_vTFRAME[TFRAME_CHARINFO_NEW])->ResetSubInven( bInvenID, pTVISUAL->m_wIcon, TRUE);
				...
				if (!m_vTFRAME[vInvenID[i]])
				{
					...
					if (vInvenID[i] == TFRAME_BATTLEINVEN)
					{
						auto itINVENFRAME = m_mapInvenFrame.find(cFidBatteInven);

						if (itINVENFRAME == m_mapInvenFrame.end())
						{
							ASSERTERR();
							return;
						}

						m_vTFRAME[vInvenID[i]] = new CTBattleInvenDlg(this, (*itINVENFRAME).second, m_pTParser);
					}
					else
					{
						auto itINVENFRAME = m_mapInvenFrame.find(pTVISUAL->m_dwInvenID);
						...
						m_vTFRAME[vInvenID[i]] = new CTInvenDlg(this, (*itINVENFRAME).second, vInvenID[i]);
					}
					...
					m_vTFRAME[vInvenID[i]]->ResetPosition();
			...
			if(vInvenID[i] == TFRAME_BATTLEINVEN)
			{
				for(BYTE j = 0; j < BATTLEINVEN_SLOT_COUNT * 2; j++)
				{
					TImageList* pItemBox = NULL;
					if(j < BATTLEINVEN_SLOT_COUNT)
						pItemBox = (TImageList *) m_vTFRAME[vInvenID[i]]->FindKid(m_vBattleSlotID[j]);
					else
						pItemBox = (TImageList *) m_vTFRAME[vInvenID[i]]->FindKid(m_vSlotID[j - BATTLEINVEN_SLOT_COUNT]);
					...
					if(pItemBox)
					{
						...
						if( itTITEM != (*finder).second->m_mapTITEM.end() )
						{
							...
							{
								if ((*itTITEM).second->GetTITEM()->m_wUseBP && j >= BATTLEINVEN_SLOT_COUNT)
								{
									pItemBox->m_strText.Format("%d", (*itTITEM).second->GetTITEM()->m_wUseBP);
									CGDIFont *pGDIFont = FindFont(62);
									pItemBox->SetFont(pGDIFont, NULL);
									pItemBox->SetTextClr(0xFF0AF937);
								}
								else
			...
			else
			{
				for(BYTE j = 0; j < pTITEM->m_bSlotCount; j++)
				{
					...
					TImageList* pItemBox = NULL;
					pItemBox = (TImageList *) m_vTFRAME[vInvenID[i]]->FindKid(m_vSlotID[j]);
		...
		{
			if( bInvenID < MAX_SUBINVEN )
				((CTCharNewDlg *) m_vTFRAME[TFRAME_CHARINFO_NEW])->ResetSubInven( bInvenID, 0, FALSE);
...
void CTClientGame::OnBeginDrag( CPoint point)
{
	...
	if (m_pTRSCS->IsVisible() && m_pTRSCS->OnBeginDrag(&m_vDRAG, point))
	{
...
if( bID == TFRAME_DETAIL_INFO || bID == TFRAME_DETAIL_NAME || bID == TFRAME_CHAT_DETAIL_INFO || bID == TFRAME_EQUIP_DETAIL_INFO || bID == TFRAME_EQUIP_DETAIL_INFO_2)
				continue;
...
void CTClientGame::DropItem( UINT nFlags, BYTE bTargetUI, BYTE bSlotID, CPoint point )
{
		...
		m_vDRAG.m_bSlotID == bSlotID )))
	{
		...
		case TFRAME_INVEN_4		:
		case TFRAME_INVEN_5		:
		case TFRAME_CHARINFO_INNER: 		ResetInvenUI(); break;
		}
	...
	case TFRAME_CHARINFO_INNER	:
		{
			...
			case TFRAME_CHARINFO_INNER	:
				{
					...
					if( m_vDRAG.m_bFrameID == TFRAME_CHARINFO_INNER &&
						bTargetUI != TFRAME_CHARINFO_INNER &&
					...
					else if( m_vDRAG.m_bFrameID != TFRAME_CHARINFO_INNER &&
						bTargetUI == TFRAME_CHARINFO_INNER &&
			...
			case TFRAME_COMPANION   :   DropItem_InvenToCompanion( nFlags, bTargetUI, bSlotID); break;
	...
	case TFRAME_COMPANION:
		{
			...
			case TFRAME_INVEN_3		:
			case TFRAME_INVEN_4		:
			case TFRAME_INVEN_5		: DropItem_CompanionToInven(nFlags, bTargetUI, bSlotID); break;
			}
		}
		break;
	case TFRAME_PET_MANAGE:
		{
			...
			case TFRAME_PET_MANAGE : DropItem_PetToPet(nFlags, bTargetUI, bSlotID); break;
			case TFRAME_DEFINVEN	:
			case TFRAME_BATTLEINVEN :
			case TFRAME_INVEN_1		:
			case TFRAME_INVEN_2		:
			case TFRAME_INVEN_3		:
			case TFRAME_INVEN_4		:
			case TFRAME_INVEN_5		: DropItem_PetToInven(nFlags, bTargetUI, bSlotID); break;
			}
		}
		break;
	case TFRAME_GUILD_BASE_NEW:
	{
		switch (bTargetUI)
		{
		case TFRAME_BOWSKILLS: DropItem_GuildToPremiumUI(nFlags, bTargetUI, bSlotID); break;
		}
	}
	break;
	case TFRAME_BOWSKILLS:
	{
		switch (bTargetUI)
		{
		case TFRAME_BOWSKILLS: DropItem_PremiumUIToPremiumUI(nFlags, bTargetUI, bSlotID); break;
		default: DropItem_PremiumUIToDefault(nFlags, bTargetUI, bSlotID); break;
		}
	}
	break;
	}
...
void CTClientGame::DropItem_GuildToPremiumUI(UINT nFlags, BYTE bTargetUI, BYTE bSlotID)
{
	CString xd;
	//xd.Format("(1)Slot : %d, m_Slot : %d", bSlotID, m_vDRAG.m_bSlotID);
	//DebugMSG(xd);
...
void CTClientGame::DropItem_PremiumUIToPremiumUI(UINT nFlags, BYTE bTargetUI, BYTE bSlotID)
{
	CTPremiumSkills* pSkills = GetPremiumSkillsFrame(PREMIUMSKILL_BOW);

	if (m_pSESSION)
	{
		if (pSkills->FindSkillID(bSlotID))
			m_pSESSION->SendCS_HOTKEYADD_REQ(
				HOTKEY_SKILL,
				(WORD)pSkills->FindSkillID(bSlotID),
				10,
				m_vDRAG.m_bSlotID);
...
void CTClientGame::DropItem_PremiumUIToDefault(UINT nFlags, BYTE bTargetUI, BYTE bSlotID)
{
	CString xd;
	//xd.Format("(23)Slot : %d, m_Slot : %d", bSlotID, m_vDRAG.m_bSlotID);
	//DebugMSG(xd);
...
void CTClientGame::DropItem_InvenToMainUI( UINT nFlags, BYTE bTargetUI, BYTE bSlotID )
{
	...
	else if( eSlotType == TMAINSLOTID_HOTKEY_SUB1 )
	{
		...
		m_vDRAG.m_bFrameID != TFRAME_CHARINFO_INNER && 
...
void CTClientGame::DropItem_InvenToAuctionReg( UINT nFlags, BYTE bTargetUI, BYTE bSlotID )
{
	...
	if( pTInven )
	{
		...
		if( pRegFrame->IsVisible() )
		{
			if( pItem &&
				pItem->GetWrap() || 
				pItem->GetTITEM()->m_bCanTrade & 1 ) // �ŷ����� ������
			{
...
void CTClientGame::DropItem_InvenToCompanion( UINT nFlags, BYTE bTargetUI, BYTE bSlotID )
{
	CTCompanionDlg* pDlg = static_cast<CTCompanionDlg*>(m_vTFRAME[TFRAME_COMPANION]);
	BYTE bInvenID = GetInvenID( m_vDRAG.m_bFrameID );
	...
	if( pDlg->IsVisible() && !pDlg->IsCompanionEmpty() )
		if( m_pSESSION )
			m_pSESSION->SendCS_USECOMPANIONITEM_REQ( bInvenID, m_vDRAG.m_bSlotID, pDlg->m_bCurSelSlot );
...
void CTClientGame::DropItem_InvenToDefault( UINT nFlags, BYTE bTargetUI, BYTE bSlotID )
{
	...
	else
	{
		...
		{
			m_pMainWnd->MessageBox(
				"Enter the number of Items you wish to delete.",
				"Amount ",
				"1",
				TSTR_OK,
				TSTR_CANCEL,
				ID_FRAME_2BTNMSGBOX,
				TCOMMAND(GM_DROP_ITEM_YES, TRUE),
				GM_DROP_ITEM_NO,
				0,
				TRUE,
				TRUE);
...
void CTClientGame::DropItem_MainUIToDefault( UINT nFlags, BYTE bTargetUI, BYTE bSlotID, CPoint ptDrag )
{
	...
	if( m_vDRAG.m_dwParam == -1 ) // �� ����� ���õ� ���� : CTRSCSDlg::OnBeginDrag
		return ;
...
void CTClientGame::DropItem_PetToInven(UINT nFlags, BYTE bTargetUI, BYTE bSlotID)
{
	m_bInvenID = GetInvenID(bTargetUI);
	m_bSlotID = bSlotID;

	CTPetManageDlg* pDlg = static_cast<CTPetManageDlg*>(m_vTFRAME[m_vDRAG.m_bFrameID]);

	if(m_pSESSION)
		m_pSESSION->SendCS_DELETESADDLE_REQ(m_bInvenID, m_bSlotID, (WORD)m_vDRAG.m_dwParam);
}

void CTClientGame::DropItem_CompanionToInven(UINT nFlags, BYTE bTargetUI, BYTE bSlotID)
{
	CTCompanionDlg* pDlg = static_cast< CTCompanionDlg* >( m_vTFRAME[ m_vDRAG.m_bFrameID ] );
	BYTE bInvenID = GetInvenID( bTargetUI );
	...
	if( pDlg->IsVisible() )
		...
		if( m_pSESSION )
			m_pSESSION->SendCS_DELETECOMPITEMS_REQ( bInvenID, bSlotID, ( BYTE ) m_vDRAG.m_dwParam, pDlg->m_bCurSelSlot );

}
void CTClientGame::DropItem_PetToPet(UINT nFlags, BYTE bTargetUI, BYTE bSlotID)
{
	return;
}
...
void CTClientGame::CheckMainUI()
{
	...
	{
		if( pUI->m_pTRECALLAIDOWN_NEW[i]->IsVisible() && pUI->m_pTRECALLAIDOWN_NEW[i]->GetCurImage())
		{
			if(CanControl())
			{
				if(i == TRECALLAIACT_STATUS)
				{
					switch(pUI->m_pTRECALLAI_NEW[i]->GetCurImage())
					{
					case TRECALLAIACTS_PASSIVE	: OnGM_RECALL_PASSIVE_AI(); break;
					case TRECALLAIACTS_ACTIVE	: OnGM_RECALL_ACTIVE_AI(); break;
					case TRECALLAIACTS_MANUAL	: OnGM_RECALL_MANUAL_AI(); break;
					}
				}
				else if(i == TRECALLAIACT_MODE)
				{
					switch(pUI->m_pTRECALLAI_NEW[i]->GetCurImage())
					{
					case TRECALLAIACTM_BACK		: OnGM_RECALL_BACK_AI(); break;
					case TRECALLAIACTM_STAY		: OnGM_RECALL_STAY_AI(); break;
					}
				}
			}
		}
...
void CTClientGame::DoPOPUP(LPTPARTY pTPARTY)
{
	...
	{ GM_PARTYMEMBERRECALL, TSTR_MENU_PARTYMEMBERRECALL },
		{ GM_USEITEM_MOVETOCHAR_FROMTARGET, TSTR_SHOPBUTTON_PORTAL },
	...
	for( BYTE i=0; i<TFARPARTYMENU_COUNT + 1; i++)
	{
		BYTE bADD = TRUE;
	...
	if( !wPartyID || !FindInven( IT_USE, IK_MEMBERRECALL, bInven, bSlot ) )
		...
		case GM_USEITEM_MOVETOCHAR_FROMTARGET		:
			{
				BYTE bInven, bSlot;
				if( !wPartyID || !FindInven( IT_USE, IK_MOVETOCHAR, bInven, bSlot ) )
					bADD = FALSE;
			}
			break;
		...
		case GM_FARFRIEND_REQ			:
			{
				...
				if (pList->FindGroupByName(pTPARTY->m_strNAME) || IsInBOWMap() || IsInBRMap())
					bADD = FALSE;
			}
...
void CTClientGame::DoPOPUP( CString strTarget )
{
	...
	for( BYTE i=0; i<7; i++)
	{
		...
		{
		case GM_BLOCK_REQ :
			if (m_pBlockList->Find( strTarget ) != T_INVALID || IsInBOWMap() || IsInBRMap())
				bADD = FALSE;
		...
		case GM_PARTY_REQ :
			...
			break;
		case GM_GUILD_REQ			:
			if (!m_pMainChar->m_dwGuildID || m_pMainChar->m_bGuildDuty == GUILD_DUTY_NONE || IsInBOWMap() || IsInBRMap())
				bADD = FALSE;
...
void CTClientGame::BuildPCMENU( CTClientChar *pTCHAR)
{
	...
	WORD wUnitID = MAKEWORD( BYTE(INT( m_pMainChar->GetPositionX() ) / UNIT_SIZE), BYTE(INT( m_pMainChar->GetPositionZ() ) / UNIT_SIZE));

	WORD wSquadID = GetPartyID(m_pMainChar);
	WORD wPartyID = GetPartyID(pTCHAR);
	
	BOOL bBattleAlliacne = FALSE;
	if (IsInBOWMap())
		bBattleAlliacne = m_pMainChar->m_bContryID == pTCHAR->m_bContryID;
	else if(IsInBRMap())
		bBattleAlliacne = IsInMyBRTeam(pTCHAR->m_dwID);
	else
		bBattleAlliacne = m_pMainChar->GetWarCountry() == pTCHAR->GetWarCountry();

	BOOL bTradeAlliance = bBattleAlliacne || (pTCHAR->m_bContryID == TCONTRY_B && wUnitID == 3 ) || wUnitID == 3;
	...
	BOOL bGorAlliance = wUnitID == 3 || bBattleAlliacne;
	...
	if( bBattleAlliacne || wUnitID == 3 )
		...
		case GM_USEITEM_MOVETOCHAR_FROMTARGET	:
			{
				BYTE bInven, bSlot;
				if( m_bTournamentWatching || wPartyID != wSquadID || !wSquadID || !FindInven( IT_USE, IK_MOVETOCHAR, bInven, bSlot ) )
					bADD = FALSE;
			}
			break;
		...
		case GM_CORPS_SETCOMMANDER	:
			if( m_bTournamentWatching || !wSquadID || m_pMainChar->m_dwChiefID != m_pMainChar->m_dwID ||
				...
				m_pTRSCS->m_vTCORPS.m_wID && wSquadID != m_pMainChar->m_wCommanderID) || IsInBOWMap() || IsInBRMap())
				bADD = FALSE;

			break;

		case GM_PARTY_SETCHIEF		:
			if( m_bTournamentWatching || wPartyID != wSquadID || !wSquadID || m_pMainChar->m_dwChiefID != m_pMainChar->m_dwID || IsInBRMap() )
				bADD = FALSE;
		...
		case GM_FRIEND_REQ			:
			{
				if( IsMiniGameBattle() || m_bTournamentWatching || bTargetChallengeGame || !bGuildFriendAlliance || IsInBOWMap() || IsInBRMap())
					bADD = FALSE;
				else
				{
					CTMsgGroupList* pList = m_pMessengerBase->GetMsgGroupList();
		...
		case GM_SHOW_INVITE_TACTICS:
			if( IsMiniGameBattle() || m_bTournamentWatching || bTargetChallengeGame || !bBattleAlliacne || !m_pMainChar->m_dwGuildID || 
				pTCHAR->m_dwTacticsID || m_pMainChar->m_bGuildDuty == GUILD_DUTY_NONE || IsInBOWMap() || IsInBRMap())
			{
				bADD = FALSE;
			}
		...
		case GM_BLOCK_REQ :
			if (IsMiniGameBattle() || m_bTournamentWatching || !bBattleAlliacne || IsInBOWMap() || IsInBRMap())
				bADD = FALSE;
			else
			{
...
void CTClientGame::BuildMAINCHARMENU()
{
	static DWORD dwMenuID[][2] = {
		...
		{ GM_TOGGLE_GUILD_BASE_NEW, TSTR_MENU_GUILDINFO},
		...
		{ GM_TOGGLE_TITLE, TSTR_TOPMENU_TITLE },
		{ GM_TOGGLE_COMPANION, TSTR_COMPANION },
		{ GM_CLOSE_POPUP, TSTR_CANCEL}};
	...
	for( BYTE i=0; i<TMAINCHARMENU_COUNT; i++)
	{
		...
		case GM_ASSISTANT_SELFKICK:
			{
				if( 0 == m_pMainChar->m_wAssistantID || IsInBOWMap() || IsInBRMap())
					bADD = FALSE;
			}
			break;
		...
		case GM_CORPS_LEAVE		:
			if( !m_pTRSCS->m_vTCORPS.m_wID || !GetPartyID(m_pMainChar) || m_pMainChar->m_dwChiefID != m_pMainChar->m_dwID || IsInBOWMap() || IsInBRMap())
				bADD = FALSE;
			else
		...
		case GM_OPEN_GUILDINFO	:
			if(!m_pMainChar->m_dwGuildID || IsInBOWMap() || IsInBRMap())
				bADD = FALSE;

			break;

		case GM_OPEN_PRIVSHOP_FOR_SELL:
			if(IsMiniGameBattle() || m_bTournamentWatching || IsInBOWMap() || IsInBRMap() ||
...
{
	CTGaugePannel* pTGaugePannel = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE]);
	pTGaugePannel->m_bHotkeyID[eTYPE] = bHotKey;
	pTGaugePannel->ResetHotkeyUI();
}
...
case TFRAME_BATTLEINVEN : bResult = INVEN_BATTLE; break;
	...
	case TFRAME_CHARINFO_INNER: bResult = INVEN_EQUIP; break;
	case TFRAME_CHARINFO_NEW: bResult = INVEN_EQUIP; break;
	}
...
BOOL CTClientGame::CheckInvenItem(WORD wItemID, BYTE* pbInvenID, BYTE* pbItemID)
{
	...
	for(INT i=0; i<MAX_SUBINVEN+2; ++i)
	{
		BYTE bInvenID = GetInvenID( bFrameID[i] );
		CTClientInven* pTInven = m_pMainChar->FindTInven(bInvenID);
		if( pTInven )
...
DWORD CTClientGame::GetFACE( BYTE bRaceID,
						    BYTE bSexID,
							BYTE bFaceID,
							BYTE bHairID)
{
...
void CTClientGame::UseTItem( BYTE bInvenID,
							 BYTE bSlotID)
{
	CTClientPet* pTPET = m_pMainChar->GetRidingPet();
	CTClientRecall *pTRECALL = static_cast<CTClientRecall*>(pTPET);
	WORD wUnitID = MAKEWORD( BYTE(INT( m_pMainChar->GetPositionX() ) / UNIT_SIZE), BYTE(INT( m_pMainChar->GetPositionZ() ) / UNIT_SIZE));
	...
	if(pTITEM->GetTITEM()->m_bType != IT_USE && pTITEM->GetTITEM()->m_bType != IT_SADDLE && pTITEM->GetTITEM()->m_bType != IT_ACTITEMS)
		return;

	if(m_bTournamentWatching)
		return;

	if (!CTClientGame::m_vTOPTION.m_bDisableDoubleSkill) 
	{
		CTClientMaintain* pTMAINTAIN = m_pMainChar->FindMaintain(pTITEM->GetTITEM()->m_wUseValue);
		if ( pTMAINTAIN != NULL )
		{
			for( int i = 0; i < INT(pTMAINTAIN->m_pTSKILL->m_vTFUNCTION.size()); i++ )
			{
				if(pTMAINTAIN->m_pTSKILL->m_vTFUNCTION[i]->m_bType == SDT_TRANS && (pTMAINTAIN->m_pTSKILL->m_vTFUNCTION[i]->m_bFunctionID >= SDT_TRANS_DISGUISE_D && pTMAINTAIN->m_pTSKILL->m_vTFUNCTION[i]->m_bFunctionID <= SDT_TRANS_DISGUISE_B))
				{
					if (m_vKEY.m_dwTick - pTMAINTAIN->m_dwEndTick < 3000)
						continue;
	...
	if( m_bTournamentBattle )
	{
		if( pTITEM->GetTITEM()->m_bKind == TOURNAMENT_CANTUSEITEM_HP ||
			...
			pTITEM->GetTITEM()->m_bKind == 36 || 
			pTITEM->GetTITEM()->m_wItemID == 1577 ||
			pTITEM->GetTITEM()->m_wItemID == 1582 ||
			pTITEM->GetTITEM()->m_wItemID == 1590 ||
			pTITEM->GetTITEM()->m_wItemID == 30425 ||
			pTITEM->GetTITEM()->m_wItemID == 8205)
		{
	...
	if( pTITEM->GetTITEM()->m_bType == IT_ACTITEMS )
	{
		if( pTPET && pTITEM->GetTITEM()->m_bKind != IK_WHIP )
		{
			pTPET->TakeDown();
			DeleteRecall(pTRECALL->m_dwID, TRUE);
		}
	...
	if( pTITEM->GetTITEM()->m_bKind != IK_REVIVAL )
	{
		...
		if(!pTPET && pTITEM->GetTITEM()->m_bKind == IK_WHIP)
		{
			...
			CTChart::LoadString( TSTR_INFO_TITLE),
			m_pMainChar->GetName(), 
			CTChart::LoadString( TSTR_WHIP_ON_MOUNT),
			TCOLOR_ERROR,
			TCHAT_FLAG_INFO);
	...
	{
	case IK_PETTRANSFORM:
		{
			CTCompanionDlg* pDlg = (CTCompanionDlg*) m_vTFRAME[TFRAME_COMPANION];
			pDlg->m_bCurSelSlot = pDlg->GetSelectedCompanion(pDlg->m_bSummonedSlot) ? pDlg->m_bSummonedSlot :
				pDlg->m_bCurSelSlot;

			pDlg->ShowComponent(TRUE);

			CCompStyleDlg* pStyleDlg = (CCompStyleDlg*) m_vTFRAME[TFRAME_COMPSTYLE];
			if (!pStyleDlg->InitData(pDlg->m_bCurSelSlot, pTITEM->GetCompanionID(), bInvenID, bSlotID))
			{
				m_pMainWnd->MessageBoxOK(
					"The companion transform cannot be accessed.",
					TSTR_OK,
					GM_CLOSE_MSGBOX,
					0 );
			}
	...
	case IK_CHANGE:
		{
			CTBoxOpenDlg* pDLG = static_cast<CTBoxOpenDlg*>(m_vTFRAME[TFRAME_OPENBOX]);
			pDLG->SetInvenID(bInvenID);
			pDLG->SetItemID(bSlotID);

			pDLG->SetSession(pTITEM->GetTITEM()->m_wItemID, bInvenID, bSlotID);

			CTBoxOpenListDlg* m_pList = static_cast<CTBoxOpenListDlg*>(m_vTFRAME[TFRAME_OPENBOXLIST]);
			m_pList->SetSession(pTITEM->GetTITEM()->m_wItemID, bInvenID, bSlotID);

			EnableUI(TFRAME_OPENBOXLIST);
		}
	...
	case IK_SADDLE:
		{
			...
			if(!pTITEM->GetEndTime() && !IsInBOWMap() && !IsInBRMap())
			{
				m_pMainWnd->MessageBoxYesNo(
					_T("The saddle will be used and the Seal will be removed, do you want to continue?"),
					TSTR_YES,
					TSTR_NO,
					TCOMMAND( GM_USE_SADDLE, dwPARAM ),
					GM_CLOSE_MSGBOX,
					GM_CLOSE_MSGBOX,
					FALSE );
			}
			else
				OnCommand(TCOMMAND( GM_USE_SADDLE, dwPARAM ));
		}
		break;
	...
	case IK_COMPHP:
		{
			...
			CTCompanionDlg* pCompanion = static_cast<CTCompanionDlg*>(m_vTFRAME[TFRAME_COMPANION]);
			if(pCompanion->IsVisible())
			{
				CTClientCompanion* pSelCompanion = pCompanion->GetSelectedCompanion(pCompanion->m_bCurSelSlot);
				if(pSelCompanion && pSelCompanion->GetLife() != 300000)
				{
					if(pSelCompanion->GetLife() + pTITEM->GetTITEM()->m_wUseValue > 300000)
					{
						m_pMainWnd->MessageBoxYesNo(
							_T("If you will use this item, part of it won't be used, because your companion cannot have more than 300000HP. Do you want to use it anyways?"),
							TSTR_YES,
							TSTR_NO,
							TCOMMAND( GM_WARN_COMPITEM, dwPARAM ),
							GM_CLOSE_MSGBOX,
							GM_CLOSE_MSGBOX,
							FALSE );
					}
					else
						m_pSESSION->SendCS_USEPETITEM_REQ( dwPARAM, pCompanion->m_bCurSelSlot );
				}
			}
		}
		break;
	case IK_COMPEXP:
		{
			...
			if(pCompanion->IsVisible())
			{
				...
				if(pSelCompanion)
				{
					if(pSelCompanion->GetExp() + pTITEM->GetTITEM()->m_wUseValue > pSelCompanion->GetNextExp())
					{
						m_pMainWnd->MessageBoxYesNo(
							_T("If you will use this item, part of it won't be used, because your companion will reach more than his maximal experience. Do you want to use it anyways?"),
							TSTR_YES,
							TSTR_NO,
							TCOMMAND( GM_WARN_COMPITEM, dwPARAM ),
							GM_CLOSE_MSGBOX,
	...
	case IK_POWDER:
		{
			...
			if(pCompanion->IsVisible())
			{
				...
				{
					m_pMainWnd->MessageBoxYesNo(
						_T("If you will use this item, bonus of your currently selected companion will be changed. Do you want to continue?"),
						TSTR_YES,
						TSTR_NO,
						TCOMMAND( GM_USE_POWDER, dwPARAM ),
						GM_CLOSE_MSGBOX,
						GM_CLOSE_MSGBOX,
						FALSE );
				}
	...
	case IK_COMPRESET:
	{
		...
		if (pCompanion->IsVisible())
		{
			...
			{
				m_pMainWnd->MessageBoxYesNo(
					_T("If you will use this item, all the companions abilities will be reseted, do you want to continue?"),
					TSTR_YES,
					TSTR_NO,
					TCOMMAND(GM_USE_COMPRESET, dwPARAM),
					GM_CLOSE_MSGBOX,
					GM_CLOSE_MSGBOX,
					FALSE);
			}
	...
	case IK_COUNTRY:
		{
			...
			{
				m_pMainWnd->MessageBoxOK(
					"You can't use this Item outside of Thebekut",
					TSTR_OK,
					GM_CLOSE_MSGBOX,
					0,
					TRUE,
...
void CTClientGame::UseTPet( WORD wPetID )
{
	...
	if (GetTickCount() - m_dwMountTick < 500)
	{
		m_wPetID = wPetID;
		return;
	}

	m_dwMountTick = GetTickCount();
	m_wPetID = 0;
	...
	if( pMainPet )
	{
		if( m_pSESSION && pMainPet->m_dwID )
		{
			...
			FLOAT fSpeed = m_pMainChar->m_fSpeedFactor;	
			if(pMainPet->GetTakeUpChar() == m_pMainChar)
				fSpeed = pMainPet->m_fSpeedFactor;
	...
	else
	{
		...
		FLOAT fSpeed = m_pMainChar->m_fSpeedFactor;
		CTClientPet* pMainPet = m_pMainChar->GetRidingPet();
		if( pMainPet  &&  pMainPet->GetTakeUpChar() == m_pMainChar)
			fSpeed = pMainPet->m_fSpeedFactor;
		SendCS_MOVE_REQ(
			m_vMAP.m_wMapID,
			m_pMainChar->GetPositionX(),
...
void CTClientGame::UseTSkill( WORD wID, BYTE bActivate, BYTE bCheckGround)
{
	try
	{
	LPTSKILL pSKILL = CTChart::FindTSKILLTEMP(wID);
	if (m_pMacroProtection)
		m_pMacroProtection->NotifySkillUse(pSKILL);

	if (pSKILL && pSKILL->m_wSkillID >= 3000 && !m_pMainChar->m_dwGuildID) {
		ShowInfoChat("You have to be in guild in order to use this skill.", TUISND_TYPE_ERROR);
		return;
	}
	else if (pSKILL && pSKILL->m_wSkillID >= 3000 && pSKILL->m_bTargetType == TSKILLTARGET_ENEMY && m_pTARGET && m_pTARGET->m_bType == OT_MON)
	{
		ShowInfoChat("Guild skills cannot be used on monsters.", TUISND_TYPE_ERROR);
		return;
	}
	if (pSKILL && pSKILL->m_bKind == 21) 
	{
	...
	if( m_bTournamentBattle )
	{
		if( wID == TOURNAMENT_CANTUSESKILL || wID == 1300 || wID == 1326 || wID == 526 || wID == 527)
		{
	...
	if (!CTClientGame::m_vTOPTION.m_bDisableDoubleSkill) 
	{
		CTClientMaintain* pTMAINTAIN = m_pMainChar->FindMaintain(wID);
		if (pTMAINTAIN != NULL)
		{
			if (m_vKEY.m_dwTick - pTMAINTAIN->m_dwEndTick < 3000)
				return;

			if ((wID == 221 || wID == 536 || wID == 537 || wID == 526 || wID == 527 )&& m_vKEY.m_dwTick - pTMAINTAIN->m_dwEndTick >= 3000) 
			{
				SendCS_SKILLEND_REQ(
					m_pMainChar->m_dwID,
					m_pMainChar->m_bType,
	...
	if (m_pMainChar->m_bSubActionLock && 
		(m_pMainChar->m_vTSUBACT.m_pANI && 
		m_pMainChar->m_vTSUBACT.m_pANI->m_pANI && 
		m_pMainChar->m_vTSUBACT.m_pANI->m_pANI->m_FromSkill && 
		!m_pMainChar->m_vTSUBACT.m_pANI->m_pANI->m_bFinished))
	{
		if (m_wLastSkillID != wID)
		{
			LPSKILLUSE pSKILLUSE = new SKILLUSE();
			pSKILLUSE->m_wID = wID;
			pSKILLUSE->m_bActivate = bActivate;
			pSKILLUSE->m_bCheckGround = bCheckGround;


			if (m_qSkillQueue.size() < MAX_SKILL_QUEUE_SKILLS)
				m_qSkillQueue.push(pSKILLUSE);
			else
			{
				delete m_qSkillQueue.front();
				m_qSkillQueue.pop();
	...
	m_pMainChar->m_dwReskillTick = GetTickCount();
	switch(bRESULT)
	{
	case SKILL_WRONGDIR		:
		if(!CanLookAt())
	...
	case SKILL_SUCCESS		:
		{
			BYTE bDismount = FALSE;

			if(pTPET)
			{
				if((pTSKILL->m_pTSKILL->m_dwSpellTick ||
				!CTChart::GetTACTION( pTSKILL->m_pTSKILL->m_bActionID[TSKILLACTION_MOVEACTIVATE] )->m_bSubAction) && (
				pTPET->m_wMoveDIR != MOVE_NONE || m_bMoveType == TMAINMOVE_CLICK ||
				pTPET->IsJump() || m_pMainChar->IsFall()) )
				{
				...
				{
					TakeUpPet(m_pMainChar->m_dwID, pTPET->m_dwID);
					pTPET->TakeDown();
					DeleteRecall(pTRECALL->m_dwID, TRUE);
					bDismount = TRUE;
				}
...
*/
			...
			if((pTSKILL->m_pTSKILL->m_dwSpellTick ||
				!CTChart::GetTACTION( pTSKILL->m_pTSKILL->m_bActionID[TSKILLACTION_MOVEACTIVATE] )->m_bSubAction) && (
				m_pMainChar->m_wMoveDIR != MOVE_NONE ||
				m_pMainChar->IsJump() || m_pMainChar->IsFall()) && !bDismount)
			{
				SkillMSG( m_pMainChar, SKILL_NOTMOVESKILL);
				break;
			}
			...
			if (m_pMacroProtection)
				m_pMacroProtection->NotifySkillWentThrough(pTSKILL->m_pTSKILL);
			...
			else
			{
				...
				if( FindSkillTarget(
					...
					&param) )
				{
					//SendCS_HACKINFO_REQ(m_pMainChar->m_bDisguise);
					SendCS_SKILLUSE_REQ(
						m_pMainChar,
						pDEFEND,
						pGROUND,
							...
							FLOAT fPosZ)
{
		...
		if(pREVIVAL == m_pMainChar)
	{
		if (IsInBOWMap())
		{
			m_pMainChar->m_vWorld._41 = m_vBOWPos[m_pMainChar->m_bContryID].x;
			m_pMainChar->m_vWorld._42 = m_vBOWPos[m_pMainChar->m_bContryID].y;
			m_pMainChar->m_vWorld._43 = m_vBOWPos[m_pMainChar->m_bContryID].z;

			CTBowRespawnDlg* pRespawn = static_cast<CTBowRespawnDlg*>(m_vTFRAME[TFRAME_BOW_RESPAWN]);
			if (pRespawn->IsVisible())
				pRespawn->ShowComponent(FALSE);
		...
		else if(m_pMainChar->m_bEnterSkygarden)
		{
			CTSkygardenMap* pSkygardenMapUI = static_cast< CTSkygardenMap* >( m_vTFRAME[TFRAME_SKYGARDEN_MAP] );
			pSkygardenMapUI->ShowComponent(TRUE);
			pSkygardenMapUI->EnableComponent(TRUE);
		}
	}
}
					...
					!m_pMainChar->IsAlliance(*itTOBJ) )
				{
					...
					if( (*itTOBJ)->m_fZValue < TPICK_DIST )
					{
						D3DXVECTOR2 vDIR(
							...
							(*itTOBJ)->GetPositionZ() - m_pMainChar->GetPositionZ()); // ĳ���Ϳ��� Ÿ���� ���� Direction. (ĳ���Ͱ� ���� ������ �ƴϴ�)
						D3DXVec2Normalize( &vDIR, &vDIR);

						int nTargetDIR = m_pMainChar->GetTargetDIR( vDIR.x, vDIR.y); // Direction�� ������ �ٲ�.
						nTargetDIR = abs(nTargetDIR-m_pMainChar->m_wDIR); // ĳ���Ͱ� ���� �ִ� ������� ���� ����.
						nTargetDIR = nTargetDIR > 900 ? 1800-nTargetDIR : nTargetDIR; // 180�� �Ѵ� ���, ���� ��� 270���� 90���� �ٲ�.

						(*itTOBJ)->m_fPickWeight =
							(TPICK_DIST- (*itTOBJ)->m_fZValue ) / TPICK_DIST * 43.0f
							+ (900.0f - nTargetDIR) / 900.0f * 57.0f; // ��ŷ����ġ, �Ÿ��� 43%, ������ 57%, �ִ밪�� 100�̴�.
						...
						if(!((*itTOBJ)->m_vTSKILLTARGET.empty()))
						{
							...
							for(; itST != endST ; ++itST)
								...
								{
									(*itTOBJ)->m_fPickWeight += 100.0f; // ���� Ÿ������ ��� �ִ� ����� �ֿ켱���� ��´�. (���� �ִ밪 100�� �����ִ� ����)
...
void CTClientGame::PickNext( TPICK ePick )
{
  ...
  LISTTOBJBASE::iterator it;
	...
	static_cast<CTClientRecall*>(pTARGET)->m_bRecallType == TRECALLTYPE_PET && !static_cast<CTClientRecall*>(pTARGET)->m_wCompanionID)
	{
...
void CTClientGame::PickBRMate(BYTE bIndex)
{
	VBRTEAM::iterator it;
	for(it = m_vBRTEAM.begin(); it != m_vBRTEAM.end(); ++it) 
	{
		if((*it)->m_nIndex == bIndex)
		{
			CTClientObjBase::m_mapTSELECTOBJ.clear();
			CTClientObjBase::m_mapTSELECTDEADOBJ.clear();

			if(!FindPC((*it)->m_dwCharID))
				return;

			ResetTargetOBJ(FindPC((*it)->m_dwCharID));
			ResetTargetINFO(FALSE);
			break;
		}
	}
...
DWORD CTClientGame::GetObjectColor( CTClientObjBase *pTOBJ)
{
	...
	if(IsInBRMap())
	{
		...
		if(pTOBJHOST)
		{
			if(IsInMyBRTeam(pTOBJHOST->m_dwID))
...
{
	if( m_pMainChar->m_wMoveDIR != MOVE_NONE || m_pMainChar->IsJump() || m_pMainChar->IsFall())
	{
		m_pChatFrame->ChatSysMSG(
			CTChart::LoadString( TSTR_INFO_TITLE),
			m_pMainChar->GetName(), 
			CTChart::LoadString( TSTR_DANCE_MOVING),
			TCOLOR_ERROR,
			TCHAT_FLAG_INFO);
...
void CTClientGame::EnableGuildUI(BOOL bEnable)
{
	...
	CTGuildMainNew* pCommDlg = static_cast<CTGuildMainNew*>(m_vTFRAME[TFRAME_GUILD_BASE_NEW]);
	pCommDlg->SetBlank( TGUILD_SUMMARY,		!bEnable, msg );
	pCommDlg->SetBlank( TGUILD_MEMBERS,		!bEnable, msg );
	pCommDlg->SetBlank( TGUILD_MERCENARY,		!bEnable, msg );
	pCommDlg->SetBlank( TGUILD_JOIN,		!bEnable, msg );
...
{
	if(m_pMainChar->m_mapTINVEN.find(INVEN_BATTLE) != m_pMainChar->m_mapTINVEN.end() && (IsInBOWMap() || IsInBRMap()))
		return;
...
BYTE CTClientGame::IsInBattle(CTClientObjBase *pTATTACK, CTClientObjBase *pTDEFEND)
{
	...
	if (IsInBRMap())
	{
		...
		if (pTATTACK == m_pMainChar && !IsInMyBRTeam(pTDEFEND->m_dwID))
			return TRUE;
		else if(pTDEFEND == m_pMainChar && !IsInMyBRTeam(pTATTACK->m_dwID))
			return TRUE;
...
BYTE CTClientGame::IsCtrlRECALL( CTClientRecall *pTOBJ)
{
	if( pTOBJ->m_bRecallType == TRECALLTYPE_SPY || (
		...
		pTOBJ->m_wCompanionID == 0))
		return TRUE;

	return FALSE;
}
BYTE CTClientGame::IsCtrlSPOLECNIK( CTClientSpolecnik *pTOBJ)
{
	if( pTOBJ->m_bSpolecnikType == TRECALLTYPE_SPY || (
		pTOBJ->m_bSpolecnikType == TRECALLTYPE_PET &&
		((CTClientPet *) pTOBJ)->GetTakeUpChar()) )
		return TRUE;
...
CTClientSpolecnik *CTClientGame::GetMainSpolecnik()
{
	MAPSPOLECNIK::iterator itTRECALL;

	if(!m_vTFRAME[TFRAME_GAUGE]->IsVisible())
		return NULL;

	for( itTRECALL = m_mapSLAVESPOLECNIK.begin(); itTRECALL != m_mapSLAVESPOLECNIK.end(); itTRECALL++)
		if( (*itTRECALL).second->m_bSpolecnikType == TRECALLTYPE_MAIN )
			return (*itTRECALL).second;
									...
									BYTE bSKILL)
{
	...
	if( pTRECALL && !pTRECALL->m_bFlash && !pTRECALL->m_bActionLock && !pTRECALL->m_bSubActionLock)
	{
		CTClientObjBase *pTARGET = FindOBJ(
			dwTargetID,
			bTargetType);
...
BYTE CTClientGame::IsHostingOBJ( DWORD dwID, BYTE bType)
{
	switch(bType)
	{
	case OT_COMPANION : return FindSlaveSpolecnik(dwID) ? TRUE : FALSE;
...
BYTE CTClientGame::DeleteRecall( DWORD dwID, bool bDeleteDirect )
{
	...
	if( finder != m_mapSLAVERECALL.end() )
	{
		...
		if( pTRECALL->m_bRecallType == TRECALLTYPE_PET)
		{
			...
			SendCS_PETRIDING_REQ(pTRECALL->m_dwID, FALSE);
			SendCS_DELRECALLMON_REQ(pTRECALL->m_dwID, pTRECALL->m_bType);
		}
...
BYTE CTClientGame::DeleteSpolecnik( DWORD dwID, bool bDeleteDirect )
{
	MAPSPOLECNIK::iterator finder = m_mapSLAVESPOLECNIK.find(dwID);
	BYTE bRESULT = FALSE;
	...
	if( finder != m_mapSLAVESPOLECNIK.end() )
	{
		CTClientSpolecnik* pTRECALL = (*finder).second;

		if( pTRECALL->m_bTPOS < TRECALL_MAX )
			m_pMainChar->m_vTRECALL[pTRECALL->m_bTPOS] = FALSE;

		m_mapSLAVESPOLECNIK.erase(finder);
		bRESULT = TRUE;

		if( pTRECALL->m_bSpolecnikType == TRECALLTYPE_MAIN)
			m_pMainChar->m_bPrevMainRecallAI = pTRECALL->m_bAI;
	}

	finder = m_mapSPOLECNIK.find(dwID);
	if( finder != m_mapSPOLECNIK.end() )
	{
		if( GetPartyID(m_pMainChar) && IsRSCSEnemy((*finder).second) )
		{
...
{
	if (IsInBRMap() && wKEY != TKEY_TRSCS_ME)
	{
		PickBRMate(BYTE(wKEY - TKEY_TRSCS_F1));
		return;
	}
								    ...
								    CTClientObjBase *pTARGET)
{
	return ( pTARGET->m_bHide && !pSEEKER->IsAlliance(pTARGET) && (
		pSEEKER != m_pMainChar || !pTARGET->m_bDetect) ) || pSEEKER->m_bGMHide ? FALSE : TRUE;
}
...
void CTClientGame::CalcDynamicHelp( DWORD dwTick)
{
	...
	if (((IsInBOWMap() || IsInBRMap()) && pTGaugePannel->m_pNotify->IsVisible()) || (!m_vTOPTION.m_bAutoHelp && pTGaugePannel->m_pNotify->IsVisible()))





		pTGaugePannel->m_pNotify->ShowComponent(FALSE);


	else if (m_vTOPTION.m_bAutoHelp && !pTGaugePannel->m_pNotify->IsVisible() && !IsInBOWMap() && !IsInBRMap())
		...
		pTGaugePannel->m_pNotify->ShowComponent(TRUE);
...
void CTClientGame::CalcUtilityBar( DWORD dwTick )
{
...
void CTClientGame::CalcDUEL(DWORD dwTick)
{
	if( m_dwDuelWaitTick )
	{
		if( m_dwDuelWaitTick > dwTick )
		{
			...
			if(m_dwDuelWaitTick && m_dwDuelWaitTick < 1000)
				if(GetDuelTarget())
					if(GetDuelTarget()->m_bHide)
						GetDuelTarget()->m_bDetect = FALSE;
	...
	else
	{
		...
		{
			if( m_pMainChar->m_bIsHolded || m_pMainChar->m_bIsStuned || m_pMainChar->m_bFlash)
				m_dwDuelOutTick = DUEL_AREAOUT_TIME*1000;
									 ...
									 LPTSKILL pTSKILL)
{
	...
	case TSKILLTARGET_GUILDEXCEPTME:
	{
		if (pTATTACK->m_bType == pTDEFEND->m_bType == OT_PC)
			return static_cast<CTClientChar*>(pTATTACK)->m_dwGuildID == static_cast<CTClientChar*>(pTDEFEND)->m_dwGuildID && pTATTACK != pTDEFEND ? SKILL_SUCCESS : SKILL_WRONGTARGET;
	}
	break;
	case TSKILLTARGET_GUILD:
	{
		if (pTATTACK->m_bType == pTDEFEND->m_bType == OT_PC)
			return static_cast<CTClientChar*>(pTATTACK)->m_dwGuildID == static_cast<CTClientChar*>(pTDEFEND)->m_dwGuildID ? SKILL_SUCCESS : SKILL_WRONGTARGET;
	}
	break;
	case TSKILLTARGET_MAINRECALL		:
	case TSKILLTARGET_RECALL			:
	...
	case	TSKILLTARGET_MOUNT:
	case	TSKILLTARGET_DEADMAINCHAR:
	case	TSKILLTARGET_ORB:
	case	TSKILLTARGET_ENEMIESINFRONT:
		return SKILL_SUCCESS;
		break;
	}
...
{
	if (pTSKILL->m_wNeedItemID == 8412)
		if (CheckInvenItem(25022) == TRUE)
			return TRUE;
								...
								LPTSKILL pTSKILL)
{
	...
	if ((pTATTACK->m_bIsHolded || 
		pTATTACK->m_bIsStuned) &&
		(pTATTACK->IsFall() || 
		pTATTACK->IsJump()))
		return SKILL_STUN;
...
void CTClientGame::CheckAttackMaintain( CTClientObjBase *pTATTACK,
									    LPTSKILL pTSKILL)
{
	if( !pTATTACK || !pTSKILL || m_pMainChar != pTATTACK)
		return;
			...
			pTSKILL->m_vTFUNCTION[i]->m_wInc )
		{
			...
			//	if ((pTATTACK->m_bType == OT_PC || pTATTACK->m_bType == OT_SELF) && !bFound)
								if ((pTATTACK->m_bType == OT_PC || pTATTACK->m_bType == OT_RECALL || pTATTACK->m_bType == OT_SELF) && !bFound)
				{
					VTSKILLTARGET vTarget; vTarget.clear();
					TSKILLTARGET pTarget = TSKILLTARGET();
					pTarget.m_dwID = pTATTACK->m_dwID;
					pTarget.m_bType = pTATTACK->m_bType;
					vTarget.push_back(&pTarget);

					FinishSkill(pTATTACK, &vTGROUND, pTSKILL, vTarget, TRUE);
				}
				else
...
void CTClientGame::CheckMaintainOBJ()
{
	...
	for( itTSPOLECNIK = m_mapSLAVESPOLECNIK.begin(); itTSPOLECNIK != m_mapSLAVESPOLECNIK.end(); itTSPOLECNIK++)
		(*itTSPOLECNIK).second->CheckMaintainOBJ( this );
...
void CTClientGame::CheckTMINIMAP( UINT nFlags, CPoint point)
{
...
#ifdef NEW_IF
	if( m_pTMinimap->HitTestArea(point))
#else
...
#endif
	{
		...
		CString test;
		test.Format("You are walking to position %d, %d", (DWORD)vTPOS.x, (DWORD)vTPOS.z);

		pMain->ResetPositionMsg(test);
...
void CTClientGame::DoDEFACTION(BOOL FromKeyPress)
{
	...
	if (FromKeyPress)
	{
		if(!m_pTARGET)
		{
			if (m_dwSelectedNPC)
			{
				m_pTARGET = static_cast<CTClientNpc*>(m_vMAP.FindTNPC(m_dwSelectedNPC));
				if (m_pTARGET)
		...
		{
			if (m_pTARGET->m_bType != OT_PC && m_dwSelectedNPC)
			{
				m_pTARGET = static_cast<CTClientNpc*>(m_vMAP.FindTNPC(m_dwSelectedNPC));
				if (m_pTARGET)
					ResetTargetOBJ(m_pTARGET);
			...
			else if (m_pTARGET && m_pTARGET->m_bType == OT_MON && m_pTARGET->IsDead())
			{
				OnGM_OPEN_MONINVEN();
			}
		}
...
case OT_MON		:
		if(!m_pMainChar->IsAlliance(m_pTARGET))
			if(!m_pTARGET->IsDead())
			{
				switch(m_pMainChar->m_bClassID)
				{
				case 0:
					{
					if(GetDifferencial(m_pMainChar->m_vWorld, m_pTARGET->m_vWorld) < 5.0f)
						m_wSkillID = 38;
					else
						m_wSkillID = 32;
...
//			ResetTargetOBJ(m_pTARGET);
		...
		if( wUnitID == 3 && ((CTClientChar*)m_pTARGET)->m_bPrivateShop && !m_pTARGET->IsDead() )
			OnGM_OPEN_PRIVSHOP_FOR_BUY();
...
void CTClientGame::OnActionCmdBtnDown(CTMiniPopupDlg* pPopupDlg, CTMiniPopupDlg::CTButtonMP* pButton)
{
	...
	CTClientChar* pChar = pGame->m_pMainChar;

	pGame->DoActCmd( pButton->m_strText );
	pPopupDlg->ShowComponent( FALSE );
}

void CTClientGame::PerformBRCommand(CTMiniPopupDlg* pPopupDlg, CTMiniPopupDlg::CTButtonMP* pButton)
{
	CTClientGame* pGame = static_cast<CTClientGame*>( pPopupDlg->GetParent() );
	CString strText = CTTextLinker::GetInstance()->BuildNetString(CString(), pButton->m_strText);

	if( pButton->m_strText.GetLength() )
		pGame->GetSession()->SendCS_CHAT_REQ(
			pGame->m_pMainChar->m_strNAME,
			CHAT_PARTY,
			strText.GetLength(),
			NULL,
			strText,
			pGame->m_pMainChar->m_bContryID,
			pGame->m_mapPLAYER);
...
BOOL CTClientGame::ShowChatDetailInfo(CTClientItem* pClientItem)
{
	...
	ITDetailInfoPtr pInfo = CTDetailInfoManager::NewItemInst(pClientItem, CRect(0,0,0,0), TRUE);
...
//	m_bUseLButtonDown = !bSTART;
	m_bUseKeyboardInput = !bSTART;
	m_bUseMouseMove = !bSTART;
...
bool CTClientGame::GetSafeMainTarget(CString& outType, unsigned int& outID)
{
	try
	{
		...
		if( pGAME->m_pTARGET )
		{
			...
			case OT_LAVA		: outType = "OT_LAVA";			break;
			case OT_WHATEVER	: outType = "OT_WHATEVER";		break;
			case OT_COMPANION	: outType = "OT_COMPANION";     break;
	...
	if(!bMonRoaming)
	{
		...
		for( itMON = m_mapSLAVEMON.begin(); itMON != m_mapSLAVEMON.end(); itMON++)
			if( (*itMON).second && (*itMON).second->m_pMON && (*itMON).second->m_pMON->m_bApplyAI )
			{
				...
				if( pMONSTER->m_bFollowType != OT_NONE ) // ���𰡸� ���󰡴� ���ʹ� �� ���� ��ü!
				{
					CTClientObjBase *pFollow = FindOBJ(
						pMONSTER->m_dwFollowID,
						pMONSTER->m_bFollowType);
...
/*	CTWebCashShopDlg* pWCSDLG = (CTWebCashShopDlg*) m_vTFRAME[ TFRAME_WEB_CASHSHOP ];
	...
	static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE])->NotifyFrameStaticMoveReset(
...
CString GetFameTitle( BYTE bType, DWORD dwRank )
{
	static DWORD ENTRYTITLE[] =
	{
		...
		TSTR_TOURNAMENT_ENTRY_2, //���������� ȭ��
		TSTR_TOURNAMENT_ENTRY_3, //�縶�ĸ�ũ�� ȭ��
		TSTR_TOURNAMENT_ENTRY_4, //���Ʈ���� ȭ��
		TSTR_TOURNAMENT_ENTRY_5, //���Ǹ����� ȭ��
		TSTR_TOURNAMENT_ENTRY_6, //�Ŀ����� ȭ��
		TSTR_TOURNAMENT_ENTRY_7, //��ī�̿��� ȭ��
		TSTR_TOURNAMENT_ENTRY_8, //������ν��� ȭ��
		TSTR_TOURNAMENT_ENTRY_9 //�̺�Ʈ
	};
...
void CTClientGame::CheckMonthRank( CTClientChar* pTOBJ )
{
	...
	if(pChar->m_wTitleID)
	{
		LPTTITLE pTitle = CTChart::FindTTITLETEMP(pChar->m_wTitleID);

		if(pTitle)
		{
			pChar->m_bUseFameTitle = TRUE;
			pChar->m_strFameTitle = pTitle->m_strTitle;
		}
	}
	else
		pChar->m_bUseFameTitle = FALSE;
	...
	if( m_bDoAutoRevival )
	{
		...
		else
		{
			...
			if (IsInBOWMap() || IsInBRMap())
			{
				...
				{
					CTBowRespawnDlg* pRespawn = static_cast<CTBowRespawnDlg*>(m_vTFRAME[TFRAME_BOW_RESPAWN]);
					pRespawn->SetTime(m_dwLeftTickAR);
					if (!pRespawn->IsVisible())
						pRespawn->ShowComponent(TRUE);
				}
...
#ifdef NEW_IF
	if(m_pMainWnd->m_bChannel != BR_SERVER_ID)
	{
		pTGaugePannel->m_pChannelNew->m_strText = CTChart::Format(TSTR_FMT_NUMBER, m_pMainWnd->m_bChannel);
	}	
	else
	{
		pTGaugePannel->m_pChannelNew->m_strText = m_pMainWnd->m_bChannel != BR_SERVER_ID ? CTChart::Format(TSTR_FMT_NUMBER, m_pMainWnd->m_bChannel) : "1";
	}
#else
	pTGaugePannel->m_pLUCKY->m_strText = CTChart::Format( TSTR_CHANNEL, m_pMainWnd->m_bChannel);
#endif
}
...
CString CTClientGame::GetWMIValue(char* type,wchar_t *value)
{ 
    CString ReturnValue = "(null)"; 
    ...
    // Wenn alles geklappt hat Wert zur�ck geben! 
    return ReturnValue; 
}

void CTClientGame::ResetPopupMenu()
{
#ifdef NEW_IF
	CTMainUI* pMainDlg = static_cast<CTMainUI*>(m_vTFRAME[TFRAME_MAIN]);

	for( INT i=0; i<TMAINBUTTONNEW_COUNT; ++i)
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[i])->SelectMenuBtn(FALSE);
#endif
}

void CTClientGame::SetMsgMenuBlinkMail(BYTE bStatus)
{
	CTPopupNewMenuDlg *pPopDlg = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE])->m_pPopupMenu;

	pPopDlg->SetBlinkMail(bStatus);
}

void CTClientGame::SetMsgMenuBlinkCD(BYTE bStatus)
{
	CTPopupNewMenuDlg *pPopDlg = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE])->m_pPopupMenu;

	pPopDlg->SetBlinkCD(bStatus);
	
}

DWORD WINAPI CTClientGame::_OpenWebsiteSafely( LPVOID lpParam)
{
	((CTClientGame *) lpParam)->OpenWebsiteSafely();
	return 0;
}

void CTClientGame::OpenWebsiteSafely()
{
	ShellExecute(NULL, _T("open"), "http://4story.com/pvp/user_cp.php?m=cash_overview", NULL, NULL, SW_SHOWDEFAULT);
	CloseHandle(GetInstance()->m_hWebsiteOpener);
	GetInstance()->m_hWebsiteOpener = NULL;

	if (GetInstance()->GetSession())
		GetInstance()->GetMainWnd()->m_mapPendingMapSession.insert(MAPTPENDINGMAPSESSION::value_type(GetInstance()->GetSession()->m_bSvrID, GetInstance()->GetSession()->m_target));

	GetInstance()->GetMainWnd()->OnClose(GetInstance()->GetSession(), 1);
}

CString CTClientGame::GetMACAddress()
{
	unsigned char szMACData[6];
	...
	for(int i = 2; i<8; i++)
		szMACData[i - 2] = uuid.Data4[i];

	strMacAddress.Format("%x-%x-%x-%x-%x-%x", szMACData[0], szMACData[1], szMACData[2], szMACData[3], szMACData[4], szMACData[5]);
...
void CTClientGame::InitPetSizes()
{
	m_mapPETSIZE.clear();
	static const FLOAT fPetSize[][2] = {
		{31176, 2.8f},
		{30024, 0.8f},
		{31169, 0.9f},
		{31803, 1.5f},
	...
	for (BYTE i = 0; i < sizeof(fPetSize) / sizeof(fPetSize[0]); ++i)
		m_mapPETSIZE.insert(MAPFLOAT::value_type((DWORD) fPetSize[i][0], fPetSize[i][1]));	
}

BYTE CTClientGame::IsInBOWMap()
{
	if( m_pINSTANCE->m_pSESSION )
		if( m_pINSTANCE->m_pSESSION->m_bSvrID == BOW_SERVER_ID)
			return TRUE;

	return FALSE;
}

BYTE CTClientGame::IsInBRMap()
{
	if (m_pINSTANCE->m_pSESSION)
		if( m_pINSTANCE->GetMainWnd()->m_bChannel == BR_SERVER_ID)
			return TRUE;
...
FLOAT CTClientGame::FindCustomPetSize(DWORD dwMonID)


{
	MAPFLOAT::iterator finder = m_mapPETSIZE.find(dwMonID);
	if (finder != m_mapPETSIZE.end())
		return (*finder).second;
...
BYTE CTClientGame::IsTournamentMap()
{
	if(m_vMAP.m_wMapID >= 501 && m_vMAP.m_wMapID < 533)
		return TRUE;
...
CTPremiumSkills* CTClientGame::GetPremiumSkillsFrame(BYTE bPremium)
{
	...
	switch (bPremium)
	{
	case PREMIUMSKILL_BOW:
		return (CTPremiumSkills*) m_vTFRAME[TFRAME_BOWSKILLS]; break;
	case PREMIUMSKILL_NORMAL:
		return (CTPremiumSkills*) m_vTFRAME[TFRAME_PREMIUMSKILLS]; break;
	default: 
		break;
	}
...
void CTClientGame::UsePremiumSkill(BYTE bID)
{
	CTPremiumSkills* pPremiumSkills = NULL;
	if (bID < PREMIUM_SKILLS)
		pPremiumSkills = GetPremiumSkillsFrame(PREMIUMSKILL_BOW);
	else
		pPremiumSkills = GetPremiumSkillsFrame(PREMIUMSKILL_NORMAL);


	if (!pPremiumSkills)
		return;

	WORD wSkillID = pPremiumSkills->FindSkillID(bID < PREMIUM_SKILLS ? bID : bID - PREMIUM_SKILLS);
	if (wSkillID)
		UseTSkill(wSkillID, FALSE, TRUE);
}

void CTClientGame::PerformActSkill(WORD wSkillID)
{
	m_wActSkillID = 0;
	if( m_pMainChar->m_bPrivateShop )
		return;
	...
	if(m_pMainChar->m_bActionLock || m_pMainChar->m_bSubActionLock)
		return;

	if( CTClientGame::m_vTOPTION.m_bAutoTargeting &&
		(!m_pTARGET || m_pTARGET->m_bAction == TA_DEAD) &&
		(!m_pACT) &&
		pTSKILL->m_bRangeType == TSKILLRANGE_NONE &&
	...
	if(!m_pACT)
		m_pACT = GetSkillTarget( NULL, pTSKILL);
	...
	BYTE bRANGE = pTSKILL->m_bRangeType == TSKILLRANGE_CIRCLE ||
	...
	case SKILL_SUCCESS		:
		{
			...
			if(pTPET)
			{
				if((pTSKILL->m_dwSpellTick ||
					!CTChart::GetTACTION( pTSKILL->m_bActionID[TSKILLACTION_MOVEACTIVATE] )->m_bSubAction) && (
					pTPET->m_wMoveDIR != MOVE_NONE || m_bMoveType == TMAINMOVE_CLICK ||
					pTPET->IsJump() || m_pMainChar->IsFall()) )
				{
					SkillMSG( m_pMainChar, SKILL_NOTMOVESKILL);
			...
			if( CTClientSkill::IsAttackSKILL(pTSKILL) && m_pMainChar->m_bMode != MT_BATTLE )
				m_pSESSION->SendCS_CHGMODE_REQ(MT_BATTLE);
			...
			TACTION vActionID = m_pMainChar->FindActionID(
				pTSKILL->m_bActionID[TSKILLACTION_SPELL],
				m_pMainChar->GetWeaponID(MT_BATTLE));

			m_bLoopLock = TRUE;

			SendCS_ACTITEMSUSE_REQ(m_pMainChar, m_pACT, pTSKILL);
...
void CTClientGame::ClearSkillQueue()
{
	while (!m_qSkillQueue.empty())
	{
		delete m_qSkillQueue.front();
...
void CTClientGame::FinishSkill(CTClientObjBase *pTATTACK,
						  	   LPD3DXVECTOR3 pTGROUND,
						       LPTSKILL pTSKILL,
							   VTSKILLTARGET& vTarget, 
							   BOOL IsLinked)
{
	try
	{
	...
	if (m_pMainChar->m_dwID == pTATTACK->GetHostID())
	{
		if (!pTSKILL)
			return;
		...
		BYTE i = 0;
		while (i < vTarget.size())
		{
			CTClientObjBase *pDEFEND = FindOBJ(
				vTarget[i]->m_dwID,
				vTarget[i]->m_bType);

			if (!pDEFEND || !CanDefend(FindPC(pTATTACK->GetHostID()), pDEFEND, pTSKILL))
		...
		{
			BOOL IsFake = FALSE;
			if (pTATTACK->m_bType == OT_RECALL)
				if (dynamic_cast<CTClientFake*>(pTATTACK) != NULL)
				{
					IsFake = TRUE;
				}

			m_pSESSION->SendCS_FINISHSKILL_ACK(
				pTATTACK->GetHostID(),
				pTATTACK->m_dwID,
				pTATTACK->m_bType,
				pTGROUND->x,
	...
	for( int i=0; i<INT(pTSKILL->m_vTFUNCTION.size()); i++)
			...
			!pTSKILL->m_vTFUNCTION[i]->m_wInc )
		{
			...
			{
				CTClientObjBase *pTDEFEND = FindOBJ(
					vTarget.front()->m_dwID,
					vTarget.front()->m_bType);

				if (!pTDEFEND || !CanDefend(FindPC(pTATTACK->GetHostID()), pTDEFEND, pTSKILL))
					return;
				...
				if( pTLINK->m_bRangeType == TSKILLRANGE_LINE )
				{
					LineSHOT(pTATTACK, &vTGROUND, pTLINK, 0, TRUE); 
				}
				else if( pTLINK->m_fAtkRange > 0.0f ) 
				{
					RangeSHOT(pTATTACK, &vTGROUND, pTLINK, 0, TRUE);
				}
			}
		}
	}
	...
	{
		CString x;
		x.Format("Exception occured in the Skill System. Code : 0x00 Please report this to the 4Story Team");
		m_pMainWnd->MessageBoxOK(
			x,
			TSTR_OK,
			GM_CLOSE_MSGBOX,
...
void CTClientGame::BackupNeededFrames()
{
	static const DWORD arFrameSkillTabID[TSKILLTAB_COUNT][TCLASS_COUNT] = {
		{
			ID_FRAME_D_WARRIOR0,
			ID_FRAME_D_RANGER0,
			ID_FRAME_D_ARCHER0,
		...
		},
		{
			...
			ID_FRAME_D_WIZARD3,
			ID_FRAME_D_PRIEST3,
			ID_FRAME_D_SORCERER3
		}
	};

	static const DWORD arFrameInvenID[] = {
		ID_FRAME_DEFINVEN,
		ID_FRAME_INVEN_4,
		ID_FRAME_INVEN_6,
		ID_FRAME_INVEN_8,
	...
	for (BYTE i = 0; i < TSKILLTAB_COUNT; ++i)
	{
		for (BYTE j = 0; j < TCLASS_COUNT; ++j)
			m_arSkillTabFrame[i][j] = m_pTParser->FindFrameTemplate(arFrameSkillTabID[i][j]);
	}
	...
	for (BYTE i = 0; i < 10; ++i)
	{
		...
		m_mapInvenFrame.insert(std::make_pair(arFrameInvenID[i], m_pTParser->FindFrameTemplate(arFrameInvenID[i])));
	}
}

BYTE CTClientGame::IsInMyBRTeam(DWORD dwCharID)
{
	for (VBRTEAM::iterator it = m_vBRTEAM.begin(); it != m_vBRTEAM.end(); ++it)
	{
...
void CTClientGame::AddEffectScrolls()
{
	LPTITEM Default = CTChart::FindTITEMTEMP(7663);
	for (BYTE i = 1; i < IE_COUNT; ++i)
	{
		LPTITEM NewScroll = new TITEM(*Default);
		NewScroll->m_wItemID = 24500 + i;
		NewScroll->m_strNAME = "Formula of Transformation";
		AddEffectString(i, NewScroll->m_strNAME);

		CTChart::m_mapTITEMTEMP.insert(MAPTITEMTEMP::value_type(NewScroll->m_wItemID, NewScroll));
	}
}

void CTClientGame::AddEffectString(BYTE bEffect, CString &strText) //Let's be honest, this is a really nice implementation
{
	switch (bEffect)
	{
	case IE_SEA:
