...
#ifdef SKYGARDEN
void CTServer::SendMW_SKYGARDENOCCUPY_REQ(BYTE bType,
									  WORD wID,
									  BYTE bCountry)
{
	CPacket * pMsg = new CPacket();
	pMsg->SetID(MW_SKYGARDENOCCUPY_REQ)
		<< bType
		<< wID
		<< bCountry;
...
#ifdef SKYGARDEN
void CTServer::SendMW_SKYGARDENENABLE_REQ(BYTE bStatus,
										  DWORD dwSecond, 
										  BYTE bSkyGardenDay, 
										  DWORD dwSkyGardenStart)
{
	CPacket * pMsg = new CPacket();
	pMsg->SetID(MW_SKYGARDENENABLE_REQ)
		<< bStatus
		<< dwSecond
		<< bSkyGardenDay
		<< dwSkyGardenStart;
...
void CTServer::SendMW_MONTEMPTEVO_REQ(DWORD dwCharID,
									  DWORD dwKey,
									  DWORD dwHostID,
									  BYTE bHostType)
{
	CPacket * pMsg = new CPacket();
	pMsg->SetID(MW_MONTEMPTEVO_REQ)
		<< dwCharID
		<< dwKey
		<< dwHostID
		<< bHostType;
...
void CTServer::SendMW_CREATESPOLECNIKMON_REQ( DWORD dwCharID,
										   DWORD dwKEY,
										   DWORD dwMonID,
										   WORD wMon,
										   DWORD dwATTR,
										   ...
										   BYTE bTargetType)
{
	...
	pMSG->SetID(MW_CREATESPOLECNIKMON_REQ)
		<< dwCharID
		<< dwKEY
		<< dwMonID
		<< wMon
...
void CTServer::SendMW_SPOLECNIKMONDEL_REQ(DWORD dwCharID,
									   DWORD dwKey,
									   DWORD dwMonID,
									   BYTE bForever)
{
	CPacket * pMSG = new CPacket();
	pMSG->SetID(MW_SPOLECNIKMONDEL_REQ)
		<< dwCharID
		<< dwKey
...
void CTServer::SendMW_BOWTIMEUPDATE_ACK(BYTE bStatus, DWORD dwSecond, BYTE bDefugelPoints, BYTE bCraxionPoints)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_BOWTIMEUPDATE_ACK)
		<< bStatus
		<< dwSecond
		<< bDefugelPoints
		<< bCraxionPoints;
...
void CTServer::SendMW_ADDTOBOWQUEUE_ACK(BYTE bResult, DWORD dwCharID, DWORD dwKEY, DWORD dwTick)
{
	CPacket* pMSG = new CPacket();
	pMSG->SetID(MW_ADDTOBOWQUEUE_ACK)
		<< bResult
		<< dwCharID
		<< dwKEY
		<< dwTick;
...
void CTServer::SendMW_CANCELBOWQUEUE_ACK(BYTE bResult, DWORD dwCharID, DWORD dwKEY, DWORD dwTick)
{
	CPacket* pMSG = new CPacket();
	pMSG->SetID(MW_CANCELBOWQUEUE_ACK)
		<< bResult
		<< dwCharID
		<< dwKEY
		<< dwTick;
...
void CTServer::SendMW_NOTIFYNONQUEUEDPLAYER_ACK(DWORD dwCharID, DWORD dwKEY)
{
	CPacket* pMSG = new CPacket();
	pMSG->SetID(MW_NOTIFYNONQUEUEDPLAYER_ACK)
		<< dwCharID
		<< dwKEY;
...
void CTServer::SendMW_PREPAREFORBOW_REQ(DWORD dwCharID, DWORD dwKEY, BYTE bTeam)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_PREPAREFORBOW_REQ)
		<< dwCharID
		<< dwKEY
		<< bTeam;
...
void CTServer::SendMW_BOWCOMMANDEXEC_REQ(BYTE bCommand)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_BOWCOMMANDEXEC_REQ)
		<< bCommand;

	Say(pMsg);
}

void CTServer::SendMW_ENDBOWWAR_REQ(const MAPBOWPLAYER& mapBOWPlayers, BYTE bWinner)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_ENDBOWWAR_REQ)
		<< bWinner
		<< BYTE(mapBOWPlayers.size());
	...
	for(MAPBOWPLAYER::const_iterator itPlayer = mapBOWPlayers.begin(); itPlayer != mapBOWPlayers.end(); ++itPlayer)
	{
		(*pMsg)
			...
			<< (*itPlayer).second->m_dwKEY;
	}

	Say(pMsg);
}

void CTServer::SendMW_ADDBOWPLAYERS_ACK(const MAPBOWPLAYER& mapBOWPlayers)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_ADDBOWPLAYERS_ACK)
		<< BYTE(mapBOWPlayers.size());
...
void CTServer::SendMW_RELEASESINGLEBOWPLAYER_REQ(DWORD dwCharID, DWORD dwKEY, BYTE bWinner)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_RELEASESINGLEBOWPLAYER_REQ)
		<< dwCharID
		<< dwKEY
		<< bWinner;
...
void CTServer::SendMW_BRTIMEUPDATE_ACK(BYTE bStatus, DWORD dwSecond, BYTE bType)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_BRTIMEUPDATE_ACK)
		<< bStatus
		<< dwSecond
		<< bType;
...
void CTServer::SendMW_BRTEAMMATEADD_ACK(DWORD dwCharID, DWORD dwKEY, BYTE bResult, CString strRequester)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_BRTEAMMATEADD_ACK)
		<< bResult
		<< dwCharID
		<< dwKEY
		<< strRequester;
...
void CTServer::SendMW_UPDATEBRTEAM_ACK(DWORD dwCharID, DWORD dwKEY, CString strChiefName, const MAPBRPLAYERS& mapBRPlayers, BYTE bTeamReady)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_UPDATEBRTEAM_ACK)
		<< dwCharID
		<< dwKEY
		<< strChiefName
		<< bTeamReady
		<< (BYTE) mapBRPlayers.size();

	for (MAPBRPLAYERS::const_iterator it = mapBRPlayers.begin(); it != mapBRPlayers.end(); ++it)
	{
		(*pMsg)
			<< (*it).second->m_dwCharID
			<< (*it).second->m_strNAME
...
void CTServer::SendMW_ADDBRTEAMS_ACK(WORD wMapID, const MAPBRTEAMS& mapTeam, BYTE Mode)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_ADDBRTEAMS_ACK)
		<< wMapID
		<< Mode
		<< BYTE(mapTeam.size());

	for(MAPBRTEAMS::const_iterator itTeam = mapTeam.begin(); itTeam != mapTeam.end(); ++itTeam)
	{
		(*pMsg)
			<< (*itTeam).first;
...
void CTServer::SendMW_PREPAREFORBR_REQ(DWORD dwCharID, DWORD dwKEY, WORD wMapID)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_PREPAREFORBR_REQ)
		<< dwCharID
		<< dwKEY
		<< wMapID;
...
void CTServer::SendMW_ADDTOBRQUEUE_ACK(BYTE bResult, DWORD dwCharID, DWORD dwKEY, DWORD dwTick)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_ADDTOBRQUEUE_ACK)
		<< bResult
		<< dwCharID
		<< dwKEY
		<< dwTick;
...
void CTServer::SendMW_BRCOMMANDEXEC_REQ(BYTE bCommand)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_BRCOMMANDEXEC_REQ)
		<< bCommand;

	Say(pMsg);
}

void CTServer::SendMW_RELEASESINGLEBRPLAYER_REQ(DWORD dwCharID, DWORD dwKEY)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_RELEASESINGLEBRPLAYER_REQ)
		<< dwCharID
		<< dwKEY;
...
void CTServer::SendMW_ENDBRWAR_REQ(const MAPBRTEAMS& mapTeams)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_ENDBRWAR_REQ)
		<< BYTE(mapTeams.size());

	for (MAPBRTEAMS::const_iterator itTeam = mapTeams.begin(); itTeam != mapTeams.end(); ++itTeam)
	{
		(*pMsg)
			<< (BYTE) (*itTeam).second->m_mapBRPlayer.size(); //don't want buffer over/underflow
...
void CTServer::SendMW_RECALCCOUNTRYBALANCE_ACK(BYTE bCountryCount[TCONTRY_B])
{
	CPacket* pMSG = new CPacket();
	pMSG->SetID(MW_RECALCCOUNTRYBALANCE_ACK)
...
void CTServer::SendMW_BATTLEMODESTATUS_ACK(DWORD dwCharID, DWORD dwKEY, CTBowSystem* pBOW, CBRSystem* pBR)
{
	CPacket* pMSG = new CPacket();
	pMSG->SetID(MW_BATTLEMODESTATUS_ACK)
		<< dwCharID
		<< dwKEY;
...
void CTServer::SendMW_GUILDSKILLACTION_ACK(DWORD dwCharID, DWORD dwKEY, BYTE Action, WORD Param[])
{
	BYTE Size = Action == GS_USESKILL ? 5 : BYTE(sizeof(Param) / sizeof(WORD));
	CPacket* pMSG = new CPacket();

	pMSG->SetID(MW_GUILDSKILLACTION_ACK)
		<< dwCharID
		<< dwKEY
		<< Action
		<< Size;
...
void CTServer::SendMW_UPDATEGUILDCOOLDOWN_REQ(DWORD dwCharID, DWORD dwKEY, WORD wSkillID, BYTE bLevel, BOOL DidRenew, BOOL DidUse)
{
	CPacket* pMSG = new CPacket();

	pMSG->SetID(MW_UPDATEGUILDCOOLDOWN_REQ)
		<< dwCharID
		<< dwKEY
		<< wSkillID
		<< bLevel
...
void CTServer::SendMW_ADDCOOLDOWN_REQ(DWORD dwGuildID, WORD SkillID)
{
	CPacket* pMSG = new CPacket();

	pMSG->SetID(MW_ADDCOOLDOWN_REQ)
		<< dwGuildID
		<< SkillID;
