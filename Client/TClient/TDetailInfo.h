...
public:
	...
	BYTE GetCanHandleSecondInfo() const { return m_bCanHandle2ndInfo; }
...
class CTBRTeamInfo : public ITDetailInfo
{
public :
	CString m_strTitle;
	CString* m_strPlayer;
...
public :
	CTBRTeamInfo(const CString& strTitle, const CString* strPlayer, const CRect& rc);
	virtual ~CTBRTeamInfo();
};
...
class CTCompanionDetInfo : public CTItemInstDetInfo
{
public:	
	virtual void Build();
	//virtual BOOL Compare(const ITDetailInfo* pTarget);
...
protected:
	...
	//virtual void _ConvertAndSetTitle(const CString& strSource, DWORD dwColor = TDEFAULT_TEXT_COLOR);
	//virtual void _ConvertAndAddLine(const CString& strSource, DWORD dwColor = TDEFAULT_TEXT_COLOR);
	virtual void _BuildCompanionIcon();
	//virtual void _BuildAttrID();
	//virtual CString _GetAttrString(TINFOTOKEN_ITEM eToken);
...
class CTAfterMathInfo : public ITDetailInfo
{
public :
	CString	m_strAfter2;
	CString m_strTitle;
...
public :
	CTAfterMathInfo( CString strTitle, CString strAfter, CString strAfter2, const CRect& rc );
	virtual ~CTAfterMathInfo();
