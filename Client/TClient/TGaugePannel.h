...
public:
	...
	TComponent *m_pNotifyComp[128];
	TComponent *m_pPartyLot;
	TComponent* m_pBB;
	TComponent* m_pMedals;

	TComponent* m_pRemainingBar;
	TComponent* m_pRemainingText;
	...
	CTBFGStatus *m_pBFGStatus;
	CTBFGAlarm *m_pBFGAlarm;
	CTUtilityBarDlg* m_pUtility;
	CTCompanionMiniUI* m_pMiniUI;
	...
	TComponent *m_pBONUSPRICE[THOTKEYBASE_COUNT][MAX_HOTKEY_POS];
...
public:
	...
	void SetMedals(DWORD dwMedals);
	void SetBonusPoints(DWORD dwBB);
	void DisconnectGaugeReset();
#ifdef NEW_IF
...
public:
	CTGaugePannel( TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc, TCMLParser *pParser, CTClientGame *pMainGame, CTClientChar *pHost);
