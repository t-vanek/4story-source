// W6-9 wire test: friend-protected refuse relay.
//
// Alice (42, peer1) tries to friend Bob (200, peer2), who has
// friend-protection on (gated map-side). World relays an auto-refuse
// to Bob's map, naming Alice.
...
int main()
{
    ...
    // Alice tries to friend protected Bob → refuse relayed to Bob's map.
    {
        std::vector<std::byte> b;
        wire::WritePOD<std::uint32_t>(b, 42);     // requester
        wire::WritePOD<std::uint32_t>(b, 0xA1);
        wire::WriteString(b, "Bob");              // protected target
        SendFramed(p1, ToUint16(MessageId::MW_FRIENDPROTECTEDASK_ACK), b);
    }
    {
        auto [w, b] = ReadFramed(p2);
        EXPECT(w == ToUint16(MessageId::MW_FRIENDADD_REQ));
        wire::Reader r(b);
        std::uint32_t cid = 0, key = 0, friend_id = 0;
        std::uint8_t  result = 0;
        std::string   name;
        r.Read(cid); r.Read(key); r.Read(result); r.Read(friend_id);
        r.ReadString(name);
        EXPECT(cid == 200);                  // delivered to Bob
        EXPECT(result == frnd::kRefuse);
        EXPECT(name == "Alice");             // requester named
    }
    ...
    if (g_fails == 0)
        std::printf("PASS test_tworldsvr_asio_friend_protected_handlers\n");
    else
        std::printf("FAIL test_tworldsvr_asio_friend_protected_handlers "
                    "(%d failure%s)\n", g_fails, g_fails == 1 ? "" : "s");
