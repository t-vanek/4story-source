...
protected:
	FRAMEDESC_SHAREDPTR m_pPosFrame;
...
public:
	...
	void InitTCUSTOMCLOAKTEX();
	void ReleaseTCUSTOMCLOAKTEX();
	BYTE AddTCUSTOMCLOAKTEX(WORD wID);
	...
	BOOL VeteranBox(BYTE bOption, BYTE bFirstLevel, BYTE bSecondLevel, BYTE bThirdLevel);
...
public:
	...
	void SendCS_SECURITYCONFIRM_ACK(CString strCode);
...
public:
	...
	int OnCS_VETERAN_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_SECURITYCONFIRM_REQ( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_SECURITYRESULT_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_BOWPLAYERNOTIFY_ACK(CTachyonSession *pSession, CPacket *pPacket);


// Menu Handler
public:
	int OnGM_BACKTO_SERVER_DIRECT();
	int OnGM_CHANGE_SELECT_SERVER();
	int OnGM_CHANGE_SELECT_CHAR();
	...
	int OnGM_CHANGE_BACKTO_CHAR();
	int OnGM_DEL_CHARMSG();
	int OnGM_START_GAME();
	int OnGM_CHANGE_START_GAME();
	int OnGM_SHOW_LOADING_TEXT();
	int OnGM_PREV_FACE();
	int OnGM_PREV_HAIR();
	int OnGM_NEXT_FACE();
	int OnGM_NEXT_HAIR();
	...
	int OnGM_CHANNEL_POPUP_CLICKED();
	int OnGM_AGREEMENT();
	int OnGM_OPEN_CHANNEL_LIST();
	int OnGM_CHANGE_NEWCHAR();
	...
	int OnGM_NEXT_CHAR();
	int OnGM_PREV_CHAR();
	int OnGM_ROTATE_CHAR_LEFT();
	int OnGM_ROTATE_CHAR_RIGHT();
	int OnGM_CHAR_NAMING_OK();
	int OnGM_CHAR_NAMING_LONG();
	int OnGM_CHAR_NAMING_WRONG();
	int OnGM_SECURITY_CONFIRM();
