...
// =======================================================
CTMailSendDlg::CTMailSendDlg(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc)
	: CTClientUIBase(pParent, pDesc), m_nInvenID(INVEN_NULL), m_nSlotID(0), m_pItem(NULL), m_nCurTabIndex(0)
{
	...
	m_pCancelBtn = static_cast<TButton*>(FindKid(8545));
	TButton* pCloseBtn = static_cast<TButton*>(FindKid(1276));
	m_pCancelBtn->m_menu[TNM_LCLICK] = GM_CANCEL_POST;
	m_menu[TNM_ESC] = GM_CANCEL_POST;
	pCloseBtn->m_menu[TNM_ESC] = GM_CANCEL_POST;
	AddKid(m_pSendRune);
	AddKid(m_pSendLuna);
	AddKid(m_pSendCron);
...
BYTE CTMailSendDlg::OnBeginDrag( LPTDRAG pDRAG, CPoint point )
{
	if( m_pItem && m_pItemIcon->HitTest(point) )
	{
		if( pDRAG )
		{
			pDRAG->m_pIMAGE = new TImageList(NULL, *m_pItemIcon);
