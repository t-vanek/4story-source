...
DWORD CTControlSvrModule::OnCT_OPLOGIN_REQ( LPPACKETBUF pBUF)
{
	...
	CString log;
	log.Format("Login : %s, IP : %s", strID, inet_ntoa(pManager->m_addr.sin_addr));
	LogSmartly(log);
	...
	// CSPOPLogin's m_nRET is an SP OUTPUT param — uninitialized until
...
DWORD CTControlSvrModule::OnCT_SERVICECONTROL_REQ(LPPACKETBUF pBUF)
{
	...
	case DCSVC_STAT_RUNNING:
		...
		{			
			...
			if( (*it).second->m_pSvrType->m_bType == SVRGRP_WORLDSVR ) // ���弭���̸�
			{
...
DWORD CTControlSvrModule::OnCT_USERKICKOUT_REQ(LPPACKETBUF pBUF)
{
	...
	strLOG.Format("TCONTROL: %s kicked %s", pManager->m_strID, strUser);
	LogEvent(strLOG);
