...
{
	DWORD dwHP = 0;
	FLOAT fRate = pow(m_pTSKILL->m_f1stRateX, (m_bLevel == NULL) ? NULL : int(m_pTSKILL->m_bStartLevel)+(m_bLevel-1)*m_pTSKILL->m_bNextLevel)/100;
...
DWORD CTSkill::GetAggro()
{
	...
	DWORD dwAggro = DWORD(m_pTSKILL->m_dwAggro * pow(m_pTSKILL->m_f1stRateX, (m_bLevel == NULL) ? NULL : m_pTSKILL->m_bStartLevel+(m_bLevel-1)*m_pTSKILL->m_bNextLevel)) / 100;
	return dwAggro;
}
...
void CTSkill::Copy(CTSkill* pSkill)
{
	m_dwChargeTick = pSkill->m_dwChargeTick;
	m_dwUseTick = pSkill->m_dwUseTick;
	m_dwDelayTick = pSkill->m_dwDelayTick;
	m_dwStartTick = pSkill->m_dwStartTick;
	m_dwMaintainTick = pSkill->m_dwMaintainTick;
	m_pTSKILL = pSkill->m_pTSKILL;
	m_bLevel = pSkill->m_bLevel;
	m_dwAttackID = pSkill->m_dwAttackID;
	m_bAttackType = pSkill->m_bAttackType;
	m_dwHostID = pSkill->m_dwHostID;
	m_bHostType = pSkill->m_bHostType;
	m_bHit = pSkill->m_bHit;
	m_wAttackLevel = pSkill->m_wAttackLevel;
	m_bAttackerLevel = pSkill->m_bAttackerLevel;
	m_dwPysMinPower = pSkill->m_dwPysMinPower;
	m_dwPysMaxPower = pSkill->m_dwPysMaxPower;
	m_dwMgMinPower = pSkill->m_dwMgMinPower;
	m_dwMgMaxPower = pSkill->m_dwMgMaxPower;
	m_bCanSelect = pSkill->m_bCanSelect;
	m_wSilencedSkill = pSkill->m_wSilencedSkill;

	m_bAttackCountry = pSkill->m_bAttackCountry;
	m_fPosX = pSkill->m_fPosX;
	m_fPosY = pSkill->m_fPosY;
	m_fPosZ = pSkill->m_fPosZ;
}