...
// ====================================================================================================
CTItemSlotBaseDlg::CTItemSlotBaseDlg(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc, int nSlotCnt)
	...
	for( BYTE i=0; i<m_nSlotCount; ++i )
	{
		if( m_ItemSlot[i].pItem && m_pSlotCtrl[i]->HitTest(point) )
		{
			if( pDRAG )
			{
				...
				if( pItem )
				{
					pDRAG->m_pIMAGE = new TImageList(NULL, *m_pSlotCtrl[i]);
