...
BYTE CTClientItem::operator == ( CTClientItem& param)
{
	if( m_wItemID != param.m_wItemID ||
		...
		m_bGem != param.m_bGem ||
		m_wMoggItemID != param.m_wMoggItemID ||
		m_wCompanionID != param.m_wCompanionID ||
		m_bGradeEffect != param.m_bGradeEffect ||
		...
		m_wCustomTex != param.m_wCustomTex ||
...
DWORD CTClientItem::GetQuality() const
{
	...
	if( GetGem() > 0)
		dwResult |= TITEM_QUALITY_GEM;

	if( GetMoggItemID())
		dwResult |= TITEM_QUALITY_MOGG;
...
WORD wGradeSkin = 0;
	if( (GetTITEM()->m_bType == 17 || GetTITEM()->m_bKind == 19) && m_bGradeEffect )
	{
...
void CTClientItem::CalcAttrID()
{
	...
	m_wAttrID += CTChart::m_vTITEMGRADE[ GetGrade() ].m_bGrade + GetGem();
...
void CTClientItem::CalcAttrID(const TITEMATTR& itemAttr)
{
	...
	for(; itrMag!=endMag; ++itrMag)
	{
		...
		else
		{
			...
			case MTYPE_MAP	:
				m_bApplyMAP = TRUE;
				m_itemAttr.m_wMinMAP	+= wMagicValue + 20;
				m_itemAttr.m_wMaxMAP	+= wMagicValue + 20;
				break;
...
void CTClientItem::SetGem(BYTE p)
{
	m_bGem = p;

	NotifyUpdate();
}
void CTClientItem::SetMoggItemID(WORD p)
{
	m_wMoggItemID = p;

	NotifyUpdate();
}
void CTClientItem::SetCompanionID(WORD p)
{
		m_wCompanionID = p;
...
void CTClientItem::SetAttrID(WORD p)
{
	m_wAttrID = p;

	NotifyUpdate();
}
void CTClientItem::SetItemAttr(LPTITEMATTR p)
{
	m_itemAttr = (*p);
...
void CTClientItem::SetCustomTex( WORD wCustomTex )
{
	m_wCustomTex = wCustomTex;
