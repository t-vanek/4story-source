...
| **T9** | Mob AI tick + spawn manager | 🟡 static spawn + **timed respawn** + AI tick (**roam → chase → melee attack**: chase nearest player in range, attack when in reach → player HP down via `CS_HPMP` floored at 1; `monster_ai`, capped per tick) done; player death/revival, pathfinding, spatial AOI + per-spawn respawn delay pending |
