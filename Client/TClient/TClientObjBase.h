...
public:
	...
	BYTE m_bDoubleHead;
	BYTE m_bCutLELBOW;
	BYTE m_bChallengeTeam;
	WORD m_wChallengeArenaID;

	DWORD m_dwLastFlashColor;
	WORD  m_wSFXCostume;
public:
...
public:
	...
	void PlayTActionSFX(
		...
		DWORD dwFuncID,
		BOOL bShow = TRUE,
		DWORD dwCustomColor = 0x00000000);

	void PlayRawSFX(
		LPVECTOROBJSFX pTACTIONSFX,
		DWORD dwSfxID,
		BYTE bUseOBJScale = FALSE,
...
void ApplyCloakTexture( CTClientItem* pCloak );
	LPOBJTEX CTClientObjBase::NewCloakTexture( LPOBJTEX pTEX,
		LPTEXTURESET pSkinTEX,
		WORD wTextureID);
	void ApplyCharacterSFX( CTClientItem* pItem );
	BYTE IsFlashed();
	DWORD GetFlashColor();
	void ApplyCustomTexture(DWORD dwID, BYTE bTexIndex);
public:
	...
	virtual BYTE IsTradeAlliance( CTClientObjBase *pTARGET );
