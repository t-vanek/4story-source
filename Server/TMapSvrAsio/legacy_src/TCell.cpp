						  ...
						  DWORD dwTick)
{
	...
	for( itSEC = m_mapCOMPANION.begin(); itSEC != m_mapCOMPANION.end(); itSEC++)

		if( (*itSEC).second->m_bMain && (IsMainCell(pPlayer->m_bChannel) ||
			pPlayer->m_bChannel == (*itSEC).second->m_bChannel) )
			pPlayer->SendCS_ADDSPOLECNIKMON_ACK( (*itSEC).second, FALSE, dwTick);
						   ...
						   DWORD dwTick)
{
	...
	case OT_COMPANION :
		for( itCHAR = m_mapPLAYER.begin(); itCHAR != m_mapPLAYER.end(); itCHAR++)
			if(pCELL->IsMainCell((*itCHAR).second->m_bChannel))
				if( (*itCHAR).second && (*itCHAR).second->m_dwID )
					(*itCHAR).second->SendCS_ADDSPOLECNIKMON_ACK( (CTCompanion *) pMON, bNewMember, dwTick);
		break;
...
void CTCell::AddMonster( CTMonster *pMON)
{
	...
	case OT_SELF	:
		...
		break;
	case OT_COMPANION : 
		if( m_mapCOMPANION.find(pMON->m_dwID) == m_mapCOMPANION.end() )
			m_mapCOMPANION.insert( MAPTCOMPANION::value_type( pMON->m_dwID, (CTCompanion*) pMON));
		break;
...
void CTCell::AddCompanion( CTMonster* pMON)
{
	//m_mapCOMPANION.insert( MAPTCOMPANION::value_type( pMON->m_dwID, (CTCompanion*) pMON));
}
						   ...
						   BYTE bExitMap)
{
	...
	for( itREC = m_mapRECALLMON.begin(); itREC != m_mapRECALLMON.end(); itREC++)
		if( (*itREC).second->m_bMain &&
			...
			pPlayer->SendCS_DELRECALLMON_ACK( (*itREC).second->m_dwHostID, (*itREC).second->m_dwID, FALSE,TRUE);

	
	for( itSEC = m_mapCOMPANION.begin(); itSEC != m_mapCOMPANION.end(); itSEC++)
		if( (*itSEC).second->m_bMain &&
			(*itSEC).second->m_bChannel != pPlayer->m_bChannel &&
			(*itSEC).second->m_bChannel == bChannel )
			pPlayer->SendCS_DELSPOLECNIKMON_ACK( (*itSEC).second->m_dwHostID, (*itSEC).second->m_dwID, FALSE);
						   ...
						   BYTE bExitMap)
{
	...
	if(!IsMainCell(pPlayer->m_bChannel))
	{
		...
		for( itSEC = m_mapCOMPANION.begin(); itSEC != m_mapCOMPANION.end(); itSEC++)
			if( (*itSEC).second->m_bMain && (*itSEC).second->m_bChannel != pPlayer->m_bChannel )
				pPlayer->SendCS_DELSPOLECNIKMON_ACK( (*itSEC).second->m_dwHostID, (*itSEC).second->m_dwID, FALSE);
	...
	case OT_COMPANION	:
		{
			for( itCHAR = m_mapPLAYER.begin(); itCHAR != m_mapPLAYER.end(); itCHAR++)
				if( (*itCHAR).second->m_bChannel != pMON->m_bChannel &&
					...
					(*itCHAR).second->SendCS_DELSPOLECNIKMON_ACK( pMON->m_dwHostID, pMON->m_dwID, bExitMap);
	...
	case OT_RECALL	:
		{
			for( itCHAR = m_mapPLAYER.begin(); itCHAR != m_mapPLAYER.end(); itCHAR++)
				if( (*itCHAR).second->m_bChannel != pMON->m_bChannel &&
					(*itCHAR).second->m_bChannel == bChannel )
					(*itCHAR).second->SendCS_DELRECALLMON_ACK( pMON->m_dwHostID, pMON->m_dwID, bExitMap, TRUE);
...
void CTCell::LeaveMonster( CTMonster *pMON, BYTE bExitMap, BYTE bForever)
{
	MAPTMONSTER::iterator itMON;
	MAPPLAYER::iterator itCHAR;
	...
	case OT_RECALL	:
		if(((CTRecallMon *) pMON)->m_bMain)
		{
			for( itCHAR = m_mapPLAYER.begin(); itCHAR != m_mapPLAYER.end(); itCHAR++)
				...
				(*itCHAR).second->SendCS_DELRECALLMON_ACK( pMON->m_dwHostID, pMON->m_dwID, bExitMap, bForever);
...
void CTCell::DelMonster( CTMonster *pMON)
{
	...
	case OT_COMPANION	:
		{
			MAPTCOMPANION::iterator finder = m_mapCOMPANION.find(pMON->m_dwID);

			if( finder != m_mapCOMPANION.end() )
				m_mapCOMPANION.erase(finder);
		}
