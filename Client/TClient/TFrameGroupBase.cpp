...
// CTFrameGroupBase
// ======================================================================
CTFrameGroupBase::CTFrameGroupBase(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc, DWORD dwInnerPosID)
...
// ----------------------------------------------------------------------
CTFrameGroupBase::CTFrameGroupBase(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc, const CPoint& ptInnerPos)
...
UINT CTFrameGroupBase::AddFrame(ITInnerFrame* pFrame, DWORD dwTabID, const CString& strBlankMsg)
{
	...
	if(dwTabID == ID_CTRLINST_TAB_MAILBOX_NEW)
	{
		TComponent *pWarReward = FindKid(ID_CTRLINST_TITLE_MAIL_WAR_NEW);
		TComponent *pTake = FindKid(ID_CTRLINST_MAIL_TAKE_NEW);
		TComponent *pCheckBox = FindKid(ID_CTRLINST_MAIL_CHECKBOX_NEW);
		TComponent *pAll = FindKid(ID_CTRLINST_MAIL_ALL_NEW);
		RemoveKid(pWarReward);
		RemoveKid(pTake);
		RemoveKid(pCheckBox);
		RemoveKid(pAll);
	}
