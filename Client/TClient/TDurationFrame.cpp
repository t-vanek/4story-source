...
// ===============================================================================
CTDurationFrame::CTDurationFrame( TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc)
...
#ifdef NEW_IF
		ID_CTRLINST_DURATION_BACK,	//ES_BACK,
#else
		NULL,						//ES_BACK,
#endif
		...
		if( m_vEquip[i] )
		{
			...
			if( GetDurationPercent(i, dwPercent) )
			{
				if( 30 < dwPercent && dwPercent <= 50 )
					m_vEquip[i]->SetCurImage( DAMAGE_NORMAL); // 30%초과 ~50%이하
				else if( dwPercent <= 30 )
					m_vEquip[i]->SetCurImage( DAMAGE_YELLOW); // 30%이하

				if( dwPercent <= 50 )
					m_vEquip[i]->ShowComponent(TRUE);
				else
					m_vEquip[i]->ShowComponent(FALSE);
			}
			else
			{
				if( dwPercent == 0 ) // 깨졌다.
...
{
#ifdef NEW_IF
	if(m_pFrame->HitTest(pt) && m_pFrame->IsVisible())
		return TRUE;
#endif
