...
CString CTChart::m_vTSTRING[TSTR_COUNT];
...
MAPTTITLETEMP CTChart::m_mapTTITLETEMP;
MAPTTITLEGROUPTEMP CTChart::m_mapTTITLEGROUPTEMP;
...
VECTORDWORD CTChart::m_vBATTLERANK;

CString CTChart::CustomStrings[][2] = 
{
	{"3000", "Double-Click Skill Disable"},
	{"3001", "Hide Companion (Others)"},
	{"3002", "Hide Audience (Tournament)"},
	{"3003", "Hide Audience (Arena)"},
	{"3004", "Hide Audience (Duel)"},
	{"3005", "Hide Auto Select Circle"},
	{"3006", "Hide Accessories for other players"},
	{"3007", "Donate Life"},
	{"3008", "Disable Chat Transparency"}
	...
	if( file.Open( LPCSTR(strPath), CFile::modeRead|CFile::typeBinary ) )
	{
		...
		for( WORD i=0; i<wCount; i++)
		{
			...
			else
			{
				...
				if(wStringID == 1908)
					CTChart::m_vTSTRING[ 1908 ] = "Enter Undred";
...
{
	for (BYTE i = 0; i < sizeof(CTChart::CustomStrings) / sizeof(CTChart::CustomStrings[0]); ++i)
		if (atoi(CTChart::CustomStrings[i][0]) == (DWORD)v)
			return CTChart::CustomStrings[i][1];
...
{
	CString Custom = NAME_NULL;

	for (BYTE i = 0; i < sizeof(CTChart::CustomStrings) / sizeof(CTChart::CustomStrings[0]); ++i)
		if (atoi(CTChart::CustomStrings[i][0]) == (DWORD)v)
			Custom = CTChart::CustomStrings[i][1];
	
	if((TSTR_NONE > v || v >= TSTR_COUNT) && Custom == NAME_NULL)
		return CString("");
	...
	va_list argList;
	va_start( argList, v );
	strRESULT.FormatV( Custom == NAME_NULL ? CTChart::LoadString( v ) : Custom, argList );
	va_end( argList );
...
//			>> pTSKILL->m_fAtkRange									///ÀÎ½Ä Æø,¹Ý°æ
...
//			>> pTSKILL->m_bActive									///ÆÐ½Ãºê=0 ¾×Æ¼ºê=1stupid stupid Konst 
//

//			>> pTSKILL->m_bLoop										///ÀÚµ¿¹Ýº¹¿©ºÎ
...
void CTChart::InitTSKILLTEMP( CString strPath )
{
	...
	for( WORD i=0; i<wCount; i++)
	{
		...
		ar	>> pTSKILL->m_strNAME
			...
			>> pTSKILL->m_wItemID[0]								///¹ß»ç¾ÆÀÌÅÛ1
			>> pTSKILL->m_wItemID[1]								///¹ß»ç¾ÆÀÌÅÛ2
			>> pTSKILL->m_bActive									///ÆÐ½Ãºê=0 ¾×Æ¼ºê=1
			>> pTSKILL->m_bLoop										///ÀÚµ¿¹Ýº¹¿©ºÎ
			>> pTSKILL->m_bActionID[TSKILLACTION_SPELL]				///ÁÖ¹®µ¿ÀÛ
			>> pTSKILL->m_bActionID[TSKILLACTION_ACTIVATE]			///¹ßµ¿µ¿ÀÛ
			>> pTSKILL->m_bActionID[TSKILLACTION_MOVEACTIVATE]		///ÀÌµ¿¹ßµ¿µ¿ÀÛ
			...
			>> pTSKILL->m_wUseMedals
			>> wWORD
			>> pTSKILL->m_bPremIconID;

			if(pTSKILL->m_wSkillID == 39)
			{
				pTSKILL->m_bRangeType = TSKILLRANGE_MCIRCLE;
				pTSKILL->m_fAtkRange = 3.5f;
			}

			if (pTSKILL->m_wSkillID == 1599)
			{
				pTSKILL->m_bKind = 23;
				pTSKILL->m_dwDelay = 200000;
				pTSKILL->m_dwGroupTick = 200000;
			}

			if (pTSKILL->m_wSkillID == 1552) {
				
				pTSKILL->m_dwDelay = 150000;
			}

			if (pTSKILL->m_wSkillID >= 3000)
				pTSKILL->m_fBufRange = pTSKILL->m_fAtkRange;

			if (pTSKILL->m_bTargetType == TSKILLTARGET_ENEMIESINFRONT)
				pTSKILL->m_bTargetType = TSKILLTARGET_ENEMY;

			if (pTSKILL->m_wSkillID == 3600)
			{
				pTSKILL->m_bTargetType = TSKILLTARGET_ENEMY;
				pTSKILL->m_bRangeType = 4;
				pTSKILL->m_fAtkRange = 7.5f;
			}
	...
	LPTSKILL pRANKBOOST50 = new TSKILL(*CTChart::FindTSKILLTEMP(941));
	pRANKBOOST50->m_wSkillID = 1700;
	pRANKBOOST50->m_strNAME = "Rank Boost (+50%)";
	pRANKBOOST50->m_dwInfoID = 0;

	CTChart::m_mapTSKILLTEMP.insert(make_pair(pRANKBOOST50->m_wSkillID, pRANKBOOST50));

	LPTSKILL pRANKBOOST75 = new TSKILL(*CTChart::FindTSKILLTEMP(941));
	pRANKBOOST75->m_wSkillID = 1701;
	pRANKBOOST75->m_strNAME = "Rank Boost (+75%)";
	pRANKBOOST75->m_dwInfoID = 0;

	CTChart::m_mapTSKILLTEMP.insert(make_pair(pRANKBOOST75->m_wSkillID, pRANKBOOST75));
}
...
void CTChart::InitTPOPUPITEM( CString strPath )
{
	...
	for( WORD i=0; i<wCount; i++)
	{
		...
		{
			delete pTITEM;
			TRACE("°æ°í : %s¿¡ Áßº¹µÈ ¾ÆÀÌµð°¡ Á¸ÀçÇÕ´Ï´Ù. (ID:%u)\n", strPath, dwID);
		}
	}
}
...
void CTChart::InitTDESTINATION( CString strPath)
{
	...
	for( WORD i=0; i<wCount; i++)
	{
		...
		if (!pTUNIT)
		{
			...
			m_mapTPORTALLINK.insert(MAPTPORTALLINK::value_type(pTUNIT->m_vID.m_dwID, pTUNIT));
		}

		if(pTUNIT)

		{
			LPTPORTAL pTPORTAL = GetTPORTAL(wPortalID);
										...
										BYTE bUnitZ)
{
	...
	for(MAPTPORTALLINK::iterator it = m_mapTPORTALLINK.begin(); it != m_mapTPORTALLINK.end(); ++it)
	{
		if ((*it).second->m_vID.m_vID.m_bUnitX == bUnitX && (*it).second->m_vID.m_vID.m_bUnitZ == bUnitZ)
			return (*it).second;
	}
...
void CTChart::InitTITEMTEMP( CString strPath )
{
	...
	for( WORD i=0; i<wCount; i++)
	{
		...
		ar	>> pTITEM->m_wItemID				///ÀÏ·Ã¹øÈ£
			...
			>> pTITEM->m_fPriceRate				///°¡°ÝºñÀ²
			>> pTITEM->m_dwPrice				///±âÁØ°¡°Ý
			>> pTITEM->m_bMinRange				///ÃÖ¼Ò»çÁ¤°Å¸®
			>> pTITEM->m_bMaxRange				///ÃÖ´ë»çÁ¤°Å¸®
			>> pTITEM->m_bStack							///ÃÖ´ë¼ö·®
			>> pTITEM->m_bSlotCount				///½½·Ô°¹¼ö
			>> pTITEM->m_bCanGamble				///ºÀÀÎ »ý¼º¿©ºÎ
			>> pTITEM->m_bGambleProb			///Ä¡È¯¿©ºÎ(Ãß°¡)
			>> pTITEM->m_bDestoryProb
			>> bla1
			>> bla1								///¼Ò¸ê¿©ºÎ(Ãß°¡)

			>> pTITEM->m_bCanGrade				///µî±Þ °¡´É¿©ºÎ
			>> pTITEM->m_bCanMagic				///¸¶¹ý »ý¼º¿©ºÎ
			>> pTITEM->m_bCanRare				///Èñ±Í »ý¼º¿©ºÎ
	...
	LPTITEM pTITEM = new TITEM(*FindTITEMTEMP(7921));
	pTITEM->m_wItemID = 21500;
	pTITEM->m_wUseValue = 1700;
	pTITEM->m_strNAME = "Rank Boost (+50%)";
	pTITEM->m_dwInfoID = 21500;

	CTChart::m_mapTITEMTEMP.insert( MAPTITEMTEMP::value_type( pTITEM->m_wItemID, pTITEM));

	pTITEM = new TITEM(*FindTITEMTEMP(7921));
	pTITEM->m_wItemID = 21501;
	pTITEM->m_wUseValue = 1701;
	pTITEM->m_strNAME = "Rank Boost (+75%)";
	pTITEM->m_dwInfoID = 21501;

	CTChart::m_mapTITEMTEMP.insert( MAPTITEMTEMP::value_type( pTITEM->m_wItemID, pTITEM));

	pTITEM = new TITEM(*FindTITEMTEMP(17013));
	pTITEM->m_wItemID = 31200;
	pTITEM->m_bKind = IK_PETTRANSFORM;
	pTITEM->m_strNAME = "Companion Transform Rune";
	pTITEM->m_dwInfoID = 31200;
	pTITEM->m_bType = IT_USE;

	CTChart::m_mapTITEMTEMP.insert(MAPTITEMTEMP::value_type(pTITEM->m_wItemID, pTITEM));

	pTITEM = new TITEM(*FindTITEMTEMP(25030));
	pTITEM->m_wItemID = 25041;
	pTITEM->m_bKind = 19;
	pTITEM->m_strNAME = "Al Pacino's Cloak";
	pTITEM->m_dwInfoID = 25041;
	pTITEM->m_bType = 2;
	pTITEM->m_wAttrID = 21148;
	pTITEM->m_wUseValue = 24;

	CTChart::m_mapTITEMTEMP.insert( MAPTITEMTEMP::value_type( pTITEM->m_wItemID, pTITEM));

	pTITEM = new TITEM(*FindTITEMTEMP(25030));
	pTITEM->m_wItemID = 25042;
	pTITEM->m_bKind = 19;
	pTITEM->m_strNAME = "eZ Gucci :^) Cloak";
	pTITEM->m_dwInfoID = 25042;
	pTITEM->m_bType = 2;
	pTITEM->m_wAttrID = 21148;
	pTITEM->m_wUseValue = 25;

	CTChart::m_mapTITEMTEMP.insert(MAPTITEMTEMP::value_type(pTITEM->m_wItemID, pTITEM));

	pTITEM = new TITEM(*FindTITEMTEMP(9584));
	pTITEM->m_wItemID = 24505;
	memcpy(pTITEM->m_wVisual, FindTITEMTEMP(11671)->m_wVisual, sizeof(pTITEM->m_wVisual));
	pTITEM->m_bKind = IK_ACCESSORY_SCROLL;
	pTITEM->m_strNAME = "Accessory Refine Scroll";
...
void CTChart::InitTTITLETEMP( CString strPath )
{
	ReleaseTTITLETEMP();
	...
	for( WORD i=0; i<wCount; i++)
	{
		LPTTITLE pTTITLE = new TTITLE();

		ar	>> pTTITLE->m_wTitleID
			>> pTTITLE->m_wCategory
			>> pTTITLE->m_bInCategoryCount
			>> pTTITLE->m_bCategoryCount
			>> pTTITLE->m_bKind
			>> pTTITLE->m_dwRequirement
			>> pTTITLE->m_bValid
			>> pTTITLE->m_strTitle
			>> pTTITLE->m_strTitleLB1
			>> pTTITLE->m_strRequirement
			>> pTTITLE->m_strDescription;							

		auto itTITTLE = CTChart::m_mapTTITLETEMP.find(pTTITLE->m_wTitleID);

		if (itTITTLE == CTChart::m_mapTTITLETEMP.end())
			CTChart::m_mapTTITLETEMP.insert(MAPTTITLETEMP::value_type(pTTITLE->m_wTitleID, pTTITLE));
		else
		{
#ifdef _DEBUG
			TRACE(_T("Duplicated entries were found in InitTTITLETEMP! ID: %d Title1: %s Title2: %s\n"), pTTITLE->m_wTitleID, (*itTITTLE).second->m_strTitle.GetString(), pTTITLE->m_strTitle.GetString());
#endif
			delete pTTITLE;
		}
	}
...
void CTChart::ReleaseTTITLETEMP()
{
	MAPTTITLETEMP::iterator itTTITLE;

	for( itTTITLE = CTChart::m_mapTTITLETEMP.begin(); itTTITLE != CTChart::m_mapTTITLETEMP.end(); itTTITLE++)
		delete (*itTTITLE).second;

	CTChart::m_mapTTITLETEMP.clear();
}

LPTTITLE CTChart::FindTTITLETEMP( DWORD dwTitleID)
{
	MAPTTITLETEMP::iterator finder = CTChart::m_mapTTITLETEMP.find(dwTitleID);

	if( finder != CTChart::m_mapTTITLETEMP.end() )
		return (*finder).second;

	return NULL;
}

void CTChart::InitTTITLEGROUPTEMP( CString strPath )
{
	ReleaseTTITLEGROUPTEMP();
	...
	for( WORD i=0; i<wCount; i++)
	{
		LPTTITLEGROUP pTTITLEGROUP = new TTITLEGROUP();

		ar	>> pTTITLEGROUP->m_bID
			>> pTTITLEGROUP->m_strCaption;						

		CTChart::m_mapTTITLEGROUPTEMP.insert( MAPTTITLEGROUPTEMP::value_type( pTTITLEGROUP->m_bID, pTTITLEGROUP));

	}
}

void CTChart::ReleaseTTITLEGROUPTEMP()
{
	...
	for( itTTITLEGROUP = CTChart::m_mapTTITLEGROUPTEMP.begin(); itTTITLEGROUP != CTChart::m_mapTTITLEGROUPTEMP.end(); itTTITLEGROUP++)
		delete (*itTTITLEGROUP).second;

	CTChart::m_mapTTITLEGROUPTEMP.clear();
}

LPTTITLEGROUP CTChart::FindTTITLEGROUPTEMP( BYTE bGroupID)
{
	MAPTTITLEGROUPTEMP::iterator finder = CTChart::m_mapTTITLEGROUPTEMP.find(bGroupID);

	if( finder != CTChart::m_mapTTITLEGROUPTEMP.end() )
		return (*finder).second;
...
void CTChart::InitTITEMVISUAL( CString strPath )
{
	...
	for( WORD i=0; i<wCount; i++)
	{
		...
		if (pTITEM->m_wID == 31134 || pTITEM->m_wID == 31133)
		{
			pTITEM->m_bHideSlot = 14;
			pTITEM->m_bHidePart = 2;
			pTITEM->m_bHideRace = 63;
		}
	...
	MAPTITEMVISUAL::iterator it = CTChart::m_mapTITEMVISUAL.find(19263);
	if (it != CTChart::m_mapTITEMVISUAL.end())
	{
		LPTITEMVISUAL pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60000;
		pTITEM->m_wIcon = 60000;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60001;
		pTITEM->m_wIcon = 60001;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60002;
		pTITEM->m_wIcon = 60002;
		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60003;
		pTITEM->m_wIcon = 60003;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60004;
		pTITEM->m_wIcon = 60004;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		it = CTChart::m_mapTITEMVISUAL.find(31120);
		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60005;
		pTITEM->m_wIcon = 60005;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60006;
		pTITEM->m_wIcon = 60006;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60007;
		pTITEM->m_wIcon = 60007;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60008;
		pTITEM->m_wIcon = 60008;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60009;
		pTITEM->m_wIcon = 60009;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60010;
		pTITEM->m_wIcon = 60010;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60011;
		pTITEM->m_wIcon = 60011;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60012;
		pTITEM->m_wIcon = 60012;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60013;
		pTITEM->m_wIcon = 60013;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60014;
		pTITEM->m_wIcon = 60014;

		CTChart::m_mapTITEMVISUAL.insert(MAPTITEMVISUAL::value_type(pTITEM->m_wID, pTITEM));

		pTITEM = new TITEMVISUAL(*it->second);
		pTITEM->m_wID = 60015;
		pTITEM->m_wIcon = 60015;
...
void CTChart::InitTMONTEMP( CString strPath )
{
	...
	for( WORD i=0; i<wCount; i++)
	{
		...
		CTChart::m_mapMONTEMP.insert( MAPMONTEMP::value_type(pTEMP->wMonID, pTEMP));
	}
...
void CTChart::InitTFORMULA( CString strPath )
{
	...
	for(auto i=0; i<TMAX_LEVEL; i++)
		CTChart::m_vLEVELRATE[i] = powf( pTFORMULA->m_fRateX, static_cast<float>(i));
}
...
void CTChart::InitTITEMGRADEVISUAL( CString strPath )
{
	...
	for( WORD i=0; i<wCount; ++i)
	{
		...
		Data.m_pSkinTex = (tagTEXTURESET*) dwTextureID;
	...
	for(BYTE c=0;c<19;++c)
	{
		for(BYTE i=0;i<29;++i)
		{
			...
			{
			case 0:
				Data.m_pSkinTex = (tagTEXTURESET*)999233543;
				break;
			case 1:
				Data.m_pSkinTex = (tagTEXTURESET*)999234560;
				break;
			case 2:
				Data.m_pSkinTex = (tagTEXTURESET*)999232641;
				break;
			case 3:
			{
			...
			case 4:
			{
				...
				Data.m_pSkinTex = (tagTEXTURESET*)285967021;
				Data.m_bPSTYPE = (INT)3;
			}
			break;
			case 5:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)999232553;
				Data.m_bPSTYPE = 1;
			}
			break;
			case 6:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30000;
				Data.m_bPSTYPE = 6;
			}
			break;
			case 7:
			...
			case 10:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30004;
				Data.m_bPSTYPE = 6;
			}
			break;
			case 11:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30005;
				Data.m_bPSTYPE = 3;
			}
			break;
			case 12:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30006;
				Data.m_bPSTYPE = 6;
			}
			break;
			case 13:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30007;
				Data.m_bPSTYPE = 4;
			}
			break;
			case 14:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30008;
				Data.m_bPSTYPE = 6;
			}
			case 15:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30009;
				Data.m_bPSTYPE = 6;
			}
			break;
			case 16:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30010;
				Data.m_bPSTYPE = 6;
			}
			break;
			case 17:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30011;
				Data.m_bPSTYPE = 6;
			}
			break;
			case 18:
			...
			case 20:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30014;
				Data.m_bPSTYPE = 6;
			}
			break;
			case 21:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30015;
				Data.m_bPSTYPE = 6;
			}
			break;
			case 22:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30016;
				Data.m_bPSTYPE = 6;
			}
			break;
			case 23:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30017;
				Data.m_bPSTYPE = 2;
			}
			break;
			case 24:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30018;
				Data.m_bPSTYPE = 2;
			}
			break;
			case 25:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30019;
				Data.m_bPSTYPE = 6;
			}
			break;
			case 26:
			...
			case 27:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30021;
				Data.m_bPSTYPE = 6;
			}
			break;
			case 28:
			{
				Data.m_pSkinTex = (tagTEXTURESET*)30022;
				Data.m_bPSTYPE = 3;
			}
...
void CTChart::InitTMINIMAP( CString strPath )
{
	...
	for( WORD i=0; i<wCount; i++)
	{
		...
		ar	>> pTMINIMAP->m_dwUnitID
			...
			>> bla;
		pTMINIMAP->m_pTEX = (tagTEXTURESET*) dwTexID;
...
void CTChart::InitBattleInsignia( CString strPath )
{
	ReleaseTArena();
	...
	{
		ar >> dwPoint;
		m_vBATTLERANK.push_back(dwPoint);
	}
}

void CTChart::ReleaseBattleInsignia()
{
	m_vBATTLERANK.clear();
}

BYTE CTChart::FindRank(DWORD dwPoint)
{
	BYTE bMaxRank = 0;
	for (VECTORDWORD::iterator it = m_vBATTLERANK.begin(); it != m_vBATTLERANK.end(); ++it)
	{
		if (dwPoint >= (*it))
			bMaxRank++;
		else
