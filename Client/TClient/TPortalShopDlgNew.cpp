...
// ====================================================================================================
CTPortalShopDlg::CTPortalShopDlg( TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc )
: CTClientUIBase(pParent, pDesc)
{
	...
	pSlot0->m_pTitleImage = FindKid( ID_CTRLINST_REGION_BASE_NEW );
...
HRESULT CTPortalShopDlg::Render(DWORD dwTickCount)
{
	if(IsVisible())
	{
		if(CTClientGame::GetInstance()->GetMainChar()->m_bIsHolded || CTClientGame::GetInstance()->GetMainChar()->m_bIsStuned || CTClientGame::GetInstance()->GetMainChar()->IsFlashed() || CTClientGame::GetInstance()->GetMainChar()->CountMaintainFunc(0, SDT_RUNAWAY, 1) || CTClientGame::GetInstance()->GetClientMap()->m_wMapID == TTOURNAMENT_LOUNGE_ID && CTClientGame::GetInstance()->GetMainChar()->m_wMoveDIR != MOVE_NONE)
			ShowComponent(FALSE);
