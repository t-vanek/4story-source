...
TMeter::TMeter(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc)
	: TComponent(pParent, pDesc)
{
	m_meterDesc = pDesc->m_vCOMP.m_vEX.meter;
	Init();
}

TMeter::TMeter(TComponent* pParent, FRAMEDESC_WEAKPTR pDesc)
	: TMeter(pParent, pDesc.lock())
{

}

TMeter::TMeter(TComponent* pParent, const TMeter& rSrcMeter)
	: TComponent(pParent, rSrcMeter)
{
	m_meterDesc = rSrcMeter.m_meterDesc;
	Init();
}
...
{
	m_nValue = 0;
	m_nSubLevel = m_meterDesc.sublevel;
	m_nSuperLevel = m_meterDesc.superlevel;

	m_dwTotalTick = 0;

	m_pSuper = FindKid(m_meterDesc.super);
	for (int i = 0; i < MAX_METER_SUBLEVEL; i++)
		m_pSub[i] = (i >= m_nSubLevel) ? NULL : FindKid(m_meterDesc.sub[i]);
}
