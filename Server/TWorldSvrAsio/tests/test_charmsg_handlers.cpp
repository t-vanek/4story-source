// W6-8 wire test: GM char message relay.
//
// The control server (peer1) sends CT_CHARMSG for "Bob"; world routes
// it to Bob's main map (peer2) as MW_CHARMSG_REQ. An unknown target
// is dropped.
...
std::vector<std::byte> CharMsgBody(const std::string& name,
                                   const std::string& msg)
{
    std::vector<std::byte> b;
    tworldsvr::wire::WriteString(b, name);
    tworldsvr::wire::WriteString(b, msg);
    return b;
}
...
int main()
{
    ...
    // Unknown target → dropped; valid one still routes (sent first so
    // the dropped one can't sneak ahead in p2's stream).
    SendFramed(p1, ToUint16(MessageId::CT_CHARMSG_ACK),
               CharMsgBody("Ghost", "ignored"));
    SendFramed(p1, ToUint16(MessageId::CT_CHARMSG_ACK),
               CharMsgBody("Bob", "Hello from a GM"));
    {
        auto [w, b] = ReadFramed(p2);
        EXPECT(w == ToUint16(MessageId::MW_CHARMSG_REQ));
        wire::Reader r(b);
        std::string name, msg;
        r.ReadString(name); r.ReadString(msg);
        EXPECT(name == "Bob");
        EXPECT(msg == "Hello from a GM");
    }
    ...
    if (g_fails == 0)
        std::printf("PASS test_tworldsvr_asio_charmsg_handlers\n");
    else
        std::printf("FAIL test_tworldsvr_asio_charmsg_handlers "
                    "(%d failure%s)\n", g_fails, g_fails == 1 ? "" : "s");
