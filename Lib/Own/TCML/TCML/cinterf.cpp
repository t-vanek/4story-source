...
int set_font_attr(int attr)
{
	...
	case FACENAME		: strcpy_s(TCMLFontCur->tlfFaceName, TCMLlval.string_value);				break;
...
int set_attr(int attr)
{
	...
	case TOOLTIP:	strcpy_s(TCMLCurrent->tooltip, TCMLlval.string_value);							break;
	case TEXT_DEF:	strcpy_s(TCMLCurrent->text, TCMLlval.string_value);								break;
	default:	return -1;
	}
