...
public:
	...
	//void SendCS_HACKINFO_REQ(BYTE Disguise);
	void SendCS_WINLDISC_REQ();
...
//	void SendCS_GETREWARDS_REQ(WORD m_wUseValue);

	void SendCS_COMPANIONUPGRADE_REQ( BYTE bIndex, BYTE bSlot );
	void SendCS_USEPETITEM_REQ( DWORD dwPARAM, BYTE bCompanionSlot );
	void SendCS_DELETECOMPANION_REQ( BYTE bSlot );
	void SendCS_CHANGECOMPANIONEFFECT_REQ( BYTE bSlot );
	void SendCS_USECOMPANIONPOWDER_REQ( BYTE bInvenID, BYTE bItemID, BYTE bCompanionSlot );
	void SendCS_USECOMPRESET_REQ(BYTE bInvenID, BYTE bItemID, BYTE bCompanionSlot);

	void SendCS_USEMEDALITEM_REQ(BYTE bInvenID, BYTE bItemID, BYTE bCount = 1); 
	void SendCS_VERIFYSESSION_ACK(DWORD dwCharID);
	
	void SendCS_CUSTOMCLOAK_LIST_REQ();
	void SendCS_CUSTOMCLOAK_CHANGE_REQ(WORD wID, DWORD dwMedals);
	...
	void SendCS_CREATECOMPANION_REQ(BYTE bInvenID, BYTE bSlotID, CString strName);
#ifdef MAGICITEMSHOP
	...
	void SendCS_COMPANIONCANCEL_REQ();
	...
	void SendCS_COMPANIONRECALL_REQ( DWORD dwMonID, BYTE bCompanionSlot );
	void SendCS_COMPANIONLUP_REQ(BYTE bCompanionSlot);
	void SendCS_PETEFFECTCHANGE_REQ(
		WORD wPetID,
		BYTE bEffect,
		WORD wPrice);
	...
	void SendCS_TOURNAMENTAPPLY_REQ( BYTE bEntryID, CString strMACAddr );
	...
	void SendCS_CREATESADDLE_REQ(BYTE m_bInvenID, BYTE m_bItemID);
	void SendCS_WARCOUNTRYBALANCE_REQ();
	void SendCS_DELETESADDLE_REQ(BYTE m_bInvenID, 
											  BYTE m_bItemID,
											  WORD m_wSaddleID);
	...
	void SendCS_CMTELEPORTBATTLEMODE_REQ(BYTE bType);
	...
	void SendCS_SOULLOTTERY_REQ();
	void SendCS_SOULLOTTERY_TRIGGER_REQ();

	void SendCS_SC_CREATECODE_REQ(CString strCode);
	void SendCS_SC_CHANGECODE_REQ(CString strOldCode, CString strNewCode);
	void SendCS_SC_DISABLECODE_REQ(CString strCode);
	void SendCS_SC_ENABLECODE_REQ();
	void SendCS_SC_UNLOCK_REQ(CString strCode);
	void SendCS_SC_CHECKCODE_REQ(CString strCode, BYTE bWrongAttempts);
	void SendCS_SC_GETLOCKTIME_REQ();

	void SendCS_GETREWARDS_REQ(BYTE bKind, WORD m_wUseValue);

	void SendCS_HIDECOMPANION_REQ();
	void SendCS_USECOMPANIONITEM_REQ( BYTE bInvenID, BYTE bSlot, BYTE bCompanionSlot );
	void SendCS_DELETECOMPITEMS_REQ( BYTE bInvenID, BYTE bItemID, BYTE bSlot, BYTE bCompanionSlot );

	void SendCS_PINGMEASUREMENT_REQ();
	void SendCS_REGISTERBOW_REQ();
	void SendCS_CANCELBOWQUEUE_REQ();
	void SendCS_CASHBOWRESPAWN_REQ();
	void SendCS_EXCHANGEBOWBP_REQ(DWORD dwMedals);
	void SendCS_BOWRANKING_REQ();
	void SendCS_FINISHSKILL_ACK(
		DWORD dwAttackID, 
		DWORD dwID,
		BYTE bType,
		FLOAT fPosX,
		...
		BOOL IsLinked,
		BOOL IsFake);
	void SendCS_LEAVEBATTLEFIELD_REQ();
	void SendCS_REGISTERBR_REQ(BYTE bOnlyReady);
	void SendCS_BRTEAMMATEADD_REQ(CString strName);
	void SendCS_BRTEAMMATEDEL_REQ(CString strName);
	void SendCS_BRTEAMMATEADDRESULT_ACK(BYTE bResult, CString strName);
	void SendCS_VOTEFORBRMAP_REQ(CString strMap, BYTE Mode);
	void SendCS_DONATELIFE_REQ(INT nID);
	void SendCS_FINISHCOMPANIONTRANSFER_ACK(BYTE bSlot, BYTE bInvenID, BYTE bItemID);
	void SendCS_QUESTPOSEXEC_REQ(DWORD dwQuestID, DWORD dwTermID);



	
	void SendCS_GUILDSKILLACTION_REQ(BYTE bAction, WORD wParam[]);
	void SendCS_ARENAINFO_REQ(BYTE Arena);
	void SendCS_ARENARANKING_REQ(BYTE Arena);
public:
	CTClientSession();
