	...
	for( BYTE i=0; i<bCount; i++)
	{
		...
		else
		{
			...
			if( pSESSION )
			{
				...
				pSESSION->m_bLogicalValid = FALSE; // CS_CONNECT_ACK??| ??????aAu??iA?? AI ??OAIA?? ??????aA?? A?E??C????? Ae??AC????????A ??E??E??U.
...
int CTClientGame::OnCS_CHARINFO_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	...
	m_dwTSHADOW = 0;
	m_bStartGame = TRUE;
	pTGaugePannel->m_bCanClickReset = TRUE;
	...
	(*pPacket)
		...
		>> m_pMainChar->m_dwRankPoint
		>> bInBOWMap;
	if (bInBOWMap)
	{
		(*pPacket)
			>> m_pMainChar->m_bDeaths;
	}
	...
	m_pMainChar->SetSoulEXP(NULL);


	m_pMainChar->ClearInven();
	m_pMainChar->ClearSkill();
	m_pMainChar->ClearMaintain();
	pTGaugePannel->m_pBFGAlarm->ShowComponent(FALSE);


	if (m_bLuckyID != 255)
	{
		//pTGaugePannel->m_pLuckyNumber->m_strText = CTChart::Format( TSTR_FMT_LUCKNUMBER, m_bLuckyID);
		pTGaugePannel->m_pLuckyNumber->m_strText = CTChart::Format(TSTR_FMT_NUMBER, m_bLuckyID);
		pTGaugePannel->m_pLuckyNumber->ShowComponent(TRUE);
	}
	else
		pTGaugePannel->m_pLuckyNumber->ShowComponent(FALSE);
	...
	for (auto i = 0; i < bCount; i++)
	{
		...
		if (pTMAINTAIN->m_pTSKILL)
		{
			...
			CTClientPet* pPET = m_pMainChar->GetRidingPet();
	...
	BYTE bType;
	WORD wSkillID;
	CTPremiumSkills* pSkills = nullptr;
	for (BYTE i = 0; i < PREMIUMSKILL_COUNT; ++i)
	{
		GetPremiumSkillsFrame(i)->m_mapPREMIUMSKILL.clear();
		GetPremiumSkillsFrame(i)->Clear();
	}

	if (bInBOWMap)
	{
		for (BYTE s = 0; s < PREMIUM_SKILLS * 2; ++s)
		{
			WORD wSkillID = s < PREMIUM_SKILLS ? m_wBOWSkills[s] : m_wPremiumSkills[s - PREMIUM_SKILLS];
			pSkills = GetPremiumSkillsFrame(s < PREMIUM_SKILLS ? PREMIUMSKILL_BOW : PREMIUMSKILL_NORMAL);

			pSkills->AddSkill(s < 6 ? s : s - 6, wSkillID);
		}
	}
	...
	for (auto i = 0; i < bCount; i++)
	{
		...
		if (bSetID == 10)
		{
			for (BYTE j = 0; j < MAX_HOTKEY_POS; j++)
			{
				...
				if (!bInBOWMap)
					GetPremiumSkillsFrame(PREMIUMSKILL_BOW)->AddSkill(j, wSkillID);
			}
		}
		else
		{
	...
	{
		if (pTGaugePannel->m_pChannelNew)
	...
	static_cast<CTCharNewDlg*>( m_vTFRAME[ TFRAME_CHARINFO_NEW ] )->SetPvPPoint(
...
if (bInBOWMap)
	{
		...
		if (!m_pMainChar->m_dwHP)
		{
			...
			m_pMainChar->SetAction( vActionID.m_dwActID, vActionID.m_dwAniID);
			...
			m_pMainChar->m_bAction = TA_DEAD;
			m_pMainChar->m_bACTLevel = CTChart::GetTACTION( TA_DEAD )->m_bLevel;

			ResetTSKY();
			ResetSkillICON();

			m_bDoAutoRevival = TRUE;
			m_dwLeftTickAR = IsInBRMap() ? BR_RESPAWN_TIME_SEC * TNOTIFY_PERIOD : BOW_RESPAWN_TIME_BASE * TNOTIFY_PERIOD + (m_pMainChar->m_bDeaths * (BOW_RESPAWN_TIME_ADDER * TNOTIFY_PERIOD));
		}
		else
		{
			TACTION vActionID = m_pMainChar->FindActionID(
			...
			m_pMainChar->m_bACTLevel = CTChart::GetTACTION( 
				TA_STAND )->m_bLevel;
			m_pMainChar->m_bAction = TA_STAND;
			m_pMainChar->m_bMode = MT_NORMAL;
			m_pMainChar->m_bGhost = FALSE;
	...
	if( pTGaugePannel->m_pHonour )
		pTGaugePannel->m_pHonour->m_strText = CTChart::Format( TSTR_HONOR, m_pMainChar->m_dwPvPMonthPoint, m_pMainChar->m_dwPvPTotalPoint );
	if( pTGaugePannel->m_pAchievement )
		pTGaugePannel->m_pAchievement->m_strText = CTChart::Format( TSTR_MERIT, m_pMainChar->m_dwPvPUseablePoint );
	...
	pTGaugePannel->SetClockTime(strTime);
	pTGaugePannel->SetMedals(dwMedals);
	...
	CTSecuritySystemDlg* pSecurityDlg = static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS]);

	pSecurityDlg->SetExist(bSecureCreated);
	if (!bSecureCreated)
		bSecureDisabled = TRUE;
	pSecurityDlg->SetEnabled(!bSecureDisabled);
	...
	if(bSecureCreated)
	{
		...
		if(!bSecureDisabled)
		{
			pSecurityDlg->PrepareDisplay();
			pSecurityDlg->SwitchMode(TDLG_SC_MUNLOCK,1);
			EnableUI(TFRAME_SECURITYSYS);
		}
	}
	else
	{
		pSecurityDlg->PrepareDisplay();
		pSecurityDlg->SwitchMode(TDLG_SC_MCREATE,1);
		EnableUI(TFRAME_SECURITYSYS);
	}

	SwitchFocus(static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE]));
	if (m_vMAP.m_wMapID == BOW_MAP_ID && m_pMainChar->m_dwHP) 
	{
		m_pMainChar->SetTAction(TA_STAND);
		StopMoveMainChar();
		BYTE bActionID = TA_SPAWN;
		TACTION vActionID = m_pMainChar->FindActionID(
			bActionID,
			m_pMainChar->GetWeaponID(m_pMainChar->m_bMode));
	...
	if (CTClientGame::IsInBRMap())
		static_cast<CBRPannels*>(m_vTFRAME[TFRAME_BRPANNELS])->UpdatePannels();


//	}
//	catch(...)
//	{
//		AfxMessageBox("__CharInfo|0xFF0A000FF");
//	}
	return TERR_NONE;
}
...
int CTClientGame::OnCS_ENTER_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	CTClientPet* pPET = pCHAR->GetRidingPet();
...
FLOAT fSpeed;
	...
	if( m_pMainChar != pCHAR )
	{
		CTClientPet* pMainPet = pCHAR->GetRidingPet();
		if( pMainPet  &&  pMainPet->GetTakeUpChar() == pCHAR)
			pMainPet->SetSaddle(WORD(fSpeed*100));

		else
			pCHAR->m_fSpeedFactor = fSpeed;
	...
	if( abs( D3DXVec3Length( &vMOVE ) ) > 0.1f )
	{
...
int CTClientGame::OnCS_MONMOVE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	for( WORD i=0 ; i < wMonCount ; ++i )
	{
		...
		if (bObjType != OT_COMPANION)
		{
			pMON->m_nPITCH = INT(wPITCH) - INT(pMON->m_wPITCH);
			pMON->m_nDIR = INT(wDIR) - INT(pMON->m_wDIR);
		}

		if(CTChart::GetTACTION( bAction )->m_bNavAction && pMON->m_bAction != bAction)
			pMON->SetTAction(bAction);

		if( abs( D3DXVec3Length(&vMOVE) ) > 2.0f )
		{
...
if (dwActID == -1 || 
		dwAniID == -1) //inherit
	{
		...
		TACTION vActionID = pOBJ->FindActionID(
			pTSKILL->m_bActionID[TSKILLACTION_SPELL],
			pOBJ->GetWeaponID(MT_BATTLE));

		bActionID = pTSKILL->m_bActionID[TSKILLACTION_SPELL];
		dwActID = vActionID.m_dwActID;
		dwAniID = vActionID.m_dwAniID;
		if (m_pMainChar == pOBJ)
		{
			if(m_bGroundLOCK)
				m_bGroundLOCK = FALSE;
			...
			if (pTSKILL->m_dwSpellTick)
			{
				...
				{
					m_wActSkillID = wSkillID;
					BeginCharge(
						pTSKILL->m_dwSpellTick,
						TCHARGETYPE_SKILL);
				}
				else
					return TERR_NONE;
...
int CTClientGame::OnCS_WHIP_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	DWORD dwSpeedFactor;
	DWORD dwTimeInUse;
	...
	m_pMainChar->GetRidingPet()->SetSpeedWhenRiding(WORD(dwSpeedFactor));
	SendCS_PETRIDING_REQ(m_pMainChar->GetRidingPet()->m_dwID, 1);
...
int CTClientGame::OnCS_SKILLUSE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	WORD wBackSkillID;	// ????A?? ??c??e??A ??Y??Y????A??(????AIAU????A??)AC ????AI??
	...
	case SKILL_SUCCESS	:
		{
			...
			if((CTChart::GetTACTION( bActionID )->m_bLevel > pATTACK->m_bACTLevel &&
				...
				!pATTACK->IsDown() &&
				(!pATTACK->IsBlock() || pTSKILL->m_bUseInHold)) || pATTACK != m_pMainChar)
			{
				...
				if( pATTACK->CanUseSubANI() && CTChart::GetTACTION( bActionID )->m_bSubAction )
				{
					...
					pATTACK->SetSubAction(
						dwActID,
						dwAniID,
						pATTACK == m_pMainChar ? TRUE : FALSE,
						wSkillID);
				}
				else
				{
			...
			//	CString strDEBUG;
			// NORAGE	strDEBUG.Format("(Info) OnCS_SKILLUSE_ACK SkillID : %d, Skill Name : %s, AtkID : %d AtkNAME : %s", pTSKILL->m_wSkillID, pTSKILL->m_strNAME, pATTACK->m_dwID, pATTACK->GetName());
			//	DebugMSG(strDEBUG, FALSE, TRUE, "SkillDebug.txt");
...
int CTClientGame::OnCS_DEFEND_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if(pTDEFEND)
	{
		...
		if (pTSKILL)
		{
			if (pTATTACK && pTDEFEND != pTATTACK && pTSKILL->m_bTargetType == TSKILLTARGET_SELF)
				pTDEFEND = pTATTACK;
			...
			{
				vActionID = pTATTACK->FindActionID(
					pTSKILL->m_bActionID[TSKILLACTION_DEFEND],
					pTATTACK->GetWeaponID(pTDEFEND->m_bMode));
			}
		...
		else if (pTATTACK && m_pMainChar == pTATTACK)
		{
			if( m_bCHARGE && (bCancelCharge || m_eCHARGETYPE == TCHARGETYPE_OPEN) &&
				pTSKILL && CTChart::IsNegativeSkill(pTSKILL) )
			{
				CancelCharge(TRUE);
				pTATTACK->CancelTAction();
			}
		}
		if( bCancelCharge || (
		...
		if( pTSKILL && (pTDEFEND->m_dwHP || pTSKILL->m_bStaticWhenDie) )
		{
			if(bMaintain)
			{
				...
				if(!pTMAINTAIN)
				{
					...
					if(wSkillID == 1218)
					{
						CTClientPet* pPet = static_cast<CTClientPet*>( FindRecall(dwHostID) );
						if(pPet && !pPet->m_wCompanionID)
						pPet->SetSpeedWhenRiding(2);
					}
					...
					CTChart::GetTACTION(bActionID)->m_bLevel >= pTDEFEND->m_bACTLevel )
				{
					if( pTDEFEND->CanUseSubANI() && CTChart::GetTACTION(bActionID)->m_bSubAction )
					{
						if(!pTDEFEND->m_dwTSUBACT)
							pTDEFEND->SetSubAction(dwActID, dwAniID, pTDEFEND == pTATTACK && m_pMainChar == pTDEFEND ? TRUE : FALSE, wSkillID);
					}
					else if( pTDEFEND->m_bType != OT_PC  || !((CTClientChar *) pTDEFEND)->GetRidingPet() )
					{
						...
						pTDEFEND->SetAction(
							dwActID,
							dwAniID,
							pTDEFEND == pTATTACK && m_pMainChar == pTDEFEND ? TRUE : FALSE,
							wSkillID);
					}
				}

		//		CString strDEBUG;
		// NPRAGE		strDEBUG.Format("(Info) OnCS_DEFEND_ACK SkillID : %d, Skill Name : %s, AtkID : %d AtkNAME : %s, DefendName : %s, DefendID: %d", pTSKILL->m_wSkillID, pTSKILL->m_strNAME, pTATTACK->m_dwID, pTATTACK->GetName(), pTDEFEND->GetName(), pTDEFEND->m_dwID);
	//			DebugMSG(strDEBUG, FALSE, TRUE, "SkillDebug.txt");
			...
			if( wBackSkillID && pTDEFEND == m_pMainChar && pTATTACK )
			{
				...
				if(CheckBackSkill( pTBACKSKILL, bBackSkillItemType))
				{
					...
					if( pTSKILL->m_bRangeType == TSKILLRANGE_LINE )
											LineSHOT( m_pMainChar, &vTSKILLDATA.m_vTGROUND, pTBACKSKILL, 0, FALSE );
					else if( pTSKILL->m_fAtkRange > 0.0f )
						RangeSHOT( m_pMainChar, &vTSKILLDATA.m_vTGROUND, pTBACKSKILL, 0, FALSE );
					else
	...
	if (pTATTACK == m_pMainChar && pTDEFEND == m_pMainChar)
		if (m_pMainChar->GetRidingPet())
			m_pMainChar->GetRidingPet()->TakeDown();
...
int CTClientGame::OnCS_LEVEL_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	>> pCHAR->m_bLevel
		...
		if( bLevel < pCHAR->m_bLevel && bShowLevelUp)
		{
			...
			CheckQuestOBJ(); // ??????????? C??A???? CO ??o AO??A Au???????? A????????| ??U??A ?????????????U.
...
int CTClientGame::OnCS_SOULUP_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	DWORD dwCharID;
	...
	if(pCHAR)
	{
		if( pCHAR == m_pMainChar )
		{
			...
			pMainUI->m_pLotBtn->ShowComponent(TRUE);
			pMainUI->m_pLotBtnBlink->ShowComponent(TRUE);
#ifdef NEW_IF
			pMainUI->m_pLotBlink->ShowComponent(TRUE);
#endif
		}
...
int CTClientGame::OnCS_SOULLOTTERY_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	DWORD dwNumber1;
	DWORD dwNumber2;
	DWORD dwNumber3;
	...
	CTSoulLotteryDlg* pSoulLottery = static_cast<CTSoulLotteryDlg*>(m_vTFRAME[TFRAME_SOULLOTTERY]);
	ToggleUI(TFRAME_SOULLOTTERY);
	pSoulLottery->Init(dwNumber1, dwNumber2, dwNumber3, dwNumber4, dwMoney);

	CTMainUI* pMainUI = static_cast<CTMainUI*>( m_vTFRAME[TFRAME_MAIN] );

	pMainUI->m_pLotBtn->ShowComponent(FALSE);
	pMainUI->m_pLotBtnBlink->ShowComponent(FALSE);
#ifdef NEW_IF
	pMainUI->m_pLotBlink->ShowComponent(FALSE);
#endif
	m_pMainChar->SetSoulEXP(NULL);
	ResetPlayerINFO(FALSE);
...
int CTClientGame::OnCS_SOULLOTTERY_TRIGGER_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	DWORD dwNumber = 0;
	DWORD dwMoney = 0;
	BYTE bRet = FALSE;
	...
	CTSoulLotteryDlg* pSoulLottery = static_cast<CTSoulLotteryDlg*>(m_vTFRAME[TFRAME_SOULLOTTERY]);
	pSoulLottery->m_bRenderNumber = FALSE;
	pSoulLottery->SetMainNumber(dwNumber);
...
CTLotWinDlg* pWinDlg = static_cast<CTLotWinDlg*>(m_vTFRAME[TFRAME_LOTWIN]);
	pWinDlg->SetReward(dwMoney);

	CPoint pt;
	pSoulLottery->GetComponentPos(&pt);
	pt.y += 240;

	pWinDlg->MoveComponent(pt);
	pWinDlg->ShowComponent(TRUE);
	SwitchFocus(pWinDlg);
...
int CTClientGame::OnCS_DIE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if( pDIE == m_pMainChar )
	{
		...
		else if( m_bTournamentBattle || m_bTournamentWatching )
		{
			m_bDoAutoRevival = TRUE;
			m_dwLeftTickAR = 5000;
		}
		...
		else
		{
			...
			if( FindInven(IT_USE, IK_REVIVAL, bInven, bSlot) )
			{
				if( CTNationOption::WAITING_REVIVAL_ITEM )
				{
					...
					if( m_dwWaitingRevivalItem == 0 ) // ??IE?? AO??????? Ac??c??e??A???AI 0AI??e ??U??I ??c??e
					{
...
int CTClientGame::OnCS_EXP_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	m_pMainChar->SetSoulEXP( dwSoulLotExp );
	ResetPlayerINFO(FALSE);
...
int CTClientGame::OnCS_RESETCOOLTIME_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	{
		if ((*itTSKILL).second->m_pTSKILL->m_wSkillID >= 3000)
			continue;
	...
	case IU_SUCCESS			:
		{
			...
			/*case IK_WHIP	:
				...
				CTClientChar *pCHAR = FindPC(m_pMainChar->m_dwID);
				pCHAR->PlayTActionSFX(
					&pCHAR->m_vTGARBAGESFX,
					1005);
...
int CTClientGame::OnCS_ITEMUPGRADE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	else if( bResult == ITEMUPGRADE_GEM_SUCCESS )
	{
		pDlg->SetItem(CTItemUpDlg::SCROLL_SLOT, NULL);
		CTClientItem* pItem = pDlg->GetLastClientItem(CTItemUpDlg::TARGET_SLOT);
		if( pItem )
		...
		strMSG = CTChart::LoadString( TSTR_GEM_SUCCESS );  
	...
	else if( bResult == ITEMUPGRADE_MOGG_SUCCESS )
	{
		pDlg->SetItem(CTItemUpDlg::SCROLL_SLOT, NULL);
		CTClientItem* pItem = pDlg->GetLastClientItem(CTItemUpDlg::TARGET_SLOT);
		if( pItem )
			pItem->SetMoggItemID(wMoggItemID);
		
		strMSG = CTChart::LoadString( TSTR_MOGG_SUCCESS );
	...
	else if( bResult == ITEMUPGRADE_UNDYE )
	{
		pDlg->SetItem(CTItemUpDlg::SCROLL_SLOT, NULL);
		CTClientItem* pItem = pDlg->GetLastClientItem(CTItemUpDlg::TARGET_SLOT);
		if( pItem )
			pItem->SetGradeEffect(0);
		
		strMSG = "The Effect has been successfully cleared.";
	...
	else
	{
		...
		case ITEMUPGRADE_DOWNGRADE:
			{
				...
				strMSG = CTChart::LoadString( TSTR_ITEMUP_DOWNGRADE ); // Aa??n ???E?????? ??C????C????A??I??U! (C????c Aa??nAC ???E?? ??i????AI CI??o??C??uA?? ?????E????AI AO??A??I??U)
		...
		case ITEMUPGRADE_GEM_FAIL:
			{
				strMSG = CTChart::LoadString( TSTR_GEM_FAIL ); 
		...
		case ITEMUPGRADE_NOGEM:
			{
				strMSG = CTChart::LoadString( TSTR_HIGHEST_GEM_LEVEL ); 
			}
			break;

		case ITEMUPGREAD_NOGRADE:
			strMSG = CTChart::LoadString( TSTR_ITEMUP_ERR_CANNOTGRADE ); // AI ????AIAUA?? ???E??CO ??o ??????A??I??U.
			break;

		case ITEMUPGREAD_MOGG_WRONGKIND:
			strMSG = CTChart::LoadString( TSTR_MOGG_WRONG_KIND );
			break;

		case ITEMUPGREAD_MOGG_SAMEITEM:
			strMSG = CTChart::LoadString( TSTR_MOGG_SAME_ITEM ); // AI ????AIAUA?? ???E??CO ??o ??????A??I??U.
			break;

		case ITEMUPGRADE_MAGICFAIL:
			strMSG = CTChart::LoadString( TSTR_ITEMUP_ERR_MAGICFAIL ); // ????AIAU A|AUAI ??C????C????A??I??U!
			break;
		...
		case ITEMUPGRADE_NOREFINE:	//A|??A??C??i AOAo ??EA??
			strMSG = CTChart::LoadString( TSTR_ITEMUP_ERR_NOREFINE );
			break;

		case ITEMUPGRADE_NOGRADEEFFECT:	//???E?? AI??a???? ????A??
			strMSG = CTChart::LoadString( TSTR_ITEMUP_ERR_UNDER17 );
			break;

		case ITEMUPGRADE_CANNOT_COLOR:	//???E?? AI??a???? ????A??
			strMSG = "You cannot color this item with normal change effect scroll.";
			break;
		...
		default:
			{
				...
				strMSG = CTChart::LoadString( TSTR_ITEMUP_FAIL); // Aa??n ???E?????? ??C????CI???? C????c Aa??n??? ??O??e??C??u??A??I??U!
			}
			break;	
		}
...
int CTClientGame::OnCS_ITEMMAGICGRADE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if (pItem && pItem->GetTITEM()->m_bType == IT_COSTUME)
		pItem->SetRefineCurrent(pItem->GetRefineCurrent() + 1);
	CString strMSG;
	DWORD dwSoundID;
	BYTE bSoundType;
	...
	case TNPC_ITEM:
		{
			...
			for( BYTE i=0; i<bCount; ++i )
			{
				...
				pInfo->m_pClientItem->SetTITEM(pInfo->m_pTItem);
				pInfo->m_pClientItem->SetGem(0);
	...
	case SKILL_SUCCESS	:
		{
			...
			if (Tick) {
				pTSKILL->m_dwReuseTick = Tick;
				pTSKILL->m_dwTick = Tick;

				EnableSkillTimer(m_pMainChar, pTSKILL, TRUE);
			}
...
{
	CTGaugePannel* pTGaugePannel = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE]);
	CTPremiumSkills* PremiumUI = static_cast<CTPremiumSkills*>(GetPremiumSkillsFrame(PREMIUMSKILL_BOW));
	...
	for( BYTE i=0; i<bCount; i++)
	{
		...
		if (bSetID == 10)
		{
			...
			PremiumUI->AddSkill(bSlotID, wSkillID);
		}
		else
		{
			(*pPacket)
...
int CTClientGame::OnCS_HELPMESSAGE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if(pTGaugePannel->m_pNotify)
	{
		pTGaugePannel->m_pNotify->m_TextColor = CTChatFrame::GetChatTypeColor(CHAT_WORLD);
		if( bID != HELPMESSAGE_NORMAL )
		{
			if (!m_vTOPTION.m_bAutoHelp)
			{
				m_vTOPTION.m_bAutoHelp = TRUE;
				pTGaugePannel->NotifyFrameStaticMove(TRUE);
			}

			m_pGameSetting->UpdateUI();
			pTGaugePannel->m_pNotify->m_TextColor = D3DCOLOR_ARGB(240,255,60,0);
			m_strHelpMsg = strTEXT;
		}

		pTGaugePannel->m_pNotify->Release();
		pTGaugePannel->m_pNotify->InitTEXT(strTEXT);
		m_strHelpMsg = strTEXT;
	}
#else
	if(m_pNotifyFrame)
					...
					else if( bQuestSt == THASQUEST_COMPLETE )
					{
						...
						if( !pTMISSION )
						{
							...
							if( file.Open( strFILE, CStdioFile::typeText | CStdioFile::modeCreate | CStdioFile::modeWrite ) )
							{
								...
								strMsg.Format( "%u Au????????AC TermA?????? ??I??aAu?????????? AO??A??? ???? Au???????? ????AI????| A??A?? ??o ??????U.",
									dwQuestID);
...
int CTClientGame::OnCS_QUESTUPDATE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	CTClientQuest *pTQuest = m_pMainChar->FindTQuest(dwQuestID);
if(pTQuest && bTermType != QTT_ITEMID)

	{
		...
		CTNPCTalkDlg *pTNPCTalkDlg = (CTNPCTalkDlg *) m_vTFRAME[TFRAME_NPCDIALOG];
		CQuestNewDlg *pTQuestDlg = (CQuestNewDlg *) m_vTFRAME[TFRAME_QUEST];
		...
		case TTERMRESULT_INCOMPLETE	:
			{
				strRESULT = CTChart::Format( TSTR_QUEST_STATUS, pTTerm->m_bCount, pTTerm->m_pTTERM ? pTTerm->m_pTTERM->m_bCount : 1);
		...
		case TTERMRESULT_COMPLETE	:
			{
				strRESULT = CTChart::LoadString( TSTR_QRESULT_COMPLETE);
					strTITLE = pTTerm->m_pTTERM ? pTTerm->m_pTTERM->m_strTermMSG : "";
...
int CTClientGame::OnCS_QUESTLIST_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	for( BYTE i=0; i<bCount; i++)
	{
		...
		if (pTQuest->m_pTQUEST)
			pTQuest->m_pTQUEST->m_bIsDaily = bCountMax;
		...
		if( pTQuest->m_pTQUEST->m_bType == QT_MISSION || pTQuest->m_pTQUEST->m_bType == QT_GUILD)
...
int CTClientGame::OnCS_CHARSTATINFO_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	CTCharNewDlg* pUI = (CTCharNewDlg *) m_vTFRAME[TFRAME_CHARINFO_NEW];

	pUI->m_vTCHARSTAT[TCHARSTAT_STR]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_wSTR);

	pUI->m_vTCHARSTAT[TCHARSTAT_DEX]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_wDEX);

	pUI->m_vTCHARSTAT[TCHARSTAT_CON]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_wCON);

	pUI->m_vTCHARSTAT[TCHARSTAT_INT]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_wINT);

	pUI->m_vTCHARSTAT[TCHARSTAT_WIS]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_wWIS);

	pUI->m_vTCHARSTAT[TCHARSTAT_MEN]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_wMEN);

	pUI->m_vTCHARBASIC[TCHARBASIC_PAP]->m_strText.Format("%d~%d",
		pTCHAR->m_StatInfo.m_dwMinPysAtkPower,
		pTCHAR->m_StatInfo.m_dwMaxPysAtkPower);

	pUI->m_vTCHARBASIC[TCHARBASIC_PDP]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_dwPysDefPower);

	pUI->m_vTCHARBASIC[TCHARBASIC_PSP]->m_strText.Format("%d~%d",
		pTCHAR->m_StatInfo.m_dwMinLongAtkPower,
		pTCHAR->m_StatInfo.m_dwMaxLongAtkPower);

	pUI->m_vTCHARBASIC[TCHARBASIC_PAL]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_wAtkLevel);

	pUI->m_vTCHARBASIC[TCHARBASIC_PDL]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_wDefLevel);

	pUI->m_vTCHARBASIC[TCHARBASIC_PCR]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_bPysCriticalProb);

	pUI->m_vTCHARBASIC[TCHARBASIC_MAP]->m_strText.Format("%d~%d",
		pTCHAR->m_StatInfo.m_dwMinMagicAP,
		pTCHAR->m_StatInfo.m_dwMaxMagicAP);

	pUI->m_vTCHARBASIC[TCHARBASIC_MDP]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_dwMagicDefPower);

	pUI->m_vTCHARBASIC[TCHARBASIC_MAL]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_wMagicAtkLevel);

	pUI->m_vTCHARBASIC[TCHARBASIC_MDL]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_wMagicDefLevel);
	
	pUI->m_vTCHARBASIC[TCHARBASIC_STA]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_bChargeProb);

	pUI->m_vTCHARBASIC[TCHARBASIC_MCR]->m_strText = CTChart::Format( TSTR_FMT_NUMBER,
		pTCHAR->m_StatInfo.m_bMagicCriticalProb);

	pUI->m_vTCHARADV[TCHARADV_NATION]->m_strText = CTChart::LoadString( (TSTRING) CTClientGame::m_vTCOUNTRYSTR[pTCHAR->m_bContryID] );

	pUI->m_vTCHARADV[TCHARADV_NAME]->m_strText = pTCHAR->GetName();
	pUI->m_vTCHARADV[TCHARADV_LEVEL]->m_strText.Format("Lv%d", pTCHAR->m_bLevel);
	pUI->m_vTCHARADV[TCHARADV_GUILD]->m_strText = pTCHAR->m_strGUILD;
	pUI->m_vTCHARADV[TCHARADV_SOULMATE]->m_strText = m_strSoulName;
	pUI->m_pCOUNTRY->SetCurImage(pTCHAR->m_bContryID);
	...
	if( pTCHAR->m_bAidCountryID == TCONTRY_N )
		...
		pUI->m_vTCHARADV[TCHARADV_AID_COUNTRY]->m_strText = CTChart::LoadString( (TSTRING) CTClientGame::m_vTCOUNTRYSTR[pTCHAR->m_bAidCountryID] );
...
int CTClientGame::OnCS_SKILLEND_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if(pOBJ)
	{
		...
		if( pTMAINTAIN != NULL )
		{
					...
					pTMAINTAIN->m_pTSKILL->m_vTFUNCTION[i]->m_bFunctionID == SDT_STATUS_FINISHCURSE )
				{
					...
					if(pTFINISH)
					{
						...
						for (BYTE i = 0; i < SKILL_EXCEPTIONS_COUNT; ++i)
							if (m_wSkillSystemExceptions[i] == pTFINISH->m_wSkillID)
							{
								bFound = TRUE;
								break;
							}
						...
						{
						//	if (pTATTACK->m_bType != OT_PC || bFound)
							...
							else
							{
								...
								FinishSkill(pTATTACK, &pTMAINTAIN->m_vTSKILLDATA.m_vTGROUND, pTFINISH, vTarget, TRUE);
							}
						}
					}
				}
			...
			// finder??| Ao??i??UA?????? AdjustSDT_STATUS_MOVESPEED??| ??CCa??AA???A AIA???A
			// AI CO??o??A AuA?? MAINTAINA?? ????E??CI??e???? ???Aa ????A?? AI????O???| A?????AIAC ??O???I ????AACI??A AIA?? CI??A???
			// finder??A ??????o??? Ao??i??o??i CN MAINTAINAI??a ???????? ??e??| ????COC?????? ????E??CI??e ??E??C??a ??????AI??U.
			if( bMoveSpeed )
			{
				pOBJ->AdjustSDT_STATUS_MOVESPEED();
				if(wSkillID == 1218)
				{
					CTClientPet* pPet = static_cast<CTClientPet*>( FindRecall(dwID) );
					if(pPet && !pPet->m_wCompanionID)
					pPet->SetSpeedWhenRiding(1);
				}
			}
...
int CTClientGame::OnCS_SENDSADDLE_REQ(CTachyonSession* pSession, CPacket* pPacket)
{
	...
	CTime tEnd(tEndTime);
	pDLG->SetSaddle((WORD)m_dwSaddleID, tEnd, m_bType);
...
int CTClientGame::OnCS_ADDSPOLECNIKMON_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if (CTClientGame::m_vTOPTION.m_bDontShowOtherCompanions || IsInBRMap() || IsInBOWMap())
		if (dwHostID != m_pMainChar->m_dwID)
			return TERR_NONE;

	BYTE bCount = 0;
	for (MAPSPOLECNIK::iterator itSpolecnik = m_mapSPOLECNIK.begin(); itSpolecnik != m_mapSPOLECNIK.end(); ++itSpolecnik)
	{
		if ((*itSpolecnik).second->m_dwHostID == dwHostID)
			bCount++;
	}

	CTClientChar *pHOST = FindPC( dwHostID );
	CTClientSpolecnik *pTSPOLECNIK;

	if(FindSpolecnik(dwMonID) || bCount > 1)
		return TERR_NONE;
		

	pTSPOLECNIK = new CTClientSpolecnik();

	pTSPOLECNIK->m_pHost = m_pMainWnd;
	pTSPOLECNIK->ReleaseData();

	(*pPacket)
		>> pTSPOLECNIK->m_bContryID
		>> pTSPOLECNIK->m_bAidCountryID
		>> pTSPOLECNIK->m_bColorID
		>> pTSPOLECNIK->m_bLevel
		>> pTSPOLECNIK->m_dwMaxHP
		>> pTSPOLECNIK->m_dwHP
		>> pTSPOLECNIK->m_dwMaxMP
		>> pTSPOLECNIK->m_dwMP
		>> pTSPOLECNIK->m_vWorld._41
		>> pTSPOLECNIK->m_vWorld._42
		>> pTSPOLECNIK->m_vWorld._43
		>> wPITCH
		>> wDIR
		...
		>> bNewMember
		>> dwRegionID
		>> pTSPOLECNIK->m_bSpolecnikType
		>> pTSPOLECNIK->m_vTSKILLDATA.m_bHIT
		>> pTSPOLECNIK->m_vTSKILLDATA.m_bLevel
		>> pTSPOLECNIK->m_vTSKILLDATA.m_wAL
		>> pTSPOLECNIK->m_vTSKILLDATA.m_bAtkLevel
		>> pTSPOLECNIK->m_vTSKILLDATA.m_dwMinAP
		>> pTSPOLECNIK->m_vTSKILLDATA.m_dwMaxAP
		>> pTSPOLECNIK->m_vTSKILLDATA.m_dwMinMP
		>> pTSPOLECNIK->m_vTSKILLDATA.m_dwMaxMP;

	pTSPOLECNIK->m_pTREGION = CTChart::FindTREGIONINFO( dwRegionID);


	pTSPOLECNIK->m_bACTLevel = CTChart::GetTACTION(pTSPOLECNIK->m_bAction)->m_bLevel;
	pTSPOLECNIK->m_nPITCH = INT(wPITCH) - INT(pTSPOLECNIK->m_wPITCH);
	pTSPOLECNIK->m_nDIR = INT(wDIR) - INT(pTSPOLECNIK->m_wDIR);

	pTSPOLECNIK->m_dwHostID = dwHostID;
	pTSPOLECNIK->m_dwID = dwMonID;
	pTSPOLECNIK->m_wMonID = wTempID;
	pTSPOLECNIK->m_bCanTeleport = TRUE;

	BOOL bFakeChar = wTempID ? FALSE : TRUE;

	pTSPOLECNIK->SetEffect(bEffect);

	pTSPOLECNIK->InitSpolecnik(
		m_pDevice,
		m_pRES,
		wTempID,
		pTSPOLECNIK->m_bLevel);
	...
	pTSPOLECNIK->DoSkillFuncAll( this );	
	m_mapSPOLECNIK.insert( MAPSPOLECNIK::value_type( pTSPOLECNIK->m_dwID, pTSPOLECNIK));
	
	m_vMAP.LandTOBJ(pTSPOLECNIK);	
	if( pHOST == m_pMainChar && !FindSlaveSpolecnik(pTSPOLECNIK->m_dwID) )
	{

		m_mapSLAVESPOLECNIK.insert( MAPSPOLECNIK::value_type( pTSPOLECNIK->m_dwID, pTSPOLECNIK));
		pTSPOLECNIK->m_bTPOS = m_pMainChar->GetFreeSpolecnikPOS();

		if( pTSPOLECNIK->m_bTPOS < TRECALL_MAX )
			m_pMainChar->m_vTSPOLECNIK[pTSPOLECNIK->m_bTPOS] = TRUE;

		if( pTSPOLECNIK->m_bSpolecnikType == TRECALLTYPE_MAIN )
			pTSPOLECNIK->m_bAI = m_pMainChar->m_bPrevMainRecallAI;

//		ResetSpolecnikINFO(FALSE);
	}
	...
	if( bNewMember && 
		pHOST &&
		pHOST->m_wMoveDIR == MOVE_NONE )
	{

		pTSPOLECNIK->ApplyTAction( m_pDevice, m_pRES, TA_SPAWN);
	}
		...
		!(m_pMainChar->m_wTDETECT & (1 << OT_PC)) )
	{
		...
		for( itTSPOLECNIK = m_mapSLAVESPOLECNIK.begin(); itTSPOLECNIK != m_mapSLAVESPOLECNIK.end(); itTSPOLECNIK++)
			if( (*itTSPOLECNIK).second != pTSPOLECNIK &&
				(*itTSPOLECNIK).second->m_bTargetType == pHOST->m_bType &&
				(*itTSPOLECNIK).second->m_dwTargetID == pHOST->m_dwID )
			{
				(*itTSPOLECNIK).second->m_bTargetType = pTSPOLECNIK->m_bType;
				(*itTSPOLECNIK).second->m_dwTargetID = pTSPOLECNIK->m_dwID;
			}
		...
		if( m_pTARGET == pHOST )
		{
			...
			ResetTargetOBJ(pTSPOLECNIK);
			ResetTargetINFO(FALSE);
		}
	}

	
	if( GetPartyID(m_pMainChar) && IsRSCSEnemy(pTSPOLECNIK) )
	{
...
int CTClientGame::OnCS_DELSPOLECNIKMON_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	CTClientSpolecnik *pTSPOLECNIK = FindSpolecnik( dwMonID );
	if( !pTSPOLECNIK )
		return TERR_NONE;
	...
	if( bExitMAP )
	{
		pTSPOLECNIK->m_dwHP = 0;
		DeleteSpolecnik( pTSPOLECNIK->m_dwID, TRUE );



	}
	else if( DeleteSpolecnik( dwMonID ) ) { }
	...
	if( dwHostID == m_pMainChar->m_dwID )
		...
		pDlg->m_bSummonedSlot = T_INVALID;
...
int CTClientGame::OnCS_ADDRECALLMON_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if(wPetID)
	{
		...
		pTPET->SetPetInfo( strPetName, wPetID, CTime(0),bEffect);
		pTPET->SetEffect(bEffect);
		pTPET->ApplyPetEffect(pTPET);
		...
		if( pHOST == m_pMainChar )
		{
			...
			if( m_pSESSION &&
				pTPET->m_dwID)
			{
				m_pMainChar->CancelMaintainByFunction( this, CTClientGame::m_dwHideMaintain, sizeof(CTClientGame::m_dwHideMaintain)/sizeof(DWORD) );
...
int CTClientGame::OnCS_WARP_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	switch(bObjType)
	{
	case OT_COMPANION: pTOBJ = FindSlaveSpolecnik(dwObjID);break;
	...
	if(pTOBJ)
	{
		...
		if(m_vMAP.CheckCollisionGATE_WARP(fPosX, fPosZ, pTOBJ->m_fRadius, fPosX_ret, fPosZ_ret))
		{
			fPosX = 0.0f;
			fPosZ = 0.0f;
			fPosY = 0.0f;
		...
		pTOBJ->AddPositionX(fPosX);
		pTOBJ->AddPositionY(fPosY);
		pTOBJ->AddPositionZ(fPosZ);

		pTOBJ->m_vWorld._41 += fPosX;
		pTOBJ->m_vWorld._42 += fPosY;
		pTOBJ->m_vWorld._43 += fPosZ;
...
int CTClientGame::OnCS_TELEPORT_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	case OT_PC		:
		if( FindPC(dwObjID) == m_pMainChar )
		{
			m_pCommandsPopup->ShowComponent(FALSE);
			pTOBJ = m_pMainChar;
...
int CTClientGame::OnCS_INVENMOVE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	ResetInvenUI();
	CancelAllItemUI();
	EnableUI(TFRAME_CHARINFO_INNER);
...
int CTClientGame::OnCS_FRIENDLIST_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if(!pMsgFrame->IsSelect(TMESSENGER_FRAME_BLOCKLIST))
...
int CTClientGame::OnCS_POSTSEND_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	case POST_SUCCESS:
		{
			...
			if( pDlg->m_bSendByCashItem == 2 ) // ???CRELUz? RCCRL?? CiA?????a??? ??? ??IRE??o??e
			{
	...
	pDlg->ClearUsingByCash(); // ???CRELUz? RCCRL?? CiA?????????? ??czeCD??A A??s?????s REA| ?z?rR????D ????L?
	return TERR_NONE;
}

int CTClientGame::OnCS_POSTRECV_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	LPTMAIL_SIMPLE mail = new TMAIL_SIMPLE;
	...
	Item->pSimple = mail;
	Item->pInfo = NULL;
	CTMailListFrame *pDlg = static_cast<CTMailListFrame*>(m_vTFRAME[TFRAME_MAIL]);
	pDlg->m_pList->AddItem(0,Item);
...
int CTClientGame::OnCS_POSTINFO_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	(*pPacket)
		>> wTotalCount
		>> wNotReadCount;
	CTMailListFrame *m_pMailList = static_cast<CTMailListFrame*>(m_vTFRAME[TFRAME_MAIL]);
...
int CTClientGame::OnCS_POSTVIEW_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	pMailSimple->m_bContain = pMail->m_bContain;
...
int CTClientGame::OnCS_POSTDEL_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if( dwPostID != 0 )
	{
		...
		//m_pMailList->RemoveMail(m_pMailList->FindIndexByPostID(dwPostID));
		INT nCount = (INT) m_pMailList->m_wTotalCount;
		m_pMailList->ClearMail();
		if( --nCount > 0 )
		{
			m_pSESSION->SendCS_POSTLIST_REQ(0);
		}
		else
		{
...
int CTClientGame::OnCS_POSTGETITEM_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if( bResult == POST_SUCCESS )
	{
		...
		LPTMAIL_SIMPLE pMAIL = m_pMailList->GetMailSimple(m_pMailList->GetSelectedIndex());
		pMAIL->m_bContain = 0;
		LPTMAIL pMAILs = m_pMailList->GetMail(m_pMailList->GetSelectedIndex());
		pMAILs->m_bContain = 0;
	}
	else
	{
	...
	case PET_SUCCESS:
		{
			...
			if( nId == T_INVALID )
			{
				pDlg->AddPet(strName, wPetID, CTime(ldwTime), 0);
				strBoxMsg = CTChart::LoadString( TSTR_PETMAKE_ACK_SUCC);
			}
			else
			{
				CTime time(ldwTime);
				pDlg->SetPet(nId, strName, wPetID, time, 0);
...
int CTClientGame::OnCS_PETLIST_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	for(BYTE i=0; i<bCount; ++i)
	{
		...
		pDlg->AddPet(strName, wPetID, CTime(ldwTime), bEffect);
	}

	return TERR_NONE;
}

int CTClientGame::OnCS_PETEFFECTCHANGE_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	BYTE bResult;
	BYTE bEffect;
	CString strMSG = "STOP HACKING YOU FAGGOT";
	...
	CTPetManageDlg* pPetDlg = static_cast<CTPetManageDlg*>(m_vTFRAME[TFRAME_PET_MANAGE]);
	CTClientPet* pPet = pPetDlg->GetSelectPet();
	...
	case EFFECT_FAIL:
		 strMSG = TSTR_ERROR;
		 break;
	case EFFECT_NEEDCASH:
         strMSG = TSTR_NEEDCASH;
		 break;
	...
	if( bResult == PET_SUCCESS )
	{
		if( bAction == PETACTION_RIDING )
		{
			...
			else
			{
				...
				if( pTCHAR && pPet && !pPet->m_wCompanionID  )
				{
					VTRECALL vTRECALL;
...
int CTClientGame::OnCS_DUELEND_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	...
	CTClientRecall* pRECALL = GetMainRecall();
	if( pRECALL )
		pRECALL->m_bSubAI = TRECALLAI_BACK;
...
int CTClientGame::OnCS_GETREWARDS_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	WORD wCount;
	...
	CTBoxOpenListDlg* pList = static_cast<CTBoxOpenListDlg*>(m_vTFRAME[TFRAME_OPENBOXLIST]);
	(*pPacket)
		>> wCount;
...
int CTClientGame::OnCS_GUILDMEMBERLIST_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	for(WORD i=0; i<wCount; ++i)
	{
	 ...
	 CTGuildMainNew* pComm = static_cast<CTGuildMainNew*>( m_vTFRAME[TFRAME_GUILD_BASE_NEW] );
	if( pComm->IsVisible() &&
		 pComm->IsSelect(TGUILD_MEMBERS) )
	 {
		 pComm->ResetInfo();
	 }
...
int CTClientGame::OnCS_GUILDVOLUNTEERLIST_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	for(DWORD i=0; i<dwCount; ++i)
	{
	 ...
	 CTGuildMainNew* pDlg = static_cast<CTGuildMainNew*>( m_vTFRAME[TFRAME_GUILD_BASE_NEW] );
	 CTGuildJoinNewDlg* pFrame = static_cast<CTGuildJoinNewDlg*>( pDlg->GetInnerFrame(TGUILD_JOIN) );
	 pFrame->ResetInfo();
...
int CTClientGame::OnCS_GUILDLOCALLIST_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	std::vector<CTGuildCommander::Mission> vMission; vMission.clear();
#ifdef ADD_MISSION
	...
	CTGuildCommander::Mission BOW, BR;
	BYTE bBOWStatus;
	DWORD dwNextBOW;
	BYTE bBOWWinner;
	BYTE bBRStatus;
	...
	BOW.m_bStatus = bBOWStatus == BS_BATTLE  || bBOWStatus == BS_PEACE ? BS_BATTLE : BS_NORMAL;
	BOW.m_strMission = "Battle of the Worlds";
	BOW.m_wMissionID = 20;
	BOW.m_NextWar = CTime(CTime::GetCurrentTime().GetTime() - dwCLT + dwNextBOW);
	BOW.m_bCountry = bBOWWinner;

	pTGUILD->m_MissionVec.push_back(BOW);

	BR.m_bStatus = bBRStatus == BS_BATTLE || bBRStatus == BS_PEACE ? BS_BATTLE : BS_NORMAL;
	BR.m_strMission = "Battle Royale "; bNextBRType ? BR.m_strMission += "(Team)" : BR.m_strMission += "(Solo)";
	BR.m_NextWar = CTime(CTime::GetCurrentTime().GetTime() - dwCLT + dwNextBR);
	BR.m_wMissionID = 21;
	BR.m_bCountry = TCONTRY_N;

	pTGUILD->m_MissionVec.push_back(BR);

	for (const auto& Mission : vMission)
		pTGUILD->m_MissionVec.push_back(Mission);
...
int CTClientGame::OnCS_GUILDTACTICSLIST_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	...
	pCmd->RecvGuildTacticsList(&pCmd->m_GuildTacticsVec);
	
	CTGuildMainNew* pComm = static_cast<CTGuildMainNew*>(m_vTFRAME[TFRAME_GUILD_BASE_NEW]);
	if (pComm->IsVisible() &&
		pComm->IsSelect(TGUILD_MERCENARY))
	{
		pComm->ResetInfo();
	}
...
int CTClientGame::OnCS_GUILDTACTICSANSWER_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	CString strMSG;
	BYTE bResult;
	DWORD dwNewTacticsMemberID; //??e??????????a????AI??
	...
	if( bResult == GUILD_SUCCESS )
	{
		if( dwNewTacticsMemberID == m_pMainChar->m_dwID ) // ??e???? ??e??oAU
		{
			EnableGuildUI( TRUE );
		}
...
int CTClientGame::OnCS_PARTYMEMBERRECALL_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if( bType == 0 )
	{
		...
		case IU_NOTFOUND		: //????AIA??????????Ao A????????e????Ao
			{
...
int CTClientGame::OnCS_PARTYMEMBERRECALLANS_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if(m_pMainChar->CountMaintainFunc(0, SDT_RUNAWAY, 1) || m_pMainChar->m_bIsHolded || m_pMainChar->m_bIsStuned)
	{
...
int CTClientGame::OnCS_TITLELIST_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	CTCharInfoNewDlg* pChar = static_cast<CTCharInfoNewDlg*>(m_vTFRAME[TFRAME_CHARINFO_INNER]);
	EnableUI(TFRAME_CHARINFO_INNER);
	pChar->m_bTab = TAB_TITLE;
	CTTitleDlg* pTitleDlg = static_cast<CTTitleDlg*>(m_vTFRAME[TFRAME_TITLE]);
	pTitleDlg->ClearLists();
	BYTE bCount;
	...
	for( int i=0; i<bCount; i++)
	{
		...
		pTitleDlg->SetTitle(wTitleID, TRUE, bSelected);
	}
...
int CTClientGame::OnCS_COMPANIONLIST_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	CTCompanionDlg* pComp = static_cast< CTCompanionDlg* >( m_vTFRAME[ TFRAME_COMPANION ] );
	BYTE bCount;
	...
	for( BYTE i = 0; i < bCount; ++i )
	{
		...
		for( BYTE k = 0; k < TCHARSTAT_COUNT; ++k )
		{
			(*pPacket)
				>> wCharAttr[ k ];
		}
		...
		for( BYTE k = 0; k < 2; ++k )
			pItems[ k ] = new CTClientItem();

		LPTITEM pItem0 = CTChart::FindTITEMTEMP( wItemID[ 0 ] );
		LPTITEM pItem1 = CTChart::FindTITEMTEMP( wItemID[ 1 ] );

		if( pItem0 && wItemID[ 0 ] )
			pItems[ 0 ]->SetTITEM( pItem0 );
		if( pItem1 && wItemID[ 1 ] )
			pItems[ 1 ]->SetTITEM( pItem1 );

		CTClientCompanion* pCompanion = new CTClientCompanion();
		pCompanion->SetCompanionInfo( ( WORD ) dwMonID, strName, dwExp, dwNextExp, wLife, bLevel, bSPoints);
		pCompanion->SetSlot( bSlot );
		pCompanion->SetCharATTR( wCharAttr );
		pCompanion->SetBonusInfo( bBonusID, fBonusValue );

		for( BYTE p = 0; p < 2; ++p )
		{
			if (pItems[ p ] )
			{
				pItems[ p ]->SetEndTime( dEndTime[ p ] );
				pItems[ p ]->SetTick( 18086, dwTick );
			}
		}

	    pCompanion->AddCompanionItems( pItems );
		pComp->m_mapSpolecnici.insert( Spolecnici::value_type( bSlot, pCompanion ) );
	}

	CCompStyleDlg* pStyleDlg = (CCompStyleDlg*) m_vTFRAME[TFRAME_COMPSTYLE];
	if (pStyleDlg->IsVisible() && pStyleDlg->m_bFinished)
		pComp->m_bCurSelSlot = pStyleDlg->m_bFromSlot;
	return TERR_NONE;
}
int CTClientGame::OnCS_TITLEGAIN_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	CTTitleDlg* pTitleDlg = static_cast<CTTitleDlg*>(m_vTFRAME[TFRAME_TITLE]);
	pTitleDlg->ClearAqList();
	BYTE bStart;
	WORD wTitleGainID;
	BYTE bCount;
	BYTE bFound = FALSE;
	...
	for( int i=0; i<bCount; i++)
	{
		...
		if(m_pMainChar->m_wTitleID == wTitleID)
			bFound = TRUE;
	}
	...
	if(bStart)
	{
		if(!bFound && m_pMainChar->m_wTitleID != 0)
			m_pSESSION->SendCS_CHANGETITLE_REQ(wTitleGainID);

		LPTTITLE pTitle = CTChart::FindTTITLETEMP(wTitleGainID);
...
int CTClientGame::OnCS_TITLERESET_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	m_pSESSION->SendCS_CHANGETITLE_REQ(0);
	return TERR_NONE;
}
...
int CTClientGame::OnCS_SOULMATESEARCH_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	DWORD dwCharID;		// ??O??i????AI???????AI??
	CString strName;	// ??O??i????AI????AI????
	DWORD dwRegion;
	...
	case SOULMATE_SILENCE	:	//????E??ACA???????a
		return TERR_SOULMATE_SILENCE;
	case SOULMATE_NEEDMONEY :	//????IA??
	...
	case GAMBLE_SUCCESS	: 
		{
			...
			pDlg->SetItem(CTItemGambleDlg::TARGET_SLOT, pTItem->GetTITEM(), bInvenID, bItemID);
		}
		break;
	case GAMBLE_REPFAIL	: 
		{
...
int CTClientGame::OnCS_ENTERSKYGARDEN_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	m_pMainChar->EnterSkygarden();

	CTSkygardenMap* pSkygardenMapUI = static_cast< CTSkygardenMap* >( m_vTFRAME[TFRAME_SKYGARDEN_MAP] );
	pSkygardenMapUI->LoadCountries(bDefendCountry, bLeftCountry, bCenterCountry, bRightCountry, bAttackCountry);
...
int CTClientGame::OnCS_LEAVESKYGARDEN_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	CTSkygardenMap* pSkygardenMapUI = static_cast< CTSkygardenMap* >( m_vTFRAME[TFRAME_SKYGARDEN_MAP] );
	pSkygardenMapUI->ShowComponent(FALSE);
	pSkygardenMapUI->EnableComponent(FALSE);

	m_pMainChar->LeaveSkygarden();
	m_pMainChar->ClearGodBallSFX();
	m_pMainChar->SetGodBallID( 0 );
	m_pMainChar->SetCamp( 0 );
	m_pMainChar->SetCastleID( 0 );
...
int CTClientGame::OnCS_SKYGARDEN_OCCUPY_CENTER_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	if( !m_pMainChar )
		return TERR_NONE;
	...
	CTSkygardenMap* pSkygardenMapUI = static_cast< CTSkygardenMap* >( m_vTFRAME[TFRAME_SKYGARDEN_MAP] );
	pSkygardenMapUI->ChangeCenterCountry(bCountry);
	pSkygardenMapUI->ShowComponent(TRUE);
	pSkygardenMapUI->EnableComponent(TRUE);
...
int CTClientGame::OnCS_SKYGARDEN_OCCUPY_LEFT_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	if( !m_pMainChar )
		return TERR_NONE;
	...
	CTSkygardenMap* pSkygardenMapUI = static_cast< CTSkygardenMap* >( m_vTFRAME[TFRAME_SKYGARDEN_MAP] );
	pSkygardenMapUI->ChangeLeftCountry(bCountry);
	pSkygardenMapUI->ShowComponent(TRUE);
	pSkygardenMapUI->EnableComponent(TRUE);
...
int CTClientGame::OnCS_SKYGARDEN_OCCUPY_RIGHT_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	if( !m_pMainChar )
		return TERR_NONE;
	...
	CTSkygardenMap* pSkygardenMapUI = static_cast< CTSkygardenMap* >( m_vTFRAME[TFRAME_SKYGARDEN_MAP] );
	pSkygardenMapUI->ChangeRightCountry(bCountry);
	pSkygardenMapUI->ShowComponent(TRUE);
	pSkygardenMapUI->EnableComponent(TRUE);
...
int CTClientGame::OnCS_CHANGECHARBASE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
		...
		case IK_TITLE:
			{
				...
				CheckMonthRank(pPLAYER);

				if(pPLAYER == m_pMainChar && static_cast<CTTitleDlg*>(m_vTFRAME[TFRAME_TITLE])->IsVisible())
				{
					CTTitleDlg* pTitleDlg = static_cast<CTTitleDlg*>(m_vTFRAME[TFRAME_TITLE]);
					pTitleDlg->UpdateTitle(wTitleID);
				}
			}
		}
	...
	else
	{
		...
		case CCB_TIME:
			{
				...
				if( dwDay != 0 )
				{
					strFMT = CTChart::Format(TSTR_NATIONALITY_WAIT_DAY_HOUR, //Ao???????? ??????A?? [%dAI %d??A???] AIEA???? ?????ECO??I??U.
						dwDay,
						dwHour );
				}
				else if( dwHour != 0 )
				{
					strFMT = CTChart::Format(TSTR_NATIONALITY_WAIT_HOUR_MIN, //Ao???????? ??????A?? [%d??A??? %d????] AIEA???? ?????ECO??I??U.
						dwHour,
						dwMin );
				}
				else if( dwMin != 0 )
				{
					strFMT = CTChart::Format(TSTR_NATIONALITY_WAIT_MIN_SEC, //Ao???????? ??????A?? [%d???? %dAE] AIEA???? ?????ECO??I??U.
						dwMin,
						dwSec );
				}
				else
		...
		case CCB_TACTICS://??e????Ao??????oAA??????????A CO??o????A??
			{
				CString strFMT;
...
int CTClientGame::OnCS_STOPTHECLOCK_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	{
		if( bItem == INVALID_SLOT ) // ???????AC ??aCN????AaAI??U.
		{
			CTClientInven* pTINVEN = m_pMainChar->FindTInven(bInvenID);
			pTINVEN->m_dEndTime = dEndTime;
		}
...
int CTClientGame::OnCS_SYSTEMMSG_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	case SM_TOURNAMENT_STEP:
		{
			...
			if( bGroup == 0 )
			{
				if( CTTournament::IsCommonSchedule( bStep ) )
				{
					...
					// ??????c ??????iAC CurrentStepA?? bCurrentStepA????I ??AAa??U.
	...
	case SM_EVENT_LOTTERY:
		...
		case SM_MISSION_NORMAL:		//%d (A|??o: E|??o) ??I??CAo????AC AuAiAI A????a??C??u??A??I??U.
		{
			WORD wValue;
			(*pPacket)
				>> wValue;
			...
			strMSG = "The unified mission map has been set to normal state.";
	...
	case SM_MISSION_START:		//%d (A|??o: E|??o) ??I??CAo????AC AuAiAI ??AAU??C??u??A??I??U.
		{
			...
			strMSG = "The war in the unified mission map has begun.";
		}
	...
	case SM_MISSION_BOSSDIE:		//%s ??I??CAo????A?? %d ?????????? A????EC????A??I??U.
		{
	...
	case SM_MISSION_START_ALARM: //%d (A|.E|) ??I??CAuAiAI %d(????/AE)EA ??AAU??E??I??U.
		{
			WORD wValue;
			DWORD dwTIME;
			...
			if( dwTIME > 60 )
			{
				...
				strMSG.Format( "The battle on the unified mission map will start in %d minutes.",
					dwTIME);
			}
			else
				...
				strMSG.Format( "The battle on the unified mission map will start in %d seconds.",
				    dwTIME);

			static_cast<CTMainUI*>(m_vTFRAME[TFRAME_MAIN])->ResetChapterMSG(strMSG);
		}
	...
	case SM_MISSION_END_ALARM:	//%d (A|.E|) ??I??CAuAiAI %d(????/AE)EA A????a??E??I??U.
		{
			...
			if( dwTIME > 60 )
			{
				...
				strMSG.Format( "The battle on the unified mission map will end in %d minutes.",
					dwTIME);
			}
			else
				...
				strMSG.Format( "The battle on the unified mission map will end in %d seconds.",
				dwTIME);
	...
	case SM_MISSION_PEACE:		//%d (A|??o: E|??o) ??I??CAo????AC AuAiAI A????a??C??u??A??I??U.
		{
			WORD wValue;
			DWORD dwSecond;
			...
			strMSG.Format( "The War in the unified mission map has ended.");
		}
		break;

	case SM_MISSION_TIMEOUT:		//%s ??I??CAo????A?? A????ECN ?????????? ??????i ??o????AI Ao????A????I AuE???C??u??A??I??U. 
		{
			CString strMissionName;
			WORD wCountry;
			(*pPacket)
			...
			strMSG = CTChart::Format( TSTR_MISSION_TIMEOUT,
				(LPCTSTR) strMissionName ); //Since the mission map %s has not been occupied by a kingdom, it now belongs to the monsters
		}
		break;
#endif
	...
	case SM_SKYGARDEN_NORMAL:	//A????Au Ao????AI AI??Y??oAA??I AuE???C??u??A??I??U.
		{
			strMSG = CTChart::Format(TSTR_SKYGARDEN_NORMAL);
		}
		break;
	...
	case SM_SKYGARDEN_START:		//A????AuAI ??AAU??C??u??A??I??U.
		{
			strMSG = CTChart::Format(TSTR_SKYGARDEN_BT_BEGIN);
		}
		break;

	case SM_SKYGARDEN_START_ALARM:	//DWORD A????AuAI %d(????/AE)EA ??AAU??E??I??U.
		{
			DWORD dwTIME;
			(*pPacket)
				>> dwTIME;
			...
			{
				dwTIME /= 60;
				strMSG = CTChart::Format(TSTR_SKYGARDEN_ALARM_MINUTES, dwTIME);
			}
			else
				strMSG = CTChart::Format(TSTR_SKYGARDEN_ALARM_SECONDS, dwTIME);
		}
		break;

	case SM_SKYGARDEN_END_ALARM:	//DWORD A????AuAI %d(????/AE)EA A????a??E??I??U.
		{
			DWORD dwTIME;
			(*pPacket)
				>> dwTIME;
			...
			{
				dwTIME /= 60;
				strMSG = CTChart::Format(TSTR_SKYGARDEN_END_MINUTES, dwTIME);
			}
			else
				strMSG = CTChart::Format(TSTR_SKYGARDEN_END_SECONDS, dwTIME);
		}
		break;
	...
	case SM_SKYGARDEN_PEACE:		//A????AuAI A????a??C??i ??oE????oAA??I AuE???C??u??A??I??U. AI Ao??????????????A AuAo??| CO ??o ??????A??I??U.
		{
			strMSG = CTChart::Format(TSTR_SKYGARDEN_PEACE);
	...
	case SM_SKYGARDEN_END:			//A????Au???????? %d ?????????? ??A????CI??????A??I??U.
		{
			CString strMissionName;
			WORD wCountry;
			DWORD dwMapID;
			...
			{
				CTSkygardenEnd* pSkygardenEnd = static_cast< CTSkygardenEnd* >( m_vTFRAME[TFRAME_SKYGARDEN] );
				pSkygardenEnd->ShowInterface(bLoseCountry);
			}
		}
		break;
	}
...
int CTClientGame::OnCS_GUILDPVPRECORD_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	for( WORD i=0 ; i < wCount ; ++i )
	{
	 ...
	 CTGuildMainNew* pDlg = static_cast<CTGuildMainNew*>( m_vTFRAME[TFRAME_GUILD_BASE_NEW] );
	 CTGuildMembersNewDlg* pFrame = static_cast<CTGuildMembersNewDlg*>( pDlg->GetInnerFrame(TGUILD_MEMBERS) );
	 if( pFrame->IsVisible() )
...
int CTClientGame::OnCS_PVPRECORD_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	DWORD dwRankOrder;
	BYTE bRankPercent;
	DWORD dwMonthRankOrder = 0;			// ????A??
	BYTE bMonthRankPercent = 0;			// ??U????????
	WORD wMonthWin = 0;					// ??A??u
	WORD wMonthLose = 0;
	...
	CTCharNewPVPDlg* pWAR = static_cast<CTCharNewPVPDlg*>( m_vTFRAME[ TFRAME_CHARINFO_PVP ] );
	CTCharDuelsDlg * pDUELS = static_cast<CTCharDuelsDlg*>( m_vTFRAME[ TFRAME_CHARINFO_DUELS ] );

	if(!bType)
	{
		pWAR->ResetData(
			bType ? TINFO_PVP : TINFO_WAR, // 0:AuAi 1:??e??a 
			dwRankOrder,
			bRankPercent,
			dwKillCount,
			dwDieCount,
	...
	case ITEMCHANGE_FAIL:
		{
			...
			dwSoundID = ID_SND_ITEM_FAILED;
			if(pDLG->IsVisible())
				pDLG->SetReward(T_INVALID, T_INVALID, strMSG);
		}
		break;
...
#ifdef TEST_MODE
	m_dwLeftTickKICKOUTMAP = 1000*60; //1????
#else
	m_dwLeftTickKICKOUTMAP = 3600000; //1??A???
#endif
...
int CTClientGame::OnCS_GUILDTACTICSREPLY_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if( dwTarget == m_pMainChar->m_dwID ) // ??e???????AO ??e??oAU
	{
		if( bResult == GUILD_SUCCESS )
			EnableGuildUI(TRUE);
	}
...
int CTClientGame::OnCS_MONTHRANKLIST_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	...
	CTRankingNewDlg* pDlg = static_cast<CTRankingNewDlg*>( pDlgMain->GetInnerFrame( 0 ) );
	...
	CTRankingNewDlg::m_bTabIndex = 0;
	pDlg->ResetData( CTRankingNewDlg::m_bTabIndex, CTRankingNewDlg::m_bTabIndex );
	pDlgMain->ShowComponent( TRUE );
	...
	for( BYTE i=0 ; i < bEntryCount ; ++i )
	{
		...
		for( int r=0 ; r < pMatch->m_bRewardCount ; ++r )
		{
			...
			(*pPacket)
				...
				>> pReward->m_bChartType // 0:Au???????? 1:????AIAUA??????
				>> pReward->m_wItemID
				>> pReward->m_bCount;
	...
	for( int i=0 ; i < bEntryCount ; ++i )
	{
		...
		for( int j=0 ; j < bJoinCount ; ++j )
		{
			...
			(*pPacket)
				...
				>> pPlayer->m_bQfWin // 8??? ??A????
				>> pPlayer->m_bSfWin // 4??? ??A????
				>> pPlayer->m_bFWin; // ??a??A ??A????
...
int CTClientGame::OnCS_TOURNAMENTEVENTLIST_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	...
	for( int i=0 ; i < bEntryCount ; ++i )
	{
		...
		(*pPacket)
			>> pMatch->m_bEntryID // A????n
			>> pMatch->m_strEntry //A????nAI????
			>> pMatch->m_bType // ????????
			>> pMatch->m_strChoice
			>> pMatch->m_bCountry
			>> pMatch->m_fRate //??nA??
			>> pMatch->m_dwAmount; //??cA???Y
...
int CTClientGame::OnCS_TOURNAMENTENTER_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	...
	// SlotID??? AUA?? ????AI LeftTeam, SlotID??? A?? ????AI RightTeam
	...
	if( m_bTournamentWatching )
	{
		...
		//Aa??E??O???? %d?????aAa???? AOAaCI??I??A??I??U. %s??OA?? AI ?????aAaAC ??u???AUAO??I??U.",
	}
	else if( m_bTournamentBattle )
	{
		...
		// Aa??E??O???? %d?????aAa???? AOAaCI??I??A??I??U. %s??OA?? AI ?????aAaAC ??????oAO??I??U.
	}
...
int CTClientGame::OnCS_ARENA_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	...
	case ARENA_MAXPARTY://AOAa AO??eAI???? AE??u
		m_pMainWnd->MessageBoxOK(
	...
	case ARENA_MINPARTY://AOAa AO??OAI???? ??I????
		m_pMainWnd->MessageBoxOK(
...
int CTClientGame::OnCS_ASSISTANT_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	...
	strMSG.Format( "????????%d %s??OAI ??E??I A??C????U??i CN??U. CO?????",
		bLevel,
		m_strAssistantInvitor );
...
int CTClientGame::OnCS_SETTIME_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	CTGaugePannel* pTGaugePannel = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE]);
	CString strTime;
...
int CTClientGame::OnCS_SC_CREATECODE_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	CTSecuritySystemDlg* pSecurityDlg = static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS]);
	...
	switch (bResult)
	{
	case TSC_ACK_CODEEXIST:

	{

		pSecurityDlg->SetEnabled(TRUE);
		pSecurityDlg->SetExist(TRUE);
	...
	break;
	case TSC_ACK_CODECREATESUCCESS:
	...
	break;
	case TSC_ACK_CODECREATEFAIL:
		pSecurityDlg->SetExist(FALSE);
		break;
	}
...
int CTClientGame::OnCS_SC_CHANGECODE_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	CTSecuritySystemDlg* pSecurityDlg = static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS]);
	...
	switch(bResult)
	{
	case TSC_ACK_CODECHANGED:
		{
			DisableUI(TFRAME_SECURITYSYS);
			pSecurityDlg->PrepareDisplay();
			pSecurityDlg->SetWrongAttempts(0);
			pSecurityDlg->SwitchMode(TDLG_SC_MCHANGED);

			EnableUI(TFRAME_SECURITYSYS);
		}
		break;
	case TSC_ACK_WRONGCODE:
		{
			pSecurityDlg->PrepareDisplay();
			pSecurityDlg->SetWrongAttempts(bTries);
			pSecurityDlg->SwitchMode(TDLG_SC_MWRONGCODE);
			EnableUI(TFRAME_SECURITYSYS);
		}
		break;
	}
...
int CTClientGame::OnCS_SC_DISABLECODE_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	CTSecuritySystemDlg* pSecurity = static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS]);
	...
	switch(bResult)
	{
	case TSC_ACK_CODEDISABLED:
		{
			pSecurity->SetEnabled(FALSE);
			pSecurity->PrepareDisplay();
			pSecurity->SwitchMode(TDLG_SC_MDISABLED);
		}
		break;
	case TSC_ACK_WRONGCODE:
		{
			pSecurity->SetWrongAttempts(bTries);
			pSecurity->PrepareDisplay();
			pSecurity->SwitchMode(TDLG_SC_MWRONGCODE);
		}
		break;
	}
...
int CTClientGame::OnCS_SC_ENABLECODE_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	CTSecuritySystemDlg* pSecurity = static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS]);
	...
	if(bResult == TSC_ACK_ENABLED)
	{
		pSecurity->SetEnabled(TRUE);
...
int CTClientGame::OnCS_SC_UNLOCK_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	CTSecuritySystemDlg* pSecurity = static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS]);
	...
	case TSC_ACK_NONE:
		{
			pSecurity->PrepareDisplay();
			pSecurity->SwitchMode(TDLG_SC_MUNLOCK);
		}
		break;
	case TSC_ACK_CODEUNLOCKED:
		{
			pSecurity->PrepareDisplay();
			pSecurity->SwitchMode(TDLG_SC_MUNLOCKED);
		}
		break;
	case TSC_ACK_WRONGCODE:
		{
...
int CTClientGame::OnCS_SC_CHECKCODE_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	CTSecuritySystemDlg* pSecurity = static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS]);
	...
	case TSC_ACK_WRONGCODE:
		...
		break;
	case TSC_ACK_CORRECTCODE:
		{
			pSecurity->SetWrongAttempts(0);
			pSecurity->PrepareDisplay();
			pSecurity->SwitchMode(pSecurity->GetPreDlgType());
		...
		break;
	case TSC_ACK_TOOMANYWRONGATTEMPTS:
		{
			pSecurity->SetWrongAttempts(bTries);
			pSecurity->SetLocked(TRUE);
			m_pSESSION->SendCS_SC_GETLOCKTIME_REQ();
		}
		break;
	}
...
int CTClientGame::OnCS_SC_GETLOCKTIME_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	CTSecuritySystemDlg* pSecurity = static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS]);
	...
	pSecurity->PrepareDisplay();
	pSecurity->SetLockTime(bHours,bMinutes);
	pSecurity->SwitchMode(TDLG_SC_TEMPLOCKED);
	EnableUI(TFRAME_SECURITYSYS);

return TERR_NONE;
}

int CTClientGame::OnCS_COMPANIONUPDATE_REQ( CTachyonSession *pSession, CPacket *pPacket )
{
	CTCompanionDlg* pDlg = static_cast<CTCompanionDlg*>(m_vTFRAME[TFRAME_COMPANION]);
	BYTE bSlot;
	BYTE bCharAttr[ 6 ] = { 0 };
	...
	pCompanion->SetCharATTR( bCharAttr );
	pCompanion->UpdateAttrPoint( bSPoints );


	return TERR_NONE;
}

int CTClientGame::OnCS_COMPANIONLUPDATE_REQ( CTachyonSession *pSession, CPacket *pPacket )
{
	CTCompanionDlg* pDlg = static_cast< CTCompanionDlg* >( m_vTFRAME[ TFRAME_COMPANION ] );
	BYTE bSlot;
	BYTE bLevel;
	...
	if( pCompanion ) {
		...
		pCompanion->UpdateExp( dwExp );
		pCompanion->UpdateLevel( bLevel );
		pCompanion->UpdateNextExp( dwNextExp );
		pCompanion->UpdateAttrPoint( bSPoints );
		pCompanion->UpdateBonus( fBonusValue );
...
int CTClientGame::OnCS_UPDATECOMPANIONBYITEM_REQ( CTachyonSession *pSession, CPacket *pPacket )
{
	CTCompanionDlg* pDlg = static_cast<CTCompanionDlg*>(m_vTFRAME[TFRAME_COMPANION]);
	DWORD dwCharID;
	BYTE bSlot;
	...
	{
		BOOL bLevelUP = FALSE;
		bLevelUP = dwExp >= pCompanion->GetNextExp() &&	
			dwExp != pCompanion->GetExp();

		pCompanion->UpdateExp( dwExp );
		pCompanion->UpdateLife( wLife );

		Spolecnici::iterator finder = pDlg->m_mapSpolecnici.find( bSlot );
		if( finder == pDlg->m_mapSpolecnici.end() )
			return TERR_NONE;
		...
		CTClientSpolecnik* pTSPOLECNIK = FindSpolecnikByMonAndHost( dwCharID, (*finder).second->GetMonID() );
		if( pTSPOLECNIK &&
			pTSPOLECNIK->GetHostID() == m_pMainChar->m_dwID ) 
		{
			pTSPOLECNIK->PlayTActionSFX( &pTSPOLECNIK->m_vTGARBAGESFX, 1013 );
		}
	}
...
int CTClientGame::OnCS_UPDATECOMPANIONBYSYSTEM_REQ( CTachyonSession *pSession, CPacket *pPacket )
{
	CTCompanionDlg* pDlg = static_cast< CTCompanionDlg* >( m_vTFRAME[ TFRAME_COMPANION ] );
	BYTE bCompanionSlot;
	...
	CTClientCompanion* pCompanion = pDlg->GetSelectedCompanion( bCompanionSlot );
	if(pCompanion)
	{
		WORD wDiferencial = WORD( dwExp - pCompanion->GetExp() );
		BOOL bExpChanged = wDiferencial > 0;
		BOOL bLevelUp = dwExp >= pCompanion->GetNextExp() && dwExp != pCompanion->GetExp();

		CString strBuiltString, strLevelUp;
		CString strTYPE = CTChart::LoadString( TSTR_INFO_TITLE );

		strBuiltString.Format( "Your summoned pet has earned %d exp.", wDiferencial );
		strLevelUp.Format( "Your summoned pet's experience is at it's maximum. If you want to level up, open the Companion window and press the Level-Up button!" );

		if( bExpChanged )
			m_pChatFrame->ChatSysMSG( strTYPE, m_pMainChar->GetName(), strBuiltString,	TCOLOR_INFO, TCHAT_FLAG_INFO );
		if( bLevelUp )
			m_pChatFrame->ChatSysMSG( strTYPE, m_pMainChar->GetName(), strLevelUp,	0xFF33CCFF, TCHAT_FLAG_INFO );
		...
		if( bLevelUp )
		{
			Spolecnici::iterator finder = pDlg->m_mapSpolecnici.find( bCompanionSlot );
			if( finder == pDlg->m_mapSpolecnici.end() )
				return TERR_NONE;

			CTClientSpolecnik* pTSPOLECNIK = FindSpolecnikByMonAndHost( m_pMainChar->m_dwID, (*finder).second->GetMonID() );
			if( pTSPOLECNIK &&
				pTSPOLECNIK->GetHostID() == m_pMainChar->m_dwID ) 
			{
				pTSPOLECNIK->PlayTActionSFX( &pTSPOLECNIK->m_vTGARBAGESFX, 1012 );
...
int CTClientGame::OnCS_UPDATESPAWNEDCOMPANION_REQ( CTachyonSession *pSession, CPacket *pPacket )
{
	CTCompanionDlg* pDlg = static_cast<CTCompanionDlg*>(m_vTFRAME[TFRAME_COMPANION]);
	BYTE bSlot;
	...
	pDlg->m_bSummonedSlot = bSlot;

	return TERR_NONE;
}

int CTClientGame::OnCS_CREATECOMPANION_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	CTCompanionDlg* pDlg = static_cast< CTCompanionDlg* >( m_vTFRAME[ TFRAME_COMPANION ] );
	BYTE bResult;
	BYTE bSlot;
	...
	case 0:
		{
			...
			pDlg->ShowComponent( TRUE );
			pDlg->m_bCurSelSlot = bSlot;


		}
		break;
	case 1:
		strMsg = "Your companion's name is empty or too long.";
		break;
	case 2:
		strMsg = "You already own this companion.";
		break;
	case 3:
		strMsg = "You cannot have more than 5 companions.";
		break;
	}
...
int CTClientGame::OnCS_UPDATECOMPANIONITEMS_REQ( CTachyonSession *pSession, CPacket *pPacket )
{
	CTCompanionDlg* pDlg = static_cast< CTCompanionDlg* >( m_vTFRAME[ TFRAME_COMPANION ] );
	BYTE bCompanionSlot;
	...
	{
		CTClientItem* pItem = new CTClientItem();
		LPTITEM pTITEM = CTChart::FindTITEMTEMP( wItemID );
		if( pTITEM && wItemID )
		{
			pItem->SetTITEM( pTITEM );
			pItem->SetEndTime( dEndTime );
		}		

		if( pItem )
			pCompanion->UpdateItems( bSlot, pItem );
	}

	return TERR_NONE;
}

int CTClientGame::OnCS_UPDATECOMPANIONBONUS_REQ( CTachyonSession *pSession, CPacket *pPacket )
{
	CTCompanionDlg* pDlg = static_cast< CTCompanionDlg* >( m_vTFRAME[ TFRAME_COMPANION ] );
	BYTE bCompanionSlot;
...
int CTClientGame::OnCS_UPDATEMEDALS_REQ( CTachyonSession *pSession, CPacket *pPacket )
{
	...
	pGauge->SetMedals( dwMedals );

	return TERR_NONE;
}

int CTClientGame::OnCS_EXITTOURNAMETMAINTAIN_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	BYTE bCount;
...
int CTClientGame::OnCS_EXITTOURNAMETHOTKEY_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	BYTE bCount;
	CTGaugePannel* pTGaugePannel = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE]);
...
int CTClientGame::OnCS_BOWTIMEUPDATE_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	CTGaugePannel* pGaugePannel = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE]);

	CTBowRegisterWaitingDlg* pWaitingDlg = static_cast<CTBowRegisterWaitingDlg*>(m_vTFRAME[TFRAME_BOW_WAITING]);
	BYTE bStatus;
	DWORD dwSecond;
	BYTE bDefugelPoints;
	BYTE bCraxionPoints;
	...
	case BS_ALARM:
		{
			if (pWaitingDlg->IsVisible())
			{
				m_bAlarm = FALSE;
				pWaitingDlg->SetTime((INT) dwSecond / 60, (INT) dwSecond % 60);
				pWaitingDlg->FindKid(ID_CTRLINST_TITLE)->m_strText = "Battle of the Worlds";
				pGaugePannel->m_pBFGAlarm->ShowComponent(FALSE);	
			}
			else
			{
				m_bAlarm = TRUE;
				pGaugePannel->m_pBFGAlarm->SetType(SYSTEM_BOW);
				pGaugePannel->m_pBFGAlarm->SetTime(dwSecond);
				pGaugePannel->m_pBFGAlarm->SetTextColor(bStatus);


				pGaugePannel->m_pBFGAlarm->ShowComponent(TRUE);
			}
		}
		break;
	...
	case BS_NORMAL:
		{
			m_bAlarm = FALSE;
			pGaugePannel->m_pBFGAlarm->ShowComponent(FALSE);
			pGaugePannel->m_pBFGStatus->ShowComponent(FALSE);
			pWaitingDlg->ShowComponent(FALSE);
		}
		break;
	default:
		{
			if (bStatus == BS_PEACE && !IsInBOWMap())
			{
				m_bAlarm = TRUE;
				pGaugePannel->m_pBFGAlarm->SetType(SYSTEM_BOW);
				pGaugePannel->m_pBFGAlarm->SetTime(dwSecond);
			...
			{
				m_bAlarm = FALSE;
				if (pGaugePannel->m_pBFGAlarm->IsVisible())
					pGaugePannel->m_pBFGAlarm->ShowComponent(FALSE);

				if (pWaitingDlg->IsVisible())
					pWaitingDlg->ShowComponent(FALSE);

				pGaugePannel->m_pBFGStatus->SetTime((INT) dwSecond / 60, (INT) dwSecond % 60);
				pGaugePannel->m_pBFGStatus->SetPoints(bDefugelPoints, bCraxionPoints);
			}
		}
		break;
	}
	...
	if (m_bAlarm)
		pGaugePannel->DettachSiegeAlarm();

	return TERR_NONE;
}

int CTClientGame::OnCS_BOWNOTIFY_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	if (!IsInBOWMap() && !IsInBRMap())
		return TERR_NONE;
	...
	pMainUI->ResetChapterMSG(strMessage);

	return TERR_NONE;
}

int CTClientGame::OnCS_PINGMEASUREMENT_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	CTMainUI* pMainUI = static_cast<CTMainUI*>(m_vTFRAME[TFRAME_MAIN]);
	DWORD dwTick;
	static BYTE PingIndex = 0;

	(*pPacket)
		>> dwTick;

	if (PingIndex++ == PING_MEASUREMENT_COUNT)
		PingIndex = 0;

	m_dwPing[PingIndex] = (GetTickCount() - dwTick);

	DWORD Holder = 0;
	for (BYTE i = 0; i < PING_MEASUREMENT_COUNT; ++i)
		Holder += m_dwPing[i];

	m_dwEstPing = (Holder / PING_MEASUREMENT_COUNT) / 2;

	return TERR_NONE;
}

int CTClientGame::OnCS_REGISTERBOW_ACK(CTachyonSession* pSession, CPacket* pPacket)
{
	CTBFGAlarm* pAlarm = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE])->m_pBFGAlarm;
	CTBowRegisterWaitingDlg* pWaitingDlg = static_cast<CTBowRegisterWaitingDlg*>(m_vTFRAME[TFRAME_BOW_WAITING]);
	BYTE bResult;
	DWORD dwTick;
	...
	case BOWREG_SUCCESS:
		{
			DisableUI(TFRAME_BOW_REGISTER);
			pAlarm->ShowComponent(FALSE);
			pWaitingDlg->SetTime(BYTE(dwTick / 60), BYTE(dwTick % 60));
			pWaitingDlg->ShowComponent(TRUE);
		}
		break;
	case BOWREG_COUNTRY:
		{
			m_pMainWnd->MessageBoxOK(
				"Your country is not valid, you cannot participate.",
				TSTR_OK,
				GM_CLOSE_MSGBOX,
				0,
				TRUE);
		...
		break;
	case BOWREG_ALREADYINQUEUE:
		{
			m_pMainWnd->MessageBoxOK(
				"You are already placed in queue.",
				TSTR_OK,
				GM_CLOSE_MSGBOX,
				0,
				TRUE );
	...
	case BOWREG_FAIL:
		{
			m_pMainWnd->MessageBoxOK(
				...
				"The Battle of the Worlds didn't start, you are not able to join.",
				TSTR_OK,
				GM_CLOSE_MSGBOX,
				0,
				TRUE );
...
int CTClientGame::OnCS_ADDTOBRQUEUE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	CTBFGAlarm* pAlarm = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE])->m_pBFGAlarm;
	CTBowRegisterWaitingDlg* pWaitingDlg = static_cast<CTBowRegisterWaitingDlg*>(m_vTFRAME[TFRAME_BOW_WAITING]);
	BYTE bResult;
	...
	case BOWREG_SUCCESS:
		{
			DisableUI(TFRAME_BOW_REGISTER);
			static_cast<CBRTeamDlg*>(m_vTFRAME[TFRAME_BRTEAMS])->ShowComponent(FALSE);
			pAlarm->ShowComponent(FALSE);
			pWaitingDlg->SetTime(BYTE(dwTick / 60), BYTE(dwTick % 60));
			pWaitingDlg->ShowComponent(TRUE);
		}
	...
	case BOWREG_FAIL:
		{
			m_pMainWnd->MessageBoxOK(
				"The Battle didn't start, you are not able to join.",
				TSTR_OK,
				GM_CLOSE_MSGBOX,
				0,
				TRUE );
...
int CTClientGame::OnCS_RELEASEFORBOW_REQ( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	DisableUI(TFRAME_BOW_RESPAWN);

	ReleaseForTELEPORT();
	m_pTMinimap->ShowComponent(TRUE);
	m_pTRSCS->ShowComponent(FALSE);

	m_pChatFrame->ResetChatList();
...
int CTClientGame::OnCS_CANCELBOWQUEUE_ACK(CTachyonSession* pSession, CPacket* pPacket)
{
	CTBFGAlarm* pAlarm = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE])->m_pBFGAlarm;
	CTBowRegisterWaitingDlg* pWaitingDlg = static_cast<CTBowRegisterWaitingDlg*>(m_vTFRAME[TFRAME_BOW_WAITING]);
	BYTE bResult;
	...
	case BOWREG_SUCCESS:
		{
			m_bAlarm = FALSE;
			pWaitingDlg->ShowComponent(FALSE);
			pAlarm->SetTime(dwTick);
			pAlarm->ShowComponent(TRUE);
		}
		break;
	case BOWREG_FAIL:
		{
...
int CTClientGame::OnCS_NOTIFYNONQUEUEDPLAYER_ACK(CTachyonSession* pSession, CPacket* pPacket)
{
	m_pChatFrame->ChatSysMSG(
		CTChatFrame::GetChatTypeString(CHAT_WORLD),
		m_pMainChar->m_strNAME,
		"You cannot enter the Battle of Worlds because the entry time has ended.",
		0xFFF6CE7F,
		TCHAT_FLAG_WORLD,
		ID_SND_INFO,
		MEDIA_TSOUND);
...
int CTClientGame::OnCS_UPDATEBONUSPOINT_ACK(CTachyonSession* pSession, CPacket* pPacket)
{
	if (!IsInBOWMap() && !IsInBRMap())
		return TERR_NONE;
	...
	pTGaugePannel->SetBonusPoints(dwBB);

	return TERR_NONE;
}

int CTClientGame::OnCS_BOWKILLNOTIFY_ACK(CTachyonSession* pSession, CPacket* pPacket)
{
	if (!IsInBOWMap() && !IsInBRMap())
		return TERR_NONE;

	CString strAttack, strMessage;
...
int CTClientGame::OnCS_BOWRANKING_ACK( CTachyonSession *pSession, CPacket *pPacket)
{	
	if (!IsInBOWMap())
		return TERR_NONE;

	CTBowRankingDlg* pRankingDlg = static_cast<CTBowRankingDlg*>(m_vTFRAME[TFRAME_BOWRANK]);
	BYTE bCount;
	BYTE bGCount;
	(*pPacket)
		>> bCount;

	pRankingDlg->ReleaseData();

	for (BYTE i = 0; i < bCount; ++i)
	{
		CTBowRank* pRank = new CTBowRank();
		(*pPacket)
			>> pRank->m_dwCharID
			>> pRank->m_strName
			>> pRank->m_bCountry
		...
		pRankingDlg->m_vTBODRANK.push_back(pRank);
	}

	(*pPacket)
		>> bGCount;

	for (BYTE i = 0; i < bGCount; ++i)
	{
		CTBowGRank* pGuild = new CTBowGRank();
		(*pPacket)
			>> pGuild->m_dwID
			>> pGuild->m_strName
			>> pGuild->m_bCountry
		...
		pRankingDlg->m_vTGUILDRANK.push_back(pGuild);
	}

	pRankingDlg->ResetRank();
	pRankingDlg->MoveComponent(CPoint(
		CTClientUIBase::m_vBasis[TBASISPOINT_CENTER_MIDDLE].x - 425,
		CTClientUIBase::m_vBasis[TBASISPOINT_CENTER_MIDDLE].y - 340));
	pRankingDlg->ShowComponent(TRUE);

	return TERR_NONE;
}

int CTClientGame::OnCS_LEAVEBATTLEFIELD_ACK(CTachyonSession* pSession, CPacket* pPacket)
{
	if (!IsInBOWMap() && !IsInBRMap())
		return TERR_NONE;
	...
	case BOWLEAVE_FAIL:
		{
			m_pMainWnd->MessageBoxOK(
				"You cannot leave the battlefield at this moment.", 
				TSTR_OK,
				GM_CLOSE_MSGBOX, 
				GM_CLOSE_MSGBOX);
		}
		break;
	case BOWLEAVE_SUCCESS:
		break;
	default: break;
	}
...
int CTClientGame::OnCS_BRTIMEUPDATE_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	CTGaugePannel* pGaugePannel = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE]);
	CBRTeamDlg* pTeamDlg = static_cast<CBRTeamDlg*>(m_vTFRAME[TFRAME_BRTEAMS]);
	CTBowRegisterWaitingDlg* pWaitingDlg = static_cast<CTBowRegisterWaitingDlg*>(m_vTFRAME[TFRAME_BOW_WAITING]);
	...
	case BS_ALARM:
		{
			if (pTeamDlg->IsVisible() && pTeamDlg->GetTeamReady())
			{
				m_bAlarm = FALSE;
				pTeamDlg->SetTime(dwSecond);
				pGaugePannel->m_pBFGAlarm->ShowComponent(FALSE);	
			}
			else
			{
				if (pWaitingDlg->IsVisible())
				{
					pTeamDlg->ShowComponent(FALSE);
					pWaitingDlg->FindKid(ID_CTRLINST_TITLE)->m_strText = "Battle Royale";
					pWaitingDlg->SetTime((INT) dwSecond / 60, (INT) dwSecond % 60);
					m_bAlarm = FALSE;
					pGaugePannel->m_pBFGAlarm->ShowComponent(FALSE);	
				}
				...
				{
					m_bAlarm = TRUE;
					pGaugePannel->m_pBFGAlarm->SetType(SYSTEM_BR, bType);
					pGaugePannel->m_pBFGAlarm->SetTextColor(bStatus, bType);
					pGaugePannel->m_pBFGAlarm->SetTime(dwSecond);
					pGaugePannel->m_pBFGAlarm->ShowComponent(TRUE);
				}
			}
	...
	default:
		{
			...
			if (pGaugePannel->m_pBFGStatus->m_bMode != MODE_BR)
				pGaugePannel->m_pBFGStatus->SetMode(MODE_BR);
...
int CTClientGame::OnCS_BRTEAMMATEADD_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	CString strNAME;
	BYTE bResult;
	...
	switch (bResult)
	{
	case TEAMADD_NOTFOUND:
		strMSG = "The Character doesn't exist, or is not on-line at the moment.";
		break;
	case TEAMADD_BUSY:
		strMSG = "The Character has not been invited, because he is busy at the moment.";
		break;
	case TEAMADD_REFUSE:
		strMSG = "The Character has refused your invite to the Battle Royal.";
		break;
	case TEAMADD_ALREADYINTEAM:
		strMSG = "The Character is already in a team.";
		break;
	}
	...
	{
		if (IsBusy() && m_pSESSION)
		{
			m_pSESSION->SendCS_BRTEAMMATEADDRESULT_ACK(TEAMADD_BUSY, m_strBRInviter);
			return TERR_NONE;
		}
		...
		m_pMainWnd->MessageBoxYesNo(
			...
			TSTR_YES,
			TSTR_NO, 
			TCOMMAND(GM_BRTEAMMATEADD_ANSWER, TRUE), 
			TCOMMAND(GM_BRTEAMMATEADD_ANSWER, FALSE), 
			TCOMMAND(GM_BRTEAMMATEADD_ANSWER, FALSE));
	}
...
int CTClientGame::OnCS_UPDATEBRTEAM_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	CBRTeamDlg* pTeamDlg = static_cast<CBRTeamDlg*>(m_vTFRAME[TFRAME_BRTEAMS]);
	BYTE bCount;
	CString strName;
	BYTE bTeamReady;
	BYTE bReady;
	std::vector<BRPREMADEPLAYER> vTeam; vTeam.clear();
	...
	for (BYTE i = 0; i < bCount; ++i)
	{
		...
		BRPREMADEPLAYER PremadePlayer;
		PremadePlayer.m_strName = strName;
		PremadePlayer.m_bReady = bReady;

		vTeam.push_back(PremadePlayer);
	}

	if (vTeam.empty())
		pTeamDlg->LeaveTeam();
	else
		pTeamDlg->UpdateTeam(vTeam, bTeamReady);

	pTeamDlg->ShowComponent(TRUE);

	return TERR_NONE;
}

int CTClientGame::OnCS_UPDATEBRMATE_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	CBRPannels* pPannels = static_cast<CBRPannels*> (m_vTFRAME[TFRAME_BRPANNELS]);
	BYTE bCount;
	WORD wKills;
	DWORD dwCharID;
	...
	for (BYTE i = 0; i < bCount; ++i)
	{
		...
		if (CreateNew || !pMATE->m_bDead)
		{
			pMATE->m_strName = strName;
			pMATE->m_bClass = bClass;
			pMATE->m_bLife = bLife;
		}
		if (CreateNew)
			m_vBRTEAM.push_back(pMATE);

		if (dwCharID == m_pMainChar->m_dwID)
			static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE])->m_pBFGStatus->UpdateTopUI(pMATE->m_bLife, wKills, 0);
	}

	pPannels->UpdatePannels();
	pPannels->ShowComponent(TRUE);

	return TERR_NONE;
}


int CTClientGame::OnCS_INITBRTRANKING_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	CBRRankingDlg* pRanking = static_cast<CBRRankingDlg*> (m_vTFRAME[TFRAME_BRRANKING]);
	CBRTRanking* pTRanking = static_cast<CBRTRanking*> (m_vTFRAME[TFRAME_BRTRANKING]);
	BYTE bCount, bTCount;
	DWORD dwTeamID;
	DWORD dwCharID;
	CString strName;
	...
	if (!pRanking->m_vBRRANK.empty())
		pRanking->ReleaseRanking();
	...
	for (BYTE i = 0; i < bCount; ++i)
	{
		...
		MAPBRTPLAYER mapPlayer; mapPlayer.clear();
		for (BYTE l = 0; l < bTCount; ++l)
		{
			(*pPacket)
				>> dwCharID
				>> strName
			...
			if (pTRanking->FindTeam(dwTeamID))
				continue;

			LPBRTPLAYER pPLAYER = new BRTPLAYER();
			pPLAYER->m_dwCharID = dwCharID;
			pPLAYER->m_strNAME = strName;
			pPLAYER->m_wKills = wKills;
			pPLAYER->m_bLifes = bLifes;

			pRanking->CloneTRanking(pPLAYER);
			mapPlayer.insert(MAPBRTPLAYER::value_type(pPLAYER->m_dwCharID, pPLAYER));
		}

		pTRanking->AddTeam(dwTeamID, mapPlayer);
	}

	pTRanking->ShowComponent(TRUE);

	return TERR_NONE;
}

int CTClientGame::OnCS_UPDATEBRTPLAYER_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	CBRTRanking* pTRanking = static_cast<CBRTRanking*> (m_vTFRAME[TFRAME_BRTRANKING]);
	DWORD dwTeamID;
	DWORD dwCharID;
	...
	pTRanking->UpdatePlayer(dwTeamID, dwCharID, wKills);
	static_cast<CBRRankingDlg*>(m_vTFRAME[TFRAME_BRRANKING])->UpdateRanking(dwCharID, wKills, bLifes);
...
int CTClientGame::OnCS_UPDATEREMAININGPLAYER_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	CTGaugePannel* pGaugePannel = (CTGaugePannel*) m_vTFRAME[TFRAME_GAUGE];
	BYTE bRemaining;

	(*pPacket)
		>> bRemaining;

	pGaugePannel->m_pRemainingText->m_strText.Format("Alive : %d", bRemaining);
...
int CTClientGame::OnCS_CUSTOMCLOAK_LIST_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	CTCustomCloakDlg* pDlg = static_cast<CTCustomCloakDlg*>(m_vTFRAME[TFRAME_CUSTOMCLOAK]);
	WORD wCount;
	(*pPacket)
		>> wCount;
	...
	for(WORD i = 0; i< wCount;i++)
	{
		...
		pDlg->AddCloak(wID,dDateCreate);
	}
	
	pDlg->ResetData(
		m_pMainChar,
		m_pRES);
	EnableUI(TFRAME_CUSTOMCLOAK);
	
	return TERR_NONE;
}
int CTClientGame::OnCS_CUSTOMCLOAK_CHANGE_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	CTCustomCloakDlg* pDlg = static_cast<CTCustomCloakDlg*>(m_vTFRAME[TFRAME_CUSTOMCLOAK]);
	DWORD dwCharID;
	WORD wID;
	...
	if (wID > 0)
	{
		strMSG.Format("Your cloak has been successfully changed to (%d).", wID);
	...
	{
		strMSG.Format("Your cloak has been successfully cleared.");
	...
	if(bRet)
	{
		CTClientChar * pPlayer = FindPlayer( dwCharID);
		if(dwCharID == m_pMainChar->m_dwID)
			pPlayer = m_pMainChar;
		if(!pPlayer)
			return TERR_NONE;

		MAPTINVEN::iterator itTINVEN = pPlayer->m_mapTINVEN.find(INVEN_EQUIP);
		if( itTINVEN != pPlayer->m_mapTINVEN.end() )
		{
			CTClientInven *pTEQUIP = (*itTINVEN).second;
			CTClientItem* pTITEM = pTEQUIP->FindTItem( ES_BACK );
			if( pTITEM )
			{
				pTITEM->SetCustomTex(wID);
			}
		}
	...
	{
		strChatMsg = "You don't have this skin for cloak!";
	}
...
int CTClientGame::OnCS_GUILDSKILLACTION_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	BYTE Action;
	BYTE ParamSize;
	WORD* wParam;
	...
	DebugMSG("Got it. Action : %d, Params : %d", Action, ParamSize);
...
int CTClientGame::OnCS_GUILDSKILLUPDATE_ACK(CTachyonSession *pSession, CPacket *pPacket)
{


	auto GuildSkill = static_cast<CGuildSkillsDlg*>(static_cast<CTGuildMainNew*>(m_vTFRAME[TFRAME_GUILD_BASE_NEW])->GetInnerFrame(TGUILD_SKILL));
	...
	for (BYTE i = 0; i < Size; ++i)
	{
		...
		GuildSkill->UpdateSkill(SkillID, Level, Time);
	}

	GuildSkill->CheckShow();

	return TERR_NONE;
}

int CTClientGame::OnCS_ARENAINFO_ACK(CTachyonSession *pSession, CPacket *pPacket)
{


	auto ArenaDlg = (CTArenaRegDlg*)m_vTFRAME[TFRAME_ARENAREG];
	...
	if (FirstTeam)
	{
		...
		ArenaDlg->AddTeam(Name, Level, Country, IsWinner ? WinCount : 0);
	}
	...
	for (BYTE i = 0; i < QueueCnt; ++i)
	{
		(*pPacket)
			>> Name;

		ArenaDlg->AddPlayersQueue(Name);
	}

	if (!ArenaDlg->IsVisible())
		ArenaDlg->ShowComponent();

	return TERR_NONE;
}

int CTClientGame::OnCS_ARENARANKING_ACK(CTachyonSession *pSession, CPacket *pPacket)
{



	auto Ranking = (ArenaRanking*)m_vTFRAME[TFRAME_ARENARANKING];

	CString name;
	BYTE Size[ARENA_MAX_TEAM];
	BYTE Wins[ARENA_MAX_TEAM];
	std::vector<ArenaPlayer> Team[ARENA_MAX_TEAM];
	...
	for (BYTE i = 0; i < Size[0]; ++i)
	{
		...
		Team[0].push_back(Player);
	}




	Ranking->AddTeam(Team[0], Wins[0]);
	...
	for (BYTE i = 0; i < Size[1]; ++i)
	{
		...
		Team[1].push_back(Player);
	}



	Ranking->AddTeam(Team[1], Wins[1]);
	Ranking->ShowComponent();
