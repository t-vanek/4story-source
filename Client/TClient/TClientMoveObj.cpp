								  ...
								  DWORD dwTick)
{
	...
	if(CanFLY())
	{
		...
		fMinSKY = max( fHeight, max(fWaterHeight, fLavaHeight));
	...
	m_bSwimLava = m_vPosition._42 > fLavaHeight ? FALSE : TRUE;
	m_bInLava = m_vPosition._42 > fLavaHeight + 2.0f ? FALSE : TRUE;
	...
	if(m_bSwim)
		...
		fHeight = max( fHeight, min( fLavaHeight, m_vPosition._42));
									   ...
									   FLOAT fTime)
{
	...
	if( m_bParashute || m_bSwim || m_bSwimLava || fTime < fDropTime )
		return 0;
	fTime -= fDropTime;
								   ...
								   CTachyonRes *pRES)
{
	...
	switch(m_bAction)
	{
	case TA_FOLLOW		: bActionID = m_bKeyDIR == TKDIR_N ? (m_bSwim || m_bSwimLava) ? TA_SWIMSTAND : TA_STAND : (m_bSwim || m_bSwimLava) ? TA_SWIMMOVE : TA_RUN; break;
	case TA_WALK		: bActionID = m_bBack ? (m_bSwim || m_bSwimLava) ? TA_SWIMBACK : TA_BACK : (m_bSwim || m_bSwimLava) ? TA_SWIMMOVE : TA_WALK; break;
	case TA_RUN			: bActionID = m_bBack ? (m_bSwim || m_bSwimLava) ? TA_SWIMBACK : TA_BACK : (m_bSwim || m_bSwimLava) ? TA_SWIMMOVE : TA_RUN; break;
	case TA_DEAD		: bActionID = m_bAction; break;
	case TA_STAND		:
		if(!m_bSwim && !m_bSwimLava)
			switch(m_bKeyDIR)
			{
			case TKDIR_L	:
				{
