								...
								FLOAT fPosZ)
{
			...
			if( 0 <= nLocX && 0 <= nLocZ )
			{
				...
				if( finder != m_mapTCELL.end() )
				{
					...
					case OT_RECALL:
						...
						break;
					case OT_COMPANION:
						for( itCOMP = pCell->m_mapCOMPANION.begin(); itCOMP != pCell->m_mapCOMPANION.end(); itCOMP++)
							if((*itCOMP).second->m_dwID == dwObjID)
	...
	{
		CString x;
		x.Format("Exception: 0x01 ; %s", ex.what());
...
void CTMap::EnterMAP( CTMonster *pMON)
{
	...
	if( pMON->m_bType == OT_RECALL )
		...
		((CTCompanion *) pMON)->m_bMain = FALSE;

	if( finder != m_mapTCELL.end() &&
		((pMON->m_bType != OT_RECALL || pMON->m_bType != OT_COMPANION) ||
...
void CTMap::LeaveMAP( CTMonster *pMON, BYTE bExitMap, BYTE bForever)
{
	try
	{
		if( !pMON )
		...
		for( it = mapTCELL.begin(); it != mapTCELL.end(); it++)
			(*it).second->LeaveMonster( pMON, bExitMap, bForever);
	}
	catch(...)
	{
		_AtlModule.LogSmartly("Exception in LeaveMAP.");
	}
}
					...
					FLOAT fPosZ)
{
	...
	if( dwOLD != dwNEW )
	{
		...
		if(pPlayer->m_bMain && pPlayer) // Check connection
		{
			MAPBYTE mapCON;
			mapCON.clear();
...
//				((CTRecallMon *) pMON)->m_bMain = pCell->IsMainCell(pMON->m_bChannel);
			...
			if( pMON->m_bType == OT_COMPANION )
			{
//				BYTE bPrevMain = ((CTRecallMon *) pMON)->m_bMain;
//				((CTRecallMon *) pMON)->m_bMain = pCell->IsMainCell(pMON->m_bChannel);
				if(((CTCompanion *) pMON)->m_bMain && !pCell->IsMainCell(pMON->m_bChannel))
				{
					_AtlModule.SendMW_RECALLMONDATA_ACK((CTRecallMon *) pMON);
					((CTCompanion *) pMON)->m_bMain = FALSE;
				}
...
void CTMap::SetSkyGarden(LPTLOCAL pSkyGarden)
{
	m_pSkyGarden = pSkyGarden;
}
