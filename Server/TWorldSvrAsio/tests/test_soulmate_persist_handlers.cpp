// W4-18 wire test: soulmate write-back persistence.
//
// Alice (42/peer1) registers Bob (200/peer2) as her soulmate → world
// persists the mutual TSOULMATETABLE rows. Alice then dissolves the
// pairing → world deletes both rows. We read the (fake) repo back to
// confirm each write.
...
int main()
{
    ...
    // Alice registers Bob as her soulmate → both rows persisted.
    {
        std::vector<std::byte> b;
        wire::WritePOD<std::uint32_t>(b, 42);
        wire::WritePOD<std::uint32_t>(b, kA);
        wire::WriteString(b, "Bob");
        wire::WritePOD<std::uint8_t>(b, 1);   // reg = register
        wire::WritePOD<std::uint8_t>(b, 0);   // npc_inven
        wire::WritePOD<std::uint8_t>(b, 0);   // npc_item
        SendFramed(p1, ToUint16(MessageId::MW_SOULMATEREG_ACK), b);
    }
    { auto [w, _] = ReadFramed(p1);
    ...
    {
        auto a = friend_repo.LoadForChar(42);
        auto b = friend_repo.LoadForChar(200);
        EXPECT(a.has_soulmate && a.soulmate_target == 200);
        EXPECT(b.has_soulmate && b.soulmate_target == 42);
    }

    // Alice dissolves the pairing → both rows deleted.
    {
        std::vector<std::byte> b;
        wire::WritePOD<std::uint32_t>(b, 42);
        wire::WritePOD<std::uint32_t>(b, kA);
        SendFramed(p1, ToUint16(MessageId::MW_SOULMATEEND_ACK), b);
    }
    { auto [w, _] = ReadFramed(p1);
    ...
    std::this_thread::sleep_for(30ms);
    {
        auto a = friend_repo.LoadForChar(42);
        auto b = friend_repo.LoadForChar(200);
        EXPECT(!a.has_soulmate);
        EXPECT(!b.has_soulmate);
    }
    ...
    if (g_fails == 0)
        std::printf("PASS test_tworldsvr_asio_soulmate_persist_handlers\n");
    else
        std::printf("FAIL test_tworldsvr_asio_soulmate_persist_handlers "
                    "(%d failure%s)\n", g_fails, g_fails == 1 ? "" : "s");
