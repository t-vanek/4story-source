...
| **T9** | Mob AI tick + spawn manager | 🟡 static spawn (`SpawnManager` + `TMAPMONCHART` → registry → CS_ADDMON on enter, real HP from `TMONATTRCHART`) + **timed respawn on kill** + AI tick with **idle-roam + aggro/chase** (`monster_ai`: chase nearest player in range, else roam → `CS_MONMOVE`, capped per tick) done; monster→player attack/damage + spatial AOI + per-spawn respawn delay pending |
