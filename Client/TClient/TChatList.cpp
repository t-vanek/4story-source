...
// ===============================================================================
CTChatList::CTChatList(CTChatFrame* pHost, FRAMEDESC_SHAREDPTR pDesc, TComponent* pOuterParent)
	: TList(pHost, pDesc), m_dwChatFlag(TCHAT_FLAG_ALL_CHAT_WITH_INFO),
	  ...
	  m_pSizeBarUP( NULL), m_pSizeBarDOWN( NULL), m_pTab( NULL ), m_pList( NULL ), m_pBottom( NULL ),
	  m_pTop( NULL ), m_pSetting( NULL ), m_pChatMenu( NULL ), m_pFrame( NULL )
{
	m_bWordBreak = FALSE;
	m_blUserColor = TRUE;
...
// ===============================================================================
void CTChatList::Init(UINT iIndex, const CString& strTitle, DWORD dwFlag, const CRect& rc, FRAMEDESC_SHAREDPTR pDescFastDown)
{
	...
	m_pTab->m_strText = strTitle;
	CD3DFont *pFont = m_pTab->Get3DFont();
	pFont->m_dwColor = 0xFFB19D67;
	m_pTab->EnableComponent(FALSE);
}
// ===============================================================================
	...
	if( m_pHost->IsLock() )
{
		...
		if( m_pTab && m_pTab->HitTest(pt) )
			return TRUE;

		if( m_pSetting && m_pSetting->HitTest(pt) )
			return TRUE;

		if( m_pChatMenu && m_pChatMenu->HitTest(pt) )
			return TRUE;
...
void CTChatList::ShowComponent( BOOL bVisible )
{
	...
	if( !m_bHideBackground && m_bVisible )
	{
		...
		if( m_pTop )
			m_pTop->ShowComponent( TRUE );
		if( m_pBottom )
			m_pBottom->ShowComponent( TRUE );
		if( m_pSetting)
			m_pSetting->ShowComponent(TRUE);
		if(IsOuter())
		{
			if( m_pTab)
			{
				m_pTab->ShowComponent(TRUE);
				m_pTab->m_strText = m_strTitle;
			}
		}
		else
			m_pTab->ShowComponent(FALSE);
		if(m_pChatMenu)
			m_pChatMenu->ShowComponent(FALSE);
	}
	else
	{
		if( m_pScroll )
		...
		if( m_pSizeBarDOWN )
			m_pSizeBarDOWN->ShowComponent( FALSE );
		if( m_pTop )
			m_pTop->ShowComponent( FALSE );
		if( m_pBottom )
			m_pBottom->ShowComponent( FALSE );
		if( m_pSetting)
			m_pSetting->ShowComponent(FALSE);
		if( m_pTab)
			m_pTab->ShowComponent(FALSE);
		if(m_pChatMenu)
			m_pChatMenu->ShowComponent(FALSE);
...
void CTChatList::SetHeight( INT nHeight )
{
	...
	if( m_pSizeBarUP )
	{
		...
		m_pSizeBarUP->MoveComponent( CPoint(0, -size.cy-5) );
		if(m_pTop)
		{
			SIZE size;
			m_pTop->GetComponentRealSize( &size );
			m_pTop->MoveComponent( CPoint(-4, -size.cy-1) );
		}
		if(m_pSetting)
		{
			SIZE size;
			m_pSetting->GetComponentRealSize( &size );
			m_pSetting->MoveComponent( CPoint(4, -(size.cy+15)) );
		}
		if(m_pTab)
		{
			SIZE size;
			m_pTab->GetComponentRealSize( &size );
			m_pTab->MoveComponent( CPoint(30, -size.cy-13) );
		}
	}
	...
	if( m_pSizeBarDOWN )
	{
		...
		if(m_pBottom)
		{			
			m_pBottom->MoveComponent( CPoint(-4, m_rc.Height()) );		
		}
	}
}
// ===============================================================================
...
void CTChatList::OnLButtonUp(UINT nFlags, CPoint pt)
{
	...
	if(m_pSetting && m_pSetting->IsVisible() && m_pSetting->HitTest(pt))
	{
		if(m_pChatMenu->IsVisible())
			m_pChatMenu->ShowComponent(FALSE);
		else
		{
			m_pChatMenu->m_pHost = this;
			m_pChatMenu->Init(m_strTitle, m_dwChatFlag, m_IndexInGroup);			
			
			m_pChatMenu->ShowComponent(TRUE);

			CPoint cp;
			m_pSetting->GetComponentPos(&cp);
			cp.y -= 400;
			m_pChatMenu->MoveComponent(cp);

			m_pHost->ClearMenuFocus();
		}
	}
}
// -------------------------------------------------------------------------------
void CTChatList::OnMouseMove(UINT nFlags, CPoint pt)
{
	...
	if (!CTClientGame::m_vTOPTION.m_bDisableChatTransparency)
	{
		if (HitItemArea(pt))
		{
			for (VECTOR_MAP_LISTITEM::iterator it = m_items.begin(); it != m_items.end(); ++it)
			{
				for (MAP_LISTITEM::iterator it2 = (*it)->begin(); it2 != (*it)->end(); ++it2)
				{
					(*it2).second->m_dwTextColor = TINSTALL_ALPHA((*it2).second->m_dwTextColor, (BYTE) 255);
...
HRESULT CTChatList::Render( DWORD dwTickCount )
{
	...
	if (!CTClientGame::m_vTOPTION.m_bDisableChatTransparency)
	{
		static DWORD dwColorTick = GetTickCount();
		dwColorTick += dwTickCount;
		if (dwColorTick > 100)
		{
			for (VECTOR_MAP_LISTITEM::iterator it = m_items.begin(); it != m_items.end(); ++it)
			{
				for (MAP_LISTITEM::iterator it2 = (*it)->begin(); it2 != (*it)->end(); ++it2)
				{
					FLOAT fAlpha = (FLOAT) GETVALUE_A((*it2).second->m_dwTextColor);
					if (fAlpha >= (255.0f / 2.0f))
						fAlpha -= 1.0f;
					else
						continue;

					(*it2).second->m_dwTextColor = TINSTALL_ALPHA((*it2).second->m_dwTextColor, (BYTE) fAlpha);
					m_bNeedUpdateRender = TRUE;
				}
			}
...
void CTChatList::DeleteThisList()
{
	m_pOuterHost->RemoveKid(this);
}
