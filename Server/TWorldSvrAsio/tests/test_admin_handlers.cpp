// W6-12 wire test: GM user-tracking relays.
//
// GM Alice (42, peer1) locates / force-moves Bob (200, peer2); world
// routes USERPOSITION / USERMOVE to Bob's map.
...
int main()
{
    ...
    // --- USERPOSITION: GM Alice locates Bob → Bob's map -----------
    {
        std::vector<std::byte> b;
        wire::WriteString(b, "Bob");      // target
        wire::WriteString(b, "Alice");    // GM
        SendFramed(p1, ToUint16(MessageId::CT_USERPOSITION_ACK), b);
        auto [w, got] = ReadFramed(p2);
        EXPECT(w == ToUint16(MessageId::MW_USERPOSITION_REQ));
        wire::Reader r(got);
        std::uint32_t cid = 0, key = 0; std::string gm;
        r.Read(cid); r.Read(key); r.ReadString(gm);
        EXPECT(cid == 200); EXPECT(gm == "Alice");
    }

    // --- USERMOVE: GM force-moves Bob → Bob's map -----------------
    {
        std::vector<std::byte> b;
        wire::WriteString(b, "Bob");
        wire::WritePOD<std::uint8_t>(b, 1);      // channel
        wire::WritePOD<std::uint16_t>(b, 1000);  // map
        wire::WritePOD<float>(b, 1.0f);
        wire::WritePOD<float>(b, 2.0f);
        wire::WritePOD<float>(b, 3.0f);
        wire::WritePOD<std::uint16_t>(b, 0);     // party
        SendFramed(p1, ToUint16(MessageId::CT_USERMOVE_ACK), b);
        auto [w, got] = ReadFramed(p2);
        EXPECT(w == ToUint16(MessageId::CT_USERMOVE_ACK));
        wire::Reader r(got);
        std::uint32_t cid = 0, key = 0; std::uint8_t channel = 0;
        std::uint16_t map_id = 0, party = 0; float x = 0, y = 0, z = 0;
        r.Read(cid); r.Read(key); r.Read(channel); r.Read(map_id);
        r.Read(x); r.Read(y); r.Read(z); r.Read(party);
        EXPECT(cid == 200); EXPECT(channel == 1); EXPECT(map_id == 1000);
        EXPECT(x == 1.0f); EXPECT(z == 3.0f);
    }
    ...
    if (g_fails == 0)
        std::printf("PASS test_tworldsvr_asio_admin_handlers\n");
    else
        std::printf("FAIL test_tworldsvr_asio_admin_handlers "
                    "(%d failure%s)\n", g_fails, g_fails == 1 ? "" : "s");
