...
CTNewQuestDlg::CTNewQuestDlg( TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc, CTClientChar* pHost)
...
void CTNewQuestDlg::ResetTQUEST( LPTQUEST pTQUEST)
{
	...
	if( m_pTQUEST && m_pTQUEST->m_bType == QT_COMPLETE )
	{
		...
		if( !pTMISSION || (pTMISSION->m_bType != QT_MISSION && pTMISSION->m_bType != QT_GUILD) )
			m_pTQUEST = NULL;
		m_pACCEPT->m_strText = CTChart::LoadString( (TSTRING) m_dwCompleteID);
	}
	else if( m_pTQUEST && (m_pTQUEST->m_bType == QT_MISSION || m_pTQUEST->m_bType == QT_GUILD))
	{
