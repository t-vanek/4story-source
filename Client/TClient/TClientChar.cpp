...
BYTE CTClientChar::GetFreeSpolecnikPOS()
{
	for( BYTE i=0; i<1; i++)
		if(!m_vTSPOLECNIK[i])
			return i;
...
BYTE CTClientChar::IsAlliance( CTClientObjBase *pTARGET)
{
	...
	if(pGame->IsInBRMap())
	{
		...
		if(pHostTarget)
			return pGame->IsInMyBRTeam(pHostTarget->m_dwID);
	...
	CTachyonRes *pRES)
{
	...
	for (auto Maintain : m_mapTMAINTAIN)
	{
		CTClientMaintain* pTMAINTAIN = Maintain.second;
...
void	CTClientChar::EnterSkygarden()
{
	m_bEnterSkygarden = TRUE;
}

void	CTClientChar::LeaveSkygarden()
{
	m_bEnterSkygarden = FALSE;
}
...
BOOL	CTClientChar::InSkygarden() const
{
	return m_bEnterSkygarden;
}
...
void CTClientChar::ApplyMantleTexture()
{
	...
	if (itTINVEN != m_mapTINVEN.end())
	{
		...
		if (pTITEM)
		{
			...
			if (CTClientGame::GetInstance()->m_wCloakPreview || pTITEM->GetCustomTex())
			{

				CTClientCustomCloak pCustom = CTClientCustomCloak(CTClientGame::GetInstance()->m_wCloakPreview ? CTClientGame::GetInstance()->m_wCloakPreview : pTITEM->GetCustomTex());
			...
			else if (pTITEM->GetTITEM()->m_wUseValue || (pTITEM->GetMoggItemID() && CTChart::FindTITEMTEMP(pTITEM->GetMoggItemID())->m_wUseValue))
			{
				ApplyCloakTexture(pTITEM);
			}
			else
			{
				...
				else if (!pTITEM->GetRegGuild() && !pTITEM->GetColor())
				{
...
DWORD CTClientChar::GetSoulEXP()
{
	return m_dwSoulEncryptEXP ^ m_zEncryptKEY;
}

void CTClientChar::SetSoulEXP(DWORD dwSoulEXP)
{
	m_dwSoulEncryptEXP = dwSoulEXP ^ m_zEncryptKEY;
}
