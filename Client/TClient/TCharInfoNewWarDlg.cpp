...
CTCharNewPVPDlg::CTCharNewPVPDlg( TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc)
...
CTCharNewPVPDlg::~CTCharNewPVPDlg()
{
	{
...
void CTCharNewPVPDlg::OnLButtonUp(UINT nFlags, CPoint pt)
{
	//GetComponentPos( &CTCharInfoDlg::m_ptPOS);
	CTClientUIBase::OnLButtonUp( nFlags, pt );
}

void CTCharNewPVPDlg::ShowComponent( BOOL bVisible)
{
	...
	if(pTGAME->GetSession())
		pTGAME->GetSession()->SendCS_PVPRECORD_REQ(FALSE);

	CTClientUIBase::ShowComponent( bVisible );
}

ITDetailInfoPtr CTCharNewPVPDlg::GetTInfoKey( const CPoint& point )
{
...
void CTCharNewPVPDlg::ResetData(
	BYTE bTabIndex,
	DWORD dwRankOrder,
	BYTE bRankPercent,
	...
	WORD wMonthLose)
{
	...
	m_pH_VALUE[1][0]->m_strText.Format("%d (%d%%)",
		dwRankOrder,
		bRankPercent);
...
void CTCharNewPVPDlg::ResetTabButton( BYTE bTabIndex )
{
