...
TGauge::TGauge(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc)
	: TComponent(pParent, pDesc)
{
	m_gaugeDesc = pDesc->m_vCOMP.m_vEX.gauge;
	Init();
}

TGauge::TGauge(TComponent* pParent, FRAMEDESC_WEAKPTR pDesc)
	: TGauge(pParent, pDesc.lock())
{

}

TGauge::TGauge(TComponent* pParent, const TGauge& rSrcGauge)
	: TComponent(pParent, rSrcGauge)
{
	m_gaugeDesc = rSrcGauge.m_gaugeDesc;
	Init();
}
...
void TGauge::Init()
{
	...
	for (unsigned int i = 0; i<m_gaugeDesc.count; ++i)
	{
		TComponent* pBar = FindKid(m_gaugeDesc.bar[i]);

		if (pBar)
			m_vBar.push_back(pBar);
	}
}
