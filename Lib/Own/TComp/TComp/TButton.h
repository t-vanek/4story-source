...
public:
	...
	void SelectMenuBtn( BOOL bSelect );
	void SetImageIndex( int nIndex );
	
	virtual BOOL HitTest( CPoint pt);
	virtual void EnableComponent( BOOL bEnable = TRUE);

	TButton(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc);
	TButton(TComponent* pParent, FRAMEDESC_WEAKPTR pDesc);
	TButton(TComponent* pParent, const TButton& rSrcBtn);
	virtual ~TButton();
...
protected:
	...
	{
		return new TButton(m_pParent, *this);
	};

	BUTTONDESC		m_btnDesc;
};
