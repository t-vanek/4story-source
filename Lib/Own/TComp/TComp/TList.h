...
{
public:
	TList(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc);
	TList(TComponent* pParent, FRAMEDESC_WEAKPTR pDesc);
	TList(TComponent* pParent, const TList& rSrcLst);
	virtual ~TList();
...
public:
	...
	void InitItemHolder(unsigned int item, int nColumnCount);
...
protected:
	...
	{
		return new TList(m_pParent, *this);
	};
};
