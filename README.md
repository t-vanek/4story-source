...
TWorldSvr         ██████████████░░░░░░   72%   (W6-45 — CMGift complete; 209/290 handlers, 99 tests)
─────────────────────────────────────────
...
| **TWorldSvrAsio** | 38 851 | ~33 900 | 209/290 — guild/party/corps/friend/soulmate/chat/TMS/mail/territory+war/combat/connection-teleport + event broadcast/update/replay + full cash-shop sale family (incl. confirm barrier + DB persist) + CMGift result + ctrl-svr identification + item-state ops tool + service/control plane (monitor echo, CCU resync, help-message, map teardown) + APEX shipped-parity stubs + GM item tools (state/find/grant) + full MonthRank + complete war/castle family + complete CMGift family (delivery + catalogue); BR/Bow/Arena/Tournament/Apex/MonthRank and the DB-bound CMGift sub-paths remain (see sub-README gaps audit) | 🟡 W3a–W6 (TGUILD* + party/corps + friend/soulmate + TMS + TITEMCHART/TCashItemSale probes) | 🟡 **W6-45 — CMGift complete** |
| `Lib/Own/FourStoryCommon` | — | (shared) | — | — | ✅ SOCI + audit + smtp + ops |

LOC weighting: `(24 213 edge-complete + ~28 000 TWorldSvr functional
[209/290 handlers ≈ 72 % of 38 851 LOC] + ~6 700 TMap scaffold) / 175 906
legacy ≈ 34 %`.
...
│   ├── TWorldSvrAsio/              # 🟡 cluster coordinator — W6-45 (cash-shop + CMGift + item tools + service plane + MonthRank + war family)
...
* [`Server/TWorldSvrAsio/README.md`](Server/TWorldSvrAsio/README.md) — 🟡 W6-45 (guild/party/corps/social/territory/combat + connection/teleport + event + cash-shop + full CMGift + ctrl-svr + item tools + service plane + APEX stubs + MonthRank + war/castle family; gaps audit inside)
...
* **TMapSvrAsio** — Layered scaffold (transport → dispatch → handlers
  ...
  **combat / loot / mob-AI grind loop** and a first **kill-count quest**
  ...
  `CONSOLIDATION.md`.
* **TWorldSvrAsio** — cluster coordinator, **~72 % ported** (209/290
  handlers, 99 in-process tests). Functionally-complete verticals:
