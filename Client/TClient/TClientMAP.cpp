...
LPTEXTURESET CTClientMAP::m_pTPORTAL = NULL;
...
BOOL CTClientMAP::CheckCollisionGATE_WARP(
								    FLOAT fPosX,
									FLOAT fPosZ,
									FLOAT fRadius,
									FLOAT & fPosX_ret,
									FLOAT & fPosZ_ret)
{
	for( int i=0; i<INT(m_vTDRAWGATE.size()); i++)
	{
		...
		for( itTGATE = m_mapTGate.begin(); itTGATE != m_mapTGate.end(); itTGATE++)
		{
			if( (*itTGATE).second->m_dwID ==  pTGATE->m_dwID)
				pGate = (*itTGATE).second;
		}

		if( D3DXVec2Dot( &vDIST, &vDIST) < fDIST * fDIST && !pGate->m_bOPEN)
		{
			fPosX_ret = pTGATE->GetPositionX();
			fPosZ_ret = pTGATE->GetPositionZ();
...
void CTClientMAP::GetLavaOBJ( LPVTOBJBASE pTLAVA,
							   FLOAT fPosX,
							   FLOAT fPosZ)
{
	MAPDWORD mapTINDEX;
				...
				default		:
					{
						...
						if( (bOBJType == OT_WATER || bOBJType == OT_LAVA) || (bOBJType == OT_COLLISION || bOBJType == OT_PATHWALL ? 1.0f : 2.0f) * TOBJCELL_LENGTH < max( fSizeX, fSizeZ) )
						{
							...
							pINST->ReleaseData();
							pINST->m_bZWrite = bOBJType == (bOBJType == OT_WATER || bOBJType == OT_LAVA) ? FALSE : TRUE;
							  ...
							  LPTSNDBLOCK pTSND)
{
	...
	if (!CTClientGame::IsInBRMap() || (CTClientGame::IsInBRMap() && (m_wMapID < 700 || m_wMapID > 712)))
	{
...

	{
		...
		pINST->m_bContryID = (*pTMAPNPC)[i]->m_bCountryID; // <- ?? ??? ??? ??? ?? ?? ? ???.
	...
	for(int i=0; i<INT(pTMAPSFX->size()); i++)
	{
	...
	for(INT i=0; i<INT(pTMAPSND->size()); i++)
	{
...
FLOAT CTClientMAP::GetLavaHeight( FLOAT fPosX,
								   FLOAT fPosZ)
{
	FLOAT fHeight = TMIN_HEIGHT;
	VTOBJBASE vTLAVA;
	...
	int nCount = INT(vTLAVA.size());
	for( int i=0; i<nCount; i++)
	{
		FLOAT fLocal = GetWaterHeight( vTLAVA[i], fPosX, fPosZ);
							  ...
							  CTRSCSDlg *pTRSCS)
{
	...
	pTRSCS->m_pDevice->m_pDevice->SetSamplerState( 0, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP);
	pTRSCS->m_pDevice->m_pDevice->SetSamplerState( 0, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP);

	pTRSCS->m_pDevice->m_pDevice->SetSamplerState( 0, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR );
	pTRSCS->m_pDevice->m_pDevice->SetSamplerState( 0, D3DSAMP_MINFILTER, D3DTEXF_LINEAR );
	pTRSCS->m_pDevice->m_pDevice->SetSamplerState( 0, D3DSAMP_MIPFILTER, D3DTEXF_NONE);
								  ...
								  LPTUNIT pTUNIT)
{
	...
	{
	case TRSCSCMD_PROTECT	: //he protecc
		{
	...
	case TRSCSCMD_ATTACK	: //he attacc
		{
	...
	case TRSCSCMD_MOVE		: //but most importantly he move XD
		{
...
//	pDevice->m_pDevice->SetRenderState( D3DRS_ALPHATESTENABLE, FALSE);
	...
	pDevice->m_pDevice->SetRenderState( D3DRS_ALPHAREF, 0x00);
	...
	/*	for(auto j=0; j<m_vTNPC[i].m_wCount; j++)
		...
		for(auto j=0; j<m_vTNPC[i].m_wCount; j++)
		{
			...
			LPTEXTURESET pTPOS = NULL;
			switch(pNPC->m_bNPCType)
			{
			case TNPC_PORTAL: pTPOS = m_pTPORTAL; break;
			}
	...
	{
		for(auto j=0; j<WORD(vCORPS[i]->m_vTUNIT.size()); ++j )
		{
	...
	if (pGame->IsInBRMap())
	{
		for (VBRTEAM::iterator itTeam = pGame->m_vBRTEAM.begin(); itTeam != pGame->m_vBRTEAM.end(); ++itTeam)
		{
			if ((*itTeam)->m_dwCharID == pHost->m_dwID ||
				(*itTeam)->m_bDead)
			{
				continue;
			}

			CTClientChar* pMate = pGame->FindPlayer((*itTeam)->m_dwCharID);
			if (!pMate)
				continue;
		...
		for (MAPPLAYER::iterator itPlayer = pGame->m_mapPLAYER.begin(); itPlayer != pGame->m_mapPLAYER.end(); ++itPlayer)
		{
			if (pGame->IsInMyBRTeam((*itPlayer).second->m_dwID))
				continue;

			if (!pGame->CanCharacterSee(pHost, (*itPlayer).second))
				continue;

			if (pGame->m_bBRStatus == BS_PEACE)
				continue;

			if (!(*itPlayer).second->m_dwHP)
				continue;
        ...
        BYTE bLoadNOW)
{
 ...
 if( bInit || IsEXTVisible(pCamera) )
 {
  ...
  vDIR *= (pCamera->m_fZoomMAX + 5.0f + CTachyonHUGEMAP::m_nMaxLength) / D3DXVec3Length(&vDIR);
...
void CTClientMAP::ResetVisibleOBJ( CD3DDevice *pDevice,
								   ...
								   DWORD dwTick)
{
	if( !pTOBJ || (pTOBJ->m_bType != OT_WATER && pTOBJ->m_bType != OT_LAVA) && (pTOBJ->m_bDETAILLevel > CTClientGame::GetMapDETAILOption()) )
	{
		if(pTOBJ)
	...
	if( pTOBJ->m_bType != OT_WATER && pTOBJ->m_bType != OT_LAVA)
	{
	...
	else if(pTOBJ->m_bType == OT_WATER)
		m_mapTWATER.insert( MAPOBJECT::value_type( pTOBJ->m_dwInstID, pTOBJ));
	else
		m_mapTLAVA.insert( MAPOBJECT::value_type( pTOBJ->m_dwInstID, pTOBJ));
...
void CTClientMAP::RenderLAVA( CD3DDevice *pDevice, CTClientCAM *pCamera)
{
	MAPOBJECT::iterator itLAVA;

	for( itLAVA = m_mapTLAVA.begin(); itLAVA != m_mapTLAVA.end(); itLAVA++)
		if((*itLAVA).second)
			(*itLAVA).second->Render( pDevice, pCamera);
}
...
void CTClientMAP::ClearTFLAG( WORD wUnitID)
{
	...
	while( itVPATH != m_vTPATH.end() )
		if( HIWORD((*itVPATH)->m_dwID) == wUnitID )
			itVPATH = m_vTPATH.erase(itVPATH);
		else
			itVPATH++;
								   ...
								   WORD wUnitID)
{
	...
	TRY
	{
		...
		DWORD dwCount = 0;
		WORD wUseless = 0;
		file.Read( &wUseless, sizeof(WORD));
