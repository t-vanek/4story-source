...
// ====================================================================================================
CTItemUpDlg::CTItemUpDlg(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc)
...
BOOL CTItemUpDlg::CheckItem(CTClientItem* pTargetItem, CTClientItem* pScrollItem) const
{
	...
	case IK_UPGRADE:	
		{
			...
			if( (dwTargetQt&TITEM_QUALITY_NORMAL) || (dwTargetQt&TITEM_QUALITY_GRADE) || (dwTargetQt&TITEM_QUALITY_GEM) || (dwTargetQt&TITEM_QUALITY_MOGG))
				return TRUE;
			else
	...
	{
		if (pTargetItem->GetTMAGIC()->size() > 4)
			return FALSE;
...
BOOL CTItemUpDlg::CanUpgrade( LPTITEM pScrollItemTemp, LPTITEM pTargetItemTemp ) const
{
	...
	case IK_ACCESSORY_SCROLL:
	{
		if (pTargetItemTemp->m_bType != IT_COSTUME)
		{
			m_strLastError = "You cannot use this scroll on other item than accessory.";
			return FALSE;
		}
	}
	return TRUE;
	...
	case TARGET_SLOT: 
		{
			...
			if((pClientItem->GetTITEM()->m_bKind == 19 && m_ItemSlot[SCROLL_SLOT].pItem && m_ItemSlot[SCROLL_SLOT].pItem->m_bKind == 105) || (m_ItemSlot[SCROLL_SLOT].pItem && m_ItemSlot[SCROLL_SLOT].pItem->m_bKind == 104))
				return TRUE;

			if( m_ItemSlot[SCROLL_SLOT].pItem )
				return CheckItem(pClientItem, GetLastClientItem(SCROLL_SLOT));
			else
				return (pItem->m_bCanGrade || pItem->m_bCanMagic || pItem->m_bCanRare || pItem->m_bCanColor || pItem->m_bType == 17);
		}
		break;
	...
	case SCROLL_SLOT: 
		{
			...
			if( m_ItemSlot[TARGET_SLOT].pItem && m_ItemSlot[SCROLL_SLOT].pItem )
				if(m_ItemSlot[TARGET_SLOT].pItem->m_bKind == 19 && m_ItemSlot[SCROLL_SLOT].pItem->m_bKind == 83)
					return FALSE;

			if((pClientItem->GetTITEM()->m_bKind == 105 && m_ItemSlot[TARGET_SLOT].pItem && m_ItemSlot[TARGET_SLOT].pItem->m_bKind == 19) || pItem->m_bKind == 104)
				return TRUE;

			if( pItem->m_bType != IT_GRADE && 
				pItem->m_bType != IT_WEAPON &&
				pItem->m_bType != IT_DEFENSIVE &&
				pItem->m_bType != IT_LONG &&
				pItem->m_bType != IT_SHIELD)
				return FALSE;
			else
			{
