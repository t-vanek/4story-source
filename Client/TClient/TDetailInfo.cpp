...
// ====================================================================================================
void ITDetailInfo::SetDir(BOOL bLR, BOOL bPriorLeft, BOOL bPriorTop, BOOL bUsePoint, const CPoint& pPoint)
{
	m_bLRType = bLR;
...
if( m_bUsePoint )
	{
		BYTE bChangePos = FALSE;
		int nDetDlgW = rcDetDlg.Width();
		int nDetDlgH = rcDetDlg.Height();
		CPoint pos;
		pos = m_pPoint == CPoint(0,0) ? ptMouse : m_pPoint;

		if( (m_rcRef.bottom + nDetDlgH > CTClientGame::GetInstance()->GetScreenY()) )
			bChangePos = 1;

		if(bChangePos)
			pos.y -= nDetDlgH - TICON_SIZE; //I want to fucking die

		return CPoint(pos.x + TICON_SIZE, pos.y);
	}
	if( m_bLRType )
		return _GetUIPosition_LR(rcDetDlg, m_bPriorLeft, m_bPriorTop);
	else
		return _GetUIPosition_TB(rcDetDlg, m_bPriorLeft, m_bPriorTop);
}
// ----------------------------------------------------------------------------------------------------
...
const CString CTItemDetInfo::TITEM_TOKENS[TINFOTOKEN_ITEM_COUNT] = {
	...
	CString(_T("/GradeLevel /GemStoneLevel")),			// TINFOTOKEN_ITEM_ATTRLEVEL
	CString(_T("/GemStoneLevel")),		// TINFOTOKEN_ITEM_NEEDSLEVEL
...
const CString CTItemDetInfo::TITEM_TYPE_TOKENS[TINFO_ITEMTYPE_COUNT] = {
	...
	CString(_T("/ItemUnique"))
//	CString(_T("/GemStoneLevel"))		// TINFO_ITEMTYPE_ITEMUNIQUE
};
// ----------------------------------------------------------------------------------------------------
...
// ====================================================================================================
CTItemDetInfo::CTItemDetInfo(TDETINFO_TYPE eType,
							 LPTITEM pItemTemp, 
							 const CRect& rc)
:
ITDetailInfo(eType, 
...
void CTItemDetInfo::Build()
{
	...
	if(m_eType == TDEFINFO_TYPE_GEM_0_W || m_eType == TDEFINFO_TYPE_GEM_1_W || m_eType == TDEFINFO_TYPE_GEM_2_W || m_eType == TDEFINFO_TYPE_GEM_3_W || m_eType == TDEFINFO_TYPE_GEM_4_W || m_eType == TDEFINFO_TYPE_GEM_5_W)
	{
		CString strMSG = " ";
...
void CTItemDetInfo::_ConvertAndSetTitle(const CString& strSource, DWORD dwColor)
{
	...
	if (m_pItemTemp->m_wItemID > 24500 && m_pItemTemp->m_wItemID <= 24500 + IE_COUNT)
		CTClientGame::AddEffectString(m_pItemTemp->m_wItemID - 24500, m_TitleB.m_strText);
}
// ----------------------------------------------------------------------------------------------------
void CTItemDetInfo::_ConvertAndAddLine(const CString& strSource, DWORD dwColor)
{
	...
	for(int i=0; i<TINFOTOKEN_ITEM_COUNT; ++i)
	{
		...
		if( nFind == -1 )
			continue;
		if(i == 9 && m_eType == TDEFINFO_TYPE_GEM_1 || i == 9 && m_eType == TDEFINFO_TYPE_GEM_1_W)
			strConv = _GetAttrString( (TINFOTOKEN_ITEM) i )+ " " + "+" + "1";
		else if(i == 9 && m_eType == TDEFINFO_TYPE_GEM_2 || i == 9 && m_eType == TDEFINFO_TYPE_GEM_2_W)
			strConv = _GetAttrString( (TINFOTOKEN_ITEM) i )+ " " + "+" + "2";
		else if(i == 9 && m_eType == TDEFINFO_TYPE_GEM_3 || i == 9 && m_eType == TDEFINFO_TYPE_GEM_3_W)
			strConv = _GetAttrString( (TINFOTOKEN_ITEM) i )+ " " + "+" + "3";
		else if(i == 9 && m_eType == TDEFINFO_TYPE_GEM_4 || i == 9 && m_eType == TDEFINFO_TYPE_GEM_4_W)
			strConv = _GetAttrString( (TINFOTOKEN_ITEM) i )+ " " + "+" + "4";
		else if(i == 9 && m_eType == TDEFINFO_TYPE_GEM_5 || i == 9 && m_eType == TDEFINFO_TYPE_GEM_5_W)
			strConv = _GetAttrString( (TINFOTOKEN_ITEM) i )+ " " + "+" + "5";
		else
			strConv = _GetAttrString( (TINFOTOKEN_ITEM) i );

		if( strConv.IsEmpty() )
			continue;
		if(i == 9 && m_eType == TDEFINFO_TYPE_GEM_1 || i == 9 && m_eType == TDEFINFO_TYPE_GEM_2 || i == 9 &&  m_eType == TDEFINFO_TYPE_GEM_3 || i == 9 && m_eType == TDEFINFO_TYPE_GEM_4 || i == 9 && m_eType == TDEFINFO_TYPE_GEM_5 || i == 9 && m_eType == TDEFINFO_TYPE_GEM_1_W || i == 9 && m_eType == TDEFINFO_TYPE_GEM_2_W || i == 9 &&  m_eType == TDEFINFO_TYPE_GEM_3_W || i == 9 && m_eType == TDEFINFO_TYPE_GEM_4_W || i == 9 && m_eType == TDEFINFO_TYPE_GEM_5_W)
		{
			dwColor = (D3DCOLOR_ARGB(255, 255, 255, 000));
		}
		else
		{
...
CTItemInstDetInfo::CTItemInstDetInfo(TDETINFO_TYPE eType,
									 ...
									 const CRect& rc)
:	CTItemDetInfo(eType, pItemInst->GetTITEM(), rc)
{
	m_eType = eType;
...
DWORD CTItemInstDetInfo::_GetAttrColor(TINFOTOKEN_ITEM eToken, DWORD dwDefaultColor )
{
	...
	case TINFOTOKEN_ITEM_NEEDSLEVEL:
		{
			if( m_pItemInst->GetELD() > 0 )
				//return D3DCOLOR_ARGB(255, 255, 255, 000);
...
// CTSealedItemDetInfo
// ====================================================================================================
CTSealedItemDetInfo::CTSealedItemDetInfo(TDETINFO_TYPE eType,
										 CTClientItem* pItemInst, 
										 const CRect& rc)
:	CTItemInstDetInfo(eType, pItemInst, rc)
{
}
// ----------------------------------------------------------------------------------------------------
...
CTCompanionDetInfo::CTCompanionDetInfo(TDETINFO_TYPE eType,
										 CTClientItem* pItemInst, 
										 const CRect& rc)
:	CTItemInstDetInfo(eType, pItemInst, rc)
{
...
// ----------------------------------------------------------------------------------------------------
CTCompanionDetInfo::~CTCompanionDetInfo()
{
}
// ====================================================================================================
void CTCompanionDetInfo::Build()
{
	if( !m_bNeedBuild )
		return;
	...
	DWORD wMonID = m_pItemInst->GetDuraMax() == 0 ? m_pItemInst->GetCompanionID() : m_pItemInst->GetDuraMax();

	LPTMONTEMP pMON = CTChart::FindTMONTEMP((WORD)wMonID);
	if (!pMON)
		return;
	m_wImageID = pMON->m_wFaceIcon;
	...
	m_TitleB.m_strText = m_pItemInst->GetTITEM()->m_bKind == IK_PETTRANSFORM ? "Companion Transformation Rune (" + pMON->m_strNAME + ")" : pMON->m_strNAME;
	if (m_pItemInst->GetTITEM()->m_bKind == IK_PETTRANSFORM)
		m_TitleB.m_dwColor = 0xFFb52e92;
	else
	m_TitleB.m_dwColor = 0xFFFFFF00;

	_ConvertAndSetTitle(m_TitleB.m_strText, 0xFFFFFF00);


	m_bNeedBuild = FALSE;
}
void CTCompanionDetInfo::_BuildCompanionIcon()
{
	if(m_pItemInst)
	{
		DWORD wMonID = m_pItemInst->GetDuraMax() == 0 ? m_pItemInst->GetCompanionID() : m_pItemInst->GetDuraMax();
...
const CString CTOptionItemDetInfo::TGEMITEM_TOKEN(_T("/GemStoneLevel"));
// ====================================================================================================
CTOptionItemDetInfo::CTOptionItemDetInfo(TDETINFO_TYPE eType, CTClientItem* pItemInst, 
									   const CRect& rc)
:	CTItemInstDetInfo(eType, pItemInst, rc),
	m_bProcOption(FALSE)
{
...
ITDetailInfo::LINE_STATE CTOptionItemDetInfo::_GetLineState(const CString& strSource, int nIdx)
{
	...
	for(int i=0; i<TINFO_ITEMTYPE_COUNT; ++i)
	{
		if( strSource.Find( TITEM_TYPE_TOKENS[i] ) != -1 )
		{
			...
			else if( q&TITEM_QUALITY_GEM )
			{
				if( i == TINFO_ITEMTYPE_ITEMGRADE )
					return LS_TITLE;
			}
			else if( q&TITEM_QUALITY_MOGG )
			{
				if( i == TINFO_ITEMTYPE_ITEMGRADE )
					return LS_TITLE;
			}
...
void CTOptionItemDetInfo::_ConvertAndSetTitle(const CString& strSource, DWORD dwColor)
{
	...
	LPTITEM pTITEM = m_pItemInst->GetTITEM();

	if (pTITEM->m_bKind == IK_1HAND ||
		pTITEM->m_bKind == IK_LHAND ||
		pTITEM->m_bKind == IK_2HAND ||
		pTITEM->m_bKind == IK_CAKRAM ||
		pTITEM->m_bKind == IK_AX ||
		pTITEM->m_bKind == IK_BOW ||
		pTITEM->m_bKind == IK_ARBALEST ||
		pTITEM->m_bKind == IK_MBAR ||
		pTITEM->m_bKind == IK_MSTICK ||
		pTITEM->m_bKind == IK_SHIELD ||
		pTITEM->m_bKind == IK_CLOTH ||
		pTITEM->m_bKind == IK_UNIFORM ||
		pTITEM->m_bKind == IK_LLEATHER ||
		pTITEM->m_bKind == IK_HLEATHER ||
		pTITEM->m_bKind == IK_LARMOR ||
		pTITEM->m_bKind == IK_HARMOR ||
		pTITEM->m_bKind == IK_BACK ||
		pTITEM->m_bType == IT_COSTUME ||
		pTITEM->m_bKind == IK_MULTIVAJRA ||
		pTITEM->m_bKind == IK_VAJRA)
	{
		CTClientGame::AddEffectString(m_pItemInst->GetGradeEffect(), m_TitleB.m_strText);

		if (m_pItemInst->GetWrap())
			_AddLine(CTChart::LoadString(TSTR_WRAP_FOOTER), D3DCOLOR_XRGB(255, 100, 100));

		if (m_pItemInst->GetCustomTex())
		{
			_AddLine(" ");
			_AddLine("Custom Cloak", 0xFF0099FF);
		}

		if (m_pItemInst->GetMoggItemID())
		{
			_AddLine(" ");

			LPTITEM pTITEM = CTChart::FindTITEMTEMP(m_pItemInst->GetMoggItemID());

			_AddLine(CTChart::LoadString(TSTR_INFO_MOGG), D3DCOLOR_XRGB(255, 102, 255));
			_AddLine(pTITEM->m_strNAME, D3DCOLOR_XRGB(255, 102, 255));
		}
	}
	
		if (m_pItemInst->GetTITEM() && m_pItemInst->GetTITEM()->m_bType == IT_COSTUME)
	{
		CString Refines;
		Refines.Format("Special Refines : %d/4", m_pItemInst->GetRefineCurrent());
		_AddLine(Refines, D3DCOLOR_XRGB(0, 255, 51));
	...
	case TINFOTOKEN_ITEM_ATTRLEVEL:
		{
			if( 0 == m_pItemInst->GetViewAttrLevel() )
				return _T("");
		if(m_pItemInst->GetGem()> 0)
		{
			strResult = CTChart::Format( TSTR_FMT_NUMBER, (m_pItemInst->GetViewAttrLevel() - m_pItemInst->GetGem()));
		}
		else
		{
...
void CTSkillDetInfo::_ConvertAndSetTitle(const CString& strSource, DWORD dwColor)
{
	...
	CString strLevel; 
	strLevel = CTChart::Format( TSTR_FMT_NUMBER, m_bLevel);
...
CString CTSkillDetInfo::_GetSkillString(TINFOTOKEN_SKILL eToken)
{
	...
	BYTE bCurLevel = (m_bLevel == NULL && !m_bProcUpgrade) ? NULL : m_pTSkill->m_bLevel + (bLevel - 1) * m_pTSkill->m_bLevelInc;
	...
	case TINFOTOKEN_SKILL_ACTTIME		: 
		{
			...
			if(strResult.IsEmpty())
				strResult = CTChart::LoadString(TSTR_SKILL_INSTANT);
		}
		break;
...
case TINFOTOKEN_SKILL_USEMP			: strResult = CTChart::Format( TSTR_FMT_NUMBER, !m_pTSkill->m_wUseMedals ? INT(CTChart::m_vLEVELRATE[bCurLevel] * FLOAT(m_pTSkill->m_wUseMP) / 100.0f) : m_pTSkill->m_wUseMedals); break;
...
CString CTSkillDetInfo::_GetSkillString(TINFOTOKEN_SKILL eToken)
{
	...
	BYTE bCurLevel = (bLevel == NULL) ? NULL : m_pTSkill->m_bLevel + (bLevel) * m_pTSkill->m_bLevelInc;
	BYTE bCurLevel2 = (bLevel == NULL) ? NULL : m_pTSkill->m_bLevel + bLevel * m_pTSkill->m_bLevelInc;
	...
	case TINFOTOKEN_SKILL_HITRATE		: strResult = CTChart::Format( TSTR_FMT_NUMBER, m_pTSkill->m_bHitInit + bLevel * m_pTSkill->m_bHitInc); break;
	case TINFOTOKEN_SKILL_DURATION		: strResult = BuildTimeFormatSTR(m_pTSkill->m_dwDuration + bLevel * m_pTSkill->m_dwDurationInc); break;
	...
	case TINFOTOKEN_SKILL_REUSE			: 
		{
			DWORD dwResult = m_pTSkill->m_dwDelay + bLevel * m_pTSkill->m_nDelayInc;
			if( m_pTSkill->m_bDelayType )
			{
				CTClientChar* pMainChar = CTClientGame::GetInstance()->GetMainChar();
				DWORD dwAD = pMainChar->m_dwAttackDelay[ m_pTSkill->m_bDelayType ];
	...
	case TINFOTOKEN_SKILL_PREVLEVEL		:
		if( m_pTSkill->m_vTUPGRADEINFO && bLevel < m_pTSkill->m_bMaxLevel )
			strResult = CTChart::Format( TSTR_FMT_LEVELB, m_pTSkill->m_vTUPGRADEINFO[bLevel].m_bPrevLevel);

		break;

	case TINFOTOKEN_SKILL_CLASSPOINT	:
		if( m_pTSkill->m_vTUPGRADEINFO && bLevel < m_pTSkill->m_bMaxLevel )
			strResult = CTChart::Format( TSTR_FMT_NUMBER, m_pTSkill->m_vTUPGRADEINFO[bLevel].m_bGroupPoint);

		break;

	case TINFOTOKEN_SKILL_SKILLPOINT	:
		if( m_pTSkill->m_vTUPGRADEINFO && bLevel < m_pTSkill->m_bMaxLevel )
			strResult = CTChart::Format( TSTR_FMT_NUMBER, m_pTSkill->m_vTUPGRADEINFO[bLevel].m_bSkillPoint);
...
CTAfterMathInfo::CTAfterMathInfo( CString strTitle, CString strAfter, CString strAfter2, const CRect& rc )
:	ITDetailInfo( TDEFINFO_TYPE_DEFTOOLTIP, 0, 0, rc ),
m_strTitle( strTitle ),
m_strAfter( strAfter ),
m_strAfter2(strAfter2)
{

}

CTAfterMathInfo::~CTAfterMathInfo()
{

}

BOOL CTAfterMathInfo::Compare( const ITDetailInfo* pTarget )
{
	if( !ITDetailInfo::Compare(pTarget) )
		return FALSE;

	const CTAfterMathInfo* pTargetInfo = static_cast<const CTAfterMathInfo*>(pTarget);
	return (pTargetInfo->m_strAfter == this->m_strAfter && pTargetInfo->m_strAfter2 == this->m_strAfter2);
}

void CTAfterMathInfo::Build()
{
	m_TitleB.m_strText = m_strTitle;
	m_TitleB.m_dwColor = D3DCOLOR_XRGB(117,204,110);

	//m_LinesB.push_back( Line(strSource, dwColor) ); 
	
	_AddLine( m_strAfter, D3DCOLOR_XRGB(69,227,226));
	_AddLine( m_strAfter2, D3DCOLOR_XRGB(205,118,108));
}

CTBRTeamInfo::CTBRTeamInfo(const CString& strTitle, const CString* strPlayer, const CRect& rc)
:	ITDetailInfo( TDEFINFO_TYPE_DEFTOOLTIP, 0, 0, rc ),
m_strTitle(strTitle)
{
	m_strPlayer = const_cast<CString*>(strPlayer);
}

CTBRTeamInfo::~CTBRTeamInfo()
{

}

BOOL CTBRTeamInfo::Compare( const ITDetailInfo* pTarget )
{
	if( !ITDetailInfo::Compare(pTarget) )
		return FALSE;

	const CTBRTeamInfo* pTargetInfo = static_cast<const CTBRTeamInfo*>( pTarget );
	return (pTargetInfo->m_strPlayer == m_strPlayer);
}

void CTBRTeamInfo::Build()
{
	m_TitleB.m_strText = m_strTitle;
	m_TitleB.m_dwColor = D3DCOLOR_XRGB( 255, 255, 255 );

	BYTE TeamSize = CTClientGame::GetInstance()->m_vBRTEAM.size();

	for (BYTE i = 0; i < TeamSize; ++i)
		_AddLine(m_strPlayer[i], D3DCOLOR_XRGB(240, 235, 255));
...
CTPlayerDetInfo::CTPlayerDetInfo(
		CString strName,
		BYTE bLevel,
		BYTE bRace,
		BYTE bSex,
		...
		BYTE bClass,
		const CRect& rc)
: ITDetailInfo(TDETINFO_TYPE_PLAYER, 0, 0, rc),
m_strName(strName),
m_bLevel(bLevel),
m_bRace(bRace),
m_bSex(bSex),
...
BOOL CTPlayerDetInfo::Compare(const ITDetailInfo* pTarget)
{
	if( pTarget->GetType() != TDETINFO_TYPE_PLAYER)
		return FALSE;

	const CTPlayerDetInfo* pTargetInfo = static_cast<const CTPlayerDetInfo*>( pTarget );

	if( pTargetInfo->m_strName != m_strName )
		return FALSE;
	
	if( pTargetInfo->m_bLevel != m_bLevel )
		return FALSE;

	if( pTargetInfo->m_bRace != m_bRace )
		return FALSE;

	if( pTargetInfo->m_bSex != m_bSex )
		return FALSE;

	if( pTargetInfo->m_bFace != m_bFace )
		return FALSE;

	if( pTargetInfo->m_bHair != m_bHair )
		return FALSE;

	if( pTargetInfo->m_bClass != m_bClass )
		return FALSE;

	return TRUE;
}

void CTPlayerDetInfo::Build()
{
	if( !m_bNeedBuild )
		return;

	m_wImageID = WORD(CTClientGame::GetFACE(
		m_bRace,
		m_bSex,
		m_bFace,
		m_bHair ));
...
CPoint CTPlayerDetInfo::GetUIPosition(const CRect& rcDetDlg, const CPoint& ptMouse )
{
	CPoint pt;
	if( m_bLRType )
		pt = _GetUIPosition_LR(rcDetDlg, m_bPriorLeft, m_bPriorTop);
	else
		pt = _GetUIPosition_TB(rcDetDlg, m_bPriorLeft, m_bPriorTop);
