...
// ======================================================================
#define TPETDLG_CAM_DIST		5.0f
// ======================================================================

// ======================================================================
CTPetDlg::CTPetDlg(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc, CD3DDevice* pDevice)
...
// ======================================================================
void CTPetDlg::SetDisplayPet(const CString& strName, WORD wPetID, const CTime& tEndTime, BYTE m_bEffect)
{
	if (m_pDisplayPet)
	{
		delete m_pDisplayPet;
	...
	m_pDisplayPet = NewDisplayPet(strName, wPetID, tEndTime, m_bEffect);
	NotifyUpdate();
}
// ======================================================================
...
HRESULT CTPetDlg::Render(DWORD dwTickCount)
{
	...
	m_pDisplayPet->ClearSFX();
	m_pDisplayPet->CTachyonObject::CalcTick(m_pDxDevice, dwTickCount);

	m_pDisplayPet->ApplyPEffect(m_pDisplayPet, m_pDisplayPet->GetPetEffect());
	if (m_pDisplayPet->m_pTEMP && m_pDisplayPet->m_pTEMP->wMonID >= 31250)
	{
		WORD wIndex = m_pDisplayPet->m_pTEMP->wMonID - 31250;
		switch (wIndex)
		{
		case 0:
		case 1:
		{
			m_pDisplayPet->ApplyCustomTexture(14, 1);
			m_pDisplayPet->ApplyCustomTexture(16, 2);
		}
		break;
		case 2:
		case 3:
		{
			m_pDisplayPet->ApplyCustomTexture(20, 1);
		}
		break;
		case 4:
		case 5:
		{
			m_pDisplayPet->ApplyCustomTexture(21, 1);
		}
		break;
		case 6:
		case 7:
		{
			m_pDisplayPet->ApplyCustomTexture(19, 1);
		}
		break;
		case 8:
		case 9:
		{
			m_pDisplayPet->ApplyCustomTexture(22, 1);
		}
		break;
...
CTClientPet* CTPetDlg::NewDisplayPet(const CString& strName, WORD wPetID, const CTime& tEndTime, BYTE m_bEffect)
{
	CTClientPet* pPet = new CTClientPet;
	pPet->m_dwHostID = CTClientGame::GetInstance()->GetMainChar()->m_dwID;
