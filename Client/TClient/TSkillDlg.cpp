...
CTSkillDlg::CTSkillDlg( TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc)
...
void CTSkillDlg::UpdateNormalMode()
{
	...
	for( BYTE bSLOT=0; bSLOT<TSKILLSLOT_COUNT; ++bSLOT)
	{
		...
		TComponent *pTRedMarker = pCURFRAME->FindKid(CTClientGame::m_vTICONNA[bSLOT]);
		
		if( !pTGauge || !pTMarker || !pTRedMarker)
			continue;
		
		pTMarker->ShowComponent(FALSE);
		pTGauge->ShowComponent(FALSE);
		pTRedMarker->ShowComponent(TRUE);
		...
		if( pTSKILL )
		{
			pTRedMarker->ShowComponent(FALSE);
