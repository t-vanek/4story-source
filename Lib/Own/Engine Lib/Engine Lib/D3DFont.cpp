						 ...
						 CPoint Hide)
{
	...
	if(nFormat&DT_CALCRECT)
	{
		...
		rect.bottom = INT(rect.top + vRECT.Height - 1) - Hide.y;
		rect.right = INT(rect.left + vRECT.Width + 1) - Hide.x;

		nFormat &= ~DT_CALCRECT;
	}

	m_nHeight = rect.Height() - Hide.y;
	m_nWidth = rect.Width() - Hide.x;
						...
						CPoint Hide)
{
	...
	MakeText( pDevice, strText, rect, nFormat, Hide);
