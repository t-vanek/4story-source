...
//////////////////////////////////////////////////////////////////////
TButton::TButton(TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc)
	:TComponent(pParent, pDesc)
{
	m_btnDesc = pDesc->m_vCOMP.m_vEX.button;
	Init();
}

TButton::TButton(TComponent* pParent, FRAMEDESC_WEAKPTR pDesc)
	: TButton(pParent, pDesc.lock())
{

}

TButton::TButton(TComponent* pParent, const TButton& rSrcBtn)
	:TComponent(pParent, rSrcBtn)
{
	m_btnDesc = rSrcBtn.m_btnDesc;
	Init();
}
...
void TButton::Init()
{
	...
	m_pUp = FindKid(m_btnDesc.up);
	m_pDown = FindKid(m_btnDesc.down);
	m_pHover = FindKid(m_btnDesc.hover);
	m_bSND = TRUE;
...
void TButton::SelectMenuBtn( BOOL bSelect )
{
	if( !m_bEnable )
		return ;
