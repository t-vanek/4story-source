...
TEdit::TEdit(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc)
	: TComponent(pParent, pDesc)
{
	m_editDesc = pDesc->m_vCOMP.m_vEX.edit;
	Init();
	...
	if (m_pCaret)
	{
		...
		m_nCaretPosY = m_pCaret->m_pDESC.lock()->m_vCOMP.m_nPosY;
		m_nCaretPosX = m_pCaret->m_pDESC.lock()->m_vCOMP.m_nPosX;
	}
}

TEdit::TEdit(TComponent* pParent, FRAMEDESC_WEAKPTR pDesc)
	: TEdit(pParent, pDesc.lock())
{

}

TEdit::TEdit(TComponent* pParent, const TEdit& rSrcEdit)
	: TComponent(pParent, rSrcEdit)
{
	m_editDesc = rSrcEdit.m_editDesc;
	Init();
	...
	if (m_pCaret)
	{
		...
		m_nCaretPosY = rSrcEdit.m_nCaretPosY;
		m_nCaretPosX = rSrcEdit.m_nCaretPosX;
	}
}
...
void TEdit::Init()
{
	...
	m_pCaret = FindKid(m_editDesc.caret);
}
...
HRESULT TEdit::DrawText()
{
	...
	if( m_pCaret && !m_blIsLocked )
	{
		...
		ptCaret.x += m_nCaretPosX;
		ptCaret.y += m_nCaretPosY;
...
{
	TextSettingVector::iterator itr;
	itr = m_TextSettingVector.begin();
