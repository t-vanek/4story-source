...
TWorldSvr         ██████████████░░░░░░   69%   (W6-41 — MonthRank live table; 199/290 handlers, 95 tests)
─────────────────────────────────────────
...
| **TWorldSvrAsio** | 38 851 | ~33 900 | 199/290 — guild/party/corps/friend/soulmate/chat/TMS/mail/territory+war/combat/connection-teleport + event broadcast/update/replay + full cash-shop sale family (incl. confirm barrier + DB persist) + CMGift result + ctrl-svr identification + item-state ops tool + service/control plane (monitor echo, CCU resync, help-message, map teardown) + APEX shipped-parity stubs + GM item tools (state/find/grant) + MonthRank live table + warlord say; BR/Bow/Arena/Tournament/Apex/MonthRank and the DB-bound CMGift sub-paths remain (see sub-README gaps audit) | 🟡 W3a–W6 (TGUILD* + party/corps + friend/soulmate + TMS + TITEMCHART/TCashItemSale probes) | 🟡 **W6-41 — MonthRank live table** |
| `Lib/Own/FourStoryCommon` | — | (shared) | — | — | ✅ SOCI + audit + smtp + ops |

LOC weighting: `(24 213 edge-complete + ~26 700 TWorldSvr functional
[199/290 handlers ≈ 69 % of 38 851 LOC] + ~6 700 TMap scaffold) / 175 906
legacy ≈ 33 %`.
...
│   ├── TWorldSvrAsio/              # 🟡 cluster coordinator — W6-41 (cash-shop + CMGift + ctrl-svr + item tools + service plane + MonthRank)
...
* [`Server/TWorldSvrAsio/README.md`](Server/TWorldSvrAsio/README.md) — 🟡 W6-41 (guild/party/corps/social/territory/combat + connection/teleport + event + full cash-shop family + CMGift + ctrl-svr + item tools + service plane + APEX stubs + MonthRank live table; gaps audit inside)
...
* **TMapSvrAsio** — Layered scaffold (transport → dispatch → handlers
  ...
  **combat / loot / mob-AI grind loop** and a first **kill-count quest**
  ...
  `CONSOLIDATION.md`.
* **TWorldSvrAsio** — cluster coordinator, **~69 % ported** (199/290
  handlers, 95 in-process tests). Functionally-complete verticals:
  ...
  + Tournament + the MonthRank rollover chain), the DB-bound CMGift
  admin sub-paths,
