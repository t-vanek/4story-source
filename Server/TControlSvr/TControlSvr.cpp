...
DWORD CTControlSvrModule::InitNetwork()
{
	...
	{
		LogSmartly("DebugSocket could not be initializised!");
...
DWORD CTControlSvrModule::ControlThread()
{
	...
	while(TRUE)
	{
		...
		else if(pOV)
		{
			...
			case COMP_CLOSE		: 
				OnCloseSession((CTControlSession *) pOV); 
				break;
			}
		}
		...
		case 4:
			{
				...
				{
					CString log;
					log.Format("Sending : %d, Title : %s", (*it).second.m_wValue, (*it).second.m_strTitle);
					LogSmartly(log);
...
void CTControlSvrModule::LogSmartly(CString strLogMessage)
{
	FILE* pDebug = fopen("C:\\TServices_GSP\\CrashLogs.log", "a+");
	CTime tTime(_AtlModule.m_timeCurrent);
	if(!pDebug)
		pDebug = fopen("C:\\TServices_GSP\\CrashLogs.log", "w+");
	
	if(!pDebug)
		return;

	CString strWhoreNewMessage;
	strWhoreNewMessage.Format("CTRL : %s - %s\n", tTime.Format("%c"), strLogMessage);
