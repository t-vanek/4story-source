...
#include "config.h"  // tmapsvr::Mode + ModeName
                 ...
                 const HandlerContext&                 ctx)
{
    ...
    spdlog::info("CS_REGISTERBOW_REQ char={} mode={} — F16 stub "
                 ...
                 "consolidation)",
        SenderCharId(sess, ctx), ModeName(ctx.mode));
    co_return;
}
                    ...
                    const HandlerContext&                 ctx)
{
    ...
    // RespawnMedalAdder constants from F2/F9 BR/Bow Settings.
...
// ---------- F17 control protocol stubs ----------
//
// All CT_* handlers are dispatch entries; F17 just decodes (no body
// fields parsed past size) and logs. The consolidation pass adds:
