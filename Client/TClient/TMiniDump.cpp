...
INT_PTR CALLBACK ErrorDlgProc(HWND hDlg,UINT iMessage,WPARAM wParam,LPARAM lParam)
{
	switch(iMessage)
	{
	case WM_INITDIALOG:
...
BOOL CALLBACK EnumerateLoadedModulesProc(PCSTR ModuleName, DWORD_PTR ModuleBase, ULONG ModuleSize, PVOID UserContext)
{
...
#ifdef _WIN64
		strTEMP.Format("rax: 0x%08x\trbx: 0x%08x\n", context.Rax, context.Rbx);
		outResult += strTEMP;

		strTEMP.Format("rcx: 0x%08x\trdx: 0x%08x\n", context.Rcx, context.Rdx);
		outResult += strTEMP;

		strTEMP.Format("rsi: 0x%08x\trdi: 0x%08x\n", context.Rsi, context.Rdi);
		outResult += strTEMP;

		strTEMP.Format("rbp: 0x%08x\trsp: 0x%08x\n\n", context.Rbp, context.Rsp);
		outResult += strTEMP;
#else
...
#define SNAPPROLOG(Cntx)                                                   \
	__asm push  RBP														   \
	__asm mov   RBP , RSP												   \
	__asm sub   RSP , __LOCAL_SIZE										   \
	__asm mov   Cntx.Rax , RAX                                             \
	__asm mov   Cntx.Rbx , RBX                                             \
	__asm mov   Cntx.Rcx , RCX                                             \
	__asm mov   Cntx.Rdx , RDX                                             \
	__asm mov   Cntx.Rdi , RDI                                             \
	__asm mov   Cntx.Rsi , RSI                                             \
	__asm xor   RAX , RAX                                                  \
	__asm mov   AX , GS                                                    \
	__asm mov   Cntx.SegGs , RAX                                           \
	__asm mov   AX , FS                                                    \
	__asm mov   Cntx.SegFs , RAX                                           \
	__asm mov   AX , ES                                                    \
	__asm mov   Cntx.SegEs , RAX                                           \
	__asm mov   AX , DS                                                    \
	__asm mov   Cntx.SegDs , RAX                                           \
	__asm mov   AX , CS                                                    \
	__asm mov   Cntx.SegCs , RAX                                           \
	__asm mov   AX , SS                                                    \
	__asm mov   Cntx.SegSs , RAX                                           \
	__asm mov  RAX , DWORD64 PTR [RBP]									   \
	__asm mov  Cntx.Rbp , RAX                                              \
	__asm mov  RAX , RBP                                                   \
	__asm add  RAX , 8                                                     \
	__asm mov  Cntx.Rsp , RAX                                              \
	__asm push RSI                                                         \
	__asm push RDI                                                         \
	__asm push RBX                                                         \
	__asm push RCX                                                         \
...
#define SNAPEPILOG(eRetVal)                                                \
	...
	__asm pop     RDI                                                      \
	__asm pop     RSI                                                      \
	__asm mov     RAX , eRetVal									           \
	__asm mov     RSP , RBP												   \
	__asm pop     RBP											           \
	__asm ret                     

#define COPYKEYCONTEXTREGISTERS(stFinalCtx,stInitialCtx)               \
	stFinalCtx.Rax   = stInitialCtx.Rax   ;                                \
	stFinalCtx.Rbx   = stInitialCtx.Rbx   ;                                \
	stFinalCtx.Rcx   = stInitialCtx.Rcx   ;                                \
	stFinalCtx.Rdx   = stInitialCtx.Rdx   ;                                \
	stFinalCtx.Rdi   = stInitialCtx.Rdi   ;                                \
	stFinalCtx.Rsi   = stInitialCtx.Rsi   ;                                \
	...
	stFinalCtx.Rbp   = stInitialCtx.Rbp
#else
#define SNAPPROLOG(Cntx)                                                   \
	__asm push  EBP														   \
	__asm mov   EBP , ESP												   \
...
BSUMDRET __declspec ( naked ) OnMemoryDump()
{
	...
	stFinalCtx.ContextFlags = CONTEXT_ALL;
