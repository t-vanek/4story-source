...
int CTSkillTemp::GetValue(LPTSKILLDATA pData, BYTE bLevel)
{
	...
	case 2:
		return int((pData->m_wValue * pow(m_f1stRateX, (bLevel == NULL) ? NULL : m_bStartLevel+(bLevel-1)*m_bNextLevel))/100);
	case 3:
...
DWORD CTSkillTemp::GetTransHPMPFromType(BYTE bType)
{
	for(DWORD i=0; i<m_vData.size(); i++)
	{
		if(m_vData[i]->m_bType == SDT_CURE)
...
DWORD CTSkillTemp::GetAggro(BYTE bLevel)
{
	...
	DWORD dwAggro = DWORD(m_dwAggro * pow(m_f1stRateX, (bLevel == NULL) ? NULL : m_bStartLevel+(bLevel-1)*m_bNextLevel)) / 100;
	return dwAggro;
}
