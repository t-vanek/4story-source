// W4-8 wire test: MW_REGION_ACK region propagation.
//
// Alice 42/peer1 0x42 is a mutual friend + soulmate of Bob 200, and
// a one-way (FT_FRIEND) friend of Carol 400 — all online. A region
// update for Alice (region 777) must store on Alice and mirror into
// Bob's reverse friend + soulmate entries, but NOT into Carol's
// one-way entry. No packet is emitted.
...
std::vector<std::byte> RegionBody(std::uint32_t cid, std::uint32_t key,
                                   std::uint32_t region)
{
    std::vector<std::byte> b;
    tworldsvr::wire::WritePOD(b, cid);
    tworldsvr::wire::WritePOD(b, key);
    tworldsvr::wire::WritePOD(b, region);
    return b;
}
...
int main()
{
    ...
    with(200, [](tworldsvr::TChar& c) {
        ...
        c.soulmate = S{42, "Alice", 0, 0, false, 0};   // not yet connected
    });
    with(400, [](tworldsvr::TChar& c) {
        c.friends.push_back({42, "Alice", frnd::kTypeFriend, true, 0, 0});
    });

    auto friend_region = [&](std::uint32_t cid, std::uint32_t fid) -> long {
        auto c = chars.Find(cid);
        if (!c) return -1;
        std::lock_guard g(c->lock);
        for (const auto& f : c->friends) if (f.id == fid) return f.region;
        return -1;
    };
    auto poll = [&](auto pred) {
        for (int i = 0; i < 200; ++i)
    ...
    SendFramed(p1, ToUint16(MessageId::MW_REGION_ACK), RegionBody(42, 0xA1, 777));

    // Alice's own region.
    EXPECT(poll([&] {
        auto c = chars.Find(42); std::lock_guard g(c->lock);
        return c->region == 777; }));
    // Bob's reverse soulmate entry: region mirrored + connected set.
    EXPECT(poll([&] {
        auto c = chars.Find(200); std::lock_guard g(c->lock);
        return c->soulmate.region == 777 && c->soulmate.connected; }));
    // Bob's reverse friend entry (mutual): region mirrored.
    EXPECT(poll([&] { return friend_region(200, 42) == 777; }));
    // Carol's one-way (FT_FRIEND) entry: NOT mirrored.
    EXPECT(friend_region(400, 42) == 0);
    ...
    if (g_fails == 0)
        std::printf("PASS test_tworldsvr_asio_region_handlers\n");
    else
        std::printf("FAIL test_tworldsvr_asio_region_handlers "
                    "(%d failure%s)\n", g_fails, g_fails == 1 ? "" : "s");
