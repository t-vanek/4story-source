...
int CTClientGame::OnGM_SYSTEM()
{
	...
	{
		if (m_pSESSION)
			m_pSESSION->SendCS_LEAVEBATTLEFIELD_REQ();
	}
	else
	{
		...
		AddKid(m_pOutro);
		m_pOutro->m_bType = 0;
		m_pOutro->ShowComponent(TRUE);


		m_pMainWnd->SetMainFrame(m_pOutro);
	}
...
int CTClientGame::OnGM_OPEN_WORLDMAP()
{
	...
	if (!static_cast<CQuestNewDlg*>(CTClientGame::GetInstance()->GetFrame(TFRAME_QUEST_NEWUI))->m_bHideMap)
		return TERR_NONE;
...
#ifdef NEW_IF
	m_pChannelPOPUP->ClearButtons();
#else
	m_pChannelPOPUP->ClearMENU();
#endif

	m_pUBarPopup->ShowComponent(FALSE);
	m_pUBarPopup->ClearButtons();

	return TERR_NONE;
}

int CTClientGame::OnGM_TITLE()
{
	/*CTTitleDlg* pTitleDlg = static_cast<CTTitleDlg*>(m_vTFRAME[TFRAME_TITLE]);

	if(pTitleDlg->m_pList3->m_bVisible == 0)
		pTitleDlg->m_pList3->ShowComponent(TRUE);
	else
		pTitleDlg->m_pList3->ShowComponent(FALSE);*/
...
int CTClientGame::OnGM_USEITEM_MOVETOCHAR_FROMTARGET()
{
	BYTE bInvenID, bSlotID;
	if( m_pMainChar->IsBlock() || 
		!m_bCanUseSkill ||
		!FindInven( IT_USE, IK_MOVETOCHAR, bInvenID, bSlotID ) ||
		( !m_pTARGET && !m_pTPARTY ) )
	{
		return TERR_NONE;
	}
	...
	CString strName = m_pTARGET ? m_pTARGET->GetName() : m_pTPARTY->m_strNAME;
	if( strName.IsEmpty() )
		return TERR_NONE;
...
int CTClientGame::OnGM_TRADE_REQ()
{
	...
	if(!m_pMainChar->IsTradeAlliance(m_pTARGET) && MAKEWORD(BYTE(INT(m_pMainChar->GetPositionX()) / UNIT_SIZE), BYTE(INT(m_pMainChar->GetPositionZ()) / UNIT_SIZE)) != 3)
	{
...
int CTClientGame::OnGM_ITEM_DO_BUY()
{
	...
	{	
			m_pSESSION->SendCS_ITEMBUY_REQ(
				pSHOPDLG->m_wShopID == 0 && m_pTARGET ? (WORD)m_pTARGET->m_dwID : (WORD)pSHOPDLG->m_wShopID,
				0,
				pTITEM->m_wItemID,
				bCount,
				INVEN_NULL,
...
int CTClientGame::OnGM_TOGGLE_QUESTINFO()
{
	if(IsInBOWMap() || IsInBRMap())
		...
		CQuestNewDlg *pTQuestDlg = static_cast<CQuestNewDlg*>(m_vTFRAME[TFRAME_QUEST_NEWUI]);

		ToggleUI(TFRAME_QUEST_NEWUI);
				pTQuestDlg->ResetTree(1);
...
int CTClientGame::OnGM_TOGGLE_CHARINFO()
{
	...
	ToggleUI(TFRAME_CHARINFO_INNER);
...
{
	if ((IsInBOWMap() || IsInBRMap()) && m_pMainChar->m_mapTINVEN.find(INVEN_BATTLE) != m_pMainChar->m_mapTINVEN.end())
		ToggleUI(TFRAME_BATTLEINVEN);
	else
...
{
	if(m_bTournamentBattle || m_bTournamentWatching || IsInBRMap())
		return TERR_NONE;
...
{
	m_pSESSION->SendCS_REQUESTSADDLE_REQ();
...
int CTClientGame::OnGM_QUEST_ACCEPT()
{
	...
	if(m_pSESSION)
	{
		...
		/*
		if (pTNPCTalkDlg->m_pACCEPT->m_strText == "Complete")
		{
			for (auto Q : CTChart::m_mapTQUESTTEMP)
			{
				if (Q.second->m_bType == QT_COMPLETE && Q.second->m_dwParentID == pTQUEST->m_dwID)
				{
					pTQUEST = Q.second;
					break;
				}
		...
		*/
		...
		if (pTNPCTalkDlg->m_pACCEPT->m_strText == "Complete")
		{
			for (auto Q : CTChart::m_mapTQUESTTEMP)
			{
				if (Q.second->m_bType == QT_COMPLETE) /* USE THIS WHEN NEW PATCH Q.second->m_dwParentID == pTQUEST->m_dwID*/
				{
					for (auto Term : Q.second->m_vTTERM)
					{
						if (Term->m_bType == QTT_COMPQUEST && Term->m_dwID == pTQUEST->m_dwID)
						{
							pTQUEST = Q.second;
							break;
						}
...
{
	CQuestNewDlg *pTQuestDlg = (CQuestNewDlg *) m_vTFRAME[TFRAME_QUEST_NEWUI];
...
int CTClientGame::OnGM_GOTO_THUMB()
{
	...
	m_pMainChar->m_fSpeedFactor = TDEF_SPEED * 1.6f;
	...
	if(pTREGION)
	{
		if( m_pMainChar->m_bEnterCastle && m_pMainChar->m_bCamp || m_pMainChar->m_bEnterSkygarden && m_pMainChar->m_bCamp)
		{
...
int CTClientGame::OnGM_USE_MEDALS()
{
	...
	m_pSESSION->SendCS_USEMEDALITEM_REQ(
		bInvenID,
		bItemID,
		(BYTE) nCount);
...
int CTClientGame::OnGM_TOGGLE_ACTLIST()
{
	...
	if( m_pActListPopup->IsVisible() )
	{
		...
		pt.x -= 368;
		pt.y += 45;
		m_pActListPopup->MoveComponent(pt);
#endif
	}
...
int CTClientGame::OnGM_TOGGLE_MAIL()
{
	...
	m_pMailList->m_pList->ClearItem();
	m_pMailList->m_pCheckAll->Select(FALSE);
...
int CTClientGame::OnGM_ITEMUP_READY()
{
	...
	else
	{
		...
		case IK_GEM			:	
			strMSG = CTChart::LoadString( TSTR_ITEMUP_GEM); 
			break;

		case IK_ACCESSORY_SCROLL:
			strMSG = "Do you really want to apply the scroll? The accessory will get random magic upgrade, and may not be reverted.";
			break;
		case IK_1HAND:
		case IK_LHAND:
		case IK_2HAND:
		...
		case IK_HARMOR:	
		case IK_BACK:
			strMSG = CTChart::LoadString( TSTR_ITEMUP_MOGG); 
			break;
		...
		case IK_COLOR:
			...
			break;
		case IK_CLOAKEFFECT:
			strMSG = "Do you really want to change your cloak effect?";
			break;
		case IK_DELEFFECT:
			strMSG = "Do you really want to delete effect of this item?";
			break;
		}
...
int CTClientGame::OnGM_NEW_NOTIFY()
{
	...
	CTGuildSummaryNewDlg* pSummary = static_cast<CTGuildSummaryNewDlg*>(pDlg->GetInnerFrame(TGUILD_SUMMARY));
	pSummary->StartNew();
...
int CTClientGame::OnGM_GMP_WHISPER2()
{
	CTGuildMainNew* pDlg = static_cast<CTGuildMainNew*>( m_vTFRAME[TFRAME_GUILD_BASE_NEW] );
	CTGuildMercenaryNewDlg* pFrame = static_cast<CTGuildMercenaryNewDlg*>( pDlg->GetInnerFrame(TGUILD_MERCENARY) );
	return pFrame->OnGM_GMP_WHISPER2();
}
...
int CTClientGame::OnGM_GMP_MAIL()
{
	CTGuildMainNew* pDlg = static_cast<CTGuildMainNew*>( m_vTFRAME[TFRAME_GUILD_BASE_NEW] );
	CTGuildMembersNewDlg* pFrame = static_cast<CTGuildMembersNewDlg*>( pDlg->GetInnerFrame(TGUILD_MEMBERS) );
	CTGuildMercenaryNewDlg* pFrame2 = static_cast<CTGuildMercenaryNewDlg*>( pDlg->GetInnerFrame(TGUILD_MERCENARY) );
	if(pFrame->IsVisible())
		return pFrame->OnGM_GMP_MAIL();
	else if(pFrame2->IsVisible())
		return pFrame2->OnGM_GMP_MAIL();
	else
		return FALSE;
}
...
int CTClientGame::OnGM_GMP_FRIEND()
{
	...
	if(pFrame->IsVisible())
		...
		return pFrame2->OnGM_GMP_FRIEND();
	else
		return FALSE;
}
...
int CTClientGame::OnGM_GMP_CANCEL()
{
	...
	if(pFrame->IsVisible())
		...
		return pFrame2->OnGM_GMP_CANCEL();
	else
		return FALSE;
}
...
int CTClientGame::OnGM_GMP_VOLUNTEER_HESED()
{
	...
	if(pFrame->IsVisible())
		...
		return pFrame2->OnGM_GMP_VOLUNTEER_HESED();
	else
		return FALSE;
}
int CTClientGame::OnGM_GMP_VOLUNTEER_ARDRI()
{
	...
	if(pFrame->IsVisible())
		...
		return pFrame2->OnGM_GMP_VOLUNTEER_ARDRI();
	else
		return FALSE;
}
int CTClientGame::OnGM_GMP_VOLUNTEER_TYCON()
{
	...
	if(pFrame->IsVisible())
		...
		return pFrame2->OnGM_GMP_VOLUNTEER_TYCON();
	else
		return FALSE;
}
int CTClientGame::OnGM_GMP_VOLUNTEER_GEHBRA()
{
	...
	if(pFrame->IsVisible())
		...
		return pFrame2->OnGM_GMP_VOLUNTEER_GEHBRA();
	else
		return FALSE;
}
int CTClientGame::OnGM_GMP_VOLUNTEER_WOODLAND()
{
	CTGuildMainNew* pDlg = static_cast<CTGuildMainNew*>(m_vTFRAME[TFRAME_GUILD_BASE_NEW]);
	...
	CTGuildMercenaryNewDlg* pFrame2 = static_cast<CTGuildMercenaryNewDlg*>(pDlg->GetInnerFrame(TGUILD_MERCENARY));
//	if (pFrame->IsVisible())
//		return pFrame->OnGM_GMP_VOLUNTEER_WOODLAND();
//	else if (pFrame2->IsVisible())
	//	return pFrame2->OnGM_GMP_VOLUNTEER_WOODLAND();
//	else
		return FALSE;
}
int CTClientGame::OnGM_GMP_VOLUNTEER_CANCEL()
{
	...
	if(pFrame->IsVisible())
		...
		return pFrame2->OnGM_GMP_VOLUNTEER_CANCEL();
	else
		return FALSE;
}
// =============================================================================================================
...
int CTClientGame::OnGM_PRIVATE_SHOP_START()
{
	...
	for(BYTE i=0; i<bCount; ++i)
	{
		...
		item[i].dwCredits = pInfo->m_dwCredits;
	}
...
int CTClientGame::OnGM_REG_ITEM_PRVSHOP()
{
	...
	if (pBox->GetCurrencyIndex() == pBox->CURRENCY_CREDIT)
		if (!pBox->GetCredits())
		{
			m_pMainWnd->MessageBoxOK(
				TSTR_GROUP_NONAME,
	...
	pInfo->m_dwCredits = pBox->GetCredits();
...
int CTClientGame::OnGM_MESSENGER_DELETE_MAIL()
{
	...
	for(int i=0;i<m_pMailList->m_pList->GetItemCnt();i++)
	{
		CTMailItem * pItem = static_cast<CTMailItem*>(m_pMailList->m_pList->GetItem(i));
		if(pItem->SelectToDel)
		{
			m_pSESSION->SendCS_POSTDEL_REQ( pItem->pSimple->m_dwPostID );
			bDeleted = TRUE;
		}
	}
...
int CTClientGame::OnGM_TOGGLE_BLOCKLIST()
{
	...
	if( pMsgFrame->IsVisible() && pMsgFrame->IsSelect(TMESSENGER_FRAME_BLOCKLIST) )
		DisableUI(TFRAME_MESSENGER);
	else
	{
		pMsgFrame->SetSelect(TMESSENGER_FRAME_BLOCKLIST);
...
int CTClientGame::OnGM_PET_RECALL()
{
	...
	if( pPet )
	{
		...
		{
			if( !pDlg->IsRecalling() && GetTickCount() - m_dwMountTick > 500 && !m_pMainChar->IsJump())
...
int CTClientGame::OnGM_PET_EFFECTCHANGE()
{
	if( !m_pSESSION )
	return TERR_NONE;

	CTPetManageDlg* pDlg = static_cast<CTPetManageDlg*>(m_vTFRAME[TFRAME_PET_MANAGE]);
	if( pDlg->GetSelectIdx() == T_INVALID )
		return TERR_NONE;
	CTClientPet* pPet = pDlg->GetSelectPet();
...
int CTClientGame::OnGM_PET_EFFECTDELETE()
{
	CTPetManageDlg* pDlg = static_cast<CTPetManageDlg*>(m_vTFRAME[TFRAME_PET_MANAGE]);
	CTClientPet* pPet = pDlg->GetSelectPet();
...
int CTClientGame::OnGM_PET_DELETEEFFECT()
{
	if( !m_pSESSION )
	return TERR_NONE;
	...
	if( pPet )
	{
		...
		m_pSESSION->SendCS_PETEFFECTCHANGE_REQ(pPet->GetPetID(), 0, 0);
	}
	else
	{
		m_pMainWnd->MessageBoxOK(
...
int CTClientGame::OnGM_PET_CHANGEEFFECT()
{
	if( !m_pSESSION )
	return TERR_NONE;
	...
	if( pPet )
	{
		...
		if( pMainPet && pMainPet->GetPetID() == pPet->GetPetID())
			...
			bCurEffect = pPet->GetPetEffect();

			do
				bNewEffect = rand() % (MAX_EFFECT - MIN_EFFECT) + MIN_EFFECT; 
			while(bCurEffect == bNewEffect);

			if(bCurEffect)
			{
				//pPet->SetEffect(bNewEffect); //?er ACK erledigen

				m_pSESSION->SendCS_PETEFFECTCHANGE_REQ(pPet->GetPetID(), bNewEffect, 400);
			}
			else
			{
				//pPet->SetEffect(bNewEffect); //?er ACK erledigen
				m_pSESSION->SendCS_PETEFFECTCHANGE_REQ(pPet->GetPetID(), bNewEffect, 1500);
			}
			
			//m_pSESSION->SendCS_PETRECALL_REQ(pPet->GetPetID()); //?er ACK erledigen
			//pPetDlg->SetRecalling(TRUE); //?er ACK erledigen
	}
	else
	{
		m_pMainWnd->MessageBoxOK(
...
{
	if (IsInBRMap() || IsInBOWMap())
	{
		m_pMainWnd->MessageBoxOK("You cannot change your channel in the Battle.", TSTR_OK, GM_CLOSE_MSGBOX, GM_CLOSE_MSGBOX);
		return TERR_NONE;
	}
	...
	{
		INT nChNum = m_pChannelPOPUP->GetLastSelIndex();
		if( nChNum == T_INVALID )
			return TERR_NONE;
		...
		size_t bCountEdit = m_pMainWnd->m_pTGROUP->m_vChannel.size() - 0;

		if( nChNum < (INT)bCountEdit) //m_pMainWnd->m_pTGROUP->m_vChannel.size() 
	...
	else
	{
		...
		for (size_t i = 0; i < bCountEdit; ++i)	//m_pMainWnd->m_pTGROUP->m_vChannel.size()
		{
			if (m_pMainWnd->m_pTGROUP->m_bSelectedChannelIndex == (BYTE)i)
				continue;

			m_pChannelPOPUP->AddButton(m_pMainWnd->m_pTGROUP->m_vChannel[i]->strNAME, GM_CHANGE_CHANNEL);//BRB
		}

		m_pChannelPOPUP->AddButton( CTChart::LoadString(TSTR_CANCEL), GM_CHANGE_CHANNEL);
		...
		CPoint point = CTClientUIBase::m_vBasis[TBASISPOINT_LEFT_TOP];
		point.x += 210;
		point.y += 45;
		...
		m_pChannelPOPUP->MoveComponent(point);
	}
#else
	if( m_pChannelPOPUP->IsVisible() )
	{
...
int CTClientGame::OnGM_CLICK_WORLDMAP()
{
	return OnGM_OPEN_WORLDMAP();;
}
...
{
	if(m_pMainChar->m_bHelmetHide == HELMET_SHOWN)
		m_pMainChar->m_bHelmetHide = ALL_HIDDEN;
	else if(m_pMainChar->m_bHelmetHide == ALL_HIDDEN)
		m_pMainChar->m_bHelmetHide = HELMET_SHOWN;
	else if(m_pMainChar->m_bHelmetHide == CLOAK_SHOWN)
		m_pMainChar->m_bHelmetHide = ALL_SHOWN;
	else if(m_pMainChar->m_bHelmetHide == ALL_SHOWN)
		m_pMainChar->m_bHelmetHide = CLOAK_SHOWN;
...
int CTClientGame::OnGM_TOGGLE_CLOAKHIDE()
{
	if(m_pMainChar->m_bHelmetHide == HELMET_SHOWN)
		m_pMainChar->m_bHelmetHide = ALL_SHOWN;
	else if(m_pMainChar->m_bHelmetHide == ALL_HIDDEN)
...
int CTClientGame::OnGM_TOGGLE_TITLE()
{
	if(!m_pSESSION || IsInBOWMap())
		return TERR_NONE;

	
	if(m_vTFRAME[TFRAME_CHARINFO_INNER]->IsVisible() && 
		static_cast<CTCharInfoNewDlg*>(m_vTFRAME[TFRAME_CHARINFO_INNER])->m_bTab == TAB_TITLE)
		DisableUI(TFRAME_CHARINFO_INNER);
	else
		m_pSESSION->SendCS_TITLELIST_REQ();
...
int CTClientGame::OnGM_PVP_INFO()
{
	...
	if( m_pSESSION )
		m_pSESSION->SendCS_PVPRECORD_REQ( TINFO_WAR );*/
...
int CTClientGame::OnGM_GVFPM_MAIL2()
{
	CTGuildMainNew* pDlg = static_cast<CTGuildMainNew*>( m_vTFRAME[TFRAME_GUILD_BASE_NEW] );
	CTGuildJoinNewDlg* pFrame = static_cast<CTGuildJoinNewDlg*>( pDlg->GetInnerFrame(TGUILD_JOIN) );
	return pFrame->OnGM_GVFPM_MAIL2();
}
...
int CTClientGame::OnGM_GMP_TACTICS_RE_CONTRACT()
{
	...
	if(pFrame->IsVisible())
		...
		return pFrame2->OnGM_GMP_TACTICS_RE_CONTRACT();
	else 
		return FALSE;
}
...
int CTClientGame::OnGM_GMP_TACTICS_KICK()
{
	...
	if(pFrame->IsVisible())
		...
		return pFrame2->OnGM_GMP_TACTICS_KICK();
	else 
		return FALSE;
}
...
int CTClientGame::OnGM_GMP_TACTICS_SELF_KICK()
{
	...
	if(pFrame->IsVisible())
		...
		return pFrame2->OnGM_GMP_TACTICS_SELF_KICK();
	else 
		return FALSE;
}
...
int CTClientGame::OnGM_GMP_TACTICS_REWARD_POINT()
{
	...
	if(pFrame->IsVisible())
		...
		return pFrame2->OnGM_GMP_TACTICS_REWARD_POINT();
	else 
		return FALSE;
}


int CTClientGame::OnGM_TOGGLE_GUILD_BASE_NEW()
{
	CTGuildMainNew* pDlg = static_cast<CTGuildMainNew*>(m_vTFRAME[TFRAME_GUILD_BASE_NEW]);
	ToggleUI(TFRAME_GUILD_BASE_NEW);
...
{
	CTGuildMainNew *pDlg = static_cast<CTGuildMainNew*>( m_vTFRAME[TFRAME_GUILD_BASE_NEW] );
	EnableUI(TFRAME_GUILD_BASE_NEW);
	pDlg->SetSelect(TGUILD_WARINFO);
...
int CTClientGame::OnGM_RANK_INFO()
{
	...
	if(pDlgMain->IsVisible() && pDlg->IsVisible())
		DisableUI(TFRAME_RANKING_NEW);
	else
	{
...
int CTClientGame::OnGM_RANK_INFO_TOTAL_TAB()
{
	...
	CTRankingNewDlg::m_bTabIndex = CTRankingNewDlg::TRANKINFO_TAB_TOTAL;
...
int CTClientGame::OnGM_RANK_INFO_COUNTRY_TAB()
{
	...
	CTRankingNewDlg::m_bTabIndex = CTRankingNewDlg::TRANKINFO_TAB_COUNTRY;
	pDlg->ResetData( CTRankingNewDlg::m_bTabIndex, CTRankingNewDlg::m_bTabIndex );
...
int CTClientGame::OnGM_RANK_INFO_TOURNAMENT_TAB()
{
	...
	CTRankingNewDlg::m_bTabIndex = CTRankingNewDlg::TRANKINFO_TAB_TOURNAMENT;
	pDlg->ResetData( CTRankingNewDlg::m_bTabIndex, CTRankingNewDlg::m_bTabIndex );
...
int CTClientGame::OnGM_TOURNAMENT_ENTRY_DO()
{
	...
	if( m_pSESSION )
	{
		m_pSESSION->SendCS_TOURNAMENTAPPLY_REQ( bEntryID, m_strMACAddr );
	}
	return TERR_NONE;
}
...
int CTClientGame::OnGM_NPC_DUEL3()
{
	...
	else
	{
		...
		auto NewDlg = (CTArenaRegDlg*)m_vTFRAME[TFRAME_ARENAREG];
		NewDlg->Release();

		if (m_pSESSION)
			m_pSESSION->SendCS_ARENAINFO_REQ(1);

		EnableUI( TFRAME_ARENAREG );
	}
...
int CTClientGame::OnGM_NPC_DUEL7()
{
	...
	else
	{
		...
		if (m_pSESSION)
			m_pSESSION->SendCS_ARENAINFO_REQ(m_pTARGET->GetName().Find("3", 0) != -1 ? 1 : 2);
...
int CTClientGame::OnGM_USE_SADDLE()
{
	m_pMainWnd->CloseMessageBox();
	...
	if( m_pSESSION)
		m_pSESSION->SendCS_CREATESADDLE_REQ(bInvenID, bItemID);

	return TERR_NONE;
}
int CTClientGame::OnGM_UB_SHOP_1()
{
	PickNext(TPICK_COUNT); 
	CTShopDlg* pDlg =static_cast<CTShopDlg*>(CTClientGame::GetInstance()->GetFrame(TFRAME_ITEM_SHOP));
	pDlg->m_wShopID = 26011;
	if(m_pSESSION)
		m_pSESSION->SendCS_NPCITEMLIST_REQ( 26011 );

	OnGM_CLOSE_POPUP();

	return TERR_NONE;
}
int CTClientGame::OnGM_UB_SHOP_2()
{
	PickNext(TPICK_COUNT); 
	CTShopDlg* pDlg =static_cast<CTShopDlg*>(CTClientGame::GetInstance()->GetFrame(TFRAME_ITEM_SHOP));
	pDlg->m_wShopID = 30006;
	if(m_pSESSION)
		m_pSESSION->SendCS_NPCITEMLIST_REQ( 30006 );

	OnGM_CLOSE_POPUP();

	return TERR_NONE;
}
int CTClientGame::OnGM_UB_SHOP_3()
{
	PickNext(TPICK_COUNT); 
	CTShopDlg* pDlg =static_cast<CTShopDlg*>(CTClientGame::GetInstance()->GetFrame(TFRAME_ITEM_SHOP));
	pDlg->m_wShopID = 26012;
	if(m_pSESSION)
		m_pSESSION->SendCS_NPCITEMLIST_REQ( 26012 );

	OnGM_CLOSE_POPUP();

	return TERR_NONE;
}
int CTClientGame::OnGM_UB_SHOP_4()
{
	PickNext(TPICK_COUNT); 
	CTShopDlg* pDlg =static_cast<CTShopDlg*>(CTClientGame::GetInstance()->GetFrame(TFRAME_ITEM_SHOP));
	pDlg->m_wShopID = 26009;
	if(m_pSESSION)
		m_pSESSION->SendCS_NPCITEMLIST_REQ( 26009 );

	OnGM_CLOSE_POPUP();

	return TERR_NONE;
}
int CTClientGame::OnGM_UB_SHOP_5()
{
	PickNext(TPICK_COUNT); 
	CTShopDlg* pDlg =static_cast<CTShopDlg*>(CTClientGame::GetInstance()->GetFrame(TFRAME_ITEM_SHOP));
	pDlg->m_wShopID = 30004;
	if(m_pSESSION)
		m_pSESSION->SendCS_NPCITEMLIST_REQ( 30004 );

	OnGM_CLOSE_POPUP();

	return TERR_NONE;
}
int CTClientGame::OnGM_UB_SHOP_6()
{
	PickNext(TPICK_COUNT); 
	CTShopDlg* pDlg =static_cast<CTShopDlg*>(CTClientGame::GetInstance()->GetFrame(TFRAME_ITEM_SHOP));
	pDlg->m_wShopID = 30008;
	if(m_pSESSION)
		m_pSESSION->SendCS_NPCITEMLIST_REQ( 30008 );

	OnGM_CLOSE_POPUP();

	return TERR_NONE;
}
int CTClientGame::OnGM_UB_SHOP_7()
{
	PickNext(TPICK_COUNT); 
	CTShopDlg* pDlg =static_cast<CTShopDlg*>(CTClientGame::GetInstance()->GetFrame(TFRAME_ITEM_SHOP));
	pDlg->m_wShopID = 26010;
	if(m_pSESSION)
		m_pSESSION->SendCS_NPCITEMLIST_REQ( 26010 );

	OnGM_CLOSE_POPUP();

	return TERR_NONE;
}
int CTClientGame::OnGM_UB_SHOP_8()
{
	PickNext(TPICK_COUNT); 
	CTShopDlg* pDlg =static_cast<CTShopDlg*>(CTClientGame::GetInstance()->GetFrame(TFRAME_ITEM_SHOP));
	pDlg->m_wShopID = 30005;
	if(m_pSESSION)
		m_pSESSION->SendCS_NPCITEMLIST_REQ( 30005 );

	OnGM_CLOSE_POPUP();

	return TERR_NONE;
}
int CTClientGame::OnGM_UB_SHOP_9()
{
	PickNext(TPICK_COUNT); 
	CTShopDlg* pDlg =static_cast<CTShopDlg*>(CTClientGame::GetInstance()->GetFrame(TFRAME_ITEM_SHOP));
	pDlg->m_wShopID = 30007;
	if(m_pSESSION)
		m_pSESSION->SendCS_NPCITEMLIST_REQ( 30007 );

	OnGM_CLOSE_POPUP();

	return TERR_NONE;
}
int CTClientGame::OnGM_UB_SHOP_10()
{
	PickNext(TPICK_COUNT); 
	CTShopDlg* pDlg =static_cast<CTShopDlg*>(CTClientGame::GetInstance()->GetFrame(TFRAME_ITEM_SHOP));
	pDlg->m_wShopID = 30009;
	if(m_pSESSION)
		m_pSESSION->SendCS_NPCITEMLIST_REQ( 30009 );

	OnGM_CLOSE_POPUP();

	return TERR_NONE;
}
int CTClientGame::OnGM_OPEN_BOX()
{
	CTBoxOpenDlg* pDlg = static_cast<CTBoxOpenDlg*>(m_vTFRAME[TFRAME_OPENBOX]);
	if(pDlg->m_pOpen->IsEnable())
	{
		pDlg->m_pOpen->EnableComponent(FALSE);
		pDlg->Open();
	}

	return TERR_NONE;
}
int CTClientGame::OnGM_CLOSE_BOX()
{
	CTBoxOpenDlg* pDlg = static_cast<CTBoxOpenDlg*>(m_vTFRAME[TFRAME_OPENBOX]);
	pDlg->Release(); //I have no clue tf im doing :D
	DisableUI(TFRAME_OPENBOX);
	return TERR_NONE;
}
int CTClientGame::OnGM_TOGGLE_LIST()
{
	CTBoxOpenDlg* pDlgBox = static_cast<CTBoxOpenDlg*>(m_vTFRAME[TFRAME_OPENBOX]);
	CTBoxOpenListDlg* pDlg = static_cast<CTBoxOpenListDlg*>(m_vTFRAME[TFRAME_OPENBOXLIST]);
	pDlgBox->m_bShowList = !pDlgBox->m_bShowList;

	if(pDlgBox->IsVisible())
	{
		pDlg->ResetPosition();
		pDlg->ShowComponent(pDlgBox->m_bShowList);
	}
	
	return TERR_NONE;
}

int CTClientGame::OnGM_UB_SHOP_CANCEL()
{
	CTClientGame* pGame = this;
	CTGaugePannel* pGauge = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE]);
	if( pGame->m_pUBarPopup->IsVisible() )
	{
		pGame->m_pUBarPopup->EnableComponent(FALSE);
		pGame->m_pUBarPopup->ShowComponent(FALSE);
	}	
	else
	{
		pGame->DisableUI(TFRAME_MAIN_MENU);

		pGame->m_pUBarPopup->ClearButtons();

		pGame->m_pUBarPopup->AddButton(
			_T("Accessories"),
			GM_UB_SHOP_1);
		...
		pGame->m_pUBarPopup->EnableComponent(TRUE);
		pGame->m_pUBarPopup->ShowComponent(TRUE);

		RemoveKid(pGame->m_pUBarPopup);
		AddKid(pGame->m_pUBarPopup);

		CPoint point;
		pGauge->m_pUtility->GetComponentPos(&point);

		pGame->m_pUBarPopup->MoveComponent(CPoint(point.x + 140, point.y +45));
	}
...
int CTClientGame::OnGM_LEVEL1()
{
	m_pMainWnd->CloseMessageBox();

	TFrame *pFRAME = m_pMainWnd->m_TNet.FindFrame(ID_FRAME_NEWCHAR_NEW);
	...
	for( BYTE i=0; i<MAX_SLOT; i++)
	{
		...
		for( BYTE j=0; j<INT(m_pMainWnd->m_vTCHARINFO.size()); j++)
		{
			if( i == m_pMainWnd->m_vTCHARINFO[j]->m_bSlotID )
			{
				bEMPTY = FALSE;
				break;
			}
		...
		if(bEMPTY)
		{
			...
			m_pMainWnd->m_vNEWTCHAR.m_bContry,
			m_pMainWnd->m_vNEWTCHAR.m_bSex,
			m_pMainWnd->m_vNEWTCHAR.m_bHair,
			m_pMainWnd->m_vNEWTCHAR.m_bFace,
			m_pMainWnd->m_vNEWTCHAR.m_bBody,
			m_pMainWnd->m_vNEWTCHAR.m_bPants,
			m_pMainWnd->m_vNEWTCHAR.m_bHand,
			m_pMainWnd->m_vNEWTCHAR.m_bFoot,
			1);

			m_pMainWnd->m_TNet.EnableComponent(FALSE);
			break;
		}
	}
...
int CTClientGame::OnGM_CANCEL_VETERAN()
{
	m_pMainWnd->CloseMessageBox();
	return TERR_NONE;
}

int CTClientGame::OnGM_OPEN_MENU_COMMUNITY()
{
	if(IsInBRMap())
	{
		m_pCommandsPopup->ShowComponent(!m_pCommandsPopup->IsVisible());

		if(m_pCommandsPopup->IsVisible())
		{
			...
			m_pCommandsPopup->MoveComponent(pt);
			m_pActListPopup->ShowComponent(FALSE);
#endif
		}
		return TERR_NONE;
	}
	...
	if(pPopDlg->IsVisible() && pPopDlg->GetCurMode() == TMAINBUTTONNEW_HELP)
	{
		pPopDlg->ShowComponent(FALSE);
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[TMAINBUTTONNEW_HELP])->SelectMenuBtn(FALSE);
	}
	else
	{
		pPopDlg->AddHelp();
		pPopDlg->ShowComponent(TRUE);
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[TMAINBUTTONNEW_HELP])->SelectMenuBtn(TRUE);
	}
#endif
	return TERR_NONE;
}

int CTClientGame::OnGM_OPEN_MENU_WAR()
{
#ifdef NEW_IF
	ResetPopupMenu();
	...
	if(pPopDlg->IsVisible() && pPopDlg->GetCurMode() == TMAINBUTTONNEW_WAR)
	{
		pPopDlg->ShowComponent(FALSE);
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[TMAINBUTTONNEW_WAR])->SelectMenuBtn(FALSE);
	}
	else
	{
		pPopDlg->AddWar();
		pPopDlg->ShowComponent(TRUE);
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[TMAINBUTTONNEW_WAR])->SelectMenuBtn(TRUE);
	}
#endif
	return TERR_NONE;
}

int CTClientGame::OnGM_OPEN_MENU_MESSENGER()
{
#ifdef NEW_IF
	ResetPopupMenu();
	...
	if(pPopDlg->IsVisible() && pPopDlg->GetCurMode() == TMAINBUTTONNEW_COMMUNITY)
	{
		pPopDlg->ShowComponent(FALSE);
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[TMAINBUTTONNEW_COMMUNITY])->SelectMenuBtn(FALSE);
	}
	else
	{
		pPopDlg->AddCommunity();
		pPopDlg->ShowComponent(TRUE);
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[TMAINBUTTONNEW_COMMUNITY])->SelectMenuBtn(TRUE);
	}
#endif
	return TERR_NONE;
}

int CTClientGame::OnGM_OPEN_MENU_CHAR()
{
#ifdef NEW_IF
	ResetPopupMenu();
	...
	if(pPopDlg->IsVisible() && pPopDlg->GetCurMode() == TMAINBUTTONNEW_CHAR)
	{
		pPopDlg->ShowComponent(FALSE);
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[TMAINBUTTONNEW_CHAR])->SelectMenuBtn(FALSE);
	}
	else
	{
		pPopDlg->AddChar();
		pPopDlg->ShowComponent(TRUE);
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[TMAINBUTTONNEW_CHAR])->SelectMenuBtn(TRUE);
	}
#endif
	return TERR_NONE;
}

int CTClientGame::OnGM_OPEN_MENU_INVEN()
{
#ifdef NEW_IF
	ResetPopupMenu();
	...
	if(pPopDlg->IsVisible() && pPopDlg->GetCurMode() == TMAINBUTTONNEW_INVEN_SLOT)
	{
		pPopDlg->ShowComponent(FALSE);
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[TMAINBUTTONNEW_INVEN_SLOT])->SelectMenuBtn(FALSE);
	}
	else
	{
		pPopDlg->AddInven();
		pPopDlg->ShowComponent(TRUE);
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[TMAINBUTTONNEW_INVEN_SLOT])->SelectMenuBtn(TRUE);
	}
#endif
	return TERR_NONE;
}

int CTClientGame::OnGM_OPEN_MENU_SYSTEM()
{
#ifdef NEW_IF
	ResetPopupMenu();
	...
	if(pPopDlg->IsVisible() && pPopDlg->GetCurMode() == TMAINBUTTONNEW_SYSTEM)
	{
		pPopDlg->ShowComponent(FALSE);
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[TMAINBUTTONNEW_SYSTEM])->SelectMenuBtn(FALSE);
	}
	else
	{
		pPopDlg->AddSystem();
		pPopDlg->ShowComponent(TRUE);
		((TButton*)pMainDlg->m_pTMAINBTN_NEW[TMAINBUTTONNEW_SYSTEM])->SelectMenuBtn(TRUE);
	}
#endif
	return TERR_NONE;
}

int CTClientGame::OnGM_CHAT_MENU()
{
	return TERR_NONE;
}

int CTClientGame::OnGM_OPEN_SOUL_LOTTERY()
{
	if(m_vTFRAME[TFRAME_SOULLOTTERY]->IsVisible())
	{
		DisableUI(TFRAME_SOULLOTTERY);
		m_pMainWnd->CloseMessageBox();
	}
	else if( m_pSESSION )
		m_pSESSION->SendCS_SOULLOTTERY_REQ();

	return TERR_NONE;
}

int CTClientGame::OnGM_SOUL_LOTTERY_TRIGGER()
{
	CTSoulLotteryDlg *pLottery = static_cast<CTSoulLotteryDlg*>(m_vTFRAME[TFRAME_SOULLOTTERY]);

	if( m_pSESSION && 
		pLottery->IsVisible())
	{
		if(pLottery->m_bClickStaus == CLICK_LOTTERY_NONE)
		{
			pLottery->m_bRenderNumber = TRUE;
			pLottery->m_bClickStaus = CLICK_LOTTERY_START;
		}
		else if(pLottery->m_bClickStaus == CLICK_LOTTERY_START)
		{
			pLottery->m_bClickStaus = CLICK_LOTTERY_END;
		}
	}
...
int CTClientGame::OnGM_SC_LEFTBTN()
{
	CTSecuritySystemDlg *pSecurity = static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS]);
	
	BYTE bDlgType = pSecurity->GetDlgType();			//Retard nigger implementation
	TEdit* pTextBox1 = pSecurity->GetTextBoxContent(1); //This is fucking retarded, why would someone
	TEdit* pTextBox2 = pSecurity->GetTextBoxContent(2);	//Make a function called GetTextBoxContent
	TEdit* pTextBox3 = pSecurity->GetTextBoxContent(3); //Which returns the pointer to an textbox
														//But returns nullptr if its not visible
														//Someone should rewrite it asap.
	BYTE bPreDlgType = pSecurity->GetPreDlgType();
...

	{
		...
		strSecurityCode = pTextBox3->m_strText;

		m_pSESSION->SendCS_SC_DISABLECODE_REQ(
			strSecurityCode);
	...
	{
		m_pSESSION->SendCS_SC_ENABLECODE_REQ();
	}
	break;
	case TDLG_SC_MUNLOCK:
	{
		CString strUnlockCode;
		strUnlockCode = pTextBox3->m_strText;





		m_pSESSION->SendCS_SC_UNLOCK_REQ(
			strUnlockCode);
	}
	break;
	/*
	...
	{
		CString strOldSecurityCode;
		CString strNewSecurityCode;

		pSecurity->GetTextBoxString(2, strNewSecurityCode);
		pSecurity->GetTextBoxString(3, strOldSecurityCode);

		if (pSecurity->CheckCode(strNewSecurityCode))

		{
			m_pSESSION->SendCS_SC_CHANGECODE_REQ(
				strOldSecurityCode,
				strNewSecurityCode);
		}
	...
	*/
	default:
	{
		...
		m_pSESSION->SendCS_SC_CHECKCODE_REQ(
			strSecurityCode,
			pSecurity->GetWrongAttempts());
	}
	break;
	}
	return TERR_NONE;
...
int CTClientGame::OnGM_SC_MIDDLEBTN()
{
	CTSecuritySystemDlg *pSecurity = static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS]);

	BYTE bDlgType = pSecurity->GetDlgType();
	TEdit* pTextBox1 = pSecurity->GetTextBoxContent(1);
	TEdit* pTextBox2 = pSecurity->GetTextBoxContent(2);
	TEdit* pTextBox3 = pSecurity->GetTextBoxContent(3);
	...
	case TDLG_SC_MCREATE:
		{
			...
			strSecurityCode = pTextBox2->m_strText;
			strConfirmCode = pTextBox3->m_strText;

			if(strSecurityCode == strConfirmCode)
			{
				if(pSecurity->CheckCode(strSecurityCode))
				{
					m_pSESSION->SendCS_SC_CREATECODE_REQ(strSecurityCode);
	...
	case TDLG_SC_MCHANGE:
		{
			CString strOldSecurityCode;
			CString strNewSecurityCode;
			CString strNewConfirmCode;

			strOldSecurityCode = pTextBox1->m_strText;
			strNewSecurityCode = pTextBox2->m_strText;
			strNewConfirmCode = pTextBox3->m_strText;

			if(strNewSecurityCode == strNewConfirmCode)
			{
				if(pSecurity->CheckCode(strNewSecurityCode))
				{
					m_pSESSION->SendCS_SC_CHANGECODE_REQ(
	...
	case TDLG_SC_MUNLOCK:
		{
			CString strUnlockCode;
			strUnlockCode = pTextBox2->m_strText;
...
int CTClientGame::OnGM_SC_RIGHTBTN()
{
	CTSecuritySystemDlg *pSecurity = static_cast<CTSecuritySystemDlg*>(m_vTFRAME[TFRAME_SECURITYSYS]);

	BYTE bDlgType = pSecurity->GetDlgType();
	
	if(bDlgType == TDLG_SC_MWRONGCODE)




		m_pMainWnd->MessageBoxOK("If you want to recover your security code, use our ticket system at www.4story.com/forum/", TSTR_OK, GM_CLOSE_MSGBOX, GM_CLOSE_MSGBOX);
...
int CTClientGame::OnGM_TOGGLE_TWITTER()
{
	ShowWindow(GetActiveWindow(), SW_MINIMIZE);
	ShellExecute(NULL, _T("open"), TSTR_TWITTERLINK ,NULL,NULL,SW_SHOWDEFAULT);

	return TERR_NONE;
}

int CTClientGame::OnGM_TOGGLE_FACEBOOK()
{
	ShowWindow(GetActiveWindow(), SW_MINIMIZE);
	ShellExecute(NULL, _T("open"), TSTR_FACEBOOKLINK ,NULL,NULL,SW_SHOWNORMAL);

	return TERR_NONE;
}
int CTClientGame::OnGM_TOGGLE_COMPANION()
{
	ToggleUI(TFRAME_COMPANION);

	return TERR_NONE;
}
int CTClientGame::OnGM_CREATE_COMPANION()
{
	BYTE bInvenID = (BYTE) LOWORD(m_dwCmdParam);
	BYTE bItemID = (BYTE) HIWORD(m_dwCmdParam);
	...
	if( m_pSESSION)
	{
		m_pSESSION->SendCS_CREATECOMPANION_REQ(
			bInvenID,
			bItemID,
			pEDIT->m_strText);
...
int CTClientGame::OnGM_WARN_COMPITEM()
{
...
if(m_pSESSION && pCompanion->IsVisible() && pSelCompanion)
		m_pSESSION->SendCS_USEPETITEM_REQ( m_dwCmdParam, pCompanion->m_bCurSelSlot );

	return TERR_NONE;
}

int CTClientGame::OnGM_COMPANION_DELETE()
{
	m_pMainWnd->MessageBoxYesNo(
		_T("Do you really want to delete this companion?"),
		TSTR_YES,
		TSTR_NO,
		GM_COMPANION_DELETEREQ,
		GM_CLOSE_MSGBOX );

	return TERR_NONE;
}

int CTClientGame::OnGM_COMPANION_DELETEREQ()
{
	CTCompanionDlg* pDlg = static_cast< CTCompanionDlg* >( m_vTFRAME[ TFRAME_COMPANION ] );
	m_pMainWnd->CloseMessageBox();
	if(pDlg->IsVisible())
	{
		BYTE bSlot = pDlg->m_bCurSelSlot;
		CTClientCompanion* pCOMPANION = pDlg->GetSelectedCompanion( bSlot );
		if( m_pSESSION && pCOMPANION )
			m_pSESSION->SendCS_DELETECOMPANION_REQ( bSlot );
	}

	return TERR_NONE;
}

int CTClientGame::OnGM_COMPANION_EFFECT()
{
	m_pMainWnd->MessageBoxYesNo(
		_T("Do you really want to change effect of this companion?"),
		TSTR_YES,
		TSTR_NO,
		GM_COMPANION_EFFECTREQ,
		GM_CLOSE_MSGBOX );

	return TERR_NONE;
}

int CTClientGame::OnGM_COMPANION_EFFECTREQ()
{
	CTCompanionDlg* pDlg = static_cast< CTCompanionDlg* >( m_vTFRAME[ TFRAME_COMPANION ] );
	m_pMainWnd->CloseMessageBox();
	...
	if( pDlg->IsVisible() )
		...
		if( m_pSESSION )
			m_pSESSION->SendCS_CHANGECOMPANIONEFFECT_REQ( pDlg->m_bCurSelSlot );
...
int CTClientGame::OnGM_USE_POWDER()
{
	CTCompanionDlg* pDlg = static_cast< CTCompanionDlg* >( m_vTFRAME[ TFRAME_COMPANION ] );
	m_pMainWnd->CloseMessageBox();
	...
	if (pDlg->IsVisible())
		...
		if (m_pSESSION)
			m_pSESSION->SendCS_USECOMPANIONPOWDER_REQ(bInvenID, bItemID, pDlg->m_bCurSelSlot);
	return TERR_NONE;
}

int CTClientGame::OnGM_USE_COMPRESET()
{
	CTCompanionDlg* pDlg = static_cast< CTCompanionDlg* >(m_vTFRAME[TFRAME_COMPANION]);
	m_pMainWnd->CloseMessageBox();
	...
	if (pDlg->IsVisible())
		if (m_pSESSION)
			m_pSESSION->SendCS_USECOMPRESET_REQ(bInvenID, bItemID, pDlg->m_bCurSelSlot);
	return TERR_NONE;
}

int CTClientGame::OnGM_CANCEL_POST()
{
	if(static_cast<CTMailSendDlg*>(m_vTFRAME[TFRAME_MAIL_SEND])->GetMessage() != NAME_NULL)
	{
		m_pMainWnd->MessageBoxYesNo(
			_T("Do you really wish to discard the draft?"),
			TSTR_YES,
			TSTR_NO,
			GM_CANCEL_POST_CONFIRM,
			GM_CLOSE_MSGBOX );
...
int CTClientGame::OnGM_CANCEL_POST_CONFIRM()
{
	DisableUI(TFRAME_MAIL_SEND);
	EnableUI(TFRAME_MAIL);
	m_pMainWnd->CloseMessageBox();
...
int CTClientGame::OnGM_OPEN_REGISTER_UI()
{
	CTBowRegisterDlg* pRegisterDlg = static_cast<CTBowRegisterDlg*>(m_vTFRAME[TFRAME_BOW_REGISTER]);
	CTBFGAlarm* pAlarm = static_cast<CTBFGAlarm*>(static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE])->m_pBFGAlarm);

	switch (pAlarm->GetType())
	{
	case SYSTEM_BOW:
		{
			pRegisterDlg->SetType(SYSTEM_BOW);
			pRegisterDlg->ShowComponent(TRUE);
		}
		break;
	case SYSTEM_BR:
		{
			if (pAlarm->GetBRType() == CTBFGAlarm::BR_SOLO)
			{
				pRegisterDlg->SetType(SYSTEM_BR);
				pRegisterDlg->ShowComponent(TRUE);
			}
			else
				EnableUI(TFRAME_BRTEAMS);
		}
		break;
	}
...
int CTClientGame::OnGM_BOW_REGISTER()
{
	CTBowRegisterDlg* pRegisterDlg = static_cast<CTBowRegisterDlg*>(m_vTFRAME[TFRAME_BOW_REGISTER]);
	if (!m_pSESSION || !pRegisterDlg->IsVisible())
		return TERR_NONE;

	switch (pRegisterDlg->GetType())
	{
	case SYSTEM_BOW:
		m_pSESSION->SendCS_REGISTERBOW_REQ();
		break;
	case SYSTEM_BR:
		m_pSESSION->SendCS_REGISTERBR_REQ(FALSE);
		break;
	}
...
int CTClientGame::OnGM_CANCEL_BOW_QUEUE()
{
	m_pMainWnd->MessageBoxYesNo(
		_T("Do you really want to cancel the queue?"),
		TSTR_YES,
		TSTR_NO,
		GM_CANCEL_BOW_QUEUE_CONFIRM,
		GM_CLOSE_MSGBOX);

	return TERR_NONE;
}

int CTClientGame::OnGM_CANCEL_BOW_QUEUE_CONFIRM()
{
	m_pMainWnd->CloseMessageBox();
	if (m_pSESSION)
		m_pSESSION->SendCS_CANCELBOWQUEUE_REQ();

	return TERR_NONE;
}

int CTClientGame::OnGM_BOW_CASH_RESPAWN()
{
	if (!IsInBOWMap() && !IsInBRMap())
		return TERR_NONE;

	if (m_pSESSION)
		m_pSESSION->SendCS_CASHBOWRESPAWN_REQ();

	return TERR_NONE;
}

int CTClientGame::OnGM_BP_EXCHANGE()
{
	if ((!IsInBOWMap() && !IsInBRMap()) || !CTBowBPTradeDlg::m_dwMedals)
		return TERR_NONE;

	CString strMSG;
	strMSG.Format("Do you really want to buy %d BP for %d Medals?", 
		CTBowBPTradeDlg::m_dwBP, 
		CTBowBPTradeDlg::m_dwMedals);

	m_pMainWnd->MessageBoxYesNo(
		strMSG,
		TSTR_YES,
		TSTR_NO,
		GM_BP_EXCHANGE_CONFIRM,
		GM_CLOSE_MSGBOX);

	return TERR_NONE;
}

int CTClientGame::OnGM_BP_EXCHANGE_CONFIRM()
{
	m_pMainWnd->CloseMessageBox();
	if (!IsInBOWMap() && !IsInBRMap())
		return TERR_NONE;
	...
	if (m_pSESSION)
		m_pSESSION->SendCS_EXCHANGEBOWBP_REQ(CTBowBPTradeDlg::m_dwMedals);

	return TERR_NONE;
}

int CTClientGame::OnGM_BRTEAMMATEADD_REQ()
{
	TFrame *pFRAME = m_pMainWnd->m_MessageBox.FindFrame(ID_FRAME_2BTNMSGBOX);
	if (!pFRAME)
		return TERR_NONE;
	...
	if (m_pSESSION)
		m_pSESSION->SendCS_BRTEAMMATEADD_REQ(pEDIT->m_strText);
...
int CTClientGame::OnGM_BRTEAMMATEADD_ANSWER()
{
	if (m_strBRInviter.IsEmpty())
		return TERR_NONE;

	if (m_pSESSION)
		m_pSESSION->SendCS_BRTEAMMATEADDRESULT_ACK(!m_dwCmdParam ? TEAMADD_REFUSE : TEAMADD_SUCCESS, m_strBRInviter);
...
int CTClientGame::OnGM_BATTLERANK_TOGGLE()
{
	CBattleRankProgressDlg* pBattleProgressDlg = static_cast<CBattleRankProgressDlg*>(m_vTFRAME[TFRAME_BATTLERANK]);
	pBattleProgressDlg->SetInfo(m_pMainChar->m_dwRankPoint);
	pBattleProgressDlg->ShowComponent(TRUE);

	return TERR_NONE;
}

int CTClientGame::OnGM_DONATE_LIFE()
{
	INT nID = static_cast<CBRPannels*>(m_vTFRAME[TFRAME_BRPANNELS])->GetLastSel();
	if (nID == T_INVALID)
		return TERR_NONE;

	if (m_pSESSION)
		m_pSESSION->SendCS_DONATELIFE_REQ(nID);

	return TERR_NONE;
}

int CTClientGame::OnGM_TOGGLE_CUSTOMCLOAK()
{
	CTCustomCloakDlg* pDlg = static_cast<CTCustomCloakDlg*>(m_vTFRAME[TFRAME_CUSTOMCLOAK]);
	
	if(!pDlg->IsVisible() && m_pSESSION ) 
		m_pSESSION->SendCS_CUSTOMCLOAK_LIST_REQ();
	else
		DisableUI(TFRAME_CUSTOMCLOAK);
	
	return TERR_NONE;
}

int CTClientGame::OnGM_CUSTOMCLOAK_WARN()
{		
	MAPTINVEN::iterator itTINVEN = m_pMainChar->m_mapTINVEN.find(INVEN_EQUIP);
	if( itTINVEN != m_pMainChar->m_mapTINVEN.end() )
	{
		...
		if( pTITEM )
		{
			...
			if(pDlg->m_pList->GetSel() != T_INVALID && pDlg->m_pList->GetSel() == 0)		
			{
				m_pMainWnd->MessageBoxYesNo(
						"0 medals will be used up. Your cloak skin will back to default. ",
						TSTR_YES,
						TSTR_NO,
						TCOMMAND( GM_CUSTOMCLOAK_APPLY ),
						GM_CLOSE_MSGBOX,
						GM_CLOSE_MSGBOX,
						FALSE );
			}
			else if(pDlg->m_pList->GetSel() != T_INVALID && pDlg->m_pList->GetSel() >= 1)
			{
				m_pMainWnd->MessageBoxYesNo(
						"4000 medals will be used up. Are you sure you want to change the cloak skin? ",
						TSTR_YES,
						TSTR_NO,
						TCOMMAND( GM_CUSTOMCLOAK_APPLY ),
						GM_CLOSE_MSGBOX,
...
int CTClientGame::OnGM_CUSTOMCLOAK_PREVIEW()
{
	MAPTINVEN::iterator itTINVEN = m_pMainChar->m_mapTINVEN.find(INVEN_EQUIP);
	if( itTINVEN != m_pMainChar->m_mapTINVEN.end() )
	{
		...
		if( pTITEM )
		{
			...
			if(pDlg->m_pList->GetSel() != T_INVALID)		
			{
				WORD wID = (WORD)pDlg->m_pList->GetItemData( pDlg->m_pList->GetSel(), 0 );	
...
int CTClientGame::OnGM_CUSTOMCLOAK_APPLY()
{		
	m_pMainWnd->CloseMessageBox();

	CTCustomCloakDlg* pDlg = static_cast<CTCustomCloakDlg*>(m_vTFRAME[TFRAME_CUSTOMCLOAK]);
	if(m_pSESSION && pDlg->m_pList->GetSel() != T_INVALID && pDlg->m_pList->GetSel() == 0)
	{
		WORD wID = (WORD)pDlg->m_pList->GetItemData( pDlg->m_pList->GetSel(), 0 );
		DWORD dwMedals = 0;
		m_pSESSION->SendCS_CUSTOMCLOAK_CHANGE_REQ(wID, dwMedals);		
	}
	else if(m_pSESSION && pDlg->m_pList->GetSel() != T_INVALID && pDlg->m_pList->GetSel() >= 1)
	{
		WORD wID = (WORD)pDlg->m_pList->GetItemData( pDlg->m_pList->GetSel(), 0 );
		DWORD dwMedals = 1400;
		m_pSESSION->SendCS_CUSTOMCLOAK_CHANGE_REQ(wID, dwMedals);		
...
int CTClientGame::OnGM_GSKILL_RENEW_YES()
{
	...
	if (m_pSESSION)
		m_pSESSION->SendCS_GUILDSKILLACTION_REQ(Action, wParam);
...
int CTClientGame::OnGM_ARENA_TOP()
{
	auto Ranking = (ArenaRanking*)m_vTFRAME[TFRAME_ARENARANKING];
	Ranking->Release();

	if (m_pSESSION)
		m_pSESSION->SendCS_ARENARANKING_REQ(0);
