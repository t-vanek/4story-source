...
TList::TList(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc)
	: TComponent(pParent, pDesc)
{
	m_listDesc = pDesc->m_vCOMP.m_vEX.list;
	Init();
}

TList::TList(TComponent* pParent, FRAMEDESC_WEAKPTR pDesc)
	: TList(pParent, pDesc.lock())
{

}

TList::TList(TComponent* pParent, const TList& rSrcLst)
	: TComponent(pParent, rSrcLst)
{
	m_listDesc = rSrcLst.m_listDesc;
	Init();
}
...
void TList::Init()
{
	...
	m_pScroll = FindKid(m_listDesc.vs);

	m_nItemPerPage = 0;

	for (int i = 0; i<m_listDesc.column; ++i)
		InitItemHolder(m_listDesc.item[i], m_nColumnCount++);
...
void TList::InitItemHolder(unsigned int item, int nColumnCount)
{
	...
	if (pItem && pItem->IsTypeOf(TCML_TYPE_BUTTON))
	{
		...
		if (nColumnCount == 0)
			m_itemholders.push_back(pHolder1st);

		//FRAMEDESC_SHAREDPTR pListItemDesc = RetrieveKidDesc( pItem->GetID(), pDesc );
		if (sizeItem.cy > 0)
		{
			if (!nColumnCount)
			...
			{
				TComponent* pListItem = AddAsNewChild(pItem);
...
MAP_LISTITEM* TList::SetCurSelItem(int nLine)
{
	...
	else if (m_nCurSel + 1 >= m_nTop + m_nItemPerPage)
	{
		...
		if (m_nTop > m_items.size() - m_nItemPerPage)
			m_nTop = static_cast<int>(m_items.size()) - m_nItemPerPage;
	}
	UpdateScrollPosition();
...
void TList::UpdateScrollPosition()
{
		...
		m_pScroll->IsTypeOf(TCML_TYPE_SCROLL))
	{
		...
		int nRange = static_cast<int>(m_items.size()) - m_nItemPerPage;
		if (nRange < 0)
			nRange = 0;
...
INT TList::GetMaxItemHeight()
{
	...
	for (INT i = 0; i < m_listDesc.column; ++i)
	{
		TComponent* pItem = FindKid(m_listDesc.item[i]);
		if (pItem && pItem->IsTypeOf(TCML_TYPE_BUTTON))
		{
			SIZE sizeItem;
			pItem->GetComponentSize(&sizeItem);
