// W6-4 wire test: recall-mon (summoned creature) cross-connection sync.
//
// Alice (42) is connected to two map servers (peer1 main, peer2). A
// CREATE with mon_id=0 is assigned a recall id and mirrored to both
// connections (verbatim apart from the patched id); DATA and DEL are
// forwarded verbatim to both.
...
int main()
{
    ...
    // --- CREATE (mon_id=0) → id assigned + mirrored to both -------
    {
        ...
        wire::WritePOD<std::uint32_t>(b, 0xDEADBEEF);  // opaque tail
        SendFramed(p1, ToUint16(MessageId::MW_CREATERECALLMON_ACK), b);
    }
    std::uint32_t mon_seen = 0;
    for (auto* s : {&p1, &p2})
    {
        auto [w, got] = ReadFramed(*s);
        EXPECT(w == ToUint16(MessageId::MW_CREATERECALLMON_REQ));
        wire::Reader r(got);
        std::uint32_t cid = 0, key = 0, mon = 0, tail = 0;
        r.Read(cid); r.Read(key); r.Read(mon); r.Read(tail);
        EXPECT(cid == 42); EXPECT(key == kKey);
        EXPECT(mon != 0);              // world assigned an id
        EXPECT(tail == 0xDEADBEEF);    // opaque tail preserved
        if (mon_seen == 0) mon_seen = mon;
        else EXPECT(mon == mon_seen);  // same id on every connection
    }

    // --- DATA → forwarded verbatim to both ------------------------
    {
        std::vector<std::byte> b;
        wire::WritePOD<std::uint32_t>(b, 42);
        wire::WritePOD<std::uint32_t>(b, kKey);
        wire::WritePOD<std::uint16_t>(b, 0x9ABC);  // opaque tail
        SendFramed(p1, ToUint16(MessageId::MW_RECALLMONDATA_ACK), b);
        for (auto* s : {&p1, &p2})
        {
            auto [w, got] = ReadFramed(*s);
            EXPECT(w == ToUint16(MessageId::MW_RECALLMONDATA_REQ));
            EXPECT(got == b);
        }
    }
    ...
    // --- DEL → forwarded verbatim to both -------------------------
    {
        ...
        wire::WritePOD<std::uint32_t>(b, 42);
        wire::WritePOD<std::uint32_t>(b, kKey);
        wire::WritePOD<std::uint32_t>(b, 123);  // mon_id
        wire::WritePOD<std::uint8_t>(b, 1);     // forever
        SendFramed(p1, ToUint16(MessageId::MW_RECALLMONDEL_ACK), b);
        for (auto* s : {&p1, &p2})
        {
            auto [w, got] = ReadFramed(*s);
            EXPECT(w == ToUint16(MessageId::MW_RECALLMONDEL_REQ));
            EXPECT(got == b);
        }
    }
    ...
    if (g_fails == 0)
        std::printf("PASS test_tworldsvr_asio_recallmon_handlers\n");
    else
        std::printf("FAIL test_tworldsvr_asio_recallmon_handlers "
                    "(%d failure%s)\n", g_fails, g_fails == 1 ? "" : "s");
