...
public:
	...
	typedef std::vector<CTChatTabButton*>	ClientChatTab;
	ClientChatTab	        m_vTabArray;
...
protected:
	...
	//CTChatTabButton*		m_vTabs[255];			///< 탭 배열
	CRect					m_vTabRect[255];		///< 슬롯에 따른 탭의 위치 배열
	...
	*/
	...
	FRAMEDESC_SHAREDPTR m_pChatFrame;
...
public:
	...
	*/
	...
	void AddList(DWORD dwChatFlag );
	void DeleteList(CTChatList *pList);
	TEdit* GetChatEditMenu();
	CTChatMenuNew* GetChatMenu();
	void ClearMenuFocus();
...
public:
	CTChatFrame(TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc, CTClientGame* pMainGame, TCMLParser *pParser);
	virtual ~CTChatFrame();
};
