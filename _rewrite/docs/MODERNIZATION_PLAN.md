...
| E — IOCP → Boost.Asio (`Session.cpp` rewrite) | this commit (Phase 1 only) | Phase 1 done — new `tnetlib::AsioSession` + `tnetlib::AsioListener` classes live alongside legacy `CSession`; C++20 coroutines + Boost.Asio, PCH-free, compiles on both Windows (vcxproj) and Linux (CMake). Echo round-trip test (`tnetlib_asio_echo`) passes on Linux. Does not yet do packet framing or replace CSession — Phase 2 extracts the codec, Phase 3 migrates servers one at a time. |
