...
void CTMapSvrModule::SendMW_CREATESPOLECNIKMON_ACK(DWORD dwCharID,
												DWORD dwKEY,
												DWORD dwMonID,
												WORD wMon,
												DWORD dwATTR,
												...
												BYTE bTargetType)
{
	...
	pMSG->SetID(MW_CREATESPOLECNIKMON_ACK)
		<< dwCharID
		<< dwKEY
		<< dwMonID
		<< wMon
...
void CTMapSvrModule::SendMW_SPOLECNIKMONDEL_ACK(DWORD dwCharID,
												DWORD dwKey,
												DWORD dwMonID,
												BYTE bForever)
{
	...
	pMSG->SetID(MW_SPOLECNIKMONDEL_ACK)
		<< dwCharID
		<< dwKey
                                                 ...
                                                 WORD wACKMsg)
{
	...
	if (wMapID == BOW_MAP_ID || pPlayer->m_bChannel == BR_SERVER_ID)
		return;
										  ...
										  WORD wACKMsg)
{
	...
	MAPSAVEDPLAYER::iterator itSaved = m_mapSAVEDPLAYER.find(pPlayer->m_dwID);
	if(itSaved != m_mapSAVEDPLAYER.end())
	{


		delete (*itSaved).second;
		m_mapSAVEDPLAYER.erase(itSaved);
	}

	CTPlayer* pSavePlayer = new CTPlayer();
	pSavePlayer->m_dwHP = pPlayer->m_dwHP;
	pSavePlayer->m_dwMP = pPlayer->m_dwMP;
	pSavePlayer->m_dwBB = pPlayer->m_dwBB;
	pSavePlayer->m_dwPoints = pPlayer->m_dwPoints;
	pSavePlayer->m_dwTotalPoints = pPlayer->m_dwTotalPoints;
	pSavePlayer->m_bDeaths = pPlayer->m_bDeaths;
	pSavePlayer->m_strGuildName = pPlayer->m_strGuildName;
	pSavePlayer->m_strNAME = pPlayer->m_strNAME;
	pSavePlayer->m_dwGuildID = pPlayer->m_dwGuildID;
	pSavePlayer->m_bGuildCountry = pPlayer->m_bGuildCountry;
#ifdef BR_COMPILE_MODE
	pSavePlayer->m_bClass = pPlayer->m_bClass;
	pSavePlayer->m_dwTeamID = pPlayer->m_dwTeamID;
#endif

	for(MAPTINVEN::iterator itInven = pPlayer->m_mapTINVEN.begin(); itInven != pPlayer->m_mapTINVEN.end(); ++itInven)
	{
		CTInven* pInven = new CTInven();
		pInven->Copy((*itInven).second);

		pSavePlayer->m_mapTINVEN.insert(MAPTINVEN::value_type((*itInven).first, pInven));
	}
	...
	for(MAPTPET::iterator itPET = pPlayer->m_mapTPET.begin(); itPET != pPlayer->m_mapTPET.end(); ++itPET)
	{
		...
		pSavePlayer->m_mapTPET.insert(MAPTPET::value_type((*itPET).first, pPet));
	}
	...
	for(MAPTSADDLE::iterator itSaddle = pPlayer->m_mapSADDLE.begin(); itSaddle != pPlayer->m_mapSADDLE.end(); ++itSaddle)
	{
		...
		pSavePlayer->m_mapSADDLE.insert(MAPTSADDLE::value_type((*itSaddle).first, pSaddle));
	}
	...
	for(MAPTSKILL::iterator itSkill = pPlayer->m_mapTSKILL.begin(); itSkill != pPlayer->m_mapTSKILL.end(); ++itSkill)
	{
		...
		pSavePlayer->m_mapTSKILL.insert(MAPTSKILL::value_type((*itSkill).first, pSkill));
	}

	for(BYTE i = 0; i < pPlayer->m_vMaintainSkill.size(); ++i)
	{
		CTSkill* pMaintainSkill = new CTSkill();
		pMaintainSkill->Copy(pPlayer->m_vMaintainSkill[i]);

		pSavePlayer->m_vMaintainSkill.push_back(pMaintainSkill);
	}

	for(BYTE i = 0; i < pPlayer->m_vRemainSkill.size(); ++i)
	{
		CTSkill* pRemainSkill = new CTSkill();
		pRemainSkill->Copy(pPlayer->m_vRemainSkill[i]);

		pSavePlayer->m_vRemainSkill.push_back(pRemainSkill);
	}

	for(MAPDWORD::iterator itCoolTime = pPlayer->m_mapItemCoolTime.begin(); itCoolTime != pPlayer->m_mapItemCoolTime.end(); ++itCoolTime)
		pSavePlayer->m_mapItemCoolTime.insert(MAPDWORD::value_type((*itCoolTime).first, (*itCoolTime).second));

	m_mapSAVEDPLAYER.insert(MAPSAVEDPLAYER::value_type(pPlayer->m_dwID, pSavePlayer));
	pPlayer->m_dwSaveTick = m_dwTick;
#else
	pPlayer->m_dwSaveTick = m_dwTick;

	if(pPlayer->m_bLoadCharError || pPlayer->IsTournamentPlayer() || pPlayer->m_bLock || pPlayer->m_wMapID == BOW_MAP_ID || pPlayer->m_bChannel == BR_SERVER_ID)
		return;
	...
	for( itQUEST = pPlayer->m_mapQUEST.begin(); itQUEST != pPlayer->m_mapQUEST.end(); itQUEST++)
	{
		...
		if( (*itQUEST).second->m_bSave == TRUE)  
		{
			...
			wQuestCount += 1;
			wTermCount += WORD((*itQUEST).second->m_vTERM.size());			
		}
	}
	...
	{
		pBUF->m_packet
			<< WORD((*itPet).second->m_pTPET->m_bID)
...
<< ( BYTE ) pPlayer->m_mapComp.size();
	...
	for(jirk=pPlayer->m_mapComp.begin(); jirk!=pPlayer->m_mapComp.end(); ++jirk)
	{
		...
		pBUF->m_packet
			...
			<< Companion->m_dwExp
			<< Companion->m_bLevel
			<< Companion->m_bBonusID
			<< Companion->m_bSPoints
			<< Companion->m_bEffect
			<< Companion->m_wSTR
			<< Companion->m_wDEX
			<< Companion->m_wCON
		...
		{
			pBUF->m_packet
				<< Companion->m_wItemID[i]
				<< Companion->m_dEndTime[i];
		}
	}
	...
	BYTE HasSecureCode = pSECURE != nullptr;
	...
	SendDM_CLEARTITLE_REQ(pPlayer->m_dwID);
	SendDM_SAVETITLE_REQ(pPlayer->m_dwID, pPlayer->m_mapTTITLE);
...
{
	if(pPlayer->ProtectTutorial() || pPlayer->m_wMapID == BOW_MAP_ID || pPlayer->m_bChannel == BR_SERVER_ID)
		return;

	if(!pPlayer->m_bMain || pPlayer->IsTournamentPlayer() || !pPlayer->m_bCheckedSession)
		return;
	...
	if(pPlayer->m_mapTINVEN.size() == 2)
	{
		...
		for( itTINVEN = pPlayer->m_mapTINVEN.begin(); itTINVEN != pPlayer->m_mapTINVEN.end(); itTINVEN++)
			nItemCount += (*itTINVEN).second->m_mapTITEM.size();

		MAPPLAYER::iterator itPLAYER = m_mapPLAYER.find(pPlayer->m_dwID);

		if(itPLAYER != m_mapPLAYER.end())
			bFoundChar = true;

		m_pDebugSocket->LogString("Save Item! CharID: %d InvenCount: %d ItemCount: %d FoundPlayer: %d", pPlayer->m_dwID, pPlayer->m_mapTINVEN.size(), nItemCount, bFoundChar);
		//return;
	}
...
void CTMapSvrModule::SendDM_CLEARTITLE_REQ(DWORD dwCharID)
{
	LPPACKETBUF pBUF = new PACKETBUF();

	pBUF->m_packet.SetID(DM_CLEARTITLE_REQ)
		<< dwCharID;

	SayToDB(pBUF);
}

void CTMapSvrModule::SendDM_SAVETITLE_REQ(DWORD dwCharID,
										  MAPTTITLE mapTITLE)
{
	MAPTTITLE::iterator itTITLE;

	for(itTITLE = mapTITLE.begin(); itTITLE != mapTITLE.end(); itTITLE++)
	{
		LPPACKETBUF pBUF = new PACKETBUF();

		pBUF->m_packet.SetID(DM_SAVETITLE_REQ)
			<< dwCharID
...
#ifdef SKYGARDEN
void CTMapSvrModule::SendDM_SKYGARDENOCCUPY_REQ(WORD wCastleID,
											 BYTE bType,
                                             BYTE bCountry)
{
	LPPACKETBUF pBUF = new PACKETBUF();

	pBUF->m_packet.SetID(DM_SKYGARDENOCCUPY_REQ)
		<< wCastleID
		<< bType
		<< bCountry;
											...
											CTPlayer * pPlayer )
{
	...
	CString strCode = NAME_NULL;
	BYTE bSecurityTries = 0;
	DWORD dwSecurityTick = 0;





	BYTE bNewSecurity = FALSE;
...

	{
		...
		bSecurityTries = pSECURE->m_bTries;
		dwSecurityTick = pSECURE->m_dwTick;




		strCode = pSECURE->m_strCode;
	}
	else
		bNewSecurity = TRUE;
	CPacket *pMSG = new CPacket();
	...
	MAPTSADDLE::iterator itSADDLE = pPlayer->m_mapSADDLE.find(pPlayer->m_dwUserID);
	if(itSADDLE != pPlayer->m_mapSADDLE.end())
	{
		(*pMSG)
			<< (*itSADDLE).second->m_wItemID
			<< (*itSADDLE).second->m_dEndTime
			<< (*itSADDLE).second->m_bType;
	}
	else
	{
		(*pMSG)
	...
	(*pMSG)
		<< WORD(pPlayer->m_mapComp.size());

	MAPTCOMP::iterator itCOMP;
	for(itCOMP = pPlayer->m_mapComp.begin(); itCOMP != pPlayer->m_mapComp.end(); itCOMP++)
	{
		(*pMSG)
	<< (*itCOMP).second->m_bSlot
			<< (*itCOMP).second->m_dwMonID
			<< (*itCOMP).second->m_dwExp
			<< (*itCOMP).second->m_dwNextExp
			<< (*itCOMP).second->m_wLife
			<< (*itCOMP).second->m_bSPoints
			<< (*itCOMP).second->m_bLevel
			<< (*itCOMP).second->m_strName
			<< (*itCOMP).second->m_bEffect
			<< (*itCOMP).second->m_wSTR
			<< (*itCOMP).second->m_wDEX
			<< (*itCOMP).second->m_wCON
			<< (*itCOMP).second->m_wINT
			<< (*itCOMP).second->m_wWIS
			<< (*itCOMP).second->m_wMEN
			<< (*itCOMP).second->m_bBonusID;
	...
	for(itCOMP = pPlayer->m_mapComp.begin(); itCOMP != pPlayer->m_mapComp.end(); itCOMP++)
	{
		if((*itCOMP).second->m_wItemID[0] || (*itCOMP).second->m_wItemID[1])
		{
			(*pMSG)
				<< (*itCOMP).second->m_bSlot
				<< (*itCOMP).second->m_dwTick;

			for(BYTE i=0;i<2;i++)
			{
				(*pMSG)
					<< (*itCOMP).second->m_wItemID[i]
					<< (*itCOMP).second->m_dEndTime[i];
			}
...
void CTMapSvrModule::SendMW_GUILDLOCALLIST_REQ(DWORD dwCharID,
	DWORD dwKEY)
{
	CPacket * pMsg = new CPacket();
	pMsg->SetID(MW_GUILDLOCALLIST_REQ)
		<< dwCharID
		<< dwKEY;
...
#ifdef SKYGARDEN
void CTMapSvrModule::SendMW_SKYGARDENOCCUPY_ACK(BYTE bType,
											 WORD wID,
											 BYTE bCountry)
{
	CPacket * pMsg = new CPacket();
	pMsg->SetID(MW_SKYGARDENOCCUPY_ACK)
		<< bType
		<< wID
		<< bCountry;
...
#ifdef SKYGARDEN
void CTMapSvrModule::SendMW_SKYGARDENENABLE_ACK(BYTE bStatus)
{
	CPacket * pMsg = new CPacket();
	pMsg->SetID(MW_SKYGARDENENABLE_ACK)
		<< bStatus;
...
void CTMapSvrModule::SendMW_MONTEMPTEVO_ACK(DWORD dwAttacker,
											DWORD dwHostID,
											BYTE bHostType)
{
	CPacket * pMsg = new CPacket();
	pMsg->SetID(MW_MONTEMPTEVO_ACK)
		<< dwAttacker
		<< dwHostID
		<< bHostType;
...
void CTMapSvrModule::SendMW_ADDTOBOWQUEUE_REQ(DWORD dwCharID, DWORD dwKEY)
{
	CPacket * pPacket = new CPacket();
	pPacket->SetID(MW_ADDTOBOWQUEUE_REQ)
		<< dwCharID
		<< dwKEY;
...
void CTMapSvrModule::SendMW_ADDTOBRQUEUE_REQ(DWORD dwCharID, DWORD dwKEY, BYTE bOnlyReady)
{
	CPacket * pPacket = new CPacket();
	pPacket->SetID(MW_ADDTOBRQUEUE_REQ)
		<< dwCharID
		<< dwKEY
		<< bOnlyReady;
...
void CTMapSvrModule::SendMW_BRTEAMMATEADD_REQ(DWORD dwCharID, DWORD dwKEY, CString strNAME)
{
	CPacket * pPacket = new CPacket();
	pPacket->SetID(MW_BRTEAMMATEADD_REQ)
		<< dwCharID
		<< dwKEY
		<< strNAME;
...
void CTMapSvrModule::SendMW_BRTEAMMATEDEL_REQ(DWORD dwCharID, DWORD dwKEY, CString strNAME)
{
	CPacket * pPacket = new CPacket();
	pPacket->SetID(MW_BRTEAMMATEDEL_REQ)
		<< dwCharID
		<< dwKEY
		<< strNAME;
...
void CTMapSvrModule::SendMW_BRTEAMMATEADDRESULT_ACK(DWORD dwCharID, DWORD dwKEY, BYTE bResult, CString strNAME)
{
	CPacket * pPacket = new CPacket();
	pPacket->SetID(MW_BRTEAMMATEADDRESULT_ACK)
		<< dwCharID
		<< dwKEY
		<< bResult
		<< strNAME;
...
void CTMapSvrModule::SendMW_VOTEFORBRMAP_REQ(DWORD dwCharID, DWORD dwKEY, CString strMap, BYTE Mode)
{
	CPacket * pPacket = new CPacket();
	pPacket->SetID(MW_VOTEFORBRMAP_REQ)
		<< dwCharID
		<< dwKEY
...
void CTMapSvrModule::SendMW_CANCELBOWQUEUE_REQ(DWORD dwCharID, DWORD dwKEY)
{
	CPacket * pPacket = new CPacket();
	pPacket->SetID(MW_CANCELBOWQUEUE_REQ)
		<< dwCharID
		<< dwKEY;
...
void CTMapSvrModule::SendMW_BATTLEMODESTATUS_REQ(DWORD dwCharID, DWORD dwKEY)
{
	CPacket * pPacket = new CPacket();
	pPacket->SetID(MW_BATTLEMODESTATUS_REQ)
		<< dwCharID
		<< dwKEY;
...
void CTMapSvrModule::SendMW_CMTELEPORTBATTLEMODE_REQ(DWORD dwCharID, DWORD dwKEY, BYTE bType)
{
	CPacket * pPacket = new CPacket();
	pPacket->SetID(MW_CMTELEPORTBATTLEMODE_REQ)
		<< dwCharID
		<< dwKEY
		<< bType;
...
void CTMapSvrModule::SendMW_BOWPOINTSUPDATE_REQ(BYTE bCountry)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_BOWPOINTSUPDATE_REQ)
		<< bCountry;
	...
	{
	case TCONTRY_D:
		strMSG = "Valorian has destroyed the Derion Divine Statue.";
		break;
	case TCONTRY_C:
		strMSG = "Derion has destroyed the Valorian Divine Statue.";
		break;
	}
	
	_AtlModule.BOWNotify(strMSG);
	m_world.Say(pMsg);
}

void CTMapSvrModule::SendMW_LEAVEBATTLEFIELD_REQ(DWORD dwCharID, DWORD dwKEY)
{
	CPacket* pMsg = new CPacket();
	pMsg->SetID(MW_LEAVEBATTLEFIELD_REQ)
		<< dwCharID
		<< dwKEY;
...
void CTMapSvrModule::SendMW_GUILDSKILLACTION_REQ(DWORD dwCharID, DWORD dwKEY, BYTE Action, WORD Param[])
{
	BYTE Size = Action == GS_USESKILL ? 5 : BYTE(sizeof(Param) / sizeof(WORD));

	CPacket * pPacket = new CPacket();
	pPacket->SetID(MW_GUILDSKILLACTION_REQ)
		<< dwCharID
		<< dwKEY
		<< Action
		<< Size;
...
void CTMapSvrModule::SendMW_UPDATEGUILDCOOLDOWN_ACK(DWORD dwGuildID, WORD wSkillID, BYTE bLevel, BOOL DidRenew, BOOL DidUse)
{
	CPacket * pPacket = new CPacket();
	pPacket->SetID(MW_UPDATEGUILDCOOLDOWN_ACK)
		<< dwGuildID
		<< wSkillID
		<< bLevel
		<< DidRenew
