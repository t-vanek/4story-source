// W4-12 wire test: TMS presence on logout (legacy LeaveTMS).
//
// Alice 42/peer1 0x42, Bob 200/peer2 0x43 share a conference.
// When Alice's map sends MW_CLOSECHAR_ACK, world drops her from the
// conference and tells the surviving member (Bob) via MW_TMSOUT_REQ
// with Alice as the departed member.
...
int main()
{
    ...
    // Open a conference {Alice, Bob} via INVITE; drain both rosters.
    std::uint32_t tms_id = 0;
    {
        std::vector<std::byte> b;
        tworldsvr::wire::WritePOD<std::uint32_t>(b, 42);
        tworldsvr::wire::WritePOD<std::uint32_t>(b, kAliceKey);
        tworldsvr::wire::WritePOD<std::uint32_t>(b, 0);
        tworldsvr::wire::WritePOD<std::uint8_t>(b, 1);
    ...
    }
    {
        ...
        std::uint32_t cid = 0, key = 0, inv = 0, t = 0;
        r.Read(cid); r.Read(key); r.Read(inv); r.Read(t);
        tms_id = t;
        EXPECT(tms_id != 0);
    }
    { auto [w, _] = ReadFramed(p2);
    ...
    // Alice logs out → Bob is told she left the conference.
    {
        std::vector<std::byte> b;
        tworldsvr::wire::WritePOD<std::uint32_t>(b, 42);
        tworldsvr::wire::WritePOD<std::uint32_t>(b, kAliceKey);
        SendFramed(p1, ToUint16(MessageId::MW_CLOSECHAR_ACK), b);
    }
    {
        ...
        EXPECT(cid == 200); EXPECT(t == tms_id); EXPECT(target == "Alice");
    }
    ...
    if (g_fails == 0)
        std::printf("PASS test_tworldsvr_asio_tms_logout_handlers\n");
    else
        std::printf("FAIL test_tworldsvr_asio_tms_logout_handlers "
                    "(%d failure%s)\n", g_fails, g_fails == 1 ? "" : "s");
