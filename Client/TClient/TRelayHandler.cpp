...
int CTClientGame::OnCS_FORWARDBOWCHAT_ACK( CTachyonSession *pSession, CPacket *pPacket)
{ 
	CTTextLinker* pTextLinker = CTTextLinker::GetInstance();
	...
	if( m_vTOPTION.m_bDenyWhisper &&
		dwSenderID != m_pMainChar->m_dwID )
		return TERR_NONE;

	DWORD dwSoundID = ID_SND_INFO;
	BYTE bSoundType = MEDIA_TSOUND;
	DWORD dwChatFlag = TCHAT_FLAG_WHISPER;
	CString strTYPE = CTChatFrame::GetChatTypeString(CHAT_WHISPER);
	DWORD dwColor = 0xFFFFFFA0;
