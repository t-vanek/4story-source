...
{
	// Bug fix: TLoginSvr was the only server process not calling srand()
	// at startup — TMapSvr / TWorldSvr / TPatchSvr all do via
	// `srand(rd())` from std::random_device. Without it, the C library's
	// default `rand()` state is identical across restarts, so every
	// TRand() call (including the per-session m_dlCheckKey generation
	// in OnCS_LOGIN_REQ) produced the same sequence after every server
	// restart. The dlCheckKey is wire-visible anyway (sent in
	// CS_LOGIN_ACK) so this wasn't a secret-leak, but it broke
	// expectation that successive sessions got "different" values.
	{
