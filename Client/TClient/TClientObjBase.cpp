...
CTClientObjBase::CTClientObjBase()
{
	...
	m_dwLastFlashColor = TFLASHCOLOR_WHITE;
	m_wSFXCostume = 0;
}
				...
				while (pTMAINTAIN->m_dwTIMMER >= TDOT_TICK)
				{
						...
						pTGAME->CanDefend(pTATTACK, this, pTMAINTAIN->m_pTIMMER))
					{
						...
						for (BYTE i = 0; i < SKILL_EXCEPTIONS_COUNT; ++i)
							if (CTClientGame::m_wSkillSystemExceptions[i] == pTMAINTAIN->m_pTIMMER->m_wSkillID)
							{
								bFound = TRUE;
								break;
							}
						...
						if ((pTATTACK->m_bType == OT_PC || pTATTACK->m_bType == OT_RECALL || pTATTACK->m_bType == OT_SELF) && !bFound)
						{
							...
							SKILLTARGET.m_dwID = m_dwID;
							SKILLTARGET.m_bType = m_bType;
							vSKILLTARGET.push_back(&SKILLTARGET);

							pTGAME->FinishSkill(pTATTACK, &pTMAINTAIN->m_vTSKILLDATA.m_vTGROUND, pTMAINTAIN->m_pTIMMER, vSKILLTARGET);
	...
	for (int i = 0; i<INT(pTGAME->m_vTMAINTAINOBJ.size()); i++)
	{
				...
				pTMAINTAINOBJ->m_bAttackType != m_bType))
		{
				...
				(!bBroadCast && finder == m_mapTMAINTAIN.end()))
			{
				...
				{
					if (pTGAME->IsInBRMap())
						if (!pTGAME->IsInMyBRTeam(pTATTACK->m_dwID))
							continue;
...
{
	// CTClientObjBase *pTOBJ ÀÚ±âÀÚ½Å
	// pTOBJ->m_mapTMAINTAINÀ» µ¹·Á¸é ½ºÅ³ÀÇ FunctionÀÌ SDT_STATUS_MOVESPEEDÀÎ °ÍµéÀ» Ã£¾Æ¼­
	// ±×°Íµé Áß¿¡ °¡Àå °ªÀÌ ³·Àº(¼Óµµ°¡ °¡Àå ³·Àº) ¾Ö¸¦ ÇöÀç pTOBJÀÇ ½ºÇÇµå·Î ÁöÁ¤ÇÑ´Ù.
	...
	for (itMAINTAIN = m_mapTMAINTAIN.begin(); itMAINTAIN != m_mapTMAINTAIN.end(); itMAINTAIN++)
	{
		...
		for (int i = 0; i<INT(pTSKILL->m_vTFUNCTION.size()); i++)
		{
				...
				pTSKILLFUNC->m_bFunctionID == SDT_STATUS_MOVESPEED)
			{
				...
				/*if(pTSKILL->m_wSkillID = 1218 && IsRide())
				fSpeedFactor += 0.5f;*/
...
void CTClientObjBase::DoSkillFunc(LPTSKILL pTSKILL,
	...
	CTClientGame* pTGAME)
{
	// CTClientObjBase *pDEFEND ÀÚ±âÀÚ½Å
	...
	for (int i = 0; i<INT(pTSKILL->m_vTFUNCTION.size()); i++)
	{
		...
		case SDT_STATUS:
		{
			...
			case SDT_STATUS_SLIDE:
				if (m_bType == OT_SELF) // ÀÇµµ:Æ÷Å¾Àº ¹Ð¸®Áö ¾Ê´Â´Ù.
					break;

				if (m_bType == OT_MON) // ³Ë¹é±ÝÁöµÈ ¸ó½ºÅÍ´Â ³Ë¹é Åë°ú.
				{
	...
	CTachyonRes *pRES)
{
	...
	BOOL bCloakHide = (m_bHelmetHide == ALL_HIDDEN || m_bHelmetHide != CLOAK_SHOWN) && m_bHelmetHide != ALL_SHOWN ? FALSE : TRUE;
	BYTE bCapeMode = pTEQUIP && pTEQUIP->FindTItem(ES_BACK) && bCloakHide ? TRUE : FALSE;
	...
	for (BYTE i = 0; i<TWEAPON_COUNT; i++)
	{
		...
		if (pTITEM[i])
		{
			...
			LPTITEM pTEMPITEM = CTChart::FindTITEMTEMP(pTITEM[i]->GetMoggItemID());

			if (pTITEM[i]->GetMoggItemID() && pTEMPITEM)
			{
				LPTITEMVISUAL pTEMPVISUAL = CTChart::FindTITEMVISUAL(pTEMPITEM->m_wVisual[0]);
				pTVISUAL = pTEMPVISUAL;
			}
			...
			case IK_BACK	:
				{
					...
					if (pTITEM[i]->GetCustomTex() || CTClientGame::GetInstance()->m_wCloakPreview)
					{
						bMode = MT_BATTLE;
						pTVISUAL = CTChart::FindTITEMVISUAL(CTChart::FindTITEMTEMP(7001)->m_wVisual[0]);
					}
	...
	for (INT i = 0; i < TWEAPON_COUNT; ++i)
	{
		...
		LPTITEMVISUAL pTVISUAL = pApplyGradeList[i]->GetVisual();
		LPTITEM pTEMPITEM = CTChart::FindTITEMTEMP(pApplyGradeList[i]->GetMoggItemID());

		if (pApplyGradeList[i]->GetMoggItemID() && pTEMPITEM)
		{
			LPTITEMVISUAL pTEMPVISUAL = CTChart::FindTITEMVISUAL(pTEMPITEM->m_wVisual[0]);
			pTVISUAL = pTEMPVISUAL;
		}
		...
		DWORD dwFuncID = pTVISUAL->m_dwEffectFuncID[bMode];
		...
		if (pTEQUIP)
		{
			...
			if (itID != m_OBJ.m_pOBJ->m_mapPIVOT.end())
			{
				...
				if (pTWEAPON)
				{
					...
					LPTITEM pTEMPITEM = CTChart::FindTITEMTEMP(pTWEAPON->GetMoggItemID());

					if (pTWEAPON->GetMoggItemID() && pTEMPITEM)
					{
						LPTITEMVISUAL pTEMPVISUAL = CTChart::FindTITEMVISUAL(pTEMPITEM->m_wVisual[0]);
						pTVISUAL = pTEMPVISUAL;
					}
	...
	CTachyonRes *pRES)
{
	...
	for (auto i = 0; i<TREPLACE_COUNT; i++)
	{
		...
		BOOL bHelmetHide = (m_bHelmetHide == ALL_HIDDEN || m_bHelmetHide != HELMET_SHOWN) && m_bHelmetHide != ALL_SHOWN ? FALSE : TRUE;
		BOOL bCloakHide = (m_bHelmetHide == ALL_HIDDEN || m_bHelmetHide != CLOAK_SHOWN) && m_bHelmetHide != ALL_SHOWN ? FALSE : TRUE;

		if (!IsDisguise() &&
			((m_bType != OT_PC || bHelmetHide) || (vREPLACE[i] != ES_HEAD && vREPLACE[i] != ES_FACE)) &&
			(bCloakHide || vREPLACE[i] != ES_BACK))
		{
			...
			if (pTITEM)
			{
				...
				LPTITEM pTEMPITEM = CTChart::FindTITEMTEMP(pTITEM->GetMoggItemID());

				if (pTITEM->GetMoggItemID() && pTEMPITEM)
				{
					LPTITEMVISUAL pTEMPVISUAL = CTChart::FindTITEMVISUAL(pTEMPITEM->m_wVisual[0]);
					pTVISUAL = pTEMPVISUAL;
				}

				if (vREPLACE[i] == ES_BACK && (pTITEM->GetCustomTex() || CTClientGame::GetInstance()->m_wCloakPreview))
					pTVISUAL = CTChart::FindTITEMVISUAL(CTChart::FindTITEMTEMP(7001)->m_wVisual[0]);
				...
				if (pTVISUAL)
				{
					...
					pTApplyGrade[i] = pTITEM; // ¹Ø¿¡¼­ Item Grade VisualÀ» Àû¿ëÇÏ±â À§ÇØ ´ë»ó ¼öÁý.
					pTApplyGradeMode[i] = bMode;
				}
			}
		}
		...
		if (bCostumeHide[i])
		{
			ClearDEFEQUIP(pDevice, vREPLACE[i]); // ÇØ´ç ºÎÀ§¸¦ ¾ø¾ÖÁÖ´Â ÄÚµå Ãß°¡
			continue;
		}
	...
	ApplyGradeEffect(pTApplyGrade, TREPLACE_COUNT); // ¹æ¾î±¸¿¡ °­È­ºñÁÖ¾ó Àû¿ë
	...
	if (m_bType == OT_PC)
		dynamic_cast<CTClientChar*>(this)->ApplyMantleTexture();
}
...
void CTClientObjBase::ResetOBJPart(CD3DDevice *pDevice)
{
	...
	if (pTEQUIP)
	{
		...
		for (BYTE i = 0; i<ES_COUNT; i++)
		{
			...
			if (IsDisguise() || (!bHelmetHide && (i == ES_HEAD || i == ES_FACE)) || (dwCostumeHide & (1 << i)))
				continue;
			...
			if (CheckOBJPart(&vTHIDE))
			{
				...
				if (pHIDE && pHIDE->GetTITEM())
				{
					...
					LPTITEM pTEMPITEM = CTChart::FindTITEMTEMP(pHIDE->GetMoggItemID());

					if (pHIDE->GetMoggItemID() && pTEMPITEM)
					{
						LPTITEMVISUAL pTEMPVISUAL = CTChart::FindTITEMVISUAL(pTEMPITEM->m_wVisual[0]);
						pTVISUAL = pTEMPVISUAL;
					}
					...
					if (i == ES_BACK && (pHIDE->GetCustomTex() || CTClientGame::GetInstance()->m_wCloakPreview))
						pTVISUAL = CTChart::FindTITEMVISUAL(CTChart::FindTITEMTEMP(7001)->m_wVisual[0]);
	...
	WORD wFromSkillID)
{
	if (m_bRootID)
		if (m_dwTSUBACT != dwActionID || m_vTSUBACT.m_dwCurANI != dwAniID)
		{
			...
			if (itACT != m_OBJ.m_pOBJ->m_mapACTION.end())
			{
				...
				(*itANI).second->m_pANI->m_bFinished = FALSE;
				(*itANI).second->m_pANI->m_FromSkill = FromSkill;
				if (FromSkill)
					(*itANI).second->m_pANI->m_wFromSkillID = wFromSkillID;
				...
				if (itANI != pACT->m_mapANI.end())
				{
					...
					m_vTSUBACT.m_pANI->m_pANI->m_bFinished = FALSE;
					m_vTSUBACT.m_pANI->m_pANI->m_FromSkill = FromSkill;
					if (FromSkill)
						m_vTSUBACT.m_pANI->m_pANI->m_wFromSkillID = wFromSkillID;
	...
	if (FromSkill)
		m_bSubActionLock = dwActionID ? TRUE : FALSE;
...
void CTClientObjBase::ResetVisible(CTClientCAM *pCamera,
	...
	DWORD dwTick)
{
	if (CTClientGame::IsInBRMap() && CTClientGame::GetInstance()->m_bBRStatus == BS_PEACE && (m_bType == OT_PC || m_bType == OT_COMPANION || m_bType == OT_RECALL))
	{
		if (CTClientGame::GetInstance()->IsInMyBRTeam(GetHostID()))
			m_bVisible = TRUE;
		else
		{
			m_bVisible = FALSE;
		...
		dwHouseID == m_dwHouseID || ((!dwHouseID || bHouseMesh) && (!m_dwHouseID || m_bHouseMesh))))
	{
		...
		if (((CTClientGame::GetInstance()->GetClientMap()->m_wMapID / 100) == 5)) // 500¹ø´ë ¸Ê
...
LPOBJTEX CTClientObjBase::NewCloakTexture(LPOBJTEX pTEX,
	LPTEXTURESET pSkinTEX,
	WORD wTextureID)
{

	CTClientGame* pTGAME = CTClientGame::GetInstance();
	if (pTGAME->GetCustomTexture(wTextureID) != NULL)
	{
		LPTEXTURESET pTextureSet = pTGAME->GetCustomTexture(wTextureID);
		LPOBJTEX pNewTEX = new OBJTEX;
		*pNewTEX = *pTEX;

		pNewTEX->m_pTEX[0] = pTGAME->GetCustomTexture(wTextureID);
		pNewTEX->m_dwOP = D3DTOP_FORCE_DWORD;
		pNewTEX->m_bType[0] = TT_TEX;
		pNewTEX->m_dwCOLOR = dwTCOLOR[TCOLOR_NONE];
	...
	if (pTACTION)
	{
		if (!m_bShoot && pTACTION->m_bEquipMode == TEM_LONG)
		{
			...
			if (pTEQUIP)
			{
				...
				LPTITEM pMOGGITEM = NULL;
				if (pWEAPON && pWEAPON->GetMoggItemID())
					pMOGGITEM = CTChart::FindTITEMTEMP(pWEAPON->GetMoggItemID());

				if (pWEAPON)
					pTACTION = CTChart::FindTACTIONTEMP(bActionID, pMOGGITEM ? pMOGGITEM->m_bWeaponID : pWEAPON->GetTITEM()->m_bWeaponID);
			}
		}
	}
...
BYTE CTClientObjBase::GetWeaponID(BYTE bMode)
{
	...
	if (m_bShoot)
	{
		...
		if (pWEAPON)
		{
			if (pWEAPON->GetMoggItemID() && CTChart::FindTITEMTEMP(pWEAPON->GetMoggItemID()))
				return CTChart::FindTITEMTEMP(pWEAPON->GetMoggItemID())->m_bWeaponID;
	...
	else
	{
		...
		LPTITEM pRIGHTITEM = pRHAND ? pRHAND->GetMoggItemID() ? CTChart::FindTITEMTEMP(pRHAND->GetMoggItemID()) : pRHAND->GetTITEM() : NULL;
		LPTITEM pLEFTITEM = pLHAND ? pLHAND->GetMoggItemID() ? CTChart::FindTITEMTEMP(pLHAND->GetMoggItemID()) : pLHAND->GetTITEM() : NULL;

		if (pRIGHTITEM && pRIGHTITEM->m_bWeaponID == WT_1HAND && pRIGHTITEM->m_bSubSlotID == ES_SNDWEAPON && !pLEFTITEM)
			pLEFTITEM = pRIGHTITEM;

		if (pLEFTITEM && (pLEFTITEM->m_bWeaponID == WT_1HAND || pLEFTITEM->m_bWeaponID == WT_LHAND))
			return WT_LHAND;
		else if (pRIGHTITEM)
			return pRIGHTITEM->m_bWeaponID;
	}
	...
	for (int i = 0; i<6; i++)
		for (int j = 0; j<2; j++)
		{
			...
			for (int k = 0; k<3; k++)
			{
				/*!
				*	@brief ±Ç¼ºÇõ
				*		pOBJ->m_vPositionÀ» ..m_vWorld·Î ¹Ù²Þ
				*		m_vPositionÀº scaleµµ Àû¿ëµÈ »óÅÂ¶ó¼­
				*		¹Ú½ºÀÇ ½ÇÁ¦ size¿¡ scaleÀÌ ÇÑ¹ø´õ Àû¿ëµÇ¼­ Ä¿Áü.
				*/
				CTMath::Transform(
					&m_vWorld,
					&vPOLY[i][j][k]);
	...
	CTClientItem* pTITEM)
{
	...
	if (pGradeVISUAL && pGradeVISUAL->m_pSkinTex)
	{
		...
		return; // ÅØ½ºÃ³È¿°ú°¡ ÀÖÀ¸¸é ÄÃ·¯È¿°ú´Â ¹«½Ã.
	}
...
void CTClientObjBase::ApplyGradeEffect(CTClientItem** pApplyGradeList, INT nCount)
{
	for (INT i = 0; i < nCount; ++i)
	{
		...
		if (pTITEM->GetTITEM()->m_strNAME == "Hat (Premium)")
		{
			LPTITEMVISUAL pTVISUAL = pTITEM->GetVisual();
			LPTITEMGRADEVISUAL pGradeVISUAL = pTITEM->GetGradeVISUAL();
			...
			if (pTVISUAL &&
				pTVISUAL->m_dwCLKID)
			{
				LPTEXTURESET pCUSTOMTEX = CTClientGame::GetInstance()->GetCustomTexture(23);
				if (!pCUSTOMTEX)
					continue;
				...
				for (; itDRAW != endDRAW; ++itDRAW)
				{
					...
					for (; itPART != endPART; ++itPART)
					{
						...
						if (pPART &&
							pPART->m_pMESH == pMESH)
						{
							pPART->m_pTEX = CTClientObjBase::NewCloakTexture(
								pPART->m_pTEX,
								pCUSTOMTEX,
								23);
						}
...
BYTE CTClientObjBase::IsAlliance(CTClientObjBase *pTARGET)
{
	if (CTClientGame::IsInBRMap())
	{
		...
		return CTClientGame::GetInstance()->IsInMyBRTeam(pTARGET->m_dwID);
	}
	...
	if (pANI && pANI->m_pANI)
	{
		...
		if (!pANI->m_pANI->m_vKey.empty())
		{
			...
			while (m_nSubKeyBase < pANI->m_pANI->m_nKeyBase)
			{
				...
				{
					/*
					if (pANI->m_pANI->m_bFinished && pANI->m_pANI->m_FromSkill)
					{
					CTClientGame::DebugMSG("Skill ended early. If this made any problems with the skills, please report this.");
					m_bSubActionLock = FALSE;
					return;
					}
					*/
					if (m_nSubKeyBase < pANI->m_pANI->m_nKeyBase)
					{
						for (int i = m_nSubKeyBase + 1; i < pANI->m_pANI->m_nKeyBase; ++i)
							if (pANI->m_pANI->m_vKey[i]->m_dwEventID)
							{
								MoreAnimations = TRUE;
								break;
							}
					...
					if (!MoreAnimations && pANI->m_pANI->m_FromSkill)
						pANI->m_pANI->m_bFinished = TRUE;
		...
		if (!pANI->m_pANI->m_vKey.empty())
		{
			...
			while (m_nKeyBase < pANI->m_pANI->m_nKeyBase)
			{
				...
				if (pANI->m_pANI->m_vKey[m_nKeyBase]->m_dwEventID > 0)
				{
					...
					*/
					if (m_nKeyBase < pANI->m_pANI->m_nKeyBase)
					{
						for (int i = m_nKeyBase + 1; i < pANI->m_pANI->m_nKeyBase; ++i)
							if (pANI->m_pANI->m_vKey[i]->m_dwEventID)
							{
								MoreAnimations = TRUE;
								break;
	...
	DWORD dwTick)
{
	...
	FLOAT fLavaHeight = pTMAP->GetLavaHeight(
		m_vPosition._41,
		m_vPosition._43) - m_fBreathHeight;
	...
	if (IsPush())
	{
		...
		if (!IsFlyOBJ() || (m_bSwim && fHeight > fWaterHeight) || (m_bSwimLava && fHeight > fLavaHeight) || fTime < m_fPushSpeed / (TPUSH_BUST * CTClientMAP::m_fGravity))
			bResult = TRUE;
	}
	else if (IsFall())
	{
		...
		if (!IsFlyOBJ() || ((m_bSwim && fHeight > fWaterHeight) || (m_bSwimLava && fHeight > fLavaHeight)))
			bResult = TRUE;
	}
	else if (IsJump())
	{
		...
		if (!IsFlyOBJ() || ((m_bSwim && fHeight > fWaterHeight) || (m_bSwimLava && fHeight > fLavaHeight)) || fTime < m_fJumpSpeed / CTClientMAP::m_fGravity)
			bResult = TRUE;
	}
	...
	if (pANI && pANI->m_pANI)
	{
		...
		if (m_fSubActTime >= fTotal)
		{
			...
			while (fTime >= fTotal)
			{
				m_fSubActTime = fTotal;
				m_fKeyTime = m_fSubActTime;
				if (pANI->m_pANI->m_FromSkill)
					m_bSubActionLock = TRUE;
...
void CTClientObjBase::CalcSUBFrame()
{
	...
	{
		pANI->m_pANI->m_fCurTime = m_fSubActTime;
		pANI->m_pANI->m_fKeyTime = m_fKeyTime;

		if (pANI->m_pANI->m_FromSkill)
		{
			if (pANI->m_pANI->m_wFromSkillID == 37)
				pANI->m_pANI->m_fKeyTime *= 2.5f;

			FLOAT fAccel = (FLOAT)CTClientGame::GetInstance()->m_dwEstPing / 35.0f;
			if (fAccel > 1.0f)
				pANI->m_pANI->m_fKeyTime *= fAccel;
		}
		else
			pANI->m_pANI->m_fKeyTime = 0.0f;
	...
	DWORD dwCustomColor)
{
	...
	ClearTActionSFX(&CTClientGame::GetInstance()->GetMainChar()->m_vTACTIONSFX);

	static MAPRES::iterator itBegin = pGame->GetResource()->m_mapSFX.find(287879416);
	LPSFX pSFX = (LPSFX) (*itBegin).second;

	CString x;
	x.Format("ID : %d", (*itBegin).first);
	pGame->DebugMSG(x);
	...
	if (itBegin != pGame->GetResource()->m_mapSFX.begin())
	--itBegin;

	return;
	*/

	LPTSFXTEMP pTEMP = CTChart::FindTSFXTEMP(dwFuncID == ICE_WINGS_ID ? 1015 : dwFuncID);
	...
	if (pTEMP->m_dwPosID)
	{
		...
		if (dwFuncID != ICE_WINGS_ID)
		{
			for (it = m_mapOBJSFXINST.begin(); it != m_mapOBJSFXINST.end(); it++)
				if ((*it).second->m_dwFuncID == pTEMP->m_dwPosID)
				{
					...
					pItem->m_pSFX->m_vROT = (*it).second->m_vROT; //TEST
		...
		{
			for (it = m_mapOBJSFXINST.begin(); it != m_mapOBJSFXINST.end(); it++)
				if ((*it).second->m_dwFuncID == ICE_WINGS_ID)
				{
					CTClientSFX *pTSFX = new CTClientSFX();
					LPOBJSFX pItem = new OBJSFX();
...
void CTClientObjBase::PlayRawSFX(LPVECTOROBJSFX pTACTIONSFX,
	DWORD dwSfxID,
	BYTE bUseOBJScale,
	D3DXVECTOR3 vPOS,
	D3DXVECTOR3 vROT,
	DWORD dwCustomColor)
{
	...
	MAPRES::iterator itSFX = pRES->m_mapSFX.find(dwSfxID);
	if (itSFX == pRES->m_mapSFX.end())
		return;

	LPSFX pSFX = (LPSFX)(*itSFX).second;
	CTClientSFX *pTSFX = new CTClientSFX();
	LPOBJSFX pItem = new OBJSFX();
	pItem->m_pUserData = pTSFX;

	pItem->m_pSFX = new SFXINST();
	pItem->m_bUseOBJScale = bUseOBJScale;
	pItem->m_bDeleteINST = TRUE;
	...
	pItem->m_pSFX->m_vPOS = vPOS;
	pItem->m_pSFX->m_vROT = vROT;
	pItem->m_pSFX->m_pSFX = pSFX;
...
{
	return m_bSwim || m_bSwimLava || (m_pTRANS && m_pTRANS->m_bCanFly) ? TRUE : FALSE;
}
...
{
	return m_bType != OT_COMPANION && (m_bDynamicOBJ && CTClientGame::GetObjSHADOWOption()) ? TRUE : FALSE;
}
...
void CTClientObjBase::ApplyCloakTexture(CTClientItem* pCloak)
{
	CTClientGame* pGame = CTClientGame::GetInstance();
	if (!pCloak)
		return;
		...
		for (; itDRAW != endDRAW; ++itDRAW)
		{
			...
			for (; itPART != endPART; ++itPART)
			{
					...
					pPART->m_pMESH == pMESH)
				{
					...
					pTEX = CTClientObjBase::NewCloakTexture(
						pPART->m_pTEX,
						pSkinTEX,
						wTextureID);
...
void CTClientObjBase::ApplyCharacterSFX(CTClientItem* pItem)
{
	CTClientInven *pTEQUIP = FindTInven(m_pTRANS ? INVEN_TRANS : INVEN_EQUIP);
	CTClientGame* pTGAME = CTClientGame::GetInstance();

	BYTE bSFX = pTEQUIP && pTEQUIP->FindTItem(ES_COSTUME_ACCESSORY) ? TRUE : FALSE;

	if (bSFX && pItem && pItem->GetTITEM()->m_wUseValue && !m_bGhost)
	{
		DWORD dwSFX = pItem->GetTITEM()->m_wUseValue;
		DWORD dwColor = 0x00000000;
		BOOL RawSFX = FALSE;

		if (pItem->GetItemID() == GREEN_CHAKRA)
			dwColor = 0xFF1AFF1A;
		else if (pItem->GetItemID() == RED_CHAKRA)
			dwColor = 0xFFFF1A1A;
		else if (pItem->GetItemID() == WHITE_WINGS_ITEM_ID)
			dwColor = 1;
		else if (pItem->GetItemID() == 25059) // NEON BLUE CHAKRA
			dwColor = 0xFFae22ab;
		else if (pItem->GetItemID() == 25060) // NEON VIOLET CHAKRA
			dwColor = 0xFF0b2a6b;
		else if (pItem->GetItemID() == 25061) // ENHANCED VIOLET
			dwColor = 0xFF6e1e91;
		else if (pItem->GetItemID() >= 25050)
		{
			switch (pItem->GetItemID())
			{
			case 25050:
				dwSFX = 285971260; // Violet Chakra
				break;
			case 25051:
				//dwSFX = 285971200; // Blue Aura
				//dwSFX = 999236343; //Spinning circle
				//dwSFX = 285971260; //Purple something
				//dwSFX = 287879900; //Spinning for the win
				//dwSFX = 285971272; //Red flame
				//dwSFX = 285971200; //Spinning blue circle
				dwSFX = 285971200;
				break;
			case 25052:
				dwSFX = 285971411; // Dark Aura
				break;
			case 25053:
				dwSFX = 287879787;  // Sacred Aura
				break;
			default:
				return;
			}
		...
		if (pItem->GetItemID() == ICE_WINGS_ITEM_ID || pItem->GetItemID() == BLUE_WINGS_ID)
		{
			dwSFX = ICE_WINGS_ID;
			if (pItem->GetItemID() == BLUE_WINGS_ID)
				dwColor = 1;
		}

		if (m_dwID == pTGAME->GetMainChar()->m_dwID && m_bHide)
		{
			ClearTActionSFX(&m_vTGARBAGESFX);

			if (!RawSFX)
				pTGAME->GetMainChar()->PlayTActionSFX(&m_vTGARBAGESFX, dwSFX, TRUE, dwColor);
			else
				pTGAME->GetMainChar()->PlayRawSFX(&m_vTGARBAGESFX, dwSFX, TRUE);
		}
		else if (!m_bHide)
		{
			pTGAME->GetMainChar()->ClearTActionSFX(&m_vTGARBAGESFX);
			if (!RawSFX)
				PlayTActionSFX(&m_vTGARBAGESFX, dwSFX, TRUE, dwColor);
			else
				PlayRawSFX(&m_vTGARBAGESFX, dwSFX, TRUE);
		}
	}
	else
		ClearTActionSFX(&m_vTGARBAGESFX);
...
BYTE CTClientObjBase::IsFlashed()
{
	MAPTMAINTAIN::iterator itMAINTAIN;
	...
	for (itMAINTAIN = m_mapTMAINTAIN.begin(); itMAINTAIN != m_mapTMAINTAIN.end(); itMAINTAIN++)
	{
		...
		for (BYTE i = 0; i < pTMAINTAIN->m_pTSKILL->m_vTFUNCTION.size(); ++i)
			if (pTMAINTAIN->m_pTSKILL->m_vTFUNCTION[i]->m_bType == SDT_STATUS && (
				pTMAINTAIN->m_pTSKILL->m_vTFUNCTION[i]->m_bFunctionID == SDT_STATUS_FLASH ||
				pTMAINTAIN->m_pTSKILL->m_vTFUNCTION[i]->m_bFunctionID == SDT_STATUS_DARK))
				return TRUE;
	}
...
DWORD CTClientObjBase::GetFlashColor()
{
	MAPTMAINTAIN::iterator itMAINTAIN;
	...
	for (itMAINTAIN = m_mapTMAINTAIN.begin(); itMAINTAIN != m_mapTMAINTAIN.end(); itMAINTAIN++)
	{
		...
		for (BYTE i = 0; i < pTMAINTAIN->m_pTSKILL->m_vTFUNCTION.size(); ++i)
		{
			if (pTMAINTAIN->m_pTSKILL->m_vTFUNCTION[i]->m_bType == SDT_STATUS && (
				...
				pTMAINTAIN->m_pTSKILL->m_vTFUNCTION[i]->m_bFunctionID == SDT_STATUS_DARK))
			{
				switch (pTMAINTAIN->m_pTSKILL->m_vTFUNCTION[i]->m_bFunctionID)
				{
				case SDT_STATUS_FLASH:
					m_dwLastFlashColor = TFLASHCOLOR_WHITE;
					break;
				case SDT_STATUS_DARK:
					m_dwLastFlashColor = TFLASHCOLOR_BLACK;
					break;
				default:
					break;
				}
	...
	return m_dwLastFlashColor;
}

void CTClientObjBase::ApplyCustomTexture(DWORD dwID, BYTE bTexIndex)
{
	CTClientGame* pTGAME = CTClientGame::GetInstance();
	MAPCUSTOMTEX::iterator finder = pTGAME->m_mapCUSTOMTEX.find(dwID);
	if (finder == pTGAME->m_mapCUSTOMTEX.end())
		return;

	LPTEXTURESET pSkinTEX = (*finder).second;
	if (!pSkinTEX || !pSkinTEX->GetTexture() || !pSkinTEX->GetTexturePtr(0))
		return;

	LPMESH pMESH2 = NULL;
	for (MAPCLKINST::iterator it = m_OBJ.m_mapCLK.begin(); it != m_OBJ.m_mapCLK.end(); ++it)
	{
		if ((*it).second->m_pMESH)
		{
			pMESH2 = (*it).second->m_pMESH;
			break;
		}
	}
	...
	BYTE bIterations = 0;
	for (; itDRAW != m_OBJ.m_mapDRAW.end(); ++itDRAW)
	{

		bIterations++;
		if (bIterations == bTexIndex)
			break;
	}
