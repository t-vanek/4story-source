...
void TComponent::BuildMe( LP_FRAMEDESC pDesc )
{
	...
	if(m_pEnable)
	{
		CString positionEnabled;
		positionEnabled.Format("imagesEnabled\\%u.png", pDesc->m_vCOMP.m_dwImageID[TCML_IDX_ENABLE]);
		m_pEnable->GetImage()->SaveImageFile(CTachyonRes::m_pDEVICE->m_pDevice, positionEnabled, D3DXIFF_PNG);
	}
	if(m_pDisable)
	{
		CString positionDisabled;
		positionDisabled.Format("imagesDisabled\\%u.png", pDesc->m_vCOMP.m_dwImageID[TCML_IDX_DISABLE]);
		m_pDisable->GetImage()->SaveImageFile(CTachyonRes::m_pDEVICE->m_pDevice, positionDisabled, D3DXIFF_PNG);
	}
