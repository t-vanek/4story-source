...
void CTClientWnd::OnClose( CTachyonSession *pSession, int nErrorCode)
{
	...
	else if( bSVR == SVR_MAP )
	{
		...
		if( it != m_mapPendingMapSession.end() )
		{
			...
			if( pNEWSESSION )
			{
				...
				pNEWSESSION->m_bLogicalValid = FALSE; // CS_CONNECT_ACK∏¶ πﬁ±‚¿¸±Ó¡¯ ¿Ã º“ƒœ¿∫ ø¨∞·¿∫ ¿Ø»ø«ÿµµ ≈ÎΩ≈«ÿº≠¥¬ æ»µ»¥Ÿ.
	...
	if (bLogout)
	{
		...
		if (!bRelogin)
		{
			...
			else if (!m_bIntendLogout) // ≥ª∞° ¿«µµ«— ººº«¡æ∑·∞° æ∆¥— ∞ÊøÏ ø°∑Ø∏ﬁΩ√¡ˆ √‚∑¬.
...
*/
	...
	CTGaugePannel* pTGaugePannel = static_cast<CTGaugePannel*>(m_MainGame.m_vTFRAME[TFRAME_GAUGE]);
	CTUtilityBarDlg* pUB = pTGaugePannel->m_pUtility; //there is actually like 1000Ä at least probably more
	if (pUB)
	{
							  ...
							  BOOL bInsertStack)
{
	...
	case TMSGBOXOPEN_NONE:	// ±‚¡∏ø° ∂∞ ¿÷¥¬∞‘ ¿÷¿∏∏È ¡ˆ±› ø≠∑¡¥¬ ∏ﬁΩ√¡ˆπ⁄Ω∫¥¬ ±◊≥… π´Ω√«œ¿⁄.
		{
	...
	case TMSGBOXOPEN_CLOSEALL_ALREADY: // ±‚¡∏ø° ø≠∑¡¿÷¥¯ ∏µÁ ∏ﬁΩ√¡ˆ π⁄Ω∫∏¶ ∞≠¡¶ ¡æ∑· Ω√≈∞∞Ì ¡ˆ±› ¿Ã ∏ﬁΩ√¡ˆπ⁄Ω∫∏¶ ø¨¥Ÿ.
		{
			ClearMessageBox();
	...
	case TMSGBOXOPEN_OVERLAP: // ±‚¡∏ ∞Õø° µ§æÓº≠ ¿ßø° ¡ˆ±› ¿Ã ∏ﬁΩ√¡ˆπ⁄Ω∫∏¶ ø¨¥Ÿ.
		{
			ClearMessageBox();
		}
		break;
