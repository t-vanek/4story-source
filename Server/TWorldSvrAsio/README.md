...
- Tournament: **state core + operator tools landed in W6-50**
  ...
  W6-51 adds the scheduler tick + step advance
  (`SM_TOURNAMENT_REQ` → `MW_TOURNAMENTENABLE_REQ` + the collapsed
  `DM_TOURNAMENTSTATUS` persist + the month-end reschedule); W6-52
  the player vertical (`MW_TOURNAMENT_ACK` registration/roster ops +
...
**War/Castle extras (W5+): complete.** W6-41 WARLORDSAY, W6-43
ENDWAR + SKYGARDENOCCUPY + WARCOUNTRYBALANCE (+ war-country index) +
CT_CASTLEGUILDCHG + DM_CASTLEAPPLY persist, W6-44 the
MW_CASTLEWARINFO top-3 / attacker-defender engine.
...
`DM_GETCHARINFO` closed in W6-52 (repository-collapsed into the
PARTYADD offline lookup, §D); `DM_ACTIVECHARUPDATE` landed in W6-43
(war-country index). `DM_CLEARDATA` is intentionally
...
`ITournamentRepository`), `DM_TOURNAMENTSTATUS_REQ` (W6-51 —
collapsed into the tick's step-advance persist via
`ITournamentRepository::SaveStatus`), `DM_GETCHARINFO_REQ/_ACK`
(W6-52 — collapsed into the PARTYADD offline-target lookup via
`ITournamentRepository::FindCharInfoByName`).
...
**Tournament player vertical** — chars can register. The wire test:
key-mismatch + unknown-char drops (sentinel), the SCHEDULE echo
(zero-period filtered, start byte-exact), APPLY at READY → TIMEOUT,
the 1st step (fame member → SUCCESS into the 1st pool + APPLYINFO
follow-up byte-walked: applied flag, 7 free slots, class-3-filtered
reward in shield-first order, 0xFF→137 max level, zero ranks; fame
outsider → DISQUALIFY), the NORMAL apply (normal-pool counts), the
already-registered short SUCCESS (Eof-checked), HWID-dup + country
FAILs, the JOINLIST PARTY gate (silent at NORMAL — sentinel),
party management (online Bob add + PARTYLIST echo + TTournamentApply
trace; Eve LEVEL-with-name; Dana country NOTFOUND; offline Frank via
the repo lookup; unknown-name NOTFOUND; duplicate ALREADYREG;
chief-self del silent; Bob del + PARTYLIST + persist-remove),
MATCHLIST with a seeded bracket player (slot 8 + QFINAL WIN), and
the boot two-pass reload (fame chief → 1st pool, member attached to
the chief's party, orphan dropped). 106 wire tests, all green.

### W6-51 — what landed

**Tournament scheduler tick** — the step-advance runtime. The wire
test: the PopDueTick walk registry-direct (not-due stop, schedule-
period fire at the resume-grafted clocks, consumed-clock idempotence,
zero-period END never rescheduling, the final END → kReschedule),
AdvanceStep (catalogue guard, QFINAL-start next-step lookup, group
flip), then over live sockets: the SM_TOURNAMENT_REQ catalogue-miss
drop (sentinel = the next ENABLE being both peers' first frame), the
step advance broadcasting MW_TOURNAMENTENABLE_REQ to two map peers
(period 600 + next=+1200 byte-checked), a tick draining a two-step
backlog in ladder order (+ 3 TTournamentStatus trace rows), and the
END tick re-running the month math (future clocks), the election
rebuild (MW_TOURNAMENTINFO_REQ, step reset to READY, entry echoed) +
TTournamentClear. 105 wire tests, all green.
...
**Castle-war info engine** — the occupation scoreboard + election
engine, the deepest single handler in the war family. The wire test
drives a two-guild report through aggregation (21/20 points, country
points, both top-3 rows), the defender/attacker election, then a
second castle whose richer G1 score triggers the recursive
cross-castle steal (castle 1 re-elects G2, both attacker slots empty
out), and the castle==0 replay path (asking map only,
sentinel-verified). 98 wire tests, all green.
...
**MonthRank live table** — `MonthRankRegistry` (3 countries × 33
slots + rank month), the byte-exact `month_rank_codec.h`, the
UPDATE re-rank ladder with dirty-range + warlord broadcast, the
RESETCHAR other-con mirror, the WARLORDSAY broadcast, and the
full-table replay on peer connect. Wire test walks: empty-table
replay on join; stale-month drop; first ranker (slot shift 1..32 +
warlord payload); displacement by a higher MonthPoint (Bob above
Alice, warlord unchanged); reset mirror skipping the reporter;
warlord-say field echo. 95 wire tests, all green.

### W6-40 — what landed

**GM item tools** — the last two §C "GM item tools" handlers.
`OnCtItemFindReq`: parse `(manager_id, item_id, name_pattern)`,
offload the CTBLItemFind query, reply `CT_ITEMFIND_ACK` to the
ctrl-svr slot (count echo even when empty; slot-empty drop — all
sentinel-tested). `OnMwAddItemAck`: char-by-(id,key) lookup;
mismatch → MIT_NOTFOUND result to the reporting map; hit →
byte-for-byte `MW_ADDITEM_REQ` forward to the main map (the test
asserts payload equality on the forwarded frame). 94 wire tests,
all green.
