...
DWORD CTMapSvrModule::OnSM_QUITSERVICE_REQ( LPPACKETBUF pBUF)
{
	...
	LogSmartly(_T("SM_QUITSERVICE_REQ detected !!"));
	if(m_bService)
...
DWORD CTMapSvrModule::OnSM_TIMER_REQ(LPPACKETBUF pBUF)
{
	...
	static DWORD dwSupplyTick = 0;
	static BYTE bSupplyIndex = 0;
	static BYTE bStaticRemaining = 0;
	BYTE bRemaining = 0;
	...
	for (MAPBRTEAM::iterator itTeam = m_mapBRTEAM.begin(); itTeam != m_mapBRTEAM.end(); ++itTeam)
	{
		...
		{
			if (FindPlayer((*itPlayer).second->m_dwCharID, (*itPlayer).second->m_dwKEY))
			{
				if ((*itPlayer).second->m_bSupplyIndex < m_vBRSUPPLIES.size())
				{
					if (dwSupplyTick >= m_vBRSUPPLIES[(*itPlayer).second->m_bSupplyIndex]->m_dwTick)
					{
						LPTITEM pTITEM = _AtlModule.FindTItem(m_vBRSUPPLIES[(*itPlayer).second->m_bSupplyIndex]->m_wItemID);
						if(pTITEM)
						{
							VTITEM vITEM;
							vITEM.clear();
							...
							pNew->m_dlID = _AtlModule.GenItemID();
							pNew->m_bCount = 1;
							pNew->m_wItemID = pTITEM->m_wItemID;
							pNew->m_bInven = INVEN_BATTLE;
							pNew->m_pTITEM = pTITEM;
							vITEM.push_back(pNew);

							CTPlayer* pPlayer = FindPlayer((*itPlayer).second->m_dwCharID, (*itPlayer).second->m_dwKEY);
							if (pPlayer)
							{
								...
								pPlayer->SendCS_BOWNOTIFY_ACK("New Supplies have been added to your bag!");
							}

							(*itPlayer).second->m_bSupplyIndex++;
						}
					}
				}
			...
			{
				MAPDWORD::iterator finder = m_mapLeftPlayer.find((*itPlayer).second->m_dwCharID);
				if (finder == m_mapLeftPlayer.end())
					m_mapLeftPlayer.insert(MAPDWORD::value_type((*itPlayer).second->m_dwCharID, 0));
				else
					(*finder).second += 1000;
			}	
	...
	if (bStaticRemaining != bRemaining)
	{
		m_bRemainingPlayers = bRemaining;
		bStaticRemaining = bRemaining;

		for (MAPPLAYER::iterator itPlayer = m_mapPLAYER.begin(); itPlayer != m_mapPLAYER.end(); ++itPlayer)
			(*itPlayer).second->SendCS_UPDATEREMAININGPLAYER_ACK(m_bRemainingPlayers);
	}
...
DWORD CTMapSvrModule::OnMW_CREATERECALLMON_REQ(LPPACKETBUF pBUF)
{
	...
	if(pPlayer->m_bMain && pPlayer->m_bStatus == OS_DEAD)
	{
		...
		SendMW_RECALLMONDEL_ACK(dwCharID, dwKEY, dwMonID, TRUE);
		return EC_NOERROR;
	}
...
//shit gem
	...
	{
		if(pPlayer->m_bMain)
			pPlayer->SendCS_DELRECALLMON_ACK(pPlayer->m_dwID, dwMonID, TRUE, FALSE);
	...
	if(pPlayer->m_bMain && !bMain)
		pPlayer->SendCS_DELRECALLMON_ACK(pPlayer->m_dwID, dwMonID, TRUE, TRUE);
	
	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnMW_CREATESPOLECNIKMON_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	DWORD dwMonID;
	...
	for(it=pPlayer->m_mapCompanion.begin(); it!=pPlayer->m_mapCompanion.end(); it++)	
		SendMW_SPOLECNIKMONDEL_ACK(pPlayer->m_dwID, pPlayer->m_dwKEY, (*it).second->m_dwID, FALSE);
	...
	CTCompanion *pMON = (CTCompanion*) CreateRecallMon(
		pPlayer,
		pTemp,
		dwATTR,
		dwMonID,
...
DWORD CTMapSvrModule::OnMW_SPOLECNIKMONDEL_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKey;
	DWORD dwMonID;
	...
	{
		CTCompanion* pPet = pPlayer->FindCompanion(dwMonID);
		if(pPet)			
				pPlayer->DeleteCompanion(dwMonID, FALSE);
	}
...
DWORD CTMapSvrModule::OnMW_CHARINFO_REQ( LPPACKETBUF pBUF)
{
	...
	pPlayer->m_bGuildCountry = bGuildCountry;
	...
	pPlayer->m_dwRankPoint = dwRankPoint;
	...
	pPlayer->SendCS_CHGCHANNEL_ACK(CHC_SUCCESS, 1);
#endif


#ifdef BOW_COMPILE_MODE
	if (BOWRelease)
		pPlayer->SendCS_RELEASEFORBOW_REQ();

	CheckMonthRank(pPlayer, pPlayer->m_bCountry, pPlayer->m_dwMonthPvPoint, pPlayer->m_dwPvPTotalPoint);
	...
	if (pTEAM)
	{
		pPlayer->m_dwTeamID = pTEAM->m_dwTeamID;
		pPlayer->SendCS_UPDATEBRMATE_ACK(pTEAM->m_mapBRPLAYER);
		pPlayer->SendCS_INITBRTRANKING_ACK(m_mapBRTEAM);
	}
#endif
	...
	pPlayer->SendCS_GUILDSKILLUPDATE_ACK();
	pPlayer->SendCS_CHARINFO_ACK(
		m_dwTick,
		pPlayer->m_pTPREVLEVEL->m_dwEXP,
		pPlayer->m_pTLEVEL->m_dwEXP);
...
DWORD CTMapSvrModule::OnDM_LOCALOCCUPY_ACK(LPPACKETBUF pBUF)
{
	...
	if(!nRet)
	{
		...
		{
			if (LocalPlayer.second->m_wLocalID != wLocalID)
				continue;

			if (LocalPlayer.second->m_dwGuildID != dwGuildID)
				continue;

			auto Local = FindLocal(wLocalID);
			if (!Local)
				continue;

			BYTE playerKills = 0;
			auto itPlayerKills = Local->m_mapKills.find(LocalPlayer.first);
			if (itPlayerKills != Local->m_mapKills.end())
				playerKills = (*itPlayerKills).second;

			guildKills += playerKills;

			LocalPlayer.second->IncreaseStatExp(playerKills * 10);
			LocalPlayer.second->IncreaseStatExp(LOCALOCCUPY_STATEXP / 2);
		}
...
/*#ifdef ADD_KINGDOMBALANCE
	...
	if(pPlayer->m_dwID == dwCharID)
	{
		DEFINE_QUERY(&m_db, CSPGetOnWarBalance)
	    query->Call();
			wVCount = query->dwValoCount;
	     	wDCount = query->dwDeroCount;
		UNDEFINE_QUERY()
	    
	    DEFINE_QUERY(&m_db, CSPGetWarBalance)
		query->Call();
		    wTotalCountV = query->dwValoCount;
		    wTotalCountD = query->dwDeroCount;
		UNDEFINE_QUERY()

		FLOAT fD = (100.0f * wVCount) / FLOAT(wVCount + wDCount);
        FLOAT fC = (100.0f * wDCount) / FLOAT(wVCount + wDCount);
		FLOAT fDTotal = (100.0f * wTotalCountV) / FLOAT(wTotalCountV + wTotalCountD);
        FLOAT fCTotal = (100.0f * wTotalCountD) / FLOAT(wTotalCountV + wTotalCountD);
		...
		pPlayer->SendCS_CHAT_ACK(CHAT_WORLD,0, GetSvrMsg(NAME_OPERATOR), BuildNetString(NAME_NULL, strWarMessage) );
	}
#endif*/
...
DWORD CTMapSvrModule::OnDM_LOADCHAR_REQ( LPPACKETBUF pBUF)
{
	...
	query->m_bStatLevel = 0;
	query->m_bStatPoint = 0;
	query->m_dwStatExp = 0;
	...
	DEFINE_QUERY(&m_db, CTBLSecureCode);
	...
	if (query->Open())
	{
		if (query->Fetch())
		{
			...
			pMSG->m_packet << CString(NAME_NULL);
	...
	if(query->Open())
	{
		...
		if(wCount == 0)
		{
			LogEvent("Character cocked : %d %s", pPlayer->m_dwID, pPlayer->m_strNAME);
			pPlayer->m_bLoadCharError = TRUE;
			CloseSession(pPlayer);
			query->Close();
	...
	{
		LogEvent("Item open query error! CharID: %d", dwCharID);
		pPlayer->m_bLoadCharError = TRUE;
	}
	UNDEFINE_QUERY()
	...
	{
		LogEvent("Skill open query error! CharID: %d", dwCharID);
		pPlayer->m_bLoadCharError = TRUE;
	}
	...
	DEFINE_QUERY( &m_db, CTBLCompanionTable)
	query->m_dwCharID = dwCharID;
	wSize = pMSG->m_packet.GetSize();
	wCount = 0;
	...
	if(query->Open())
	{
		while(query->Fetch())
		{
			pMSG->m_packet
				...
				<< query->m_dwMonID
				<< query->m_dwExp
				<< DWORD( pow( query->m_bLevel, 2 ) * 3600 )
				<< query->m_wLife
				<< query->m_bSPoints
				<< query->m_bLevel
				<< query->m_szName
	...
	DEFINE_QUERY( &m_db, CTBLCompanionItemTable)
	query->m_dwCharID = dwCharID;
	wSize = pMSG->m_packet.GetSize();
	wCount = 0;
	...
	DEFINE_QUERY(&m_db, CSPGetLastCompanion);
	query->m_dwCharID = dwCharID;
	query->m_bCompanionSlot = ( BYTE ) -1;


	query->Call();

	pMSG->m_packet
		<< query->m_bCompanionSlot;

	UNDEFINE_QUERY();

	DEFINE_QUERY(&m_db, CSPGetMedals);
	query->m_dwCharID = dwCharID;
	query->m_dwMedals = 0;
	...
	DEFINE_QUERY(&m_db, CSPGetRankPoint);
	query->m_dwCharID = dwCharID;
	query->m_dwRankPoint = BATTLERANK_FIRST;
	...
	DEFINE_QUERY(&m_db, CSPGetPlayTime);
	{
	...
	if(pPlayer->m_bLoadCharError)
	{
		LogEvent("Load Char Error! CharID: %d", pPlayer->m_dwID);
		CloseSession(pPlayer);
	}
...
DWORD CTMapSvrModule::OnDM_LOADCHAR_ACK( LPPACKETBUF pBUF)
{
	...
	if(!bLogin)
	{
		...
		if(!bNewSecurity)
		{
...

			{
				...
				pSECURE->m_dwTick = dwSecurityTick;
				pSECURE->m_bTries = bTries;
	...
	else
	{
		...
		if (strCode != NAME_NULL)
		{
			...
			pSECURE->m_strCode = strCode;
			pSECURE->m_bDisabled = !pSECURE->m_bDisabled;
		...
		case STORAGE_INVEN:
			{
				...
				{
					CString Log;
					Log.Format("Storage_inven Problem : %d %d", dwCharID, pTITEM->m_wItemID);
					LogSmartly(Log);
		...
		case STORAGE_CABINET:
			{
				...
				{
					CString Log2;
					Log2.Format("Storage_cabinet Problem : %d %d", dwCharID, pTITEM->m_wItemID);
					LogSmartly(Log2);
	...
	for(WORD i=0; i<wCount; i++)
	{
		...
		if(pTemp)
		{
			...
			//1oA3 1o¸±¶§ m_vRemainSkill?!1­ CO´c 1oA3 2A ≫￢°I
	...
	LPTSADDLE Saddle = new TSADDLE();
	DWORD m_dwItemID = 0;
	BYTE m_bType;
	__time64_t m_dEndTime = 0;
	...
	Saddle->m_wItemID = m_dwItemID;
	Saddle->m_dEndTime = m_dEndTime;
	Saddle->m_bType = m_bType;

	if(m_dwItemID)
	{
		pPlayer->m_mapSADDLE.insert(MAPTSADDLE::value_type(pPlayer->m_dwUserID, Saddle));
		pPlayer->SendCS_SENDSADDLE_REQ(m_dwItemID, m_dEndTime, m_bType);
	}
	else
	...
	if(wCount)
	{
		...
		MAPTTITLETEMP::iterator find = m_mapTTITLE.find(NOTITLE);
		if(find != m_mapTTITLE.end())
		{
			LPTTITLE pTITLE = new TTITLE();

			pTITLE->m_wTitleID = NOTITLE;
			pTITLE->m_bSelected = !bTitleSelected;

			pPlayer->m_mapTTITLE.insert( MAPTTITLE::value_type( pTITLE->m_wTitleID, pTITLE));

			if(pTITLE->m_bSelected)
				pPlayer->m_wTitleID = NOTITLE;
		}
	}
	...
	if(wCount)
	{
		...
		for(BYTE j = 0; j < PETITEMS_COUNT; ++j)
			{
				pCOMP->m_wItemID[j] = 0;
				pCOMP->m_dEndTime[j] = 0;
				pCOMP->m_dwTick = 0;
			}
	...
	{
		BYTE bCompanionSlot;
		WORD wItemID[PETITEMS_COUNT] = {0};
		__time64_t dEndTime[PETITEMS_COUNT] = {0};
		DWORD dwTick;
...

		{
			pBUF->m_packet
				...
				MAPTCOMP::iterator pCOMP = pPlayer->m_mapComp.find(bCompanionSlot);
				if (pCOMP != pPlayer->m_mapComp.end())

				{
					(*pCOMP).second->m_wItemID[j] = wItemID[j];
					(*pCOMP).second->m_dEndTime[j] = dEndTime[j];
					(*pCOMP).second->m_dwTick = dwTick;
	...
	LPBRTEAM pTEAM = GetBRTeam(pPlayer->m_dwID);
	LPBRPLAYER pBRPLAYER = NULL;
	if (pTEAM)
	{
		MAPBRPLAYER::iterator itPlayer = pTEAM->m_mapBRPLAYER.find(pPlayer->m_dwID);
		if (itPlayer != pTEAM->m_mapBRPLAYER.end())
			pBRPLAYER = (*itPlayer).second;
	}

	if (pTEAM && pBRPLAYER && !pBRPLAYER->m_bDead)
	{
		pPlayer->m_dwTeamID = pTEAM->m_dwTeamID;

		CTPlayer* pMoveTo = NULL;
		FLOAT fPosX, fPosY, fPosZ;
		for (MAPBRPLAYER::iterator it = pTEAM->m_mapBRPLAYER.begin(); it != pTEAM->m_mapBRPLAYER.end(); ++it)
		{
			if ((*it).second->m_dwCharID == pPlayer->m_dwID || (*it).second->m_bDead)
				continue;

			pMoveTo = _AtlModule.FindPlayer((*it).second->m_dwCharID, (*it).second->m_dwKEY);
			if (!pMoveTo || !pMoveTo->m_dwHP)
				continue;
			else
				break;
		}

		pPlayer->m_bChannel = BR_SERVER_ID;
		pPlayer->SendCS_CHGCHANNEL_ACK(CHC_SUCCESS, BR_SERVER_ID);
		pPlayer->m_wMapID = m_wBRMapID;

		if (!pMoveTo || pMoveTo->m_fPosX <= 0.0f)
		{
			BRRAWPOS pPOS = GetBRRandomSpawnPos();
			fPosX = pPOS.m_fPosX;
			fPosY = pPOS.m_fPosY;
			fPosZ = pPOS.m_fPosZ;
		}
		...
		{
			fPosX = pMoveTo->m_fPosX;
			fPosY = pMoveTo->m_fPosY;
			fPosZ = pMoveTo->m_fPosZ;
		}
	...
	else
	{
		...
		LogSmartly("Ending here.");

		CloseSession(pPlayer);
		return EC_NOERROR;
	}

	MAPSAVEDPLAYER::iterator finder = m_mapSAVEDPLAYER.find(pPlayer->m_dwID);
	if (finder != m_mapSAVEDPLAYER.end() && ((*finder).second->m_mapTINVEN.empty() || (*finder).second->m_mapTSKILL.empty()))
	{
		m_mapSAVEDPLAYER.erase(finder);
		delete (*finder).second;
	}
	finder = m_mapSAVEDPLAYER.find(pPlayer->m_dwID);

	pPlayer->ReplaceBOWEquipment(finder != m_mapSAVEDPLAYER.end());

	if (finder != m_mapSAVEDPLAYER.end())
	{
		MAPTINVEN::iterator itInven = pPlayer->m_mapTINVEN.begin();

		while (itInven != pPlayer->m_mapTINVEN.end())
		{
			...
			for (MAPTITEM::iterator itItem = pInven->m_mapTITEM.begin(); itItem != pInven->m_mapTITEM.end(); ++itItem)
			{
				if ((*itItem).second)
				{
					pPlayer->SendCS_DELITEM_ACK(pInven->m_bInvenID, (*itItem).second->m_bItemID);
			...
			if (pInven->m_bInvenID != INVEN_EQUIP)
				pPlayer->SendCS_INVENDEL_ACK(INVEN_SUCCESS, pInven->m_bInvenID, FALSE);

			itInven = pPlayer->m_mapTINVEN.erase(itInven);
			delete pInven;
		}

		for (MAPTPET::iterator itPET = pPlayer->m_mapTPET.begin(); itPET != pPlayer->m_mapTPET.end(); ++itPET)
			delete (*itPET).second;
		pPlayer->m_mapTPET.clear();

		for (MAPTSADDLE::iterator itSaddle = pPlayer->m_mapSADDLE.begin(); itSaddle != pPlayer->m_mapSADDLE.end(); ++itSaddle)
			delete (*itSaddle).second;
		pPlayer->m_mapSADDLE.clear();

		//for (MAPTCOMP::iterator itComp = pPlayer->m_mapComp.begin(); itComp != pPlayer->m_mapComp.end(); ++itComp)
		//	delete (*itComp).second;
		//pPlayer->m_mapComp.clear();

		for (MAPTSKILL::iterator itSkill = pPlayer->m_mapTSKILL.begin(); itSkill != pPlayer->m_mapTSKILL.end(); ++itSkill)
			delete (*itSkill).second;
		pPlayer->m_mapTSKILL.clear();

		for (BYTE i = 0; i < pPlayer->m_vMaintainSkill.size(); ++i)
			delete pPlayer->m_vMaintainSkill[i];
		pPlayer->m_vMaintainSkill.clear();

		for (BYTE i = 0; i < pPlayer->m_vRemainSkill.size(); ++i)
			delete pPlayer->m_vRemainSkill[i];
		pPlayer->m_vRemainSkill.clear();
		pPlayer->m_mapItemCoolTime.clear();

		for (MAPTINVEN::iterator itInven = (*finder).second->m_mapTINVEN.begin(); itInven != (*finder).second->m_mapTINVEN.end(); ++itInven)
		{
			CTInven* pInven = new CTInven();
			pInven->Copy((*itInven).second);
			...
			for (MAPTITEM::iterator itItem = pInven->m_mapTITEM.begin(); itItem != pInven->m_mapTITEM.end(); ++itItem)
				if ((*itItem).second)
					pPlayer->SendCS_ADDITEM_ACK((*itItem).second, pInven->m_bInvenID);

			pPlayer->m_mapTINVEN.insert(MAPTINVEN::value_type((*itInven).first, pInven));
		}

		for (MAPTPET::iterator itPet = (*finder).second->m_mapTPET.begin(); itPet != (*finder).second->m_mapTPET.end(); ++itPet)
		{
			LPTPET pPET = new TPET();
			pPET->m_wPetID = (*itPet).second->m_wPetID;
			pPET->m_strName = (*itPet).second->m_strName;
			pPET->m_pTPET = (*itPet).second->m_pTPET;
			pPET->m_ldwTime = (*itPet).second->m_ldwTime;
			pPET->m_bEffect = (*itPet).second->m_bEffect;

			pPlayer->m_mapTPET.insert(MAPTPET::value_type((*itPet).first, pPET));
		}

		for (MAPTSADDLE::iterator itSaddle = (*finder).second->m_mapSADDLE.begin(); itSaddle != (*finder).second->m_mapSADDLE.end(); ++itSaddle)
		{
			LPTSADDLE pSADDLE = new TSADDLE();
			pSADDLE->m_wItemID = (*itSaddle).second->m_wItemID;
			pSADDLE->m_bType = (*itSaddle).second->m_bType;
			pSADDLE->m_dEndTime = (*itSaddle).second->m_dEndTime;

			pPlayer->m_mapSADDLE.insert(MAPTSADDLE::value_type((*itSaddle).first, pSADDLE));
		}

		for (MAPTSKILL::iterator itSkill = (*finder).second->m_mapTSKILL.begin(); itSkill != (*finder).second->m_mapTSKILL.end(); ++itSkill)
		{
			CTSkill* pSkill = new CTSkill();
			pSkill->Copy((*itSkill).second);

			pPlayer->m_mapTSKILL.insert(MAPTSKILL::value_type((*itSkill).first, pSkill));
		}

		for (BYTE i = 0; i < (*finder).second->m_vMaintainSkill.size(); ++i)
		{
			CTSkill* pMaintainSkill = new CTSkill();
			pMaintainSkill->Copy((*finder).second->m_vMaintainSkill[i]);

			pPlayer->m_vMaintainSkill.push_back(pMaintainSkill);
		}
		
		for (BYTE i = 0; i < (*finder).second->m_vRemainSkill.size(); ++i)
		{
			CTSkill* pRemainSkill = new CTSkill();
			pRemainSkill->Copy((*finder).second->m_vRemainSkill[i]);

			pPlayer->m_vRemainSkill.push_back(pRemainSkill);
		}
		
		for (MAPDWORD::iterator itCoolTime = (*finder).second->m_mapItemCoolTime.begin(); itCoolTime != (*finder).second->m_mapItemCoolTime.end(); ++itCoolTime)
			pPlayer->m_mapItemCoolTime.insert(MAPDWORD::value_type((*itCoolTime).first, (*itCoolTime).second));

		pPlayer->m_dwHP = (*finder).second->m_dwHP;
		pPlayer->m_dwMP = (*finder).second->m_dwMP;
		pPlayer->m_dwBB = (*finder).second->m_dwBB;
		pPlayer->m_dwPoints = (*finder).second->m_dwPoints;
		pPlayer->m_dwTotalPoints = (*finder).second->m_dwTotalPoints;
		pPlayer->m_bDeaths = (*finder).second->m_bDeaths;
	}
	...
	{
		MAPTSPAWNPOS::iterator finder = m_mapTSPAWNPOS.find(BOW_DEFAULT_SPAWNID + (*finderPlayer).second->m_bCountry);
		if (finder != m_mapTSPAWNPOS.end())
		{
			LPTSPAWNPOS pPOS = (*finder).second;
			pPlayer->m_wMapID = pPOS->m_wMapID;
			...
			pPlayer->m_fPosY = pPOS->m_fPosY;
			pPlayer->m_fPosZ = pPOS->m_fPosZ;
			pPlayer->m_wSpawnID = BOW_DEFAULT_SPAWNID + (*finderPlayer).second->m_bCountry;
		}
	}
	else
	{
...
#ifdef BOW_COMPILE_MODE
		pPlayer->m_bMain = FALSE;
	//	LogSmartly("release main true, bow");
		SendMW_RELEASEMAIN_ACK(TRUE, pPlayer);
#else
		//LogSmartly("release main protect tutorial");
...
DWORD CTMapSvrModule::OnDM_SAVECHAR_REQ( LPPACKETBUF pBUF)
{
	...
	if (m_mapSAVEDPLAYER.find(dwCharID) != m_mapSAVEDPLAYER.end())
		return EC_NOERROR;
	...
	BYTE bPlayerCount = 0;
	BOOL bError = FALSE;
	MAPPLAYER::iterator jirk;

	for(jirk = m_mapSESSION.begin(); jirk!=m_mapSESSION.end(); ++jirk)
	{
		if((*jirk).second->m_dwID == dwCharID)
			bPlayerCount++;
	}

	if(bPlayerCount > 1)
		LogEvent("Alright I guess we got attacked.");
	...
	if(!bLevel)
	{
		strError.Format("Curaci zas mrdaj save funkci//Player ID : %d UID : %d", dwCharID, dwUserID);
		LogSmartly(strError);
		//worst case scenario bypass
	...
	CString xD;
	xD.Format("Level: %d, Point : %d, Exp: %d", query->m_StatLevel, query->m_StatPoint, query->m_StatExp);
	LogSmartly(xD);

	query->m_bStartAct = bStartAct;
	query->m_bLevel = bLevel;
	query->m_dwCharID = dwCharID;

	if (!query->Call()) {
		LogSmartly("QUERY_CALL_FAIL");
		return EC_NOERROR;
	}

	if (query->m_nRET) {
		LogSmartly("QUERY_RET_FAIL");
		return EC_NOERROR;
	}
	...
	if (!query->Call()) {
		LogSmartly("NO SAVECHARDATASTART");
		return EC_NOERROR;
	}
	...
	for( WORD i=0; i<bInvenCount; i++)
	{
		...
		CString db;
		db.Format("IID : %d, Item: %d", bInvenID, query->m_wItemID);
		LogSmartly(db);
		...
		{
			CString xd;
			xd.Format("Item: %d", query->m_item.m_wItemID);
			LogSmartly(xd);
	...
	for(WORD i=0; i<bCabinetCount; i++)
	{
		BYTE bCabinetID;
		BYTE bUse;
	...
	DEFINE_QUERY(&m_db, CSPSaveCompanion);
	query->m_dwCharID = dwCharID;
...
for(BYTE i = 0; i < bCount; ++i )
	{
		...
		lstrcpy(query->m_strName, strName);

			__time64_t dEndTime[PETITEMS_COUNT];
		WORD wItemID[PETITEMS_COUNT];
		...
		for (BYTE j = 0; j < PETITEMS_COUNT; ++j)
		{
			pBUF->m_packet
				...
				query->m_wItemID[j] = wItemID[j];
				__TIMETODB(dEndTime[j], query->m_dEndTime[j]);
		}
	...
	DEFINE_QUERY(&m_db, CSPSaveLastCompanion);
	query->m_dwCharID = dwCharID;
	pBUF->m_packet
		>> query->m_bCompanionSlot;
...
BYTE HasSecureCode;
	...
	if (HasSecureCode)
	{
		...
		strcpy(query->m_strCode, strCode);
		query->m_bEnabled = !query->m_bEnabled;
...
DWORD CTMapSvrModule::OnDM_SAVEITEM_REQ( LPPACKETBUF pBUF)
{
	...
	if (m_mapSAVEDPLAYER.find(pPlayer->m_dwID) != m_mapSAVEDPLAYER.end())
		return EC_NOERROR;
...
DWORD CTMapSvrModule::OnMW_GUILDESTABLISH_REQ( LPPACKETBUF pBUF)
{
	...
	if(!bRet)
	{
		...
		for (auto& GSkill : m_mapGuildSkill)
			if (pPlayer->m_mapGuildSkill.find(GSkill.first) == pPlayer->m_mapGuildSkill.end()) {
				GUILDSKILL Skill;
				Skill.m_wSkillID = GSkill.second.m_wSkillID;
				Skill.m_bType = GSkill.second.m_bType;
				Skill.m_EndTime = 0;
				Skill.m_bLevel = 1;

				pPlayer->m_mapGuildSkill.insert(std::make_pair(Skill.m_wSkillID, Skill));
			}
		SaveGuildSkill(pPlayer);
		pPlayer->SendCS_GUILDSKILLUPDATE_ACK();
...
DWORD CTMapSvrModule::OnMW_GUILDJOIN_REQ( LPPACKETBUF pBUF)
{
	...
	/*
	pPlayer->m_mapGuildSkill.clear();
	
	for (auto& GuildSkill : GetGuildSkill(dwCharID, GUILD_DUTY_NONE, dwGuildID))
	{
		GUILDSKILL Skill;
		Skill.m_wSkillID = GuildSkill.m_wSkillID;
		Skill.m_bLevel = GuildSkill.m_bLevel;
		Skill.m_bType = GuildSkill.m_bType;
		Skill.m_EndTime = GuildSkill.m_EndTime;

		pPlayer->m_mapGuildSkill.insert(std::make_pair(Skill.m_wSkillID, Skill));
	}

	for (auto& Temp : m_mapGuildSkill)
	{
		auto Skill = pPlayer->m_mapTSKILL.find(Temp.first);
		if (Skill != pPlayer->m_mapTSKILL.end())
			pPlayer->m_mapTSKILL.erase(Temp.first);

		CString strQuery;
		strQuery.Format("DELETE FROM TSKILLTABLE WHERE dwCharID = %d AND wSkillID = %d", dwCharID, Temp.second.m_wSkillID);
	...
	for (auto NewGS : pPlayer->m_mapGuildSkill)
	{
		CTSkill * pSkill = new CTSkill();
		pSkill->m_pTSKILL = FindTSkill(NewGS.first);
		pSkill->m_bLevel = NewGS.second.m_bLevel;

		pPlayer->m_mapTSKILL.insert(MAPTSKILL::value_type(NewGS.first, pSkill));
		pPlayer->RemainSkill(pSkill, 0);

		pPlayer->SendCS_SKILLBUY_ACK(SKILL_SUCCESS, NewGS.first, pSkill->m_bLevel);
	}
	*/
	...
	if (pPlayer->m_strNAME == strTarget)
		{
			...
			/*
			pPlayer->m_mapGuildSkill.clear();
			for (auto& GuildSkill : GetGuildSkill(dwCharID, bDuty, pPlayer->m_dwGuildID))
			{
				GUILDSKILL Skill;
				Skill.m_wSkillID = GuildSkill.m_wSkillID;
				Skill.m_bLevel = GuildSkill.m_bLevel;
...
DWORD CTMapSvrModule::OnMW_GUILDLEAVE_REQ( LPPACKETBUF pBUF)
{
	...
	if(pPlayer->m_strNAME == strTarget)
	{
		...
		auto GuildSkill = pPlayer->m_mapGuildSkill.begin();
		while (GuildSkill != pPlayer->m_mapGuildSkill.end())
		{
			auto& Temp = m_mapGuildSkill.find(GuildSkill->first);
			if (Temp == m_mapGuildSkill.end())
				continue;

			if (Temp->second.m_bType)
				GuildSkill = pPlayer->m_mapGuildSkill.erase(GuildSkill);
			else
				++GuildSkill;
		}
...
{
	static BYTE bCurStatus = BS_NORMAL;
...
DWORD CTMapSvrModule::OnMW_SKYGARDENOCCUPY_REQ(LPPACKETBUF pBUF)
{
	...
	pSkyGarden->m_timeNextDefend += DAY_ONE;
	pSkyGarden->m_mapTPoint.clear();
	pSkyGarden->m_mapKills.clear();
...
DWORD CTMapSvrModule::OnMW_SKYGARDENENABLE_REQ(LPPACKETBUF pBUF)
{
	...
	for(DWORD ll=0; ll<m_vSkygarden.size(); ll++)
	{
		...
		if (pSkygarden)
		{
			pSkygarden->m_bDay = nCD;

			if (dwCLT < dwSkyGardenStart)
				pSkygarden->m_timeNextDefend = curtime.GetTime() - dwCLT + dwSkyGardenStart;
			else
				pSkygarden->m_timeNextDefend = curtime.GetTime() - dwCLT + dwSkyGardenStart + DAY_ONE;
		}
...
DWORD CTMapSvrModule::OnMW_GETBLOOD_REQ( LPPACKETBUF pBUF)
{
	...
	case OT_COMPANION:pOBJ = pPlayer->FindCompanion(dwAtkID); break;
...
DWORD CTMapSvrModule::OnMW_STARTTELEPORT_REQ( LPPACKETBUF pBUF)
{
...
#if defined(BR_COMPILE_MODE)
	if (bChannel == BR_SERVER_ID && pPlayer->m_bChannel != BR_SERVER_ID)
	{
		BOOL bFound = FALSE;
		for (MAPBRTEAM::iterator itTeam = m_mapBRTEAM.begin(); itTeam != m_mapBRTEAM.end(); ++itTeam)
		{
			for (MAPBRPLAYER::iterator itPlayer = (*itTeam).second->m_mapBRPLAYER.begin(); itPlayer != (*itTeam).second->m_mapBRPLAYER.end(); ++itPlayer)
			{
				if ((*itPlayer).second->m_dwCharID == pPlayer->m_dwID && !(*itPlayer).second->m_bDead)
				{
					bFound = TRUE;
					break;
				}
	...
	if (wMapID == BOW_MAP_ID &&
		pPlayer->m_wMapID != BOW_MAP_ID)
	{
		if(m_mapBOWPLAYER.find(dwCharID) == m_mapBOWPLAYER.end())
			return EC_NOERROR;
	}
#endif
...
DWORD CTMapSvrModule::OnMW_RECALLMONDATA_REQ(LPPACKETBUF pBUF)
{
	...
	CTCompanion * pComp = pPlayer->FindCompanion(dwID);
	if(pComp)
		pRecall = pComp;
...
DWORD CTMapSvrModule::OnMW_CHANGECHARBASE_REQ(LPPACKETBUF pBUF)
{
	...
	case IK_RACE:
		...
		if(pPlayer->m_bMain)
		{
			...
			if(pRe && !pRe->m_wCompanionID)
				SendMW_RECALLMONDEL_ACK(pPlayer->m_dwID, pPlayer->m_dwKEY, pRe->m_dwID, TRUE);
...
DWORD CTMapSvrModule::OnMW_CASHSHOPSTOP_REQ(LPPACKETBUF pBUF)
{
	...
	if(bSendPlayer)
	{
		...
		for(it = m_mapPLAYER.begin(); it != m_mapPLAYER.end(); it++)
		{
			if((*it).second->m_bMain && (*it).second)
		...
		for(itTP=m_mapTNMTPlayer.begin(); itTP!=m_mapTNMTPlayer.end(); itTP++)
		{
			...
			if(itTP->second->m_wMapID)
			{
				...
				if(pChar)
				{
					...
					if(pChar->IsTournamentPlayer())
						pChar->ExitTournamentCharClear();
				}
			}
		}
		...
		for(it=m_mapPLAYER.begin();it!=m_mapPLAYER.end(); it++)
			if((*it).second->m_bMain)
			{
					...
					(*it).second->m_wMapID == GetLoungeMapID())
				{
					...
					if((*it).second->IsTournamentPlayer())
						(*it).second->ExitTournamentCharClear();
				}
			}
	}
...
DWORD CTMapSvrModule::OnMW_MISSIONENABLE_REQ(LPPACKETBUF pBUF)
{
	...
	for(DWORD ll=0; ll<m_vMission.size(); ll++)
	{
		...
		if (!pMission->m_timeNextDefend || (bCurStatus != BS_NORMAL && bStatus == BS_NORMAL))
		{
			...
			pMission->m_timeNextDefend = next.GetTime() + dwStart;
		}
...
DWORD CTMapSvrModule::OnMW_BOWTIMEUPDATE_ACK(LPPACKETBUF pBUF)
{
	BYTE bStatus;
	DWORD dwSecond;
	BYTE bDefugelPoints;
...
#ifdef BOW_COMPILE_MODE
	m_bBOWStatus = bStatus;

	if (m_bBOWStatus != bStatus && bStatus == BS_ALARM)
		ClearSavedPlayers();
#endif

	for (MAPPLAYER::iterator itPlayer = m_mapPLAYER.begin(); itPlayer != m_mapPLAYER.end(); ++itPlayer)
		(*itPlayer).second->SendCS_BOWTIMEUPDATE_ACK(bStatus, dwSecond, bDefugelPoints, bCraxionPoints);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnMW_ADDTOBOWQUEUE_ACK(LPPACKETBUF pBUF)
{
	BYTE bResult;
	DWORD dwCharID;
	DWORD dwKEY;
	...
	CTPlayer* pPlayer = FindPlayer(dwCharID, dwKEY);
	if (pPlayer)
		pPlayer->SendCS_REGISTERBOW_ACK(bResult, dwTick);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnMW_CANCELBOWQUEUE_ACK(LPPACKETBUF pBUF)
{
	BYTE bResult;
	DWORD dwCharID;
	DWORD dwKEY;
	...
	CTPlayer* pPlayer = FindPlayer(dwCharID, dwKEY);
	if (pPlayer)
		pPlayer->SendCS_CANCELBOWQUEUE_ACK(bResult, dwTick);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnMW_NOTIFYNONQUEUEDPLAYER_ACK(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	...
	pPlayer->SendCS_NOTIFYNONQUEUEDPLAYER_ACK();

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnMW_PREPAREFORBOW_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	BYTE bTeam;
	...
	if (!pPlayer || pPlayer->IsTournamentPlayer())
		return EC_NOERROR;
	...
	if (pPlayer->m_dwID)
	{
		...
		Teleport(pPlayer, BOW_DEFAULT_SPAWNID + bTeam);
	}

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnMW_RECALCCOUNTRYBALANCE_ACK(LPPACKETBUF pBUF)
{
	BYTE bCountryCount[TCONTRY_B] = { 0 };

	pBUF->m_packet
		>> bCountryCount[TCONTRY_D]
		>> bCountryCount[TCONTRY_C];

	wVCount = bCountryCount[TCONTRY_D];
	wDCount = bCountryCount[TCONTRY_C];

	FLOAT fD = (100.0f * bCountryCount[TCONTRY_D]) / FLOAT(bCountryCount[TCONTRY_D] + bCountryCount[TCONTRY_C]);
	FLOAT fC = (100.0f * bCountryCount[TCONTRY_C]) / FLOAT(bCountryCount[TCONTRY_D] + bCountryCount[TCONTRY_C]);

	if (fD < fC)
		_AtlModule.m_bBonusCountry = 0;
	else
		_AtlModule.m_bBonusCountry = 1;

	FLOAT fDifference = fD > fC ? fD - fC : fC - fD;
	CString szStage = "No Penalty";

	if (fDifference > 15.0f)
		szStage = "3";
	else if (fDifference > 10.0f)
		szStage = "2";
	else if (fDifference > 5.0f)
		szStage = "1";

	if (szStage != "No Penalty") {
		BYTE bStage = atoi(szStage);
		_AtlModule.m_bStage = bStage;
	}
	else
		_AtlModule.m_bStage = 0;

	return EC_NOERROR;
}

#ifdef BOW_COMPILE_MODE
DWORD CTMapSvrModule::OnMW_BOWCOMMANDEXEC_REQ(LPPACKETBUF pBUF)
{
	BYTE bCommand;
	...
	CTMap* pBOWMAP = FindTMap(DEFAULT_CHANNEL, 0, BOW_MAP_ID);
	if (!pBOWMAP)
		return EC_NOERROR;
...
DWORD CTMapSvrModule::OnMW_ENDBOWWAR_REQ(LPPACKETBUF pBUF)
{
	BYTE bWinCountry;
	BYTE bCount;
	DWORD dwCharID;
	...
	for (BYTE i = 0; i < bCount; ++i)
	{
		...
		if (pPlayer)
		{
			...
			{
				LPTSPAWNPOS pPOS = (*finder).second;
				SafeTeleport(pPlayer, 0, pPOS->m_wMapID, pPOS->m_fPosX, pPOS->m_fPosY, pPOS->m_fPosZ);
			}
		}
		else
		{
			MAPSAVEDPLAYER::iterator itSaved = m_mapSAVEDPLAYER.find(dwCharID);
			if (itSaved == m_mapSAVEDPLAYER.end())
				continue;
			else
...
DWORD CTMapSvrModule::OnMW_RELEASESINGLEBOWPLAYER_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	BYTE bWinCountry;
...
DWORD CTMapSvrModule::OnMW_ADDBOWPLAYERS_ACK(LPPACKETBUF pBUF)
{
	BYTE bCount;
	DWORD dwCharID;
	DWORD dwKEY;
	...
	for (BYTE i = 0; i < bCount; ++i)
	{
		...
		m_mapBOWPLAYER.insert(MAPBOWPLAYER::value_type(dwCharID, pPLAYER));
	}
...
DWORD CTMapSvrModule::OnMW_BRTIMEUPDATE_ACK(LPPACKETBUF pBUF)
{
	BYTE bStatus;
	DWORD dwSecond;
	BYTE bType;
	...
	for (MAPPLAYER::iterator itPlayer = m_mapPLAYER.begin(); itPlayer != m_mapPLAYER.end(); ++itPlayer)
		(*itPlayer).second->SendCS_BRTIMEUPDATE_ACK(bStatus, dwSecond, bType);
#ifdef BR_COMPILE_MODE
	m_bBRStatus = bStatus;
#endif
	return EC_NOERROR;
...
DWORD CTMapSvrModule::OnMW_ADDTOBRQUEUE_ACK(LPPACKETBUF pBUF)
{
	BYTE bResult;
	DWORD dwCharID;
	DWORD dwKEY;
	...
	pPlayer->SendCS_ADDTOBRQUEUE_ACK(bResult, dwTick);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnMW_BRTEAMMATEADD_ACK(LPPACKETBUF pBUF)
{
	BYTE bResult;
	...
	CTPlayer* pReceiver = FindPlayer(dwCharID, dwKEY);
	if (!pReceiver)
		return EC_NOERROR;

	pReceiver->SendCS_BRTEAMMATEADD_ACK(bResult, strNAME);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnMW_UPDATEBRTEAM_ACK(LPPACKETBUF pBUF)
{
	BYTE bCount;
	DWORD dwCharID;
	DWORD dwKEY;
	...
	if (strChiefName != NAME_NULL)
	{
		BRPREMADEPLAYER Chief;
		Chief.m_strName = strChiefName;
		Chief.m_bReady = TRUE;

		vTeam.push_back(Chief);
	}
	...
	for (BYTE i = 0; i < bCount; ++i)
	{
		...
		if (strName != strChiefName)
			vTeam.push_back(PremadePlayer);
	}

	pPlayer->SendCS_UPDATEBRTEAM_ACK(vTeam, bTeamReady);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnMW_PREPAREFORBR_REQ(LPPACKETBUF pBUF)
{
	WORD wMapID;
	DWORD dwCharID;
	DWORD dwKEY;
	...
	if (pPlayer->m_dwID)
	{
		...
		Teleport(pPlayer, BR_SERVER_ID, wMapID, UNIT_SIZE / 2, 0.0f, UNIT_SIZE / 2); //The coords will be automatically adjusted.
	}
...
#ifdef BR_COMPILE_MODE
DWORD CTMapSvrModule::OnMW_ADDBRTEAMS_ACK(LPPACKETBUF pBUF)
{
	DWORD dwTeamID;
	...
	CTMap* pBRMap = FindTMap(BR_SERVER_ID, 0, m_wBRMapID);
	if (!pBRMap)
		return EC_NOERROR;
	...
	for (BYTE i = 0; i < bCount; ++i)
	{
		...
		for (BYTE l = 0; l < BR_TEAMMATE_MAX_COUNT(m_Mode); ++l)
		{
			pBUF->m_packet
				>> dwCharID
				>> dwKEY
			...
			LPBRPLAYER pPLAYER = new BRPLAYER();
			pPLAYER->m_dwCharID = dwCharID;
			pPLAYER->m_dwKEY = dwKEY;
			pPLAYER->m_bClass = bClass;
			pPLAYER->m_strNAME = strNAME;
			...
			pTEAM->m_mapBRPLAYER.insert(MAPBRPLAYER::value_type(dwCharID, pPLAYER));
		}


		m_mapBRTEAM.insert(MAPBRTEAM::value_type(dwTeamID, pTEAM));
	}

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnMW_BRCOMMANDEXEC_REQ(LPPACKETBUF pBUF)
{
	BYTE bCommand;
	...
	case BR_START:
		{
			BOWNotify("Battle Royale has started. Kill, or be killed!");
		}
		break;
	case BR_END:
		{
			...
			MAPBRTEAM::iterator itTeam = m_mapBRTEAM.find(dwTeamID);
			if (itTeam != m_mapBRTEAM.end())
			{
				for (MAPBRPLAYER::iterator itPlayer = (*itTeam).second->m_mapBRPLAYER.begin(); itPlayer != (*itTeam).second->m_mapBRPLAYER.end(); ++itPlayer)
				{
					strMSG += (*itPlayer).second->m_strNAME += ", ";
				}
			}

			strMSG += "Won the Battle Royale.";
...
DWORD CTMapSvrModule::OnMW_ENDBRWAR_REQ(LPPACKETBUF pBUF)
{
	BYTE bTCount, bCount;
	DWORD dwCharID;
	DWORD dwKEY;
	...
	for (BYTE i = 0; i < bTCount; ++i)
	{
		...
		for (BYTE l = 0; l < bCount; ++l)
		{
			...
			if (pPlayer)
			{
				...
				if (pBRPLAYER)
				{
					DEFINE_QUERY(&m_db, CSPGiveBRReward);
					{
...
DWORD CTMapSvrModule::OnMW_RELEASESINGLEBRPLAYER_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	...
	if (pPlayer)
	{
		...
		if (pBRPLAYER)
		{
			...
			if (finder != m_mapBRTEAM.end())
			{
				...
				{
					(*finder).second->m_mapBRPLAYER.erase(itPlayer);
...
DWORD CTMapSvrModule::OnMW_BATTLEMODESTATUS_ACK(LPPACKETBUF pBUF)
{
	...
	pReceiver->SendCS_GUILDLOCALLIST_ACK(bBOWStatus, bBOWWinner, dwNextBOW, bBRStatus, dwNextBR, bNextBRType);
...
DWORD CTMapSvrModule::OnMW_GUILDSKILLACTION_ACK(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	BYTE Action;
	...
	for (BYTE i = 0; i < ParamSize; ++i)
	{
		...
		CString dbug;
		dbug.Format("Action : %d, ParamSize : %d, Param[%d] = %d", Action, ParamSize, i, Param[i]);
		LogSmartly(dbug);
	}

	CTPlayer* pPlayer = FindPlayer(dwCharID, dwKEY);
	if (!pPlayer || !pPlayer->m_bMain || !pPlayer->m_dwGuildID)
		return EC_NOERROR;
	...
	case GS_BUY:
	{
		...
		CTSkill* pSkill = pPlayer->FindTSkill(Param[0]);
		if (!pSkill)
		{
			CTSkillTemp* pTemp = FindTSkill(Param[0]);
			if (!pTemp)
				return EC_NOERROR;
			...
			pPlayer->m_mapTSKILL.insert(MAPTSKILL::value_type(Param[0], pSkill));
			pPlayer->RemainSkill(pSkill, 0);

		}
		else if (pSkill->m_bLevel < pSkill->m_pTSKILL->m_bMaxLevel)
			pSkill->m_bLevel++;
		else
			return EC_NOERROR;
		...
		if (!Param[1]) 
		{
			if (!pPlayer->m_StatPoint)
				return EC_NOERROR;

			pPlayer->m_StatPoint--;
		}
		else
			SendMW_UPDATEGUILDCOOLDOWN_ACK(pPlayer->m_dwGuildID, Param[0], pSkill->m_bLevel, TRUE, FALSE);

		auto& itGuildSkill = pPlayer->m_mapGuildSkill.find(Param[0]);
		if (itGuildSkill != pPlayer->m_mapGuildSkill.end()) {
			(*itGuildSkill).second.m_bLevel++;
		}
		else
		{
			GUILDSKILL GSkill = { Param[0], 0, 1, m_timeCurrent + (DAY_ONE * 31) };
			pPlayer->m_mapGuildSkill.insert(std::make_pair(Param[0], GSkill));
		}

		if (pSkill)
			pPlayer->SendCS_SKILLBUY_ACK(SKILL_SUCCESS, Param[0], pSkill->m_bLevel);
	}
	break;
	case GS_RENEW:
	{
		auto GSTemp = m_mapGuildSkill.find(Param[0]);
		if (GSTemp == m_mapGuildSkill.end())
			return EC_NOERROR;
		...
		if (pPlayer->m_dwPvPUseablePoint < SKILL_RENEW_PRICE)
			return EC_NOERROR;

		if (!pPlayer->CheckGuildDuty(GSTemp->second.m_bType))
			return EC_NOERROR;

		if (itGuildSkill->second.m_EndTime > m_timeCurrent)
			return EC_NOERROR;
		...
		pPlayer->UsePvPoint(SKILL_RENEW_PRICE, 0, PVP_USEABLE);
		pPlayer->SendCS_PVPPOINT_ACK(pPlayer->m_dwPvPTotalPoint, pPlayer->m_dwPvPUseablePoint, 0, pPlayer->m_dwMonthPvPoint);
		pPlayer->SendCS_MONEY_ACK();

		itGuildSkill->second.m_EndTime = m_timeCurrent + (DAY_ONE * 31);

		if (GSTemp->second.m_bType)
			SendMW_UPDATEGUILDCOOLDOWN_ACK(pPlayer->m_dwGuildID, Param[0], pSkill->m_bLevel, TRUE, TRUE);
	}
	break;
	}
	...
	pPlayer->SendCS_GUILDSKILLUPDATE_ACK();
	pPlayer->SendCS_GUILDSKILLACTION_ACK(dwCharID, dwKEY, Action, Param);

	return EC_NOERROR;
}

DWORD CTMapSvrModule::OnMW_UPDATEGUILDCOOLDOWN_REQ(LPPACKETBUF pBUF)
{
	DWORD dwCharID;
	DWORD dwKEY;
	WORD wSkillID;
	...
	if (!pPlayer || !pPlayer->m_pMAP)
		return EC_NOERROR;
	/*
	auto GuildSkill = pPlayer->m_mapGuildSkill.find(wSkillID);
	if (GuildSkill == pPlayer->m_mapGuildSkill.end() && DidRenew)
	{
		...
		GUILDSKILL GSkill = { wSkillID, 0, bLevel, m_timeCurrent + (DAY_ONE * 31) };
		pPlayer->m_mapGuildSkill.insert(std::make_pair(wSkillID, GSkill));
	}

	if (DidUse && !DidRenew)
	{
		auto Skill = pPlayer->FindTSkill(wSkillID);
		if (Skill) {
			pPlayer->SkillUse(Skill, m_dwTick);
			pPlayer->SendCS_SKILLBUY_ACK(SKILL_SUCCESS, wSkillID, bLevel, Skill->m_dwUseTick);
		}
	}

	GuildSkill = pPlayer->m_mapGuildSkill.find(wSkillID);
	if (GuildSkill != pPlayer->m_mapGuildSkill.end()) {
		GuildSkill->second.m_bLevel = bLevel;
		if (DidUse && DidRenew)
			GuildSkill->second.m_EndTime = m_timeCurrent + (DAY_ONE * 31);

		CTSkill* pSkill = pPlayer->FindTSkill(wSkillID);
		if (pSkill) {
			pSkill->m_bLevel = bLevel;
			pPlayer->SendCS_SKILLBUY_ACK(SKILL_SUCCESS, wSkillID, bLevel);
		}
	}
	*/
...
DWORD CTMapSvrModule::OnMW_ADDCOOLDOWN_REQ(LPPACKETBUF pBUF)
{
	DWORD dwGuildID;
	WORD SkillID;
	...
	auto pSkill = FindTSkill(SkillID);
	if (!pSkill)
		return EC_NOERROR;

	auto GuildCD = m_mapGuildCooldown.find(dwGuildID);
	if (GuildCD == m_mapGuildCooldown.end())
	{
		tagGUILDCOOLDOWN Cooldown;
		Cooldown.m_vSkill.push_back(std::make_pair(SkillID, GetTickCount() + pSkill->m_dwReuseDelay));
		m_mapGuildCooldown.insert(std::make_pair(dwGuildID, Cooldown));
	}
	else
		(*GuildCD).second.m_vSkill.push_back(std::make_pair(SkillID, GetTickCount() + pSkill->m_dwReuseDelay));

	return EC_NOERROR;
}

std::vector<GUILDSKILL> CTMapSvrModule::GetGuildSkill(DWORD dwCharID, BYTE bDuty, DWORD dwGuildID)
{
	std::vector<GUILDSKILL> Buffer; Buffer.clear();
	CString dbug;
	...
	{
		_AtlModule.LogSmartly("VICE");
		DEFINE_QUERY(&m_db, CTBLGuildSkillDuty);
		{
			for (auto GuildSkill : m_mapGuildSkill)
			{
				query->m_wSkillID = GuildSkill.second.m_wSkillID;
				query->m_dwGuildID = dwGuildID;
				if (query->Open())
				{
					if (bDuty >= GuildSkill.second.m_bType)
					{
						dbug.Format("Bigger. bDuty : %d, Type : %d", bDuty, GuildSkill.second.m_bType);
						_AtlModule.LogSmartly(dbug);
						while (query->Fetch()) {
							Buffer.push_back(GUILDSKILL{ query->m_wSkillID, bDuty, query->m_bLevel, __DBTOTIME(query->m_tEndTime) });
							_AtlModule.LogSmartly(":D fetching");
						}
					}
				}
	...
	DEFINE_QUERY(&m_db, CTBLGuildSkillMember);
	{
		query->m_dwCharID = dwCharID;
		...
		{
			while (query->Fetch())
				Buffer.push_back(GUILDSKILL{ query->m_wSkillID, GUILD_DUTY_NONE, query->m_bLevel, __DBTOTIME(query->m_tEndTime) });
