							  ...
							  BYTE bProgress)
{
	...
	for( size_t i=0 ; i < CTachyonRes::m_vTLOADINGSCENE.size() ; ++i )
	{
						   ...
						   int nTotal) 
{
	...
	while( dwPOS < dwLENGTH )
	{
		...
		CString pos;
		pos.Format("Textury\\%u.png", dwPOS);
							  ...
							  int nTotal)
{
	...
	while (dwPOS < dwLENGTH)
	{
		...
		/*CString pos;
		pos.Format("ImgDecomp\\%u.png", dwID);

		pTEX->SaveImageFile(pos, D3DXIFF_PNG, pTEX->GetTEX());*/
	...
	for( it = m_mapTEX.begin(); it != m_mapTEX.end(); it++)
	{
		...
		if(pTEX)
		{
			...
			for( int i=0; i<nCount; i++)
			{
				...
				CT3DTexture *pText = nullptr;

				if (finder != m_mapTEXSRC.end())
					pText = static_cast<CT3DTexture*>((*finder).second);

				pTEX->SetTexturePtr( i, pText);
			}
		}
	}
}
	...
	for( it = m_mapIMG.begin(); it != m_mapIMG.end(); it++)
	{
		...
		if(pIMG)
		{
			...
			for( int i=0; i<nCount; i++)
			{
				...
				CD3DImage *pImg = nullptr;

				if (finder != m_mapIMGSRC.end())
					pImg = static_cast<CD3DImage*>((*finder).second);

				pIMG->m_vImage[i] = pImg;
			}
		}
	}
}
	...
	for( it = m_mapANI.begin(); it != m_mapANI.end(); it++)
	{
		...
		if(pANI)
		{
			...
			for( int i=0; i<nCount; i++)
			{
				...
				CTachyonAnimation *pAni = nullptr;

				if (finder != m_mapANISRC.end())
					pAni = static_cast<CTachyonAnimation*>((*finder).second);

				pANI->m_vAni[i]->m_pAni = pAni;
			}
		}
	}
}
	...
	for( it = m_mapOBJ.begin(); it != m_mapOBJ.end(); it++)
	{
		...
		if(pOBJ)
		{
			...
			for( itSFX = pOBJ->m_mapSFX.begin(); itSFX != pOBJ->m_mapSFX.end(); itSFX++)
			{
				...
				if (itRES != m_mapSFX.end())
					pSFX = static_cast<LPSFX>((*itRES).second);

				(*itSFX).second->m_pSFX = pSFX;
			}
			...
			for( itACT = pOBJ->m_mapACTION.begin(); itACT != pOBJ->m_mapACTION.end(); itACT++)
			{
				...
				for( itANI = pACT->m_mapANI.begin(); itANI != pACT->m_mapANI.end(); itANI++)
				{
					...
					LPANISET pAniSet = nullptr;

					if (finder != m_mapANI.end())
						pAniSet = static_cast<LPANISET>((*finder).second);

					(*itANI).second->m_pANI = pAniSet;
			...
			for( itCLK = pOBJ->m_mapCLKIND.begin(); itCLK != pOBJ->m_mapCLKIND.end(); itCLK++)
			{
				...
				for( itCL = pCLK->m_mapCLOTH.begin(); itCL != pCLK->m_mapCLOTH.end(); itCL++)
				{
					...
					for( itMESH = pCL->m_mapMESH.begin(); itMESH != pCL->m_mapMESH.end(); itMESH++)
					{
						...
						CTachyonMesh *pMesh = nullptr;

						if (finder != m_mapMESH.end())
							pMesh = static_cast<CTachyonMesh*>((*finder).second);
