...
#define REG_SET_CHATTITLE   "CHATTITLE%d"
...
#define REG_SET_OUTMAXLIST			"OUTERFRAMENUM"
#define REG_SET_OUTCHATPOSX		"OUTERCHATPOSX%d"
#define REG_SET_OUTCHATPOSY		"OUTERCHATPOSY%d"
#define REG_SET_OUTCHATTAB			"OUTERCHATTAB%d"
#define REG_SET_OUTCHATFLAG		"OUTERCHATFLAG%d"
#define REG_SET_OUTCHATBASIS		"OUTERCHATBASIS%d"
#define REG_SET_OUTCHATHEIGHT	"OUTERCHATHEIGHT%d"
#define REG_SET_OUTCHATTITLE   "OUTERCHATTITLE%d"
#define REG_SET_FRAMENUM "FRAMENUM"
#define REG_SET_FRAMEPOSX "CHATFRAMEPOSX"
#define REG_SET_FRAMEPOSY "CHATFRAMEPOSY"
#define REG_SET_FRAMEBASIS "CHATFRAMEBASIS"
// ===============================================================================
...
// ===============================================================================
CTChatFrame::CTChatFrame(TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc, CTClientGame* pMainGame, TCMLParser *pParser)
	  ...
	  m_dwChatTargetID(0), m_pChatEditBack(NULL), m_pChatTabBack(NULL), m_dwNotifyTime(0)
{
	...
	{
		ID_CTRLINST_CHATNEW_TAB0,
		ID_CTRLINST_CHATNEW_TAB1,
		ID_CTRLINST_CHATNEW_TAB2,
		ID_CTRLINST_CHATNEW_TAB3,
		ID_CTRLINST_CHATNEW_TAB4,
		ID_CTRLINST_CHATNEW_TAB5
	};

	ZeroMemory(m_vPopup, sizeof(m_vPopup));
	CTClientUIBase *pChat = new CTClientUIBase( this, pParser->FindFrameTemplate(ID_FRAME_CHAT));

	CTClientUIBase *pChatBox = new CTClientUIBase( this, CTClientGame::GetInstance()->m_pTParser->FindFrameTemplate(ID_FRAME_CHATBOX));

	m_pListTemplate = FindKid( ID_CTRLINST_CHAT_LIST );
	CRect rc;
	m_pListTemplate->GetComponentRect(&rc);
	rc.right =440;
	m_pListTemplate->SetComponentRect(rc);
	m_pListTemplate->GetComponentRect(&m_rcDefaultList);	
	RemoveKid( m_pListTemplate );
	m_pListTemplate->SaveFramedata();

	FRAMEDESC_SHAREDPTR pSizeBarFrameDesc = CTClientGame::GetInstance()->m_pTParser->FindFrameTemplate( ID_FRAME_CHATNEW );
	m_pChatFrame = pSizeBarFrameDesc;
	TFrame* pSizeBarFrame = new TFrame( NULL, pSizeBarFrameDesc );

	TComponent* pListTemplate2 = pChatBox->FindKid( ID_CTRLINST_CHAT_LIST );
	RemoveKid( pListTemplate2 );

	TButton* pBtn = (TButton*) pSizeBarFrame->FindKid( ID_CTRLINST_BTN_UP );
	FRAMEDESC_WEAKPTR pSizeBarButtonDesc = pBtn->m_pDESC;

	TScroll* pBtns = (TScroll*)  pSizeBarFrame->FindKid( ID_CTRLINST_CHAT_LIST );
	FRAMEDESC_WEAKPTR pSizeBarScrollDesc = pBtns->m_pDESC;

	TComponent* pBottom =  pSizeBarFrame->FindKid( ID_CTRLINST_CHATNEW_BOTTOM );
	FRAMEDESC_WEAKPTR pSizeBarBottomDesc = pBottom->m_pDESC;
	
	TComponent* pTop =  pSizeBarFrame->FindKid( ID_CTRLINST_CHATNEW_TOP );
	FRAMEDESC_WEAKPTR pSizeBarTopDesc = pTop->m_pDESC;

	TButton* pTool = (TButton*) pSizeBarFrame->FindKid(ID_CTRLINST_CHATNEW_TOOL);
	FRAMEDESC_WEAKPTR pBarToolDesc = pTool->m_pDESC;

	TButton* pTab = (TButton*) pSizeBarFrame->FindKid(ID_CTRLINST_CHATNEW_TAB);
	FRAMEDESC_WEAKPTR pBarTabDesc = pTab->m_pDESC;

	RemoveKid(FindKid(ID_CTRLINST_CHATNEW_BOTTOM));
	RemoveKid(FindKid(ID_CTRLINST_CHATNEW_TOP));
	RemoveKid(FindKid(ID_CTRLINST_CHATNEW_TOOL));
	RemoveKid(FindKid(ID_CTRLINST_CHATNEW_TAB));
	RemoveKid(FindKid(ID_CTRLINST_BTN_UP));

	FRAMEDESC_SHAREDPTR pChatMenuDesc = CTClientGame::GetInstance()->m_pTParser->FindFrameTemplate( ID_FRAME_CHATMENU );

	m_pChatMenuFrame = pChatMenuDesc;

	CString title;
	for( INT i=0; i<TCHAT_SET_COUNT; ++i)
	{
		CPoint cp;
		TComponent* pTabTemplate;

		pTabTemplate = pChatBox->FindKid( dwChatTab[i] );
		pChatBox->RemoveKid( pTabTemplate );

		pTabTemplate->GetComponentRect(&m_vTabRect[i]);
		pTabTemplate->ShowComponent(FALSE);

		CTChatTabButton* m_vTabs = new CTChatTabButton(this, pTabTemplate->m_pDESC.lock());
		m_vTabs->m_id = GetUniqueID(NULL);
		
		//delete pTabTemplate;

		CTChatList*	m_vList = new CTChatList(this, m_pListTemplate->m_pDESC.lock(), pParent);
		m_vList->m_id = GetUniqueID(pParent);
		m_vList->m_bHitTestType = HITTEST_TYPE_RECT;

		m_vList->m_pSizeBarUP = new TButton( m_vList, pSizeBarButtonDesc );
		m_vList->m_pSizeBarDOWN = new TButton( m_vList, pSizeBarButtonDesc );
		
		m_vList->m_pBottom = new  TComponent(m_vList,pSizeBarBottomDesc);
		m_vList->m_pTop = new  TComponent(m_vList,pSizeBarTopDesc);

		m_vList->m_pSetting = new TButton(m_vList,pBarToolDesc);
		m_vList->m_pTab = new TButton(m_vList,pBarTabDesc);

		m_vList->m_pChatMenu = new CTChatMenuNew(m_vList, pChatMenuDesc, m_vList);
		

		m_vList->m_pSizeBarUP->m_id = GetUniqueID( m_vList);
		m_vList->m_pSizeBarDOWN->m_id = GetUniqueID(m_vList);
		
		m_vList->m_pBottom->m_id = GetUniqueID( m_vList);
		m_vList->m_pTop->m_id = GetUniqueID( m_vList);
		
		m_vList->m_pSetting->m_id = GetUniqueID( m_vList);
		m_vList->m_pTab->m_id = GetUniqueID( m_vList);

		m_vList->m_pChatMenu->m_id = GetUniqueID( m_vList);

		m_vList->AddKid( m_vList->m_pBottom );
		m_vList->AddKid( m_vList->m_pTop );
		m_vList->AddKid( m_vList->m_pSizeBarUP );
		m_vList->AddKid( m_vList->m_pSizeBarDOWN );
		m_vList->AddKid( m_vList->m_pSetting);
		m_vList->AddKid( m_vList->m_pTab);
		m_vList->AddKid( m_vList->m_pChatMenu);
		
		CRect rt;
		m_vList->GetComponentRect(&rt);

		TFRAMEOFFSET offset;
		CTClientUIBase::GetUIOffset( CTClientUIBase::m_rtDEFAULT_SCREEN, rt, &offset);
		m_vList->m_bBasisPoint = offset.m_bBasisPoint;
		m_vList->m_vCompOffset = offset.m_vOffset;
				
		title = CTChart::LoadString( (TSTRING) dwTabTitle[i]);
        m_vList->Init( i, title, DEFAULT_CHAT_FLAGS[i], m_rcDefaultList, NULL);
		m_vListArray.push_back(m_vList);
		m_vTabs->Init( i, m_vListArray[i] );
		m_vTabArray.push_back(m_vTabs);

		AddKid( m_vTabArray[i] );
		AddKid( m_vListArray[i] );
	}

	TComponent* pFrame = pChatBox->FindKid(ID_CTRLINST_CHATNEW_BOXFRAME);
	FRAMEDESC_WEAKPTR pFrameDESC = pFrame->m_pDESC;

	m_pNotifyList = new CTChatList(this, pListTemplate2->m_pDESC.lock(), pParent);
	...
	m_pNotifyList->m_vCompOffset = CPoint(300, -600);
	m_pNotifyList->m_pFrame = new  TComponent(m_pNotifyList,pFrameDESC );
	m_pNotifyList->m_pFrame->m_id = GetUniqueID( m_pNotifyList);
	m_pNotifyList->AddKid(m_pNotifyList->m_pFrame);
	
	m_pUpgradeShowList = new CTChatList(this, pListTemplate2->m_pDESC.lock(), pParent);
	...
	m_pUpgradeShowList->m_vCompOffset = CPoint(0, -444);
	m_pUpgradeShowList->m_pFrame = new  TComponent(m_pUpgradeShowList,pFrameDESC );
	m_pUpgradeShowList->m_pFrame->m_id = GetUniqueID( m_pUpgradeShowList);
	m_pUpgradeShowList->AddKid(m_pUpgradeShowList->m_pFrame);

	m_pChatEditCtrl = static_cast<TEdit*>( pChatBox->FindKid(ID_CTRLINST_CHAT_INPUT) );
	m_pChatWhisperTargetEditCtrl = static_cast<TEdit*>( pChatBox->FindKid(ID_CTRLINST_CHAT_TYPE_EDIT) );
	m_pChatMode = pChatBox->FindKid( ID_CTRLINST_CHAT_TYPE );
	AddKid(m_pChatEditCtrl);
	AddKid(m_pChatWhisperTargetEditCtrl);
	AddKid(m_pChatMode);

	m_pChatTypeCtrl = pChatBox->FindKid(ID_CTRLINST_CHATNEW_TYPECTRL);
	m_pChatTypeCtrl->m_strText = CTChart::LoadString( TSTR_CHAT_NORMAL );
	AddKid(m_pChatTypeCtrl);

	m_pChatImeCtrl = static_cast<TButton*>( pChatBox->FindKid(ID_CTRLINST_CHAT_LANGUAGE) );
	m_pChatImeCtrl->SetStateButton();
	
	m_pChatEditBack = pChatBox->FindKid(ID_CTRLINST_CHAT_INPUT_BASE);
	AddKid(m_pChatEditBack);

	AddKid(m_pChatImeCtrl);
	NotifyImeLocalModeChange();

	m_pChatTabBack =  pChat->FindKid(ID_CTRLINST_CHAT_BOTTOM);
	TComponent *pBack = pChatBox->FindKid(ID_CTRLINST_CHATNEW_BOXFRAME);
	pChatBox->RemoveKid(pBack);
	m_bSND = FALSE;
	m_ptPrev = CPoint(0,0);
	m_tip.Reset();
	m_blDragging = FALSE;
	...
	case POPUP_CHATTYPE:
		{
			...
			for(BYTE i=0; i<TCHAT_POPUP_COUNT; ++i)
			{
				...
				switch(i)
				{
				case 0	:  dwColor = TCOLOR_CHAT_WHISPER; break;
				case 1	:  dwColor =  TCOLOR_CHAT_NEAR; break;
				case 2	:  dwColor =  TCOLOR_CHAT_MAP; break;
				case 3	:  dwColor = TCOLOR_CHAT_PARTY; break;
				case 4	:  dwColor =  TCOLOR_CHAT_GUILD;break;
				case 5	:  dwColor = TCOLOR_CHAT_FORCE;break;
				case 6	:  dwColor =  TCOLOR_CHAT_OPERATOR;break;
				}

				m_vPopup[ePopup]->SetButtonColor( i, dwColor );//p5idat
			}
			...
			m_vPopup[ePopup]->MoveComponent( CPoint(pt.x-15, pt.y-sz.cy) );
...
void CTChatFrame::ShowPopup(PopupEnum ePopup, BOOL bShow)
{
	...
	if( bShow && POPUP_TO_LIST[ePopup] != T_INVALID )
	{
		...
		CTChatTabButton* pTab = FindTab( m_vListArray[nListIndex] );
...
void CTChatFrame::UpdateChatFlagToPopup()
{
	...
	for(INT i=0; i<POPUP_COUNT; ++i)
	{
		...
		for(INT j=0; j<nBtnCnt; ++j)
		{
			...
			m_vPopup[i]->SelectButton(j, m_vListArray[nListIdx]->CheckChatFlag(dwBtnFlag));
		}
		break;
	}
}
...
void CTChatFrame::SetHeightAllInnerChatList( INT nHeight )
{
	...
	for( int i=0 ; i < m_vListArray.size() ; ++i )
	{
		if( m_vListArray[ i ] && !m_vListArray[ i ]->IsOuter() )
		{
			
			m_vListArray[ i ]->SetHeight( nHeight );

			CRect rtDefault = GetDefaultListRect();

			m_vListArray[ i ]->MoveComponent( CPoint(
				rtDefault.left,
				rtDefault.bottom - m_vListArray[ i ]->m_rc.Height()) );

			CRect rtDefaults ;
			CPoint pt;
			m_vListArray[i]->GetComponentPos(&pt);
			pt.y += -37;
			m_vTabArray[i]->GetComponentRect(&rtDefaults);
			if(m_vTabArray[i])
				{
				m_vTabArray[ i ]->MoveComponent( CPoint(
				rtDefaults.left,
				pt.y ));
				}		
...
void CTChatFrame::Update(DWORD dwTick)
{
	if( IsVisible() )
	{
		BYTE bChatMenu = FALSE;

		for(INT i=0; i<m_vListArray.size(); i++)
			if(m_vListArray[i]->IsVisible())
				bChatMenu = TRUE;

		if( !IsChatEditON() && !bChatMenu)
		{
			m_dwTick += dwTick;
	...
	for( UINT i=0; i<m_vTabArray.size(); ++i )
		m_vTabArray[i]->Update(dwTick);
...
{
	ChangeSelection( m_vTabArray[ set ] );
}
// ===============================================================================
void CTChatFrame::SwapTab(CTChatTabButton* pTab1, CTChatTabButton* pTab2)
{
	...
	m_vTabArray[iSlot1] = pTab2;
	m_vTabArray[iSlot2] = pTab1;
...
void CTChatFrame::AddTabAndList(CTChatTabButton* pTab)
{
	...
	if( pTab != m_vTabArray[m_iTabCount] )
		SwapTab(pTab, m_vTabArray[m_iTabCount]);
...
void CTChatFrame::RemoveTabAndList(CTChatTabButton* pTab)
{
	...
	{
		m_vTabArray[i] = m_vTabArray[i+1];
		m_vTabArray[i]->SetSlot(i);
	}

	m_vTabArray[m_iTabCount] = pTab;
...
BOOL CTChatFrame::SnapList(CTChatList* pList, CRect& outRect)
{
	...
	if( m_iTabCount < TCHAT_SET_COUNT )
	{
		for(INT i=0; i<m_vListArray.size(); ++i)
		{
			CTChatList* pCur = m_vListArray[i];
...
void CTChatFrame::ResetSetting()
{
	...
	for( INT i=0; i<TCHAT_SET_COUNT; ++i )
	{
		...
		str.Format( _T(REG_SET_CHATTITLE), i);
		theApp.WriteProfileString(_T(REG_KEY), str, CTChart::LoadString( (TSTRING)dwChatString[i] ));
	}

	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_NOTCHATPOSX), (DWORD)(-211));
	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_NOTCHATPOSY), (DWORD)(132));
	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_NOTCHATBASIS), TBASISPOINT_CENTER_TOP);
	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_SHOWCHATPOSX), (DWORD)(-440));
	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_SHOWCHATPOSY), (DWORD)(-79));
	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_SHOWCHATBASIS), TBASISPOINT_RIGHT_MIDDLE);
	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_RESET), CHATFRAME_RESET_IDENTITY);


	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_OUTMAXLIST), 1);
	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_FRAMEPOSX), (DWORD)(0));
	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_FRAMEPOSY), (DWORD)(-335));
	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_FRAMEBASIS), TBASISPOINT_LEFT_BOTTOM);

	//Result Chat
	{
		str.Format(_T(REG_SET_OUTCHATFLAG), 0);
		theApp.WriteProfileInt(_T(REG_KEY), str, TCHAT_FLAG_INFO);

		str.Format(_T(REG_SET_OUTCHATPOSX), 0);	
		theApp.WriteProfileInt(_T(REG_KEY), str, DWORD(-440));

		str.Format(_T(REG_SET_OUTCHATPOSY), 0);	
		theApp.WriteProfileInt(_T(REG_KEY), str, DWORD(11));

		str.Format(_T(REG_SET_OUTCHATBASIS), 0);
		theApp.WriteProfileInt(_T(REG_KEY), str, TBASISPOINT_RIGHT_MIDDLE);

		str.Format( _T(REG_SET_OUTCHATHEIGHT), 0);
		theApp.WriteProfileInt(_T(REG_KEY), str, m_rcDefaultList.Height() );

		str.Format( _T(REG_SET_OUTCHATTITLE), 0);
		theApp.WriteProfileString(_T(REG_KEY), str, CTChart::LoadString( TSTR_CHAT_TAB_RESULT ));
	}
	
	str.Format( _T(REG_SET_FRAMENUM), TCHAT_SET_COUNT);
	theApp.WriteProfileInt(_T(REG_KEY), str, TCHAT_SET_COUNT );
...
void CTChatFrame::LoadSetting( BOOL bReset )
{
	...
	for( INT i=0; i<nCount; ++i )
	{
		...
		str.Format(_T(REG_SET_CHATTITLE), i);
		CString strTitle = theApp.GetProfileString(_T(REG_KEY), str, "");
		...
		m_vListArray[i]->SetDragEnable(TRUE);
		m_vListArray[i]->m_vCompOffset = CPoint(x,y);
		m_vListArray[i]->m_bBasisPoint = basis;

		if(height)
			m_vListArray[i]->SetHeight( height );

		CRect rtDefault = GetDefaultListRect();
		CRect rc;
		m_vListArray[i]->m_strTitle = strTitle;
		m_vListArray[i]->GetComponentRect( &rc );
		m_vListArray[i]->MoveComponent( CPoint(rtDefault.left, rtDefault.bottom - rc.Height()) );
		...
		if( flag != T_INVALID )
			m_vListArray[i]->SetChatFlag(flag);
	}

	for( INT i=0; i<nCount; ++i )
	{
		CTChatList* pList = m_vListArray[vTAB[i]];
        
		m_vTabArray[i]->Init(i, pList);
		m_vTabArray[i]->ShowComponent(FALSE);
	...
	x = theApp.GetProfileInt(_T(REG_KEY), _T(REG_SET_FRAMEPOSX), T_INVALID);
	LONG y = theApp.GetProfileInt(_T(REG_KEY), _T(REG_SET_FRAMEPOSY), T_INVALID);
	BYTE basis = theApp.GetProfileInt( _T(REG_KEY), _T(REG_SET_FRAMEBASIS), 0xFF);

	m_vCompOffset = CPoint(x,y);
	m_bBasisPoint = basis;
	
	for(int l=TCHAT_SET_COUNT; l<m_vListArray.size(); l++)
		DeleteList(m_vListArray[l]);

	UINT nCountOut = theApp.GetProfileInt(_T(REG_KEY), _T(REG_SET_OUTMAXLIST), 0);
	for( INT i=0; i< nCountOut; ++i )
	{
		str.Format( _T(REG_SET_OUTCHATHEIGHT), i );
		LONG height = theApp.GetProfileInt( _T(REG_KEY), str, m_rcDefaultList.Height() );
				
		str.Format(_T(REG_SET_OUTCHATPOSX), i);
		LONG x = theApp.GetProfileInt(_T(REG_KEY), str, T_INVALID);

		str.Format(_T(REG_SET_OUTCHATTITLE), i);
		CString strTitle = theApp.GetProfileString(_T(REG_KEY), str, "");


		str.Format(_T(REG_SET_OUTCHATPOSY), i);
		LONG y = theApp.GetProfileInt(_T(REG_KEY), str, T_INVALID);


		str.Format( _T(REG_SET_OUTCHATBASIS), i);
		BYTE basis = theApp.GetProfileInt( _T(REG_KEY), str, 0xFF);


		str.Format(_T(REG_SET_OUTCHATFLAG), i);
		DWORD flag = theApp.GetProfileInt(_T(REG_KEY), str, T_INVALID);

		AddList(flag);
		
		m_vListArray[i+TCHAT_SET_COUNT]->m_vCompOffset = CPoint(x,y);
		m_vListArray[i+TCHAT_SET_COUNT]->m_bBasisPoint = basis;

		m_vListArray[i+TCHAT_SET_COUNT]->SetHeight( height );
		m_vListArray[i+TCHAT_SET_COUNT]->SetOuter(TRUE);
		m_vListArray[i+TCHAT_SET_COUNT]->SetDragEnable(TRUE);
		m_vListArray[i+TCHAT_SET_COUNT]->ResetPosition();
		m_vListArray[i+TCHAT_SET_COUNT]->m_strTitle = strTitle;

		if( flag != T_INVALID )
			m_vListArray[i+TCHAT_SET_COUNT]->SetChatFlag(flag);
	}
...
void CTChatFrame::SaveSetting()
{
	...
	for( INT i=0; i<m_vListArray.size(); ++i )
	{
		if( !m_vListArray[i]->IsOuter() )
		{
			
			offset = m_vListArray[i]->m_vCompOffset;
			basis = m_vListArray[i]->m_bBasisPoint;
		
			

		str.Format(_T(REG_SET_CHATPOSX), k);	
		theApp.WriteProfileInt(_T(REG_KEY), str, offset.x);

		str.Format(_T(REG_SET_CHATPOSY), k);	
		theApp.WriteProfileInt(_T(REG_KEY), str, offset.y);

		str.Format( _T("CHATBASIS%d"), k);
		theApp.WriteProfileInt(_T(REG_KEY), str, basis);

		str.Format(_T(REG_SET_CHATFLAG), k);
		theApp.WriteProfileInt(_T(REG_KEY), str, m_vListArray[i]->GetChatFlag());

		str.Format(_T(REG_SET_CHATTAB), k);	
		theApp.WriteProfileInt(_T(REG_KEY), str, m_vTabArray[i]->GetChatListIndex());

		CRect rt;
		m_vListArray[i]->GetComponentRect( &rt );
		str.Format(_T(REG_SET_CHATHEIGHT), k );
		theApp.WriteProfileInt(_T(REG_KEY), str, rt.Height() );


		str.Format( _T(REG_SET_CHATTITLE), k);
		theApp.WriteProfileString(_T(REG_KEY), str, m_vListArray[i]->GetTitle());
	...
	offset = this->m_vCompOffset;
	basis = this->m_bBasisPoint;

	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_FRAMEPOSX), offset.x);
	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_FRAMEPOSY), offset.y);
	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_FRAMEBASIS), basis);



	k=0;
	theApp.WriteProfileInt(_T(REG_KEY), _T(REG_SET_OUTMAXLIST), m_vListArray.size()-TCHAT_SET_COUNT);
	...
	for( INT i=0; i<m_vListArray.size(); ++i )
	{
		...
		str.Format(_T(REG_SET_OUTCHATPOSX), k);	
		theApp.WriteProfileInt(_T(REG_KEY), str, offset.x);

		str.Format(_T(REG_SET_OUTCHATPOSY), k);	
		theApp.WriteProfileInt(_T(REG_KEY), str, offset.y);

		str.Format( _T("OUTERCHATBASIS%d"), k);
		theApp.WriteProfileInt(_T(REG_KEY), str, basis);

		str.Format(_T(REG_SET_OUTCHATFLAG), k);
		theApp.WriteProfileInt(_T(REG_KEY), str, m_vListArray[i]->GetChatFlag());
		
		CRect rt;
		m_vListArray[i]->GetComponentRect( &rt );
		str.Format(_T(REG_SET_OUTCHATHEIGHT), k );
		theApp.WriteProfileInt(_T(REG_KEY), str, rt.Height() );


		str.Format( _T(REG_SET_OUTCHATTITLE), k);
		theApp.WriteProfileString(_T(REG_KEY), str, m_vListArray[i]->GetTitle());
	...
	theApp.WriteProfileInt(_T(REG_KEY), REG_SET_FRAMENUM, m_vListArray.size());
}
// ===============================================================================
CTChatTabButton* CTChatFrame::GetHitTab(const CPoint& pt)
{
	if( HitTest(pt) )
	{
		...
		{
			if( m_vTabArray[i]->HitTest(pt) )
				return m_vTabArray[i];
		}
	}
...
CTChatTabButton* CTChatFrame::FindTab(CTChatList* pList)
{
	...
	for(; i < e ; ++i)
	{
		if( m_vTabArray[i]->GetChatList() == pList )
			return m_vTabArray[i];
	}
	return NULL;
}
...
CTChatList* CTChatFrame::GetHitList(const CPoint& pt)
{
	for( INT i=0; i<m_vListArray.size(); ++i )
	{
		if( m_vListArray[i]->TList::HitTest(pt) )
			return m_vListArray[i];
	}
	...
	for( INT i=0; i<POPUP_COUNT; ++i )
	{
		if( m_vPopup[i] == pPopup )
		{
			if( POPUP_TO_LIST[i] != T_INVALID )
				return m_vListArray[ POPUP_TO_LIST[i] ];
		}
	}
...
{	
	for( INT i=0 ; i < m_vListArray.size() ; ++i )
		if(m_vListArray[ i ]->GetChatFlag() == dwChatFlag)
			return m_vListArray[i];
...
void CTChatFrame::ResetChatMsg(BOOL bEnable)
{
	...
	SetHideBackgroundOfList(!bEnable);
...
void CTChatFrame::ShowWhisperTargetInput( BOOL bShow )
{
	...
	else
	{
		...
		CTClientUIBase::OnLoseFocus(m_pChatWhisperTargetEditCtrl);
		ForceFocus();
...
void CTChatFrame::LoseFocusWhisperTargetInput( BOOL bCancel )
{
	...
	OnLoseFocus(m_pChatWhisperTargetEditCtrl);
	ForceFocus();
...
CString CTChatFrame::ProcChatMsgInEdit()
{
    ...
    if( nLen > 1 && strMSG.GetAt(0) == CHAT_CMD_MARK )
    {
        ...
        {
#ifdef USE_DYNAMIC_COMMANDS
            CTClientSession pSession = m_pMainGame->GetSession();


            pSession->SendCS_DYNCMD_REQ(m_pMainGame->GetMainChar()->GetName(), strMSG);
#else
            ...
            ChatSysMSG(strTYPE, pMainChar->GetName(), strMSG, TCOLOR_ERROR, TCHAT_FLAG_INFO, ID_SND_ERROR, MEDIA_TSOUND);
#endif
        }
							  ...
							  DWORD dwSenderID )
{
	...
	while( !strLINE.IsEmpty() )
	{
		...
		case TCHAT_FLAG_SHOW:
			{
				szITEM = m_vListArray[0]->GetItemSize(TCHATCOL_INFO);
				m_vListArray[0]->GetTextExtentPoint( strTYPENAME, szHeader );
		...
		default:
			{
				szITEM = m_vListArray[0]->GetItemSize(TCHATCOL_MSG);
				szITEM.cx -= TCHAT_MARGINE;
			}
			break;
		};

		strResult += pTextLinker->SplitTextByComp(m_vListArray[0], szITEM.cx, strPMSG, vLINES);
		...
		for( INT i=0; i<nLineSize; ++i)
		{
			...
			{
				for( INT j=0; j<m_vListArray.size(); ++j )
				{
					CTChatList* pList = m_vListArray[j];
					if( !pList->CheckChatFlag(dwChatFlag) )
						continue;

					if( CHAT_SHOW_NEWMSG[pList->GetIndexInGroup()] && !m_vTabArray[j]->IsSelect() )
						m_vTabArray[j]->NotifyNewMsg();
	...
	if( !m_bChatFrameVisible )
	{
		...
		{
			BOOL bFind = FALSE;
			for( INT i=0; i<m_vTabArray.size(); ++i )
			{
				CTChatList* pList = m_vTabArray[i]->GetChatList();
				if( (m_vTabArray[i]->IsSelect() || pList->IsOuter()) &&
...
void CTChatFrame::SetHideBackgroundOfList(BOOL bHide)
{
	...
	for(INT i=0; i<m_vListArray.size(); ++i)
	{
		m_vListArray[i]->SetHideBackground(bHide);
		if( !m_vListArray[i]->m_bHideBackground && m_vListArray[i]->m_bVisible)
			visible =TRUE;
	}	

	for(INT i=0; i<m_vListArray.size(); ++i)
	{
		if( !m_vListArray[i]->m_bHideBackground && visible)	
			m_vTabArray[i]->ShowComponent(TRUE);
		else
			m_vTabArray[i]->ShowComponent(FALSE);
	}	
}
...
void CTChatFrame::ShowComponent(BOOL bVisible)
{
	...
	{
		if( m_vListArray[i]->IsOuter() )
			m_vListArray[i]->ShowComponent(bVisible);
		else
		{
			if( bVisible )
				m_vListArray[i]->ShowComponent(i == m_pSelectTab->GetChatListIndex());
			else
				m_vListArray[i]->ShowComponent(FALSE);
		}
	}
...
BOOL CTChatFrame::HitTest( CPoint pt)
{
	...
	m_vTabArray[0]->ComponentToScreen(&rc);
	if( rc.PtInRect(pt) )
		return TRUE;
...
void CTChatFrame::OnLButtonDown(UINT nFlags, CPoint pt)
{
	...
	for( INT i=0; i<m_vListArray.size(); ++i)
	{
		if(m_vListArray[i]->m_pSizeBarUP->HitTest(pt))		
			m_bDraging = FALSE;		
	}		
}
// -------------------------------------------------------------------------------
...
void CTChatFrame::OnMouseMove(UINT nFlags, CPoint pt)
{
	...
	if( m_bLock )
	{
		if(!m_pChatEditCtrl->m_bVisible)
		{
			for( INT i=0 ; i < m_vListArray.size(); ++i )
				m_vListArray[i]->m_pScroll->ShowComponent(bHit);
		}			
		else
		for( INT i=0 ; i < m_vListArray.size(); ++i )
			m_vListArray[i]->m_pScroll->ShowComponent(TRUE);
		m_pHitTab = NULL;
	}
	else
	{
		SetHideBackgroundOfList(FALSE);
	}
}
// -------------------------------------------------------------------------------
...
void CTChatFrame::_LOG(LPCSTR func)
{
	...
	for( INT i=0; i<m_vTabArray.size(); ++i )
	{
		...
		temp.Format("TAB#%d(SLOT:%u VIS:%u) LIST#%u(NAME:%s OUT:%u VIS:%u POS:(%d,%d) SIZE:(%d,%d))\n", 
			i, 
			m_vTabArray[i]->GetSlot(), 
			m_vTabArray[i]->IsVisible(),
...
TEdit* CTChatFrame::GetCurEditMenu()
{
	if( !IsVisible() )
		return NULL;

	return m_vListArray[m_nCurTabIndex]->m_pChatMenu->GetCurEdit();
}

BOOL CTChatFrame::GetApplyCursor( LPTCURSOR_TYPE pbCursor )
{
	for( INT i=0 ; i < m_vListArray.size() ; ++i )
		if( m_vListArray[ i ]->GetApplyCursor( pbCursor ) )
			return TRUE;
	return FALSE;
}
...
TListItem* CTChatFrame::HitTestChatList( CPoint pt )
{
	for( INT i=0; i<m_vListArray.size(); ++i)
	{
		CTChatList* pList = m_vListArray[i];
...
void CTChatFrame::AddList(DWORD dwChatFlag)
{
	TFrame* pSizeBarFrame = new TFrame( nullptr, m_pChatFrame);
	TButton* pBtn = (TButton*) pSizeBarFrame->FindKid( ID_CTRLINST_BTN_UP );
	TScroll* pBtns = (TScroll*) pSizeBarFrame->FindKid( ID_CTRLINST_CHAT_LIST );
	TComponent* pBottom = pSizeBarFrame->FindKid( ID_CTRLINST_CHATNEW_BOTTOM );
	TComponent* pTop = pSizeBarFrame->FindKid( ID_CTRLINST_CHATNEW_TOP );
	...
	int i = m_vListArray.size();

	CTChatTabButton* m_vTabs = new CTChatTabButton(this, pTab->m_pDESC.lock());
	m_vTabs->m_id = GetUniqueID(NULL);

	CTChatList*	m_vList = new CTChatList(this, m_pListTemplate->GetSavedFramedata(), m_pParent);
	m_vList->m_id = GetUniqueID(m_pParent);
	m_vList->m_bHitTestType = HITTEST_TYPE_RECT;

	m_vList->m_pSizeBarUP = new TButton( m_vList, pBtn->m_pDESC);
	m_vList->m_pSizeBarDOWN = new TButton( m_vList, pBtn->m_pDESC);
		
	m_vList->m_pBottom = new  TComponent(m_vList, pBottom->m_pDESC);
	m_vList->m_pTop = new  TComponent(m_vList, pTop->m_pDESC);

	m_vList->m_pSetting = new TButton(m_vList, pTool->m_pDESC);
	m_vList->m_pTab = new TButton(m_vList, pTab->m_pDESC);

	m_vList->m_pChatMenu = new CTChatMenuNew(m_vList, m_pChatMenuFrame,m_vList);
	...
	m_vList->m_bBasisPoint = TBASISPOINT_CENTER_MIDDLE;
	m_vList->m_vCompOffset = CPoint(0, 0);
	m_vList->Init( i, "" , dwChatFlag, m_rcDefaultList, NULL);		
	m_vListArray.push_back(m_vList);
	AddKid( m_vListArray[i] );
	m_vListArray[i]->SetHeight( 125 );
	m_vListArray[i]->SetOuter(TRUE);
	m_vListArray[i]->SetDragEnable(TRUE);
	m_vListArray[i]->ResetPosition();
	m_vTabs->Init( i, m_vListArray[i] );
	m_vTabArray.push_back(m_vTabs);
...
void CTChatFrame::DeleteList(CTChatList *pList)
{
	int i;
	ClientChatList::iterator it, end;
	it = m_vListArray.begin();
	end = m_vListArray.end();
	...
	for(; it != end ; ++it )
	{
		...
		{
			RemoveKid(*it);
			i = (*it)->GetIndexInGroup();
			m_vListArray.erase( it );
			break;
		}
	}

	ClientChatTab::iterator its, ends;
	its = m_vTabArray.begin();
	ends = m_vTabArray.end();
	for(; its != ends ; ++its)
	{
		if ((*its)->GetChatList() == pList)
		{
			m_vTabArray.erase(its);
			break;
		}
	}


	pList->DeleteThisList();
}

TEdit* CTChatFrame::GetChatEditMenu()
{
	for(int i=0; i < m_vListArray.size(); i++)
	{
		TEdit *pEdit = m_vListArray[i]->m_pChatMenu->GetCurEdit();

		if(pEdit && m_vListArray[i]->m_pChatMenu->IsVisible())
			return pEdit;
	}
...
CTChatMenuNew* CTChatFrame::GetChatMenu()
{
	for(int i=0; i < m_vListArray.size(); i++)
	{
		TEdit *pEdit = m_vListArray[i]->m_pChatMenu->GetCurEdit();
		...
		if(pEdit && m_vListArray[i]->m_pChatMenu->IsVisible())
			return m_vListArray[i]->m_pChatMenu;
	}

	return NULL;
}

void CTChatFrame::ClearMenuFocus()
{
	for(int i=0; i < m_vListArray.size(); i++)
	{
		TEdit *pEdit = m_vListArray[i]->m_pChatMenu->GetCurEdit();
...
void CTChatFrame::SetInnerTextTab(CString strTitle)
{
	INT nIndex = -1;
	...
	if(nIndex != -1)
		m_vTabArray[nIndex]->m_strText = strTitle;
}
	  ...
	  m_dwChatTargetID(0), m_pChatEditBack(NULL), m_pChatTabBack(NULL), m_dwNotifyTime(0)
{
	...
	FRAMEDESC_SHAREDPTR pSizeBarFrameDesc = CTClientGame::GetInstance()->m_pTParser->FindFrameTemplate( ID_FRAME_SIZEBAR );
	TFrame* pSizeBarFrame = new TFrame( NULL, pSizeBarFrameDesc );
	TButton* pBtn = (TButton*) pSizeBarFrame->FindKid( ID_CTRLINST_BTN );
	FRAMEDESC_SHAREDPTR pSizeBarButtonDesc = pBtn->m_pDESC;
