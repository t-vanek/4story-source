...
void CTClientNET::DefaultPosition(CPoint* vBASIS, BOOL bRestore)
{
	...
	static const DWORD TCHARFRAME_COMP_COUNT = 20;
	...
	else
	{
		// ID_FRAME_LOGIN_NEW
		{
			...
			{ ID_CTRLINST_LOGO, TBASISPOINT_CENTER_BOTTOM },
			...
			for (INT i = 0; i < TLOGINFRAME_COMP_COUNT; ++i)
			{
				...
				if (i == 9)
				{
					pCOMP->LoadOwnIMG(".\\Data\\IMG\\Custom\\Logo.png");
				}
		...
		// ID_FRAME_CHAR_NEW
		{
			...
			{ ID_CTRLINST_BACK, TBASISPOINT_LEFT_BOTTOM },
			{ ID_CTRLINST_COUNTRY, TBASISPOINT_CENTER_TOP },
			{ ID_CTRLINST_COUNTRY_NAME, TBASISPOINT_CENTER_TOP },
			//{ ID_CTRLINST_SERVER_NAME, TBASISPOINT_LEFT_TOP }
			{ ID_CTRLINST_SERVER_COMP1, TBASISPOINT_CENTER_TOP },
			{ ID_CTRLINST_SERVER_COMP2, TBASISPOINT_CENTER_TOP },
			{ ID_CTRLINST_SERVER_COMP3, TBASISPOINT_CENTER_TOP },
			{ ID_CTRLINST_SERVER_COMP4, TBASISPOINT_CENTER_TOP },
			{ ID_CTRLINST_CHAR_SERVER_IMAGE, TBASISPOINT_CENTER_TOP }
			};
		...
		// ID_FRAME_LOADING
		{
			...
			for (INT i = 0; i < TLOADINGCOMP_COMP_COUNT; ++i)
			{
				TComponent* pCOMP = pFRAME->FindKid(vCOMP[i][0]);
				if (i == 0 || i == 1)
					pCOMP->LoadOwnIMG(".\\Data\\IMG\\Custom\\Load.png");
				if (pCOMP)
				{
					CPoint pt;
					pCOMP->GetComponentPos(&pt);
					...
					m_vLOADINGCOMP[i].m_vOffset = CPoint(pt.x, pt.y);
				}
			}
		}
