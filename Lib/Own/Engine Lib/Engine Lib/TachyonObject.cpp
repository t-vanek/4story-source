								...
								WORD wFromSkillID)
{
	...
	if( itACT != m_OBJ.m_pOBJ->m_mapACTION.end() )
	{
		...
		{
			pAI->m_pANI->m_pANI->m_FromSkill = FromSkill;
			pAI->m_pANI->m_pANI->m_bFinished = FALSE;

			if (FromSkill)
				pAI->m_pANI->m_pANI->m_wFromSkillID = wFromSkillID;
		}
...
void CTachyonObject::CalcFrame( BOOL bUpdate)
{
	...
	{
		pANI->m_pANI->m_fCurTime = m_fActTime;
		pANI->m_pANI->m_fKeyTime = pANI->m_pANI->m_fCurTime;
		if (pANI->m_pANI->m_FromSkill)
		{
			if (pANI->m_pANI->m_wFromSkillID == 37)
				pANI->m_pANI->m_fKeyTime *= 2.5f;

			FLOAT fAccel = (FLOAT) CTachyonWnd::m_dwPing / 35.0f;
			if (fAccel > 1.0f)
		...
		if(!pANI->m_pANI->m_vKey.empty())
			while( m_nKeyBase < pANI->m_pANI->m_nKeyBase )
			{
				...
				{
					if (m_nKeyBase < pANI->m_pANI->m_nKeyBase && pANI->m_pANI->m_FromSkill)
					{
...
void CTachyonObject::CalcTick( LPDIRECT3DDEVICE9 pDevice, DWORD dwTick)
{
	...
	if( pANI && pANI->m_pANI )
	{
		...
		m_fKeyTime = m_fActTime;

		if( m_fActTime >= fTotal && fTotal != 0.0f )
		{
...
void CTachyonObject::Render(CD3DDevice *pDevice, CD3DCamera *pCamera, BYTE bNoBlend)
{
	static DWORD vCOLOROP[PS_COUNT] = {
		D3DTOP_DISABLE,						// PS_STAGE1
		D3DTOP_MODULATE,					// PS_MODULATE
	...
	for( it = m_OBJ.m_mapDRAW.begin(); it != m_OBJ.m_mapDRAW.end(); it++)
	{
		...
		for( int i=0; i<INT(pDRAW->size()); i++)
		{
			...
			DWORD dwCOLOROP = vCOLOROP[bNoBlend ? D3DTOP_MODULATE2X : nPS];
