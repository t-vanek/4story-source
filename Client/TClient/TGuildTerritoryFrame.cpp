...
// ====================================================================
CTGuildTerritoryFrame::CTGuildTerritoryFrame(TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc)
	: ITInnerFrame(pParent, pDesc, TCOMMUNITY_FRAME_GUILDTERRITORY), m_nCurMode(MODE_CASTLE)
{
	...
	m_pBox = FindKid(ID_CTRLINST_SKYGARDEN_BT);
...
{ 
	...
	if( m_nCurMode == MODE_CASTLE || m_nCurMode == MODE_LOCAL )
	{
		...
		for(INT i=0; i<nCNT; ++i) //14
		{
			...
			{
			case TCASTLE_HESED : color = COLOR_CASTLE_HESED; break;
			case TCASTLE_ARDRI : color = COLOR_CASTLE_ARDRI; break;
			case TCASTLE_GEHBRA : color = COLOR_CASTLE_GEHBRA; break;
			case TCASTLE_TYCON : color = COLOR_CASTLE_TYCON; break;
			case TCASTLE_WOODLAND : color = COLOR_CASTLE_WOODLAND; break;
			...
			else if( m_nCurMode == MODE_LOCAL )
			{
				...
				for(INT j=0; j<nLOCNT; ++j)
				{
					...
					if(local.m_bStatus)
						m_pList->SetUserColor( nLine, 2, dwColor );
					else if(local.m_bStatus == 0 || local.m_bStatus == BS_PEACE)
						m_pList->SetUserColor( nLine, 2, color );

					DWORD dwInfoIndex = MAKELONG( j, i );
					m_pList->SetItemDataAllColumn( nLine, dwInfoIndex );

					if( (i == (nCNT - 1)) && (j == (nLOCNT - 1)) )
					{
						int nLine = m_pList->AddString("");
						m_pList->SetUserColor( nLine, 0, COLOR_CASTLE_NONE );
						m_pList->SetItemString(nLine, 1, "");
						m_pList->SetUserColor( nLine, 1, COLOR_CASTLE_NONE );
						CString strCountry;
						strCountry = "";
						m_pList->SetItemString(nLine, 2, "");
						m_pList->SetUserColor( nLine, 2, COLOR_CASTLE_NONE );
						DWORD dwInfoIndex = MAKELONG( nLOCNT , nCNT + 1 );
						m_pList->SetItemDataAllColumn( nLine, dwInfoIndex );
					}
				}		
			}
