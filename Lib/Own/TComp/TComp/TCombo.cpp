...
TCombo::TCombo(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc)
	:TComponent(pParent, pDesc)
{
	m_comboDesc = pDesc->m_vCOMP.m_vEX.combo;
	Init();
}

TCombo::TCombo(TComponent* pParent, FRAMEDESC_WEAKPTR pDesc)
	: TCombo(pParent, pDesc.lock())
{

}

TCombo::TCombo(TComponent* pParent, const TCombo& rSrcCombo)
	:TComponent(pParent, rSrcCombo)
{
	m_comboDesc = rSrcCombo.m_comboDesc;
	Init();
}
...
void TCombo::Init()
{
	...
	m_pDrop = FindKid(m_comboDesc.drop);
	m_pDownList = FindKid(m_comboDesc.downlist);
