								   ...
								   BYTE bLevel)
{
	...
	int nIndex = (bLevel == NULL) ? NULL : INT(pTSKILL->m_bLevel) + INT(bLevel - 1) * INT(pTSKILL->m_bLevelInc);
	...
	case TSKILLDATAOP_EXP		: nRESULT = INT(CTChart::m_vLEVELRATE[nIndex] * INT(pTSKILL->m_vTFUNCTION[bINDEX]->m_wValue) / 100); break;
	}
