...
public:
	...
	static CString GetSHA1String(const CString& strSTRING);
	static CString GetSHA512String(const CString& strString);
	...
	HANDLE m_hWebsiteOpener;

	static DWORD WINAPI _OpenWebsiteSafely( LPVOID lpParam);
	static void OpenWebsiteSafely();
	// Option calculator
	...
	static DWORD m_vTICONNA[TSKILLSLOT_COUNT];
	static WORD m_vSlotID[MAX_DRAGSLOT];						// Slot ID pool
	static WORD m_vBattleSlotID[BATTLEINVEN_SLOT_COUNT];
	static WORD m_vBattleGauge[BATTLEINVEN_SLOT_COUNT];
	...
	static DWORD m_dwHideMaintain[2];
	static D3DXVECTOR3 m_vBOWPos[TCONTRY_B];
	static WORD m_wPremiumSkills[PREMIUM_SKILLS];
	static WORD m_wBOWSkills[PREMIUM_SKILLS];
	static WORD m_wSkillSystemExceptions[SKILL_EXCEPTIONS_COUNT];
	static CString m_strBRCommands[13];

	static void GetSecondToRemainTime( DWORD dwSecond, DWORD* dwDay, DWORD* dwHour, DWORD* dwMin, DWORD* dwSec);
	static void AddEffectString(BYTE bEffect, CString& strText);
...
public:
	...
	MAPSPOLECNIK m_mapSLAVESPOLECNIK;							// Slave recall pool	
	MAPSPOLECNIK m_mapSPOLECNIK;								// Recall pool
	MAPSPOLECNIK m_mapFAKETEMPS;							// Fake character temporary pool
	MAPFLOAT m_mapPETSIZE;
...
public:
	...
	MAPCUSTOMTEX m_mapCUSTOMTEX;

	QSKILLQUEUE m_qSkillQueue;
	BYTE m_bAlarm;
public:
	...
	CString m_strMACAddr;								// Mac Address
	...
	MacroProtection* m_pMacroProtection;
...
#ifdef NEW_IF
	CTMiniPopupDlg *m_pChannelPOPUP;
	CTMiniPopupDlg *m_pUBarPopup;
#else
	CTPopupMenu *m_pChannelPOPUP;
#endif
	...
	CTMiniPopupDlg* m_pCommandsPopup;
	TComponent* m_pResolutionBtn;
	TButton* m_pScreenModeBtn;
	...
	BYTE m_bLastSVR;
	BYTE m_bAutoHelp;
	DWORD m_dwPing[PING_MEASUREMENT_COUNT];
	DWORD m_dwEstPing;
	BYTE m_bSkillUseLocked;
	WORD m_wCloakPreview;
public:
	CTBlockListDlg* m_pBlockList;

	BYTE m_bCanRender;
	CD3DImage* m_pCustomImages[TCUSTOMTEX_COUNT];
protected:
	...
	BYTE m_bCharLoaded;
	DWORD m_dwLastQueueTick;
	CString m_strBRInviter;
	DWORD m_dwSelectedNPC;

	FRAMEDESC_SHAREDPTR m_arSkillTabFrame[TSKILLTAB_COUNT][TCLASS_COUNT];
	std::map<DWORD, FRAMEDESC_SHAREDPTR> m_mapInvenFrame;
...
public:
	...
	CTPremiumSkills* GetPremiumSkillsFrame(BYTE bPremium);
	void UsePremiumSkill(BYTE bID);
	void PerformActSkill(WORD wSkillID);

	void SetTournamentRemainTime( BOOL onoff, DWORD dwTIME );
	void OnTournamentEnd();
	void OnTournamentEnter();
	LPTEXTURESET ImagesToTextures(DWORD dwID, BOOL bResetDevice = FALSE, CD3DDevice* pDevve = NULL);
	LPTEXTURESET GetCustomTexture(DWORD dwID);
	void ReleaseCustomTexture();
	static BYTE IsInBOWMap();
	static BYTE IsInBRMap();
	...
	void ClearTASSITANT();
	void SortTASSITANT();
	CString GetWMIValue(char* type,wchar_t *value);
	void ResetPopupMenu();
	void SetMsgMenuBlinkMail(BYTE bStatus);
	void SetMsgMenuBlinkCD(BYTE bStatus);
	CString GetMACAddress();
	void InitPetSizes();
	FLOAT FindCustomPetSize(DWORD dwMonID);
	BYTE IsTournamentMap();
	...
	void DropItem_GuildToPremiumUI(UINT nFlags, BYTE bTargetUI, BYTE bSlotID);

	void DropItem_PremiumUIToPremiumUI(UINT nFlags, BYTE bTargetUI, BYTE bSlotID);

	void DropItem_PremiumUIToDefault(UINT nFlags, BYTE bTargetUI, BYTE bSlotID);
	...
	void DropItem_InvenToCompanion( UINT nFlags, BYTE bTargetUI, BYTE bSlotID );
	...
	void DropItem_PetToPet(UINT nFlags, BYTE bTargetUI, BYTE bSlotID);
	void DropItem_PetToInven(UINT nFlags, BYTE bTargetUI, BYTE bSlotID);
	void DropItem_CompanionToInven(UINT nFlags, BYTE bTargetUI, BYTE bSlotID);
	...
	void ResetNPCTalkUI();
	void ResetQuestUI(BYTE bReset);
	void ResetInvenUI();
	void ResetSkillUI();
	void ResetPremiumSkillUI();
	...
	BYTE IsAutoPath() const {return m_bTAUTOPATH;}
	...
	void LoadCustomTextures(DWORD dwID, const CString strPath);
	...
	void DoDEFACTION(BOOL FromKeyPress);
	void DoCHAT();
	...
	BYTE GetLucky() { return m_bLuckyID; };
	CString GetHelpMsg() { return m_strHelpMsg; };

	void DeleteFixRecall( DWORD dwID, bool bDeleteDirect=true );
	BYTE DeleteRecall( DWORD dwID, bool bDeleteDirect=true );

	void DeleteFixSpolecnik( DWORD dwID, bool bDeleteDirect=true );
	BYTE DeleteSpolecnik( DWORD dwID, bool bDeleteDirect=true );
	...
	BYTE IsCtrlSPOLECNIK( CTClientSpolecnik *pTOBJ);
	...
	// Utility
	CTClientObjBase *SeekSpolecnikTarget( CTClientSpolecnik *pSPOLECNIK);
	...
	CTClientRecall *FindSlaveCompanion( DWORD dwID);
	CTClientRecall *FindFixRecall( DWORD dwID);
	CTClientRecall *FindRecall( DWORD dwID);
	CTClientSpolecnik *FindSlaveFixSpolecnik( DWORD dwID);
	CTClientSpolecnik *FindSlaveSpolecnik( DWORD dwID);
	CTClientSpolecnik *FindFixSpolecnik( DWORD dwID);
	CTClientSpolecnik *FindSpolecnik( DWORD dwID);
	CTClientSpolecnik *FindSpolecnikByMon( WORD wID);
	CTClientSpolecnik* FindSpolecnikByMonAndHost( DWORD dwHostID, WORD wMonID );
	...
	void FindSlaveSpolecnikByHost( DWORD dwHostID, LPVTSPOLECNIK );
	void FindSpolecnikByHost( DWORD dwHostID, LPVTSPOLECNIK );
	...
	void FinishSkill(CTClientObjBase *pTATTACK,
		LPD3DXVECTOR3 pTGROUND,
		LPTSKILL pTSKILL,
		VTSKILLTARGET& vTarget,
		BOOL IsLinked = FALSE);

	BYTE IsInMyBRTeam(DWORD dwCharID);
	void AddEffectScrolls();
	...
	CTClientSpolecnik *GetMainSpolecnik();
	...
	void CalcUtilityBar( DWORD dwTick);
	...
	void CalcSLAVESPOLECNIK( DWORD dwTick);
	...
	void CalcSPOLECNIK( DWORD dwTick);
	void CalcBREATH( DWORD dwTick);
	void CalcLAVA( DWORD dwTick);
	...
	void UpdateBoxGauge(DWORD dwTick);
	...
	BYTE RenderTRANK( CTClientObjBase *pTOBJ, const D3DXVECTOR3 &vPROJ, FLOAT fBaseHeight, FLOAT fRight);
	void RenderTOBJ(
		CTClientObjBase *pTOBJ,
		DWORD dwTick);
	...
	FLOAT GetDifferencial(D3DXMATRIX vCharPos, D3DXMATRIX vDefendPos);
...
public:
	...
	//void SendCS_HACKINFO_REQ(BYTE bDisguise);
	...
	void SendCS_PETRIDING_REQ(
		DWORD dwMonID, BOOL bRide);

	void SendCS_DELRECALLMON_REQ(
		DWORD dwMonID, BYTE bMonType);

	void SendCS_DELSPOLECNIKMON_REQ(
		DWORD dwMonID, BYTE bMonType);
	...
	void SendCS_ACTITEMSUSE_REQ(CTClientObjBase *pATTACK,
								CTClientObjBase *pDEFEND,
								LPTSKILL pTSKILL);
	...
	int OnGM_COMPANION_DELETEREQ();
	int OnGM_COMPANION_EFFECT();
	int OnGM_COMPANION_EFFECTREQ();
	...
	int OnGM_TOGGLE_CUSTOMCLOAK();
	int OnGM_CUSTOMCLOAK_WARN();
	int OnGM_CUSTOMCLOAK_PREVIEW();
	int OnGM_CUSTOMCLOAK_APPLY();
	...
	int OnGM_TOGGLE_GUILD_BASE_NEW();
	...
	int OnGM_UB_SHOP_9();
	int OnGM_UB_SHOP_10();
	int OnGM_UB_SHOP_CANCEL(); 

	int OnGM_OPEN_BOX();
	int OnGM_CLOSE_BOX();
	int OnGM_TOGGLE_LIST();
	...
	int OnGM_GMP_VOLUNTEER_WOODLAND();
	...
	int OnGM_PET_RECALL();
	int OnGM_PET_EFFECTCHANGE();
	int OnGM_PET_EFFECTDELETE();
	int OnGM_PET_DELETEEFFECT();
	int	OnGM_PET_CHANGEEFFECT();
	int OnGM_MENU_RIDEPET();
	int OnGM_MENU_PETMANAGE();

	int OnGM_SELECT_TEXTURE_DETAIL();
	int OnGM_CHANGE_CHANNEL();
	int OnGM_CLICK_WORLDMAP();
...
//	int OnGM_TEXTOOL_COMPO();
	...
	int OnGM_LEVEL2();
	int OnGM_LEVEL3();
	int OnGM_CANCEL_VETERAN();
	int OnGM_OPEN_MENU_COMMUNITY();
	int OnGM_OPEN_MENU_WAR();
	int OnGM_OPEN_MENU_MESSENGER();
	int OnGM_OPEN_MENU_CHAR();
	int OnGM_OPEN_MENU_INVEN();
	int OnGM_OPEN_MENU_SYSTEM();
	int OnGM_CHAT_MENU();
	int OnGM_OPEN_SOUL_LOTTERY();
	int OnGM_SOUL_LOTTERY_TRIGGER();
	int OnGM_SC_LEFTBTN();
	int	OnGM_SC_MIDDLEBTN();
	int	OnGM_SC_RIGHTBTN();
	int OnGM_TOGGLE_FACEBOOK();
	int	OnGM_TOGGLE_TWITTER();
	int OnGM_USE_POWDER();
	int OnGM_USE_COMPRESET();
	int OnGM_CANCEL_POST();
	int OnGM_CANCEL_POST_CONFIRM();

	int OnGM_OPEN_REGISTER_UI();
	int OnGM_BOW_REGISTER();
	int OnGM_CANCEL_BOW_QUEUE();
	int OnGM_CANCEL_BOW_QUEUE_CONFIRM();
	int OnGM_BOW_CASH_RESPAWN();
	int OnGM_BP_EXCHANGE();
	int OnGM_BP_EXCHANGE_CONFIRM();
	int OnGM_BRTEAMMATEADD_REQ();
	int OnGM_BRTEAMMATEADD_ANSWER();
	int OnGM_BATTLERANK_TOGGLE();
	int OnGM_DONATE_LIFE();

	int OnGM_GSKILL_RENEW_YES();
	...
	int OnCS_CUSTOMCLOAK_LIST_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_CUSTOMCLOAK_CHANGE_ACK( CTachyonSession *pSession, CPacket *pPacket);
	...
	int OnCS_SENDSADDLE_REQ (CTachyonSession* pSession, CPacket* pPacket);
	...
	int OnCS_WHIP_ACK( CTachyonSession *pSession, CPacket *pPacket);
	...
	int OnCS_SOULUP_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_SOULLOTTERY_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_SOULLOTTERY_TRIGGER_ACK( CTachyonSession *pSession, CPacket *pPacket);
	...
	int OnCS_ADDSPOLECNIKMON_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_DELSPOLECNIKMON_ACK( CTachyonSession *pSession, CPacket *pPacket);
	...
	int OnCS_PETEFFECTCHANGE_ACK( CTachyonSession *pSession, CPacket *pPacket);
	...
	int OnCS_GETREWARDS_ACK(CTachyonSession *pSession, CPacket *pPacket);
	// SMS message
	...
	// Title Message
	int OnCS_TITLELIST_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_COMPANIONLIST_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_TITLEGAIN_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_TITLERESET_ACK( CTachyonSession *pSession, CPacket *pPacket);
	...
	int OnCS_LEAVESKYGARDEN_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_SKYGARDEN_OCCUPY_CENTER_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_SKYGARDEN_OCCUPY_LEFT_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_SKYGARDEN_OCCUPY_RIGHT_ACK( CTachyonSession *pSession, CPacket *pPacket);
	...
	int OnCS_SETTIME_ACK( CTachyonSession *pSession, CPacket *pPacket );
	
	// Securitysystem
	int OnCS_SC_CREATECODE_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_SC_CHANGECODE_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_SC_DISABLECODE_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_SC_ENABLECODE_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_SC_UNLOCK_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_SC_CHECKCODE_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_SC_GETLOCKTIME_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_COMPANIONUPDATE_REQ( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_COMPANIONLUPDATE_REQ( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_UPDATECOMPANIONBYITEM_REQ( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_UPDATECOMPANIONBYSYSTEM_REQ( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_UPDATESPAWNEDCOMPANION_REQ( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_CREATECOMPANION_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_UPDATECOMPANIONITEMS_REQ( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_UPDATECOMPANIONBONUS_REQ( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_UPDATEMEDALS_REQ( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_EXITTOURNAMETMAINTAIN_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_EXITTOURNAMETHOTKEY_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_BOWTIMEUPDATE_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_BOWNOTIFY_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_PINGMEASUREMENT_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_REGISTERBOW_ACK(CTachyonSession* pSession, CPacket* pPacket);
	int OnCS_RELEASEFORBOW_REQ( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_CANCELBOWQUEUE_ACK(CTachyonSession* pSession, CPacket* pPacket);
	int OnCS_NOTIFYNONQUEUEDPLAYER_ACK(CTachyonSession* pSession, CPacket* pPacket);
	int OnCS_UPDATEBONUSPOINT_ACK(CTachyonSession* pSession, CPacket* pPacket);
	int OnCS_BOWKILLNOTIFY_ACK(CTachyonSession* pSession, CPacket* pPacket);
	int OnCS_BOWRANKING_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_LEAVEBATTLEFIELD_ACK(CTachyonSession* pSession, CPacket* pPacket);

	int OnCS_FORWARDBOWCHAT_ACK( CTachyonSession *pSession, CPacket *pPacket);

	int OnCS_ADDTOBRQUEUE_ACK( CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_BRTIMEUPDATE_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_BRTEAMMATEADD_ACK(CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_UPDATEBRTEAM_ACK(CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_UPDATEBRMATE_ACK(CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_INITBRTRANKING_ACK(CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_UPDATEBRTPLAYER_ACK(CTachyonSession *pSession, CPacket *pPacket);

	int OnCS_UPDATEREMAININGPLAYER_ACK(CTachyonSession *pSession, CPacket *pPacket);
	int OnCS_GUILDSKILLACTION_ACK(CTachyonSession * pSession, CPacket * pPacket);

	int OnCS_GUILDSKILLUPDATE_ACK(CTachyonSession * pSession, CPacket * pPacket);

	int OnCS_ARENAINFO_ACK(CTachyonSession * pSession, CPacket * pPacket);

	int OnCS_ARENARANKING_ACK(CTachyonSession * pSession, CPacket * pPacket);
