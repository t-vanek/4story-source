...
void CTTextLinkData::PackItem(LPTITEM pItem, CTClientItem* pClientItem)
{
	...
	if( pClientItem )
	{
		(*this) 
			...
			<< pClientItem->GetWrap()
			<< pClientItem->GetELD()
			<< pClientItem->GetMoggItemID()
			<< pClientItem->GetCompanionID()
			<< pClientItem->GetGradeEffect()
			<< pClientItem->GetEndTime();
...
CTClientItem* CTTextLinkData::UnPackItem()
{
	...
	WORD wCompanionID;
		...
		m_pLastUnpackItem->SetMoggItemID(wMoggItemID);
		m_pLastUnpackItem->SetGradeEffect(bGradeEffect);
		m_pLastUnpackItem->SetEndTime(dEndTime);
		m_pLastUnpackItem->SetCompanionID(wCompanionID);
		// jkchoi
	}
