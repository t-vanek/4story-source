...
int CTClientWnd::OnGM_TOGGLE_SCREEN_MODE()
{
	...
	{
		if (m_Device.m_option.m_dwScreenX != GetSystemMetrics(SM_CXSCREEN) &&
			m_Device.m_option.m_dwScreenY != GetSystemMetrics(SM_CYSCREEN))
		{
...
int CTClientWnd::OnGM_CHANGE_SELECT_CHAR()
{
	...
	if (m_TNet.m_bBOWSlot != (BYTE) T_INVALID)
	{
		MessageBoxYesNo(
			"Do you want to get back to the battleground?",
			TSTR_YES,
			TSTR_NO,
			GM_START_GAME,
			GM_BACKTO_SERVER,
		...
		for (BYTE i = 0; i < m_vTCHARINFO.size(); ++i)
		{
			if (m_vTCHARINFO[i]->m_bSlotID == m_TNet.m_bBOWSlot)
			{
				m_bSlotID = i;
				break;
			}
...
int CTClientWnd::OnGM_DEL_CHAR()
{
	...
	strPW = CTClientGame::GetSHA1String(pEDIT->m_strText);
...
int CTClientWnd::OnGM_SECURITY_CONFIRM()
{
	TFrame *pFRAME = m_MessageBox.FindFrame(1300);
	CString strCode = "";
	
	if(pFRAME)
		strCode = pFRAME->FindKid(27428)->m_strText;

	if(!strCode.IsEmpty())
		SendCS_SECURITYCONFIRM_ACK(strCode);
