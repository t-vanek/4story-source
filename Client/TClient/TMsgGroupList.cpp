...
// ======================================================================
CTMsgGroupList::CTMsgGroupList(TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc)
...
// ======================================================================
void CTMsgGroupList::InitPopup(FRAMEDESC_SHAREDPTR pPopupDESC)
{
	...
	: CItem(pParent,nIdx,nLine), m_pParentGrp(pParentGrp)
{
	...
	if( pTplt )
	{
		for(INT i=0; i<ISTATE_COUNT; ++i)
		{
			m_pTopFrame[i]  = new TComponent(pParent, *pTplt->m_pTopFrame[i]);
			m_pDownFrame[i] = new TComponent(pParent, *pTplt->m_pDownFrame[i]);
		...
		for(INT i=0; i<MTXT_COUNT; ++i)
		{
			m_vTxt[i] = new TComponent(pParent, *pTplt->m_vTxt[i]);
