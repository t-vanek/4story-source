...
CTDynamicHelpDlg::CTDynamicHelpDlg( TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc)
: CTClientUIBase( pParent, pDesc)
{
	...
	m_TextColor = CTChatFrame::GetChatTypeColor(CHAT_WORLD);
...
void CTDynamicHelpDlg::InitTEXT( CString& strTEXT)
{
	...
	m_pHEIGHT[m_nPartCount - 1] = nHeight;
	m_pWIDTH[m_nPartCount - 1] = nWidth;

	D3DXCreateTexture(
		CTachyonRes::m_pDEVICE->m_pDevice,
		m_pWIDTH[m_nPartCount - 1],
		m_pHEIGHT[m_nPartCount - 1],
		1, 0,
		D3DFMT_A4R4G4B4,
		D3DPOOL_MANAGED,
		&m_pTEX[m_nPartCount - 1]);
...
HRESULT CTDynamicHelpDlg::Render( DWORD dwTickCount)
{
	...
	DWORD dwTextREF = ((m_TextColor & 0xFF000000) >> 24) * (TGDITEXTCLR_MIN - 1) / TGDITEXTCLR_MAX;
	...
	if (m_TextColor != CTChatFrame::GetChatTypeColor(CHAT_WORLD))
	{
		static DWORD dwStart = GetTickCount();
		static FLOAT fCount = 0;
		fCount = (FLOAT) ((GetTickCount() - dwStart) + 1) / 117.64f;
		...
		BYTE bRed = (BYTE) (sin(fFreq * (FLOAT)fCount + 0) * 127) + 128;
		BYTE bGreen = (BYTE) (sin(fFreq * (FLOAT)fCount + 2) * 127) + 128;
		BYTE bBlue = (BYTE) (sin(fFreq * (FLOAT)fCount + 4) * 127) + 128;

		CTachyonRes::m_pDEVICE->m_pDevice->SetRenderState( D3DRS_TEXTUREFACTOR, Color(bRed, bGreen, bBlue).GetValue());
	}
	else
		CTachyonRes::m_pDEVICE->m_pDevice->SetRenderState( D3DRS_TEXTUREFACTOR, m_TextColor);
