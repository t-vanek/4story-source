...
CTBlockList::CTBlockList(CTBlockListDlg *pParent, FRAMEDESC_SHAREDPTR pDesc)
: TList( pParent, pDesc ),
m_pParent( pParent )
{
}
...
// =======================================================
CTBlockListDlg::CTBlockListDlg(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc)
	: ITInnerFrame( pParent, pDesc, TMESSENGER_FRAME_BLOCKLIST)
{
	TList* pList = static_cast<TList*>(FindKid(ID_CTRLINST_LIST));
	RemoveKid( pList );

	m_pList = new CTBlockList( this, pList->m_pDESC.lock() );
	...
	m_pOpButton[0] = dynamic_cast<TButton*>(FindKid( ID_CTRLINST_OP_1 ));
	m_pOpButton[1] = dynamic_cast<TButton*>(FindKid( ID_CTRLINST_OP_2 ));
	m_pOpButton[2] = dynamic_cast<TButton*>(FindKid( ID_CTRLINST_OP_3 ));
	m_pOpButton[3] = dynamic_cast<TButton*>(FindKid( ID_CTRLINST_OP_4 ));
	...
	for( INT i=0 ; i < m_pList->GetItemPerPage() ; ++i )
	{
		...
		if( bValid )
		{
			...
			else
			{
				...
				vOP.m_pOpButton[2] = new TButton( this, *m_pOpButton[2] );
				vOP.m_pOpButton[3] = new TButton( this, *m_pOpButton[3] );
