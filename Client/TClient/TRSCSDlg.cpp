...
#ifdef NEW_IF
CTRSCSDlg::CTRSCSDlg( TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc, CTClientUIBase *pRaidNew)
:CTClientUIBase( pParent, pDesc)
{
	...
	m_pArea->GetComponentRect(&m_vTAREA);
	...
	if( m_pTSQUAD->GetSel() < 0 )
	{
		if(!m_vTCORPS.m_vTSQUAD.empty())
			...
			for(BYTE i=0;i<BYTE(m_vTCORPS.m_vTSQUAD.size());++i)
				if(m_vTCORPS.m_vTSQUAD[i]->m_wPartyID == CTClientGame::GetInstance()->GetMainChar()->m_wPartyID)
					bSquad = i;

			m_pTCURSQUAD = m_vTCORPS.m_vTSQUAD[bSquad];
			m_pTSQUAD->SetCurSelItem(bSquad);
		}
		else
			m_pTCURSQUAD = NULL;
...
void CTRSCSDlg::ResetUNIT()
{
	...
	if( m_pTUNIT->GetSel() < 0 )
	{
		...
		{
			BYTE bUnit = 0;
			for(BYTE i=0;i<m_pTCURSQUAD->m_vTUNIT.size();++i)
				if(m_pTCURSQUAD->m_vTUNIT[i]->m_dwCharID == CTClientGame::GetInstance()->GetMainChar()->m_dwID)
					bUnit = i;

			m_pTCURUNIT = m_pTCURSQUAD->m_vTUNIT[bUnit];
			m_pTUNIT->SetCurSelItem(bUnit);
		}
		else
			m_pTCURUNIT = NULL;
...
void CTRSCSDlg::RenderTRSCS()
{
	if(IsVisible())
	{
		...
		CRect rect(
			...
			TMINIMAPTEX_SIZE);
		rect.OffsetRect(
			m_rc.left + m_vTAREA.left - (160 - m_vTAREA.Width()) / 2,
			m_rc.top + m_vTAREA.top - (160 - m_vTAREA.Height()) / 2);
		TNLVERTEX vRECT[8] = {
...
BOOL CTRSCSDlg::HitTestArea( CPoint pt)
{
	LPIMAGESET pImageset = m_pArea->m_pEnable;
...
CTRSCSDlg::CTRSCSDlg( TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc)
:CTClientUIBase( pParent, pDesc)
{
	static DWORD dwSquadNameID[TRSCSHOTKEY_COUNT] = {
