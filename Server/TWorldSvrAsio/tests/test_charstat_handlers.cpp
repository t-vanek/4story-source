// W4-10 wire test: the inspect-player stat relay (CHARSTATINFO /
// CHARSTATINFOANS) over a two-peer loopback session.
//
// Alice 42/peer1 0x42 inspects Bob 200/peer2 0x43.
//   - CHARSTATINFO_ACK(req=42, char=200) → CHARSTATINFOANS_REQ to
//     Bob's map (asking it to gather Bob's stats).
//   - CHARSTATINFOANS_ACK(req=42, <stat block>) → CHARSTATINFO_REQ
//     forwarded verbatim to Alice's map.
...
int main()
{
    ...
    // --- Step 1: Alice requests Bob's stats → ANS_REQ to Bob's map --
    {
        ...
        SendFramed(p1, ToUint16(MessageId::MW_CHARSTATINFO_ACK), b);
    }
    {
        auto [w, b] = ReadFramed(p2);   // Bob's map asked to gather
        EXPECT(w == ToUint16(MessageId::MW_CHARSTATINFOANS_REQ));
        tworldsvr::wire::Reader r(b);
        std::uint32_t req = 0, cid = 0;
        r.Read(req); r.Read(cid);
        EXPECT(req == 42); EXPECT(cid == 200);
    }

    // --- Step 2: Bob's map answers with a stat block → relay to p1 --
    {
        std::vector<std::byte> b;
        tworldsvr::wire::WritePOD<std::uint32_t>(b, 42);   // req
    ...
    {
        auto [w, b] = ReadFramed(p1);   // forwarded to the requester
        EXPECT(w == ToUint16(MessageId::MW_CHARSTATINFO_REQ));
        tworldsvr::wire::Reader r(b);
    ...
    if (g_fails == 0)
        std::printf("PASS test_tworldsvr_asio_charstat_handlers\n");
    else
        std::printf("FAIL test_tworldsvr_asio_charstat_handlers "
                    "(%d failure%s)\n", g_fails, g_fails == 1 ? "" : "s");
