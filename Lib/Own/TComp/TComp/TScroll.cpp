...
TScroll::TScroll(TComponent* pParent, FRAMEDESC_SHAREDPTR pDesc)
	: TComponent(pParent, pDesc)
{
	m_scrollDesc = pDesc->m_vCOMP.m_vEX.scroll;
	Init();
}

TScroll::TScroll(TComponent* pParent, FRAMEDESC_WEAKPTR pDesc)
	: TScroll(pParent, pDesc.lock())
{

}

TScroll::TScroll(TComponent* pParent, const TScroll& rSrcScroll)
	: TComponent(pParent, rSrcScroll)
{
	m_scrollDesc = rSrcScroll.m_scrollDesc;
	Init();
}
...
void TScroll::Init()
{
	...
	m_pBar = FindKid(m_scrollDesc.bar);
	m_pUpLeft = FindKid(m_scrollDesc.ul);
	m_pDownRight = FindKid(m_scrollDesc.dr);
}
