...
struct tagANISET
{
	...
	inline ANIKEY GetAniKey()
	{
		...
		for( ; i<nCount; i++)
		{
			FLOAT fRealTime = m_fKeyTime != 0.0f ? m_fKeyTime : m_fCurTime;
			if( fRealTime < m_vKey[i]->m_fTime )
				break;

			if( fRealTime < m_vKey[i + 1]->m_fTime )
			{
				FLOAT fTotal = m_vKey[i + 1]->m_fTime - m_vKey[i]->m_fTime;
				FLOAT fTime = fRealTime - m_vKey[i]->m_fTime;
