...
boost::asio::awaitable<void>
SendMwStartTeleportReq(std::shared_ptr<PeerSession> peer,
                       std::uint32_t                char_id,
                       std::uint32_t                key,
                       std::uint8_t                 channel,
                       std::uint16_t                map_id,
                       ...
                       float                        pos_z)
{
    ...
    co_await peer->Wire()->SendPacket(
        tnetlib::protocol::ToUint16(
            tnetlib::protocol::MessageId::MW_STARTTELEPORT_REQ),
        std::move(body));
}
...
boost::asio::awaitable<void>
SendMwTeleportReq(std::shared_ptr<PeerSession> peer,
                  std::uint32_t                char_id,
                  std::uint32_t                key,
                  std::uint8_t                 channel,
                  std::uint16_t                map_id,
                  ...
                  std::uint8_t                 result)
{
    ...
    co_await peer->Wire()->SendPacket(
        tnetlib::protocol::ToUint16(
            tnetlib::protocol::MessageId::MW_TELEPORT_REQ),
        std::move(body));
}

boost::asio::awaitable<void>
SendMwConListReq(std::shared_ptr<PeerSession> peer,
                 std::uint32_t                char_id,
                 std::uint32_t                key,
                 std::uint8_t                 channel,
                 std::uint16_t                map_id,
                 ...
                 float                        pos_z)
{
    ...
    co_await peer->Wire()->SendPacket(
        tnetlib::protocol::ToUint16(
            tnetlib::protocol::MessageId::MW_CONLIST_REQ),
        std::move(body));
}
