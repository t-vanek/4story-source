...
#define ID_CTRLINST_WORLD_OB			(0x00000C7A)

CTWorldmapDlg::CTWorldmapDlg( TComponent *pParent, FRAMEDESC_SHAREDPTR pDesc)
:CTClientUIBase( pParent, pDesc)
{
	...
	m_pTALLWORLDMAP2 = FindKid(ID_CTRLINST_WORLDMAP);
	...
	CPoint pt2;
	m_pTITLE->GetComponentPos(&pt2);
	pt2.SetPoint(pt2.x - 0, pt2.y - 7);
	m_pTITLE->MoveComponent(pt2);
...
{
	static const INT TWORLDMAP_COMP_COUNT = 11;
	...
	static DWORD vCOMP[TWORLDMAP_COMP_COUNT][2] =
	{
		...
		{ ID_CTRLINST_WORLD_OB, TBASISPOINT_CENTER_TOP }
	};
	...
	if (bRestore)
	{
		...
		static CPoint MIN = 5;
		m_pTALLWORLDMAP2->MoveComponent(
			CTClientUIBase::m_vBasis[TBASISPOINT_CENTER_MIDDLE] + test);
	...
	else
	{
		for (INT i = 0; i < TWORLDMAP_COMP_COUNT; ++i)
		{
			...
			if (pCOMP)
			{
				...
				vCOMP_OFFSET[i].x = pt.x - 1;
				vCOMP_OFFSET[i].y = pt.y - 26;
			}
		}
...
void CTWorldmapDlg::OnLButtonDown( UINT nFlags, CPoint pt)
{
	...
	if(pMainChar->CountMaintainFunc(0, SDT_RUNAWAY, 1))
		return;
