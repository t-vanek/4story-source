// W6-6 wire test: monster-result relays.
//
// Alice (42) has her main session on peer1. A monster result is
// resolved on peer2 (where the monster lives) and naming Alice; world
// routes MONSTERDIE / TAKEMONMONEY verbatim back to her main map.
...
std::vector<std::byte> MonBody(std::uint32_t char_id, std::uint32_t key,
                               std::uint32_t tail)
{
    std::vector<std::byte> b;
    tworldsvr::wire::WritePOD<std::uint32_t>(b, char_id);
    tworldsvr::wire::WritePOD<std::uint32_t>(b, key);
    tworldsvr::wire::WritePOD<std::uint32_t>(b, tail);  // opaque tail
    return b;
}
...
int main()
{
    ...
    // MONSTERDIE resolved on p2 → routed verbatim to Alice's map (p1).
    {
        auto b = MonBody(42, kKey, 0xABCD1234);
        SendFramed(p2, ToUint16(MessageId::MW_MONSTERDIE_ACK), b);
        auto [w, got] = ReadFramed(p1);
        EXPECT(w == ToUint16(MessageId::MW_MONSTERDIE_REQ));
        EXPECT(got == b);
    }

    // TAKEMONMONEY likewise.
    {
        auto b = MonBody(42, kKey, 0x55667788);
        SendFramed(p2, ToUint16(MessageId::MW_TAKEMONMONEY_ACK), b);
        auto [w, got] = ReadFramed(p1);
        EXPECT(w == ToUint16(MessageId::MW_TAKEMONMONEY_REQ));
        EXPECT(got == b);
    }
    ...
    if (g_fails == 0)
        std::printf("PASS test_tworldsvr_asio_monresult_handlers\n");
    else
        std::printf("FAIL test_tworldsvr_asio_monresult_handlers "
                    "(%d failure%s)\n", g_fails, g_fails == 1 ? "" : "s");
