...
BOOL CTClientGuildMark::OnDeviceReset( BOOL bRelease )
{
	...
	if( bRelease ) // Release
	{
		...
		MAPCUSTOMTEX::iterator jit, jend;
		jit = pTGAME->m_mapCUSTOMTEX.begin();
		jend = pTGAME->m_mapCUSTOMTEX.end();

		for(; jit != jend ; ++jit )
		{
			CT3DTexture *pJEX = jit->second->GetTexturePtr(0);

			if(pJEX)
				delete pJEX;
			if(jit->second)
				jit->second->ClearTexture();
		}
	}
	else // Restore
	{
		...
		for(; jit != jend ; ++jit )
		{
			LPTEXTURESET pTextureS = pTGAME->ImagesToTextures( jit->first, TRUE, &pMainWnd->m_Device);
			jit->second->PushTexturePtr( pTextureS->GetTexturePtr(0) );
			delete pTextureS;
		}
