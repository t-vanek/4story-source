...
BOOL CSFX::InitSFX( CD3DDevice *pDevice, LPSFX pSFX, DWORD dwID)
{
	Release();
	m_pSFX = pSFX;
...
void CSFX::RenderSINGLE( CD3DDevice *pDevice, CD3DCamera *pCamera, BYTE bMaxAlpha, DWORD dwCustomColor )
{
	if(m_pSFX)
	{
		DWORD dwCLR = dwCustomColor == 0x00000000 || dwCustomColor == 1 ? m_pSFX->GetColor(CTMath::GetTimeTick(m_fTime)) : dwCustomColor;
		...
		case SFXSRC_SFX	:
			{
				...
				m_SRC.m_pSFX->m_vGlobal *= m_vGlobal;
				m_SRC.m_pSFX->m_vPFactor = m_vPFactor;
				m_SRC.m_pSFX->Render( pDevice, pCamera, bMaxAlpha, dwCustomColor );
			}
		...
		case SFXSRC_TEX	:
			{
				...
				if(CSFX::m_pTEXVB)
				{
					...
					if(dwCustomColor != 0x00000000 && dwCustomColor != 1)
					{
						pDevice->m_pDevice->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_ADDSIGNED);
...
void CSFX::Render( CD3DDevice *pDevice, CD3DCamera *pCamera, BYTE bMaxAlpha, DWORD dwCustomColor )
{
	if(m_pSFX)
	{
		DWORD dwCLR = dwCustomColor == 0x00000000 ? m_pSFX->GetColor(CTMath::GetTimeTick(m_fTime)) : dwCustomColor;
...
BOOL CTachyonSFX::InitSFX( CD3DDevice *pDevice, LPSFX pSFX, DWORD dwID)
{
	Release();
	m_pSFX = pSFX;
		...
		case SFX_SINGLE		:
			{
				...
				m_SINGLE.InitSFX( pDevice, pSFX, dwID);
			}
...
{
	auto itPSFX = m_vPSFX.begin();
	auto itCSFX = m_vCSFX.begin();
...
void CTachyonSFX::Render( CD3DDevice *pDevice, CD3DCamera *pCamera, BYTE bMaxAlpha, DWORD dwCustomColor )
{
	if( m_pSFX->m_dwLodLevel < m_dwGlobalLodLevel )
		return ;
	...
	if( m_pSFX && m_pSFX->m_bSRCType == SFXSRC_TEX )
	{
		...
		pDevice->m_pDevice->SetRenderState( D3DRS_TEXTUREFACTOR, 0xFFFF0000);
		...
		(m_SINGLE.m_SRC.m_pOBJ || m_pSFX->m_bSRCType == SFXSRC_TEX) )
	{
		...
		m_SINGLE.RenderSINGLE( pDevice, pCamera, bMaxAlpha, dwCustomColor );
	}
	...
	for( int i=0; i<nCount; i++)
	{
		...
		m_vPSFX[i]->Render( pDevice, pCamera, bMaxAlpha, dwCustomColor );
	}
	...
	for(auto i=0; i<nCount; i++)
	{
		...
		m_vCSFX[i]->Render( pDevice, pCamera, bMaxAlpha, dwCustomColor );
	}
}
...
DWORD CTachyonSFX::RegSFX( CTachyonSFX *pSFX, BYTE bBias, DWORD dwCustomColor)
{
	pSFX->m_dwCustomColor = dwCustomColor;
							 ...
							 FLOAT fDistMAX)
{
	...
	for( it = m_mapINST.begin(); it != m_mapINST.end(); it++)
		...
		{
			(*it).second->Render( pDevice, pCamera, (*it).second->m_bMaxAlpha, (*it).second->m_dwCustomColor );
		}
}
