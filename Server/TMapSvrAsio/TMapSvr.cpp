...
void CTMapSvrModule::servicetest(CString vals)
{
	_AtlModule.SendDM_POSTRECV_REQ(
		0, 0, "local", "localservicetest",
		_AtlModule.BuildNetString(NAME_NULL, "values"), _AtlModule.BuildNetString(NAME_NULL, vals), POST_PACKATE,
		9999, 0, 0, 0);
}
...
DWORD CTMapSvrModule::OnReceive( LPPACKETBUF pBUF)
{
	...
	if(pBUF->m_pSESSION)
	{
		...
		else if(pBUF->m_pSESSION->m_bSessionType == SESSION_SERVER && pBUF->m_pSESSION->m_addr.sin_port == m_wGamePort)
