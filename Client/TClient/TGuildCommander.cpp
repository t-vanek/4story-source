...
void CTGuildCommander::RecvGuildEstablish(BYTE bResult, DWORD dwGuildID, const CString& strGuild)
{
	...
	if( bResult == GUILD_SUCCESS )
	{
		...
		if( pMainChar )
		{
			...
			CTGuildMainNew* pDlg = static_cast<CTGuildMainNew*>( pGame->GetFrame(TFRAME_GUILD_BASE_NEW) );
			CTGuildSummaryNewDlg* pComm = static_cast<CTGuildSummaryNewDlg*>(pDlg->GetInnerFrame(TGUILD_SUMMARY));
			CTGuildMembersNewDlg* pComm2 = static_cast<CTGuildMembersNewDlg*>(pDlg->GetInnerFrame(TGUILD_MEMBERS));
			if( pDlg->IsSelect(TGUILD_SUMMARY) && pComm->IsVisible()) 
			{
				pComm->RequestInfo();
			}

			if( pDlg->IsSelect(TGUILD_MEMBERS) && pComm2->IsVisible()) 
			{
				pComm2->RequestInfo();
			}
		}
...
void CTGuildCommander::RecvGuildDetInfo(const GuildDetInfo& info)
{
	...
	if (pDlg->IsSelect(TGUILD_SUMMARY) && pComm->IsVisible())
		pComm->ResetInfo();
	else if (pDlg->IsSelect(TGUILD_SKILL) && pDlg->IsVisible())
		static_cast<CGuildSkillsDlg*>(pDlg->GetInnerFrame(TGUILD_SKILL))->ResetInfo();
}
// --------------------------------------------------------------------
...
void CTGuildCommander::RecvGuildTacticsList(GuildTacticsVec* pMembers)
{
	...
	CTGuildMainNew* pDlg = static_cast<CTGuildMainNew*>( pGame->GetFrame(TFRAME_GUILD_BASE_NEW) );
	CTGuildMembersNewDlg* pComm = static_cast<CTGuildMembersNewDlg*>(pDlg->GetInnerFrame(TGUILD_MEMBERS));
	if( pDlg->IsSelect(TGUILD_SUMMARY) && pComm->IsVisible() )
		pComm->ResetInfo();
}
// ====================================================================
...
void CTGuildCommander::RecvGuildNotifyList(GuildNotifyVec* pNotVec)
{
	...
	if( pDlg->IsSelect(TGUILD_SUMMARY) && pSummary->IsVisible())
					pSummary->UpdateInfo();
...
void CTGuildCommander::RecvTerritoryList(TerritoryVec* pTerr)
{
	...
	CTGuildMainNew* pDlg = static_cast<CTGuildMainNew*>( pGame->GetFrame(TFRAME_GUILD_BASE_NEW) );
	CTGuildWarInfoNewDlg* pSummary = static_cast<CTGuildWarInfoNewDlg*>(pDlg->GetInnerFrame(TGUILD_WARINFO));
	if( pDlg->IsSelect(TGUILD_WARINFO) && pSummary->IsVisible()  )
		pSummary->ResetInfo();
}
// ----------------------------------------------------------------------
...
CTGuildCommander::GuildLatestPvP* CTGuildCommander::GetGuildLatestPvPbyID( DWORD dwCharID )
{
	CTGuildCommander::GuildLatestPvPVec::iterator it, end;
	it = m_GuildLatestPvPVec.begin();
	end = m_GuildLatestPvPVec.end();
...
CTGuildCommander::GuildWeekPvP* CTGuildCommander::GetGuildWeekPvPbyID( DWORD dwCharID )
{
	CTGuildCommander::GuildWeekPvPVec::iterator it, end;
	it = m_GuildWeekPvPVec.begin();
	end = m_GuildWeekPvPVec.end();
