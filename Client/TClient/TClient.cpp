...
BOOL CTClientApp::InitInstance()
{
	...
	if (iStart != -1)
	{
		...
		if (iStart != -1)
		{
			CString strWorkingDir = strCmdLine.Tokenize(_T(" "), iStart);
			SetCurrentDirectory(strWorkingDir);
		}
	}*/
...
//	JHWIDLib::Start();
	...
	if( CTNationOption::KOR || CTNationOption::UNITEDKINGDOM)
...
BYTE CTClientApp::CreateLoadThread()
{
	...
	if(!::SetThreadPriority( m_hLoadThread, THREAD_PRIORITY_NORMAL))
		return FALSE;
...
void CTClientApp::LoadStdProfileSettings()
{
	...
	CTClientGame::m_vTOPTION.m_bDisableDoubleSkill = (BYTE) GetProfileInt(
		_T("Settings"),
		_T("DISABLE_DOUBLE_SKILL"),
		CTClientGame::m_vTOPTION.m_bDisableDoubleSkill);

	CTClientGame::m_vTOPTION.m_bDontShowOtherCompanions = (BYTE) GetProfileInt(
		_T("Settings"),
		_T("DISABLE_OTHER_COMPANIONS"),
		CTClientGame::m_vTOPTION.m_bDontShowOtherCompanions);

	CTClientGame::m_vTOPTION.m_bHideTournament = (BYTE) GetProfileInt(
		_T("Settings"),
		_T("DISABLE_TOURNAMENT_AUDIENCE"),
		CTClientGame::m_vTOPTION.m_bHideTournament);

	CTClientGame::m_vTOPTION.m_bHideArena = (BYTE) GetProfileInt(
		_T("Settings"),
		_T("DISABLE_ARENA_AUDIENCE"),
		CTClientGame::m_vTOPTION.m_bHideArena);

	CTClientGame::m_vTOPTION.m_bHideDuel = (BYTE) GetProfileInt(
		_T("Settings"),
		_T("DISABLE_DUEL_AUDIENCE"),
		CTClientGame::m_vTOPTION.m_bHideDuel);

	CTClientGame::m_vTOPTION.m_bDisableAutoSelectSFX = (BYTE) GetProfileInt(
		_T("Settings"),
		_T("DISABLE_AUTO_SELECT_SFX"),
		CTClientGame::m_vTOPTION.m_bDisableAutoSelectSFX);

	CTClientGame::m_vTOPTION.m_bDisableChatTransparency = (BYTE) GetProfileInt(
		_T("Settings"),
		_T("DISABLE_CHAT_TRANSPARENCY"),
		CTClientGame::m_vTOPTION.m_bDisableChatTransparency);

	CTClientGame::m_vTOPTION.m_bUIBar = (BYTE) GetProfileInt(
		_T("Settings"),
		_T("UTILITYBAR"),
...
int CTClientApp::Run()
{
	// Set this to 0 if you do not want to see the MFC memoryleak-detection output. If you want to, comment this line out and #define new DEBUG_NEW for tracing the location.
	_CrtSetDbgFlag(0);
	BYTE bHackPass = 0;
	...
	while(m_pTachyonWnd)
	{
		...
		if ( !CTNationOption::INSTALL_HACKSHIELD && !CTNationOption::INSTALL_GAMEGUARD && !CTNationOption::INSTALL_XTRAP)
		{
...
BOOL CTClientApp::HackShield_Init()
{
	...
	dwOption = AHNHS_CHKOPT_ALL | AHNHS_CHKOPT_STANDALONE | AHNHS_DISPLAY_HACKSHIELD_TRAYICON
		...
		| AHNHS_CHKOPT_UPDATED_FILE_CHECK | AHNHS_CHKOPT_LOCAL_MEMORY_PROTECTION;
	...
	if (nRet != HS_ERR_OK)
	{
		...
		{
			szMsg = "Compatibilitymode has been detected!";
			break;
		}
		case HS_ERR_NEED_ADMIN_RIGHTS:
		{
			szMsg = "Please restart the game as administrator!";
			break;
		}
		...
		{
			szMsg = "Initialsation has failed!";
			break;
		}
		case HS_ERR_DEBUGGER_DETECT:
		{
			szMsg = "A running debugger has been detected!";
			break;
		}
		...
		{
			szMsg = "HackShield has not been initialized!";
			break;
		}
...
int __stdcall CTClientApp::HS_CallbackProc ( long lCode, long lParamSize, void* pParam )
{
	...
	if (lCode == AHNHS_ENGINE_DETECT_WINDOWED_HACK || (lCode == AHNHS_ACTAPC_DETECT_HOOKFUNCTION && lParamSize == 256))
		return TRUE;
