...
class CTBLGuildStats : public CSqlQuery
{
public:
	DWORD m_dwGuildID;
	BYTE m_bSkillPoint;
	...
	DEF_QUERY(CTBLGuildStats, _T("SELECT dwGuildID, bSkillPoint, bLevel, dwExp FROM TGUILDSTATSTABLE"));
...
{
public:
	DEF_QUERY(CTBLStopMap,_T("SELECT dwCharID FROM OTESTER"))
...
class CTBLProtectChar : public CSqlQuery
{
	...
	public:
	DEF_QUERY(CTBLProtectChar,_T("SELECT dwCharID FROM OTESTER"))
};

class CTBLDeleteLevel : public CSqlQuery
{
public:
	DEF_QUERY(CTBLDeleteLevel,_T("SELECT dwCharID FROM OTESTER"))
...
class CTBLCustomTimeChart : public CSqlQuery
{
public:
	BYTE m_bType;
	DWORD m_dwTime;

DEF_QUERY(CTBLCustomTimeChart, _T("SELECT \
	bType, \
	dwTime \
	FROM TCUSTOMTIMECHART"))
...
class CTBLBOWSettingsChart : public CSqlQuery
{
public:
	WORD m_wMapID;
...
DEF_QUERY(CTBLBOWSettingsChart, _T("SELECT \
	wMapID, \
	bMinPlayersCount, \
	bMaxNationDifference, \
	dwAlarmDur, \
	dwBuyTimeDur, \
	dwBattleDur \
...
BEGIN_COLUMN(6)
	...
	COLUMN_ENTRY(m_bMinPlayersCount)
	COLUMN_ENTRY(m_bMaxNationDifference)
	COLUMN_ENTRY(m_dwAlarmDur)
	COLUMN_ENTRY(m_dwBuyTimeDur)
	COLUMN_ENTRY(m_dwBattleDur)
END_COL()
};

class CTBLBRSettingsChart : public CSqlQuery
{
public:
...
DEF_QUERY(CTBLBRSettingsChart, _T("SELECT \
	bMinPlayerCount, \
	dwAlarmDur, \
	dwBuyTimeDur, \
	dwBattleDur \
	FROM TBRSETTINGSCHART"))

BEGIN_COLUMN(4)
	COLUMN_ENTRY(m_bMinPlayerCount)
	COLUMN_ENTRY(m_dwAlarmDur)
	COLUMN_ENTRY(m_dwBuyTimeDur)
	COLUMN_ENTRY(m_dwBattleDur)
END_COL()
...
class CSPSaveGuildStats : public CSqlQuery
{
public:
	DWORD m_GuildID;
	BYTE m_SkillPoint;
	...
	DEF_QUERY(CSPSaveGuildStats, _T("{ CALL TSaveGuildStats(?,?,?,?) }"))

		BEGIN_PARAM(4)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_GuildID)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_SkillPoint)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_Level)
		PARAM_ENTRY(SQL_PARAM_INPUT, m_Exp)
		END_PARAM()
};
...
class CSPAddBOWPlayer : public CSqlQuery
{
public:
	DWORD m_dwCharID;
	DWORD m_dwUserID;

DEF_QUERY(CSPAddBOWPlayer, _T("{ CALL TAddBOWPlayer(?,?)}"))
...
class CSPDeleteSingleBOWPlayer : public CSqlQuery
{
public:
	DWORD m_dwUserID;

DEF_QUERY(CSPDeleteSingleBOWPlayer, _T("{ CALL TDeleteSingleBOWPlayer(?)}"))
...
class CSPAddBRPlayer : public CSqlQuery
{
public:
	DWORD m_dwCharID;
	DWORD m_dwUserID;

DEF_QUERY(CSPAddBRPlayer, _T("{ CALL TAddBRPlayer(?,?)}"))
...
class CSPDeleteSingleBRPlayer : public CSqlQuery
{
public:
	DWORD m_dwUserID;

DEF_QUERY(CSPDeleteSingleBRPlayer, _T("{ CALL TDeleteSingleBRPlayer(?)}"))
...
class CSPCash : public CSqlQuery
{
public:
	DWORD m_dwUserID;
	DWORD m_dwCash;
...
DEF_QUERY(CSPCash, _T("{ CALL TCash(?,?,?,?)}"))
