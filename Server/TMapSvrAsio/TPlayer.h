...
public:
	...
	MAPTCOMPANION m_mapCompanion;
	MAPTPET m_mapTPET;
	MAPTTITLE m_mapTTITLE;
	MAPTSADDLE m_mapSADDLE;
	MAPTCOMP m_mapComp;

	MAPTCUSTOMCLOAK m_mapTCUSTOMCLOAK;

	TDURINGITEM m_diPremium;
	TDURINGITEM m_diExp;

MAPTINVEN m_mapTINVEN_bak;
	MAPTSKILL m_mapTSKILL_bak;
	VTSKILL m_vMaintainSkill_bak;
	VTSKILL m_vRemainSkill_bak;
	MAPTHOTKEYINVEN m_mapHotkeyInven_bak;
	BYTE m_bLevel_bak;
	BYTE m_bIsTournamentPlayer;
...
public:
	...
	void SendCS_SETTIME_ACK(DWORD dwHour, DWORD dwMinute);

	void SendCS_SOULLOTTERY_ACK(DWORD dwNumber1,
								DWORD dwNumber2, 
								DWORD dwNumber3, 
								DWORD dwNumber4,
								DWORD dwMoney);

	void SendCS_SOULLOTTERY_TRIGGER_ACK(DWORD dwNumber, DWORD dwMoney, BYTE bRet);




	void SendCS_EXITTOURNAMETMAINTAIN_ACK();
	void SendCS_EXITTOURNAMETHOTKEY_ACK();
	...
	WORD m_wTitleID;
	DWORD m_dwSkillTick;
	VUSEDSKILL m_vUsedSkill;

	INT m_nCS;
	int m_nPacketCount;

	BYTE m_bLock;
	BYTE m_bProcessingSkill;
	...
	DWORD m_dwLotNumber1;
	DWORD m_dwLotNumber2;
	DWORD m_dwLotNumber3;
	DWORD m_dwLotNumber4;
	DWORD m_dwLotMoney;
...
WORD m_wLastSkillID;
	...
	DWORD m_dwRealMoveTick;
	DWORD m_dwBPTick;

	DWORD m_dwBB; 
	DWORD m_dwPoints;
	DWORD m_dwTotalPoints;
	BYTE m_bDeaths;
#endif
...
public:
	...
	//BYTE m_bSecureLocked;
	//BYTE m_bSecureEnabled;
...
public:
	...
	DWORD GetLeftTick(BYTE m_bKind, DWORD dwTick);
	FLOAT CalcPetBonus(BYTE bType);
	...
	void ClearSpolecnikMon(BYTE bExitMap);
	...
	LPTSECURE GetSecureCode();
	void IncreaseStatExp(WORD Inc);
	void CheckGuildSkillTick();
	void LeaveAllRecallMon();
	void LeaveAllCompanion();
	void AddHotKey(
		BYTE bInven,
		BYTE bPos,
		BYTE bType,
	...
	CTCompanion *FindSpolecnikMon(DWORD dwMonID);
	CTSelfObj *FindSelfObj(DWORD dwMonID);

	CTCompanion * FindCompanion(DWORD dwMonID);
	DWORD GetPetItemUse();
	DWORD GetExpVital();
	FLOAT GetLevelRate(BYTE bLevel);
	BYTE SetAftermath(DWORD dwTick, BYTE bStep);
	void ResetAftermath(DWORD dwTick);
	void DeleteRecallMon(DWORD dwID, BYTE bForever = TRUE);
	void DeleteCompanion(DWORD dwID, BYTE bForever = TRUE);
	...
	void ClearSoulmate();
	CTCompanion * FindRecalledCompanion();
	...
	BYTE ChangeCharBase(BYTE bType, WORD wTitleID, CString strName = NAME_NULL, BYTE bValue=0);
	...
	LPTCOMP GetCompanion( BYTE bCompanionSlot );
...
void TournamentCharClear();
	void ExitTournamentCharClear();
	void ClearChar(BYTE bRemoveHotKey);
	void ClearBakMaps();
	void ResetChar();
	BYTE HaveInvenBlank();
	void ResetRPS();
	BYTE IsTournamentPlayer();
	BYTE CanTalk(BYTE bChatType, BYTE bCountry, BYTE bAidCountry);
	BYTE CheckChat(CString strTalk);
	DWORD GetAidLeftTime();
	void GetTitle(BYTE bType, DWORD dwValue, BYTE bStart);
	void CzechPetHP();
#if defined(BOW_COMPILE_MODE) || defined(BR_COMPILE_MODE)
	void IncrementBonusPoints(WORD wBPlus);
	void DecreaseBonusPoints(WORD wBPDecrease);
#endif

	void RespawnCompanion();
	BOOL CanUseRaceChange();

#if defined(BOW_COMPILE_MODE) || defined(BR_COMPILE_MODE)
	void ReplaceBOWEquipment(BOOL bSaved);
	void ReleaseForBOW(BOOL bSaved);
	BYTE IsWeaponButNotMain(BYTE bKind);
#endif
public:
	...
	void SendCS_DEALITEMADD_ACK(
		...
		DWORD dwCooper,
		LPVTITEM pvItem);
	void SendCS_UPDATEMEDALS_REQ(DWORD dwMedals);
	...
	void SendCS_TITLEGAIN_ACK(WORD wTitleID, BYTE bStart);
	...
	void SendCS_ADDSPOLECNIKMON_ACK(
		CTCompanion * pMON,
		BYTE bNewMember,
		DWORD dwTick);
	...
	void SendCS_DELRECALLMON_ACK(
		...
		BYTE bExitMap,
		BYTE bForever);
	void SendCS_DELSPOLECNIKMON_ACK(
		DWORD dwHostID,
		DWORD dwMonID,
		BYTE bExitMap);
	...
	void SendCS_LEVEL_ACK(
		DWORD dwCharID,
			BYTE bLevel,
		BYTE bShowLevelUp = TRUE);

	void SendCS_SOULUP_ACK(
		DWORD dwCharID);

	void SendCS_WHIP_ACK(
		DWORD dwSpeedFactor,
		DWORD dwTimeInUse);
	...
	void SendCS_SKILLBUY_ACK(BYTE bRet, WORD wSkillID, BYTE bLevel, DWORD Tick = 0);
	...
	void SendCS_CUSTOMCLOAK_LIST_ACK();
	void SendCS_CUSTOMCLOAK_CHANGE_ACK(DWORD dwCharID,WORD wID,BYTE bRet);
	...
	void SendCS_GUILDLOCALLIST_ACK(BYTE bBOWStatus, BYTE bBOWWinner, DWORD dwNextBOW, BYTE bBRStatus, DWORD dwNextBR, BYTE bNextBRType);
	...
	void SendCS_CHAT_ACK(
		...
		CString strName,
		CString strTalk);
	void SendCS_SENDSADDLE_REQ(DWORD m_dwSaddleID, __time64_t tEndTime, BYTE m_bType, BOOL m_bOpenUI = FALSE);
	...
	void SendCS_GETREWARDS_ACK(BYTE bKind, WORD m_wUseValue);
...
#ifdef SKYGARDEN
	void SendCS_SKYGARDENOCCUPY_ACK(BYTE bType, WORD wID, BYTE bCountry);
#endif
	void SendCS_ITEMUPGRADE_ACK(BYTE bResult, BYTE bTargetInven, BYTE bTargetItemID, BYTE bGrade, BYTE bGem, BYTE bGradeEffect, WORD wColor, WORD wMoggItemID);
	...
	void SendCS_INVENADD_ACK( BYTE bResult, BYTE bInvenID, WORD wItemID, __time64_t dEndTime, BYTE bShowIF = TRUE);
	void SendCS_INVENDEL_ACK( BYTE bResult, BYTE bInvenID, BYTE bShowIF = TRUE);
	...
	void SendCS_PETEFFECTCHANGE_ACK(BYTE bResult, BYTE bEffect);
	...
	void SendCS_TITLELIST_ACK();

	void CheckPetEndTime(__time64_t dCurrentTime);
	void SendCS_TITLERESET_ACK();
...
#ifdef SKYGARDEN
	void SendCS_ENTERSKYGARDEN_ACK(WORD wSkyGardenID, BYTE bCamp, BYTE bDefendCountry, BYTE bLeftCountry, BYTE bCenterCountry, BYTE bRightCountry, BYTE bAttackCountry);
	void SendCS_LEAVESKYGARDEN_ACK();
	void SendCS_SKYGARDEN_OCCUPY_LEFT_ACK(BYTE bCountry);
	void SendCS_SKYGARDEN_OCCUPY_CENTER_ACK(BYTE bCountry);
	void SendCS_SKYGARDEN_OCCUPY_RIGHT_ACK(BYTE bCountry);
#endif
	...
	//Securitysystem
	void SendCS_SC_CREATECODE_ACK(BYTE bResult);
	void SendCS_SC_CHANGECODE_ACK(BYTE bResult);
	void SendCS_SC_DISABLECODE_ACK(BYTE bResult);
	void SendCS_SC_ENABLECODE_ACK(BYTE bResult);
	void SendCS_SC_UNLOCK_ACK(BYTE bResult);
	void SendCS_SC_CHECKCODE_ACK(BYTE bResult);
	void SendCS_SC_GETLOCKTIME_ACK(BYTE bHours, BYTE bMinutes);

	void SendCS_COMPANIONLIST_ACK();
	void SendCS_COMPANIONUPDATE_REQ( BYTE bSlot, BYTE bSTR, BYTE bDEX, BYTE bCON, BYTE bINT, BYTE bWIS, BYTE bMEN, BYTE bStatusPoints );
	void SendCS_COMPANIONLUPDATE_REQ( BYTE bSlot, BYTE bLevel, DWORD dwExp, BYTE bSPoints, DWORD dwNextExp, FLOAT fBonusValue );
	void SendCS_UPDATECOMPANIONBYITEM_REQ( DWORD dwCharID, BYTE bCompanionSlot, DWORD wLife, DWORD dwExp );
	void SendCS_UPDATECOMPANIONBYSYSTEM_REQ( BYTE bCompanionSlot, DWORD wLife, DWORD dwExp );
	void SendCS_UPDATESPAWNEDCOMPANION_REQ( BYTE bCompanionSlot );
	void SendCS_CREATECOMPANION_ACK( BYTE bResult, BYTE bSlot );
	void SendCS_UPDATECOMPANIONITEMS_REQ( BYTE bCompanionSlot, BYTE bSlot, WORD wItemID, __time64_t dEndTime );
	void SendCS_UPDATECOMPANIONBONUS_REQ( BYTE bCompanionSlot );

	void SendCS_BOWNOTIFY_ACK(const CString& strMessage);
	void SendCS_REGISTERBOW_ACK(BYTE bResult, DWORD dwTick);
	void SendCS_CANCELBOWQUEUE_ACK(BYTE bResult, DWORD dwTick);
	void SendCS_NOTIFYNONQUEUEDPLAYER_ACK();
	void SendCS_PINGMEASUREMENT_ACK(DWORD dwTick);
	void SendCS_BOWTIMEUPDATE_ACK(BYTE bStatus, DWORD dwSecond, BYTE bDefugelPoints, BYTE bCraxionPoints);
	void SendCS_RELEASEFORBOW_REQ();
#if defined(BOW_COMPILE_MODE) || defined(BR_COMPILE_MODE)
	void SendCS_UPDATEBONUSPOINT_ACK(DWORD dwBB);
	void SendCS_BOWKILLNOTIFY_ACK(CString strAttack = NAME_NULL /*smart switch; if party member got kill*/);
	void SendCS_BOWRANKING_ACK(const VBOWRANKING& vPlayer, const MAPBOWGRANKING& mapGuild);
	void SendCS_LEAVEBATTLEFIELD_ACK(BYTE bResult);
#endif

	void SendCS_ADDTOBRQUEUE_ACK(BYTE bResult, DWORD dwTick);
	void SendCS_BRTIMEUPDATE_ACK(BYTE bStatus, DWORD dwSecond, BYTE bType);
	void SendCS_BRTEAMMATEADD_ACK(BYTE bResult, CString strNAME);
	void SendCS_UPDATEBRTEAM_ACK(const std::vector<BRPREMADEPLAYER>& vTeam, BYTE bTeamReady);
#ifdef BR_COMPILE_MODE
	void SendCS_INITBRTRANKING_ACK(const MAPBRTEAM& mapTeam);
	void SendCS_UPDATEBRMATE_ACK(const MAPBRPLAYER& mapPlayer);
	void SendCS_UPDATEBRTPLAYER_ACK(DWORD dwTeamID, DWORD dwCharID, WORD wKills, BYTE bLife);
	void SendCS_UPDATEREMAININGPLAYER_ACK(BYTE bRemaining);
#endif

	void SendCS_GUILDSKILLACTION_ACK(DWORD dwCharID, DWORD dwKEY, BYTE Action, WORD Param[]);
	void SendCS_GUILDSKILLUPDATE_ACK();
	void SendCS_ARENAINFO_ACK(LPTARENA Arena);
	void SendCS_ARENARANKING_ACK();
public:
	CTPlayer();
	virtual ~CTPlayer();
};
